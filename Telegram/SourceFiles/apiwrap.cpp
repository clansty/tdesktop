...
[[nodiscard]] std::shared_ptr<ChatHelpers::Show>
ShowForPeer(not_null<PeerData *> peer) {
...
[[nodiscard]] QString FormatVideoTimestamp(TimeId seconds) {
  ...
  return hours ? u"%1h%2m%3s"_q.arg(hours).arg(minutes % 60).arg(seconds % 60)
         : minutes ? u"%1m%2s"_q.arg(minutes).arg(seconds % 60)
...
TimeId UnixtimeFromMsgId(mtpMsgId msgId) { return TimeId(msgId >> 32); }

} // namespace Api

ApiWrap::ApiWrap(not_null<Main::Session *> session)
    : MTP::Sender(&session->account().mtp()), _session(session),
  ...
  crl::on_main(session, [=] {
    ...
    _session->data().chatsFilters().changed() | rpl::filter([=] {
      return _session->data().chatsFilters().archiveNeeded();
    }) |
        rpl::on_next([=] { requestMoreDialogsIfNeeded(); },
                     _session->lifetime());
...
void ApiWrap::ProcessRecentSelfForwards(not_null<Main::Session *> session,
...
Main::Session &ApiWrap::session() const { return *_session; }

Storage::Account &ApiWrap::local() const { return _session->local(); }

Api::Updates &ApiWrap::updates() const { return _session->updates(); }
...
void ApiWrap::setupSupportMode() {
  ...
  _session->settings().supportChatsTimeSliceValue() |
      rpl::on_next(
          [=](int seconds) {
            _dialogsLoadTill =
                seconds ? std::max(base::unixtime::now() - seconds, 0) : 0;
            refreshDialogsLoadBlocked();
          },
          _session->lifetime());
}

void ApiWrap::requestChangelog(const QString &sinceVersion,
...
void ApiWrap::requestDeepLinkInfo(
  ...
  _deepLinkInfoRequestId =
      request(MTPhelp_GetDeepLinkInfo(MTP_string(path)))
          .done([=](const MTPhelp_DeepLinkInfo &result) {
            ...
            if (result.type() == mtpc_help_deepLinkInfo) {
              ...
              callback(TextWithEntities{qs(data.vmessage()),
                                        Api::EntitiesFromMTP(
                                            _session,
...
void ApiWrap::requestTermsUpdate() {
  ...
  _termsUpdateRequestId =
      request(MTPhelp_GetTermsOfServiceUpdate())
          .done([=](const MTPhelp_TermsOfServiceUpdate &result) {
            ...
            const auto requestNext = [&](auto &&data) {
              ...
              _termsUpdateSendAt =
                  crl::now() + std::clamp(timeout * crl::time(1000),
            ...
            default:
              Unexpected("Type in requestTermsUpdate().");
            }
          })
          .fail([=] {
            _termsUpdateRequestId = 0;
...
void ApiWrap::acceptTerms(bytes::const_span id) {
  request(MTPhelp_AcceptTermsOfService(MTP_dataJSON(MTP_bytes(id))))
      .done([=] { requestTermsUpdate(); })
      .send();
}

void ApiWrap::checkChatInvite(const QString &hash,
                              FnMut<void(const MTPChatInvite &)> done,
                              Fn<void(const MTP::Error &)> fail) {
  request(base::take(_checkInviteRequestId)).cancel();
  _checkInviteRequestId = request(MTPmessages_CheckChatInvite(MTP_string(hash)))
                              .done(std::move(done))
                              .fail(std::move(fail))
                              .handleFloodErrors()
                              .send();
...
void ApiWrap::checkFilterInvite(
    const QString &slug, FnMut<void(const MTPchatlists_ChatlistInvite &)> done,
    Fn<void(const MTP::Error &)> fail) {
  request(base::take(_checkFilterInviteRequestId)).cancel();
  _checkFilterInviteRequestId =
      request(MTPchatlists_CheckChatlistInvite(MTP_string(slug)))
          .done(std::move(done))
          .fail(std::move(fail))
          .send();
}
...
void ApiWrap::savePinnedOrder(Data::Folder *folder) {
  ...
  ranges::transform(order, ranges::back_inserter(peers), input);
  request(MTPmessages_ReorderPinnedDialogs(
              MTP_flags(MTPmessages_ReorderPinnedDialogs::Flag::f_force),
              MTP_int(folder ? folder->id() : 0), MTP_vector(peers)))
      .send();
}
...
void ApiWrap::toggleHistoryArchived(not_null<History *> history, bool archived,
                                    Fn<void()> callback) {
  ...
  const auto requestId =
      request(MTPfolders_EditPeerFolders(MTP_vector<MTPInputFolderPeer>(
                  1, MTP_inputFolderPeer(history->peer->input(),
              ...
              if (settings.hideAllChatsFolder()) {
                if (const auto window = Core::App().activeWindow()) {
                  if (const auto controller = window->sessionController()) {
                    ...
                    const auto lookup_id = filters->lookupId(
          ...
          .fail([=] { _historyArchivedRequests.remove(history); })
          .send();
  _historyArchivedRequests.emplace(history, requestId, callback);
}

void ApiWrap::sendMessageFail(const MTP::Error &error,
                              not_null<PeerData *> peer, uint64 randomId,
                              FullMsgId itemId) {
  sendMessageFail(error.type(), peer, randomId, itemId);
}

void ApiWrap::sendMessageFail(const QString &error, not_null<PeerData *> peer,
                              uint64 randomId, FullMsgId itemId) {
  const auto show = ShowForPeer(peer);
  const auto paidStarsPrefix = u"ALLOW_PAYMENT_REQUIRED_"_q;
  if (show && error == u"PEER_FLOOD"_q) {
    show->showBox(
        Ui::MakeInformBox(PeerFloodErrorText(&session(), PeerFloodType::Send)),
        Ui::LayerOption::CloseOther);
  } else if (show && error == u"USER_BANNED_IN_CHANNEL"_q) {
    const auto link = tr::link(tr::lng_cant_more_info(tr::now),
                               session().createInternalLinkFull(u"spambot"_q));
    show->showBox(Ui::MakeInformBox(tr::lng_error_public_groups_denied(
                      tr::now, lt_more_info, link, tr::marked)),
                  Ui::LayerOption::CloseOther);
    ...
    show->showToast(peer->isBroadcast()
                    ...
                    : peer->isUser() ? tr::lng_error_noforwards_user(tr::now)
    ...
    if (show) {
      show->showToast(u"Payment requirements changed. Please, try again."_q);
    }
...
void ApiWrap::requestMessageData(PeerData *peer, MsgId msgId, Fn<void()> done) {
...
ApiWrap::collectMessageIds(const MessageDataRequests &requests) {
...
auto ApiWrap::messageDataRequests(ChannelData *channel, bool onlyExisting)
  ...
  return &_channelMessageDataRequests.emplace(channel, MessageDataRequests())
              .first->second;
}
...
void ApiWrap::resolveMessageDatas() {
  ...
  if (!ids.isEmpty()) {
    const auto requestId =
        request(MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(ids)))
  ...
  for (auto j = _channelMessageDataRequests.begin();
       j != _channelMessageDataRequests.cend();) {
    if (j->second.empty()) {
    ...
    if (!ids.isEmpty()) {
      ...
      const auto requestId =
          request(MTPchannels_GetMessages(channel->inputChannel(),
...
void ApiWrap::finalizeMessageDataRequest(ChannelData *channel,
                                         mtpRequestId requestId) {
  ...
  for (auto i = requests->begin(); i != requests->cend();) {
    if (i->second.requestId == requestId) {
      ...
      if (callbacks.empty()) {
        ...
        callbacks.insert(end(callbacks), std::make_move_iterator(begin(list)),
...
void ApiWrap::exportMessageAsBase64(not_null<HistoryItem *> item,
                                    Fn<void(const QString &)> done,
                                    Fn<void()> fail) {
  auto ids = QVector<MTPInputMessage>{MTP_inputMessageID(MTP_int(item->id))};
  auto requestDone = [=](const MTPmessages_Messages &result,
  ...
  if (item->history()->peer->isChannel()) {
    request(MTPchannels_GetMessages(
                ...
                MTP_vector<MTPInputMessage>(ids)))
        .done(requestDone)
        .fail([=](const MTP::Error &error, mtpRequestId requestId) { fail(); })
        .send();
  } else {
    request(MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(ids)))
        .done(requestDone)
...
QString ApiWrap::exportDirectMessageLink(not_null<HistoryItem *> item,
  ...
  const auto fallback = [&] {
    ...
    if (inRepliesContext) {
      ...
      const auto rootId =
          linkThreadIsTopic ? item->topicRootId() : item->replyToTop();
      if (rootId) {
        const auto root = item->history()->owner().message(channel->id, rootId);
        const auto sender =
            root ? root->discussionPostOriginalSender() : nullptr;
    ...
    const auto base =
        (linkChannel->hasUsername() && !forceNonPublicLink)
    ...
    const auto query =
        ...
        (linkCommentId
         ...
         : linkThreadId ? (QString::number(linkThreadId.bare) + '/' + post)
                        : post);
  ...
  const auto current =
      (i != end(_unlikelyMessageLinks)) ? i->second : fallback();
  request(MTPchannels_ExportMessageLink(
              ...
              channel->inputChannel(), MTP_int(item->id)))
      ...
      .send();
  const auto addTimestamp =
      channel->hasUsername() && !inRepliesContext && videoTimestamp.has_value();
  const auto addedSeparator = (current.indexOf('?') >= 0) ? '&' : '?';
  const auto addedTimestamp =
      addTimestamp
...
QString ApiWrap::exportDirectStoryLink(not_null<Data::Story *> story) {
  ...
  const auto fallback = [&] {
    ...
    const auto id = story->call() ? u"live"_q : QString::number(storyId.story);
    const auto query = base + "/s/" + id;
    return session().createInternalLinkFull(query);
  };
  const auto i = _unlikelyStoryLinks.find(storyId);
  const auto current = (i != end(_unlikelyStoryLinks)) ? i->second : fallback();
  request(MTPstories_ExportStoryLink(peer->input(), MTP_int(story->id())))
...
void ApiWrap::requestContacts() {
  ...
  _contactsRequestId =
      request(MTPcontacts_GetContacts(MTP_long(0) // hash
                                      ))
...
void ApiWrap::requestMoreDialogs(Data::Folder *folder) {
  ...
  state->requestId =
      request(MTPmessages_GetDialogs(
                  MTP_flags(flags), MTP_int(folder ? folder->id() : 0),
                  MTP_int(state->offsetDate), MTP_int(state->offsetId),
                  (state->offsetPeer ? state->offsetPeer->input()
                                     : MTP_inputPeerEmpty()),
                  MTP_int(loadCount), MTP_long(hash)))
          .done([=](const MTPmessages_Dialogs &result) {
            const auto state = dialogsLoadState(folder);
            const auto count = result.match(
                [](const MTPDmessages_dialogsNotModified &) {
                  LOG(("API Error: not-modified received for requested "
                       "dialogs."));
                  return 0;
                },
                ...
                [&](const MTPDmessages_dialogsSlice &data) {
                  updateDialogsOffset(folder, data.vdialogs().v,
                                      data.vmessages().v);
            ...
            result.match(
                ...
                [&](const auto &data) {
                  ...
                  _session->data().applyDialogs(folder, data.vmessages().v,
                                                data.vdialogs().v, count);
                });
          ...
          .fail([=] { dialogsLoadState(folder)->requestId = 0; })
          .send();
...
void ApiWrap::updateDialogsOffset(Data::Folder *folder,
...
void ApiWrap::dialogsLoadFinish(Data::Folder *folder) {
  const auto notify = [&] {
    Core::App().postponeCall(
        crl::guard(_session, [=] { _session->data().chatsListDone(folder); }));
  };
  const auto state = dialogsLoadState(folder);
...
void ApiWrap::requestPinnedDialogs(Data::Folder *folder) {
  ...
  state->pinnedRequestId =
      request(MTPmessages_GetPinnedDialogs(MTP_int(folder ? folder->id() : 0)))
...
void ApiWrap::requestWallPaper(const QString &slug,
  ...
  _wallPaperRequestId =
      request(MTPaccount_GetWallPaper(MTP_inputWallPaperSlug(MTP_string(slug))))
...
void ApiWrap::requestFullPeer(not_null<PeerData *> peer) {
  ...
  const auto requestId = [&] {
    ...
    if (const auto user = peer->asUser()) {
      ...
      return request(MTPusers_GetFullUser(user->inputUser()))
      ...
      return request(MTPmessages_GetFullChat(chat->inputChat()))
      ...
      return request(MTPchannels_GetFullChannel(channel->inputChannel()))
          .done([=](const MTPmessages_ChatFull &result) {
            gotChatFull(peer, result);
...
void ApiWrap::processFullPeer(not_null<PeerData *> peer,
                              const MTPmessages_ChatFull &result) {
  gotChatFull(peer, result);
}

void ApiWrap::gotChatFull(not_null<PeerData *> peer,
                          const MTPmessages_ChatFull &result) {
  ...
  _fullPeerRequests.remove(peer);
  _session->changes().peerUpdated(peer, Data::PeerUpdate::Flag::FullInfo);
}

void ApiWrap::gotUserFull(not_null<UserData *> user,
  ...
  _session->changes().peerUpdated(user, Data::PeerUpdate::Flag::FullInfo);
}
...
void ApiWrap::requestPeerSettings(not_null<PeerData *> peer) {
  ...
  request(MTPmessages_GetPeerSettings(peer->input()))
...
void ApiWrap::migrateChat(not_null<ChatData *> chat,
  ...
  if (const auto channel = chat->migrateTo()) {
    session().changes().peerUpdated(chat, Data::PeerUpdate::Flag::Migration);
    crl::on_main([=] { migrateDone(chat, channel); });
  } else if (chat->isDeactivated()) {
    crl::on_main([=] {
      migrateFail(chat, MTP::Error::Local("BAD_MIGRATION",
    ...
    crl::on_main([=] {
      migrateFail(chat, MTP::Error::Local(
                            "BAD_MIGRATION",
  ...
  request(MTPmessages_MigrateChat(chat->inputChat()))
      .done([=](const MTPUpdates &result) {
        applyUpdates(result);
      ...
      .fail([=](const MTP::Error &error) { migrateFail(chat, error.type()); })
      .send();
}

void ApiWrap::migrateDone(not_null<PeerData *> peer,
...
void ApiWrap::markContentsRead(
  ...
  auto channelMarkedIds =
      base::flat_map<not_null<ChannelData *>, QVector<MTPint>>();
  ...
  if (!markedIds.isEmpty()) {
    request(MTPmessages_ReadMessageContents(MTP_vector<MTPint>(markedIds)))
  ...
  for (const auto &channelIds : channelMarkedIds) {
    request(
        MTPchannels_ReadMessageContents(channelIds.first->inputChannel(),
...
void ApiWrap::deleteAllFromParticipant(not_null<ChannelData *> channel,
...
void ApiWrap::deleteAllFromParticipantSend(not_null<ChannelData *> channel,
                                           not_null<PeerData *> from) {
  request(MTPchannels_DeleteParticipantHistory(channel->inputChannel(),
                                               from->input()))
      .done([=](const MTPmessages_AffectedHistory &result) {
        ...
        if (offset > 0) {
          deleteAllFromParticipantSend(channel, from);
                       ...
                       _session->data().historyLoaded(channel)) {
...
void ApiWrap::requestStickerSets() {
  for (auto &[id, info] : _stickerSetRequests) {
    ...
    info.id = request(MTPmessages_GetStickerSet(
                          MTP_inputStickerSetID(MTP_long(id),
                  ...
                  .fail([=, setId = id] { _stickerSetRequests.remove(setId); })
                  .afterDelay(kSmallDelayMs)
                  .send();
  }
}

void ApiWrap::saveStickerSets(const Data::StickersSetsOrder &localOrder,
                              const Data::StickersSetsOrder &localRemoved,
                              Data::StickersType type) {
  auto &setDisenableRequests =
      (type == Data::StickersType::Emoji)   ? _customEmojiSetDisenableRequests
      : (type == Data::StickersType::Masks) ? _maskSetDisenableRequests
                                            : _stickerSetDisenableRequests;
  const auto reorderRequestId = [=]() -> mtpRequestId & {
    return (type == Data::StickersType::Emoji)   ? _customEmojiReorderRequestId
           : (type == Data::StickersType::Masks) ? _masksReorderRequestId
  ...
  const auto stickersSaveOrder = [=] {
    ...
    const auto flags = (type == Data::StickersType::Emoji)   ? Flag::f_emojis
                       : (type == Data::StickersType::Masks) ? Flag::f_masks
                                                             : Flag(0);
    reorderRequestId() =
        request(MTPmessages_ReorderStickerSets(MTP_flags(flags),
                                               MTP_vector<MTPlong>(mtpOrder)))
            .done([=] { reorderRequestId() = 0; })
            .fail([=] {
              reorderRequestId() = 0;
  ...
  auto writeInstalled = true, writeRecent = false, writeCloudRecent = false,
       writeCloudRecentAttached = false, writeFaved = false,
       writeArchived = false;
  ...
  for (const auto removedSetId : localRemoved) {
    if ((removedSetId == Data::Stickers::CloudRecentSetId) ||
      ...
      auto &requestId = isAttached ? _stickersClearRecentAttachedRequestId
                                   : _stickersClearRecentRequestId;
      const auto finish = [=] {
        (isAttached ? _stickersClearRecentAttachedRequestId
                    : _stickersClearRecentRequestId) = 0;
      };
      requestId = request(MTPmessages_ClearRecentStickers(MTP_flags(flags)))
                      .done(finish)
                      .fail(finish)
                      .send();
      continue;
    ...
    if (it != sets.cend()) {
      ...
      if (!archived) {
        ...
        auto requestId =
            request(MTPmessages_UninstallStickerSet(setId))
  ...
  for (auto &[id, set] : sets) {
    const auto archived = !!(set->flags & Flag::Archived);
    const auto thatType =
        !!(set->flags & Flag::Emoji)   ? Data::StickersType::Emoji
        : !!(set->flags & Flag::Masks) ? Data::StickersType::Masks
  ...
  for (const auto setId : std::as_const(localOrder)) {
    ...
    if (archived && !localRemoved.contains(set->id)) {
      ...
      const auto requestId =
          request(MTPmessages_InstallStickerSet(mtpSetId, MTP_boolFalse()))
  ...
  for (auto it = sets.begin(); it != sets.cend();) {
    const auto set = it->second.get();
    if ((set->flags & Flag::Featured) || (set->flags & Flag::Installed) ||
        (set->flags & Flag::Archived) || (set->flags & Flag::Special) ||
        (set->flags & Flag::Emoji) || (set->locked > 0)) {
      ++it;
    } else {
      it = sets.erase(it);
    }
...
void ApiWrap::joinChannel(not_null<ChannelData *> channel) {
  if (channel->amIn()) {
    session().changes().peerUpdated(channel,
                                    Data::PeerUpdate::Flag::ChannelAmIn);
  } else if (!_channelAmInRequests.contains(channel)) {
    const auto requestId =
        request(MTPchannels_JoinChannel(channel->inputChannel()))
            .done([=](const MTPUpdates &result) {
...
void ApiWrap::leaveChannel(not_null<ChannelData *> channel) {
  if (!channel->amIn()) {
    ...
    auto requestId = request(MTPchannels_LeaveChannel(channel->inputChannel()))
                         .done([=](const MTPUpdates &result) {
                           _channelAmInRequests.remove(channel);
                           applyUpdates(result);
                         })
                         .fail([=] { _channelAmInRequests.remove(channel); })
                         .send();
...
void ApiWrap::requestNotifySettings(const MTPInputNotifyPeer &peer) {
  const auto bad = peer.match(
      [](const MTPDinputNotifyUsers &) { return false; },
      [](const MTPDinputNotifyChats &) { return false; },
      [](const MTPDinputNotifyBroadcasts &) { return false; },
  ...
  const auto peerFromInput = [&](const MTPInputPeer &inputPeer) {
    return inputPeer.match(
        [&](const MTPDinputPeerSelf &) { return _session->userPeerId(); },
        [](const MTPDinputPeerEmpty &) { return PeerId(0); },
  ...
  const auto key = peer.match(
      [](const MTPDinputNotifyUsers &) {
  ...
  const auto requestId =
      request(MTPaccount_GetNotifySettings(peer))
          ...
          .fail([=] {
            _session->data().notifySettings().apply(
                peer, MTP_peerNotifySettings(
                          MTP_flags(0), MTPBool(), MTPBool(), MTPint(),
                          MTPNotificationSound(), MTPNotificationSound(),
                          MTPNotificationSound(), MTPBool(), MTPBool(),
                          MTPNotificationSound(), MTPNotificationSound(),
...
void ApiWrap::updateNotifySettingsDelayed(
  ...
  if (_updateNotifyTopics.emplace(topic).second) {
    topic->destroyed() |
        rpl::on_next([=] { _updateNotifyTopics.remove(topic); },
...
void ApiWrap::sendNotifySettingsUpdates() {
  ...
  for (const auto &topic : base::take(_updateNotifyTopics)) {
    request(MTPaccount_UpdateNotifySettings(
                MTP_inputNotifyForumTopic(topic->peer()->input(),
  ...
  for (const auto &peer : base::take(_updateNotifyPeers)) {
    request(MTPaccount_UpdateNotifySettings(MTP_inputNotifyPeer(peer->input()),
...
void ApiWrap::updatePrivacyLastSeens() {
  ...
  if (!_session->premium()) {
    _session->data().enumerateUsers([&](not_null<UserData *> user) {
      if (user->isSelf() || !user->isLoaded() || user->lastseen().isHidden()) {
        return;
      }

      const auto till = user->lastseen().onlineTill();
      user->updateLastseen(
          (till + 3 * 86400 >= now)    ? Data::LastseenStatus::Recently(true)
          : (till + 7 * 86400 >= now)  ? Data::LastseenStatus::WithinWeek(true)
          : (till + 30 * 86400 >= now) ? Data::LastseenStatus::WithinMonth(true)
  ...
  _contactsStatusesRequestId =
      request(MTPcontacts_GetStatuses())
          .done([=](const MTPVector<MTPContactStatus> &result) {
            ...
            for (const auto &status : result.v) {
              ...
              if (const auto user = _session->data().userLoaded(userId)) {
                const auto status =
                    LastseenFromMTP(data.vstatus(), user->lastseen());
          ...
          .fail([this] { _contactsStatusesRequestId = 0; })
          .send();
}
...
void ApiWrap::deleteConversation(not_null<PeerData *> peer, bool revoke) {
  if (const auto chat = peer->asChat()) {
    request(MTPmessages_DeleteChatUser(MTP_flags(0), chat->inputChat(),
...
void ApiWrap::deleteHistory(not_null<PeerData *> peer, bool justClear,
                            bool revoke) {
  ...
  if (const auto channel = peer->asChannel()) {
    if (!justClear && !revoke) {
      ...
      if (deleteTillId || (!justClear && revoke)) {
        history->owner().histories().deleteAllMessages(history, deleteTillId,
                                                       justClear, revoke);
      }
    }
  } else {
...
void ApiWrap::applyAffectedMessages(
    not_null<PeerData *> peer, const MTPmessages_AffectedMessages &result) {
...
void ApiWrap::saveDraftsToCloud() {
  for (auto i = begin(_draftsSaveRequestIds);
       i != end(_draftsSaveRequestIds);) {
    ...
    if (!_session->supportMode()) {
      ...
      cloudDraft =
          history->createCloudDraft(topicRootId, monoforumPeerId, nullptr);
    }
    ...
    auto entities = Api::EntitiesToMTP(
        _session, TextUtilities::ConvertTextTagsToEntities(textWithTags.tags),
        Api::ConvertOption::SkipLocal);

    history->startSavingCloudDraft(topicRootId, monoforumPeerId);
    cloudDraft->saveRequestId =
        request(MTPmessages_SaveDraft(
                    MTP_flags(flags), ReplyToForMTP(history, cloudDraft->reply),
                    history->peer->input(), MTP_string(textWithTags.text),
                    entities,
            ...
            .done([=](const MTPBool &result, const MTP::Response &response) {
              ...
              const auto cloudDraft =
                  history->cloudDraft(topicRootId, monoforumPeerId);
              if (cloudDraft) {
...
void ApiWrap::gotStickerSet(uint64 setId,
...
void ApiWrap::resolveWebPages() {
                ...
                idsByChannel
                    .emplace(channel,
                             IndexAndMessageIds(
                                 idsByChannel.size(),
                                 QVector<MTPInputMessage>(
                                     1, MTP_inputMessageID(MTP_int(item->id)))))
                    .first;
          } else {
  ...
  for (auto i = idsByChannel.cbegin(), e = idsByChannel.cend(); i != e; ++i) {
    reqsByIndex[i->second.first] =
        request(MTPchannels_GetMessages(
                    i->first->inputChannel(),
                    MTP_vector<MTPInputMessage>(i->second.second)))
            .done([=, channel = i->first](const MTPmessages_Messages &result,
                                          mtpRequestId requestId) {
...
void ApiWrap::requestFileReference(Data::FileOrigin origin,
  ...
  request(std::move(data))
      .done([=](const auto &result) {
        ...
        for (const auto &p : parsed.data) {
          ...
          const auto documentId = std::get_if<DocumentFileLocationId>(&origin);
          if (documentId) {
            _session->data()
...
void ApiWrap::refreshFileReference(
    Data::FileOrigin origin, not_null<Storage::DownloadMtprotoTask *> task,
    int requestId, const QByteArray &current) {
  return refreshFileReference(
      origin, crl::guard(task, [=](const UpdatedFileReferences &data) {
        task->refreshFileReferenceFrom(data, requestId, current);
      }));
}

void ApiWrap::refreshFileReference(Data::FileOrigin origin,
                                   FileReferencesHandler &&handler) {
  const auto fail = [&] { handler(UpdatedFileReferences()); };
  const auto request = [&](auto &&data, Fn<void()> &&additional = nullptr) {
    requestFileReference(origin, std::move(handler), std::move(data));
    if (additional) {
      const auto i = _fileReferenceHandlers.find(origin);
      Assert(i != end(_fileReferenceHandlers));
      if (i->second.size() == 1) {
        i->second.push_back([=](auto &&) { additional(); });
      }
    }
  };
  v::match(
      origin.data,
      [&](Data::FileOriginMessage data) {
        if (const auto item = _session->data().message(data)) {
          ...
          const auto storyId =
              mediaStory
                  ? mediaStory
                  : FullStoryId{
                        (IsStoryMsgId(item->id) ? item->history()->peer->id
                                                : PeerId()),
                        (IsStoryMsgId(item->id) ? StoryIdFromMsgId(item->id)
                                                : StoryId())};
          if (storyId) {
            ...
            const auto realId = _session->scheduledMessages().lookupId(item);
            ...
            request(MTPmessages_GetQuickReplyMessages(
                ...
                MTP_vector<MTPint>(1, MTP_int(realId)), MTP_long(0)));
            ...
            request(MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(
                1, MTP_inputMessageID(MTP_int(item->id)))));
          }
        } else {
          fail();
      ...
      [&](Data::FileOriginUserPhoto data) {
        if (const auto user = _session->data().user(data.userId)) {
          request(MTPphotos_GetUserPhotos(user->inputUser(), MTP_int(-1),
                                          MTP_long(data.photoId), MTP_int(1)));
        } else {
          fail();
        }
      },
      ...
      [&](Data::FileOriginPeerPhoto data) {
        ...
        if (const auto channel = peer->asChannel()) {
          request(MTPchannels_GetFullChannel(channel->inputChannel()));
        } else if (const auto chat = peer->asChat()) {
      ...
      [&](Data::FileOriginStickerSet data) {
        ...
        if (data.setId == Data::Stickers::CloudRecentSetId ||
            data.setId == Data::Stickers::RecentSetId || isRecentAttached) {
          auto done = [=] {
          ...
          request(MTPmessages_GetStickerSet(
                      MTP_inputStickerSetID(MTP_long(data.setId),
      ...
      [&](Data::FileOriginSavedGifs data) {
        request(MTPmessages_GetSavedGifs(MTP_long(0)), [=] {
      ...
      [&](Data::FileOriginWallpaper data) {
        ...
        request(MTPaccount_GetWallPaper(
            useSlug ? MTP_inputWallPaperSlug(MTP_string(data.slug))
                    : MTP_inputWallPaper(MTP_long(data.paperId),
                                         MTP_long(data.accessHash))));
      },
      [&](Data::FileOriginTheme data) {
        request(MTPaccount_GetTheme(
            MTP_string(Data::CloudThemes::Format()),
            MTP_inputTheme(MTP_long(data.themeId), MTP_long(data.accessHash))));
      },
      ...
      [&](Data::FileOriginWebPage data) {
        request(MTPmessages_GetWebPage(MTP_string(data.url), MTP_int(0)));
      },
...
void ApiWrap::gotWebPages(ChannelData *channel,
...
void ApiWrap::requestSpecialStickersForce(bool faved, bool recent,
                                          bool attached) {
  if (faved) {
...
void ApiWrap::setGroupStickerSet(not_null<ChannelData *> megagroup,
                                 const StickerSetIdentifier &set) {
  Expects(megagroup->mgInfo != nullptr);

  megagroup->mgInfo->stickerSet = set;
  request(MTPchannels_SetStickers(megagroup->inputChannel(),
                                  Data::InputStickerSet(set)))
      .send();
  _session->data().stickers().notifyUpdated(Data::StickersType::Stickers);
}

void ApiWrap::setGroupEmojiSet(not_null<ChannelData *> megagroup,
                               const StickerSetIdentifier &set) {
  Expects(megagroup->mgInfo != nullptr);

  megagroup->mgInfo->emojiSet = set;
  request(MTPchannels_SetEmojiStickers(megagroup->inputChannel(),
                                       Data::InputStickerSet(set)))
      .send();
  _session->changes().peerUpdated(megagroup, Data::PeerUpdate::Flag::EmojiSet);
  _session->data().stickers().notifyUpdated(Data::StickersType::Emoji);
}

std::vector<not_null<DocumentData *>> *
ApiWrap::stickersByEmoji(const QString &key) {
  ...
  if (sendRequest) {
    const auto hash =
        (it != _stickersByEmoji.end()) ? it->second.hash : uint64(0);
    request(MTPmessages_GetStickers(MTP_string(key), MTP_long(hash)))
  ...
  const auto done = [=](const MTPmessages_AllStickers &result) {
    ...
    result.match([&](const MTPDmessages_allStickersNotModified &) {},
                 [&](const MTPDmessages_allStickers &data) {
                   _session->data().stickers().setsReceived(data.vsets().v,
                                                            data.vhash().v);
                 });
  };
  _stickersUpdateRequest = request(MTPmessages_GetAllStickers(MTP_long(
                                       Api::CountStickersHash(_session, true))))
                               .done(done)
                               .fail([=] {
...
void ApiWrap::requestMasks(TimeId now) {
  ...
  const auto done = [=](const MTPmessages_AllStickers &result) {
    ...
    result.match([&](const MTPDmessages_allStickersNotModified &) {},
                 [&](const MTPDmessages_allStickers &data) {
                   _session->data().stickers().masksReceived(data.vsets().v,
                                                             data.vhash().v);
                 });
  };
  _masksUpdateRequest = request(MTPmessages_GetMaskStickers(MTP_long(
                                    Api::CountMasksHash(_session, true))))
                            .done(done)
                            .fail([=] {
...
void ApiWrap::requestCustomEmoji(TimeId now) {
  ...
  const auto done = [=](const MTPmessages_AllStickers &result) {
    ...
    result.match([&](const MTPDmessages_allStickersNotModified &) {},
                 [&](const MTPDmessages_allStickers &data) {
                   _session->data().stickers().emojiReceived(data.vsets().v,
                                                             data.vhash().v);
                 });
  };
  _customEmojiUpdateRequest =
      request(MTPmessages_GetEmojiStickers(
...
void ApiWrap::requestRecentStickers(std::optional<TimeId> now, bool attached) {
  const auto needed =
      !now       ? true
      : attached ? _session->data().stickers().recentAttachedUpdateNeeded(*now)
                 : _session->data().stickers().recentUpdateNeeded(*now);
  if (!needed) {
    return;
  }
  const auto requestId = [=]() -> mtpRequestId & {
    return attached ? _recentAttachedStickersUpdateRequest
  ...
  const auto flags = attached ? MTPmessages_getRecentStickers::Flag::f_attached
                              : MTPmessages_getRecentStickers::Flags(0);
  requestId() =
      request(MTPmessages_GetRecentStickers(
                  MTP_flags(flags), MTP_long(now ? Api::CountRecentStickersHash(
                                                       _session, attached)
                                                 : 0)))
          .done([=](const MTPmessages_RecentStickers &result) {
            ...
            switch (result.type()) {
            case mtpc_messages_recentStickersNotModified:
              return;
            case mtpc_messages_recentStickers: {
              auto &d = result.c_messages_recentStickers();
              _session->data().stickers().specialSetReceived(
                  attached ? Data::Stickers::CloudRecentAttachedSetId
                           : Data::Stickers::CloudRecentSetId,
                  tr::lng_recent_stickers(tr::now), d.vstickers().v,
                  d.vhash().v, d.vpacks().v, d.vdates().v);
            }
              return;
            default:
              Unexpected("Type in ApiWrap::recentStickersDone()");
            }
          })
          .fail([=] {
            finish();
...
void ApiWrap::requestFavedStickers(std::optional<TimeId> now) {
  ...
  _favedStickersUpdateRequest =
      request(MTPmessages_GetFavedStickers(
          ...
          .done([=](const MTPmessages_FavedStickers &result) {
            ...
            switch (result.type()) {
            case mtpc_messages_favedStickersNotModified:
              return;
            case mtpc_messages_favedStickers: {
              auto &d = result.c_messages_favedStickers();
              _session->data().stickers().specialSetReceived(
                  Data::Stickers::FavedSetId, Lang::Hard::FavedSetTitle(),
                  d.vstickers().v, d.vhash().v, d.vpacks().v);
            }
              return;
            default:
              Unexpected("Type in ApiWrap::favedStickersDone()");
            }
          })
          .fail([=] {
            _session->data().stickers().setLastFavedUpdate(crl::now());
...
void ApiWrap::requestFeaturedEmoji(TimeId now) {
  ...
  _featuredEmojiUpdateRequest =
      request(MTPmessages_GetFeaturedEmojiStickers(
                  MTP_long(Api::CountFeaturedStickersHash(_session))))
          .done([=](const MTPmessages_FeaturedStickers &result) {
...
void ApiWrap::requestSavedGifs(TimeId now) {
  ...
  _savedGifsUpdateRequest =
      request(
          MTPmessages_GetSavedGifs(MTP_long(Api::CountSavedGifsHash(_session))))
          .done([=](const MTPmessages_SavedGifs &result) {
            _session->data().stickers().setLastSavedGifsUpdate(crl::now());
            _savedGifsUpdateRequest = 0;

            switch (result.type()) {
            case mtpc_messages_savedGifsNotModified:
              return;
            case mtpc_messages_savedGifs: {
              auto &d = result.c_messages_savedGifs();
              _session->data().stickers().gifsReceived(d.vgifs().v,
                                                       d.vhash().v);
            }
              return;
            default:
              Unexpected("Type in ApiWrap::savedGifsDone()");
            }
          })
          .fail([=] {
            _session->data().stickers().setLastSavedGifsUpdate(crl::now());
...
void ApiWrap::readFeaturedSets() {
  ...
  if (!wrappedIds.empty()) {
    auto requestData =
        MTPmessages_ReadFeaturedStickers(MTP_vector<MTPlong>(wrappedIds));
    request(std::move(requestData))
        .done([=] {
...
void ApiWrap::resolveJumpToDate(
  ...
  if (const auto peer = chat.peer()) {
    ...
    resolveJumpToHistoryDate(peer, rootId, monoforumPeerId, date,
                             std::move(callback));
  }
}

template <typename Callback>
void ApiWrap::requestMessageAfterDate(not_null<PeerData *> peer,
                                      MsgId topicRootId, PeerId monoforumPeerId,
                                      const QDate &date, Callback &&callback) {
  // API returns a message with date <= offset_date.
  // So we request a message with offset_date = desired_date - 1 and add_offset
  // = -1. This should give us the first message with date >= desired_date.
  const auto offsetId = 0;
  const auto offsetDate =
      static_cast<int>(date.startOfDay().toSecsSinceEpoch()) - 1;
  ...
  auto send = [&](auto &&serialized) {
    request(std::move(serialized))
        .done([=, callback = std::forward<Callback>(callback)](
                  const MTPmessages_Messages &result) {
  ...
  if (topicRootId) {
    send(MTPmessages_GetReplies(
        peer->input(), MTP_int(topicRootId), MTP_int(offsetId),
        MTP_int(offsetDate), MTP_int(addOffset), MTP_int(limit), MTP_int(maxId),
        MTP_int(minId), MTP_long(historyHash)));
  } else if (monoforumPeerId) {
    send(MTPmessages_GetSavedHistory(
        MTP_flags(MTPmessages_GetSavedHistory::Flag::f_parent_peer),
        peer->input(), session().data().peer(monoforumPeerId)->input(),
        MTP_int(offsetId), MTP_int(offsetDate), MTP_int(addOffset),
        MTP_int(limit), MTP_int(maxId), MTP_int(minId), MTP_long(historyHash)));
  } else {
    send(MTPmessages_GetHistory(peer->input(), MTP_int(offsetId),
                                MTP_int(offsetDate), MTP_int(addOffset),
                                MTP_int(limit), MTP_int(maxId), MTP_int(minId),
                                MTP_long(historyHash)));
  }
}

void ApiWrap::resolveJumpToHistoryDate(
    not_null<PeerData *> peer, MsgId topicRootId, PeerId monoforumPeerId,
    const QDate &date, Fn<void(not_null<PeerData *>, MsgId)> callback) {
  if (const auto channel = peer->migrateTo()) {
    return resolveJumpToHistoryDate(channel, topicRootId, monoforumPeerId, date,
                                    std::move(callback));
  }
  const auto jumpToDateInPeer = [=] {
    requestMessageAfterDate(peer, topicRootId, monoforumPeerId, date,
                            [=](MsgId itemId) { callback(peer, itemId); });
  };
  const auto migrated =
      (topicRootId || monoforumPeerId) ? nullptr : peer->migrateFrom();
  if (migrated) {
    requestMessageAfterDate(migrated, MsgId(), PeerId(), date,
                            [=](MsgId itemId) {
                              if (itemId) {
...
void ApiWrap::requestHistory(not_null<History *> history, MsgId messageId,
                             SliceType slice) {
  ...
  histories.sendRequest(history, requestType, [=](Fn<void()> finish) {
    return request(std::move(prepared))
        .done([=](const Api::HistoryRequestResult &result) {
          ...
          auto parsed = Api::ParseHistoryResult(peer, messageId, slice, result);
          history->messages().addSlice(std::move(parsed.messageIds),
                                       parsed.noSkipRange, parsed.fullCount);
          finish();
        })
        .fail([=] {
          _historyRequests.remove(key);
...
void ApiWrap::requestSharedMedia(not_null<PeerData *> peer, MsgId topicRootId,
                                 PeerId monoforumPeerId, SharedMediaType type,
                                 MsgId messageId, SliceType slice) {
  const auto key = SharedMediaRequest{
      peer, topicRootId, monoforumPeerId, type, messageId, slice,
  };
  ...
  const auto prepared = Api::PrepareSearchRequest(
      peer, topicRootId, monoforumPeerId, type, QString(), messageId, slice);
  if (!prepared) {
    return;
  }
  ...
  histories.sendRequest(history, requestType, [=](Fn<void()> finish) {
    return request(std::move(*prepared))
        .done([=](const Api::SearchRequestResult &result) {
          ...
          auto parsed =
              Api::ParseSearchResult(peer, type, messageId, slice, result);
          sharedMediaDone(peer, topicRootId, monoforumPeerId, type,
                          std::move(parsed));
          finish();
        })
        .fail([=] {
...
void ApiWrap::sharedMediaDone(not_null<PeerData *> peer, MsgId topicRootId,
                              PeerId monoforumPeerId, SharedMediaType type,
  ...
  _session->storage().add(Storage::SharedMediaAddSlice(
      peer->id, topicRootId, monoforumPeerId, type,
      std::move(parsed.messageIds), parsed.noSkipRange, parsed.fullCount));
...
mtpRequestId
ApiWrap::requestGlobalMedia(Storage::SharedMediaType type, const QString &query,
                            int32 offsetRate,
                            Data::MessagePosition offsetPosition,
                            Fn<void(Api::GlobalMediaResult)> done) {
  auto prepared = Api::PrepareGlobalMediaRequest(_session, offsetRate,
                                                 offsetPosition, type, query);
  if (!prepared) {
    done({});
    return 0;
  }
...
void ApiWrap::finishForwarding(const SendAction &action) {
  ...
  const auto topicRootId = action.replyTo.topicRootId;
  const auto monoforumPeerId = action.replyTo.monoforumPeerId;
  auto toForward = history->resolveForwardDraft(topicRootId, monoforumPeerId);
  if (!toForward.items.empty()) {
    const auto error =
        GetErrorForSending(history->peer, {
...
void ApiWrap::forwardMessages(Data::ResolvedForwardDraft &&draft,
                              SendAction action,
                              FnMut<void()> &&successCallback) {
  Expects(!draft.items.empty());

  const auto fullAyuForward =
  ...
  const auto ayuIntelligentForwardNeeded =
      AyuForward::isAyuForwardNeeded(draft.items);
  ...
  const auto shared = successCallback ? std::make_shared<SharedCallback>()
  ...
  const auto genClientSideMessage =
      action.generateLocal && (count < 2) &&
  ...
  const auto monoforumPeerId = action.replyTo.monoforumPeerId;
  const auto monoforumPeer =
      monoforumPeerId ? session().data().peer(monoforumPeerId).get() : nullptr;
  ...
  const auto sendAccumulated = [&] {
    ...
    const auto starsPaid =
        std::min(action.options.starsApproved,
    ...
    auto buildMessage =
        [=](not_null<History *> history,
            FullReplyTo replyTo) -> Data::Histories::PreparedMessage {
      const auto kGeneralId = Data::ForumTopic::kGeneralId;
      const auto realTopMsgId =
          (replyTo.topicRootId == kGeneralId) ? MsgId(0) : replyTo.topicRootId;
      auto flags = oneFlags;
      if (realTopMsgId) {
      ...
      return MTPmessages_ForwardMessages(
          MTP_flags(flags), forwardFrom->input(), MTP_vector<MTPint>(ids),
          MTP_vector<MTPlong>(randomIds), history->peer->input(),
          MTP_int(realTopMsgId),
          (action.options.suggest ? ReplyToForMTP(history, replyTo)
           : monoforumPeer ? MTP_inputReplyToMonoForum(monoforumPeer->input())
                           : MTPInputReplyTo()),
          MTP_int(action.options.scheduled),
          MTP_int(action.options.scheduleRepeatPeriod),
          (sendAs ? sendAs->input() : MTP_inputPeerEmpty()),
          Data::ShortcutIdToMTP(&history->session(), action.options.shortcutId),
          MTP_long(action.options.effectId), MTPint(), MTP_long(starsPaid),
          Api::SuggestToMTP(action.options.suggest));
    };
    histories.sendPreparedMessage(
        history, FullReplyTo{.topicRootId = topicRootId}, uint64(0),
        ...
        [=](const MTP::Error &error, const MTP::Response &) {
          if (idsCopy) {
            for (const auto &[randomId, itemId] : *idsCopy) {
              _session->api().sendMessageFail(error, peer, randomId, itemId);
            }
          } else {
  ...
  for (const auto &item : draft.items) {
    ...
    if (genClientSideMessage) {
      const auto newId =
          FullMsgId(peer->id, _session->data().nextLocalMessageId());
...
void ApiWrap::shareContact(const QString &phone, const QString &firstName,
                           const QString &lastName, const SendAction &action,
                           Fn<void(bool)> done) {
...
void ApiWrap::shareContact(not_null<UserData *> user, const SendAction &action,
                           Fn<void(bool)> done) {
  ...
  return sendSharedContact(phone, user->firstName, user->lastName, userId,
                           action, std::move(done));
}

void ApiWrap::sendSharedContact(const QString &phone, const QString &firstName,
                                const QString &lastName, UserId userId,
                                const SendAction &action, Fn<void(bool)> done) {
  sendAction(action);
  ...
  const auto item = history->addNewLocalMessage(
      ...
      TextWithEntities(),
      MTP_messageMediaContact(MTP_string(phone), MTP_string(firstName),
                              MTP_string(lastName),
                              MTP_string(), // vcard
                              MTP_long(userId.bare)));

  const auto media = MTP_inputMediaContact(
      MTP_string(phone), MTP_string(firstName), MTP_string(lastName),
      MTP_string()); // vcard
...
void ApiWrap::sendVoiceMessage(QByteArray result, VoiceWaveform waveform,
                               crl::time duration, bool video,
  ...
  _fileLoader->addTask(std::make_unique<FileLoadTask>(FileLoadTask::VoiceArgs{
      .session = &session(),
      .voice = result,
      .duration = duration,
      .waveform = waveform,
...
void ApiWrap::editMedia(Ui::PreparedList &&list, SendMediaType type,
                        TextWithTags &&caption, const SendAction &action) {
  if (list.files.empty())
    return;
  ...
  if (existing && existing->computeSuggestionActions() ==
    ...
    to.replyTo.messageId = {action.history->peer->id, to.replaceMediaOf};
...
void ApiWrap::sendFiles(Ui::PreparedList &&list, SendMediaType type,
  ...
  for (auto &file : list.files) {
    const auto uploadWithType = !album ? type
...
void ApiWrap::sendFile(const QByteArray &fileContent, SendMediaType type,
                       const SendAction &action) {
  const auto to = FileLoadTaskOptions(action);
  auto caption = TextWithTags();
  const auto spoiler = false;
  _fileLoader->addTask(
      std::make_unique<FileLoadTask>(FileLoadTask::Args{.session = &session(),
                                                        .filepath = QString(),
                                                        .content = fileContent,
                                                        .information = nullptr,
                                                        .videoCover = nullptr,
...
void ApiWrap::sendUploadedPhoto(FullMsgId localId, Api::RemoteFileInfo info,
...
void ApiWrap::sendUploadedDocument(FullMsgId localId, Api::RemoteFileInfo info,
                                   Api::SendOptions options) {
  if (const auto item = _session->data().message(localId)) {
    ...
    const auto media = Api::PrepareUploadedDocument(item, std::move(info));
...
void ApiWrap::sendShortcutMessages(not_null<PeerData *> peer,
                                   BusinessShortcutId id) {
  markReadAfterAction(peer->owner().history(peer));
  auto ids = QVector<MTPint>();
  auto randomIds = QVector<MTPlong>();
  request(MTPmessages_SendQuickReplyMessages(peer->input(), MTP_int(id),
...
void ApiWrap::sendMessage(MessageToSend &&message,
  ...
  const auto replyTo = action.replyTo.messageId
                           ...
                           : nullptr;
  const auto topicRootId = draftTopicRootId ? draftTopicRootId
                           : replyTo        ? replyTo->topicRootId()
                                            : Data::ForumTopic::kGeneralId;
  const auto topic = peer->forumTopicFor(topicRootId);

  const bool canSendTexts =
      topic ? Data::CanSendTexts(topic) : Data::CanSendTexts(peer);

  if (clearReplyTo) {
    message.action.replyTo.messageId =
        FullMsgId(message.action.replyTo.messageId.peer,
                  message.action.replyTo.topicRootId);
    action.replyTo.messageId =
        FullMsgId(action.replyTo.messageId.peer, action.replyTo.topicRootId);
  }

  if ((!canSendTexts && !AyuForward::isForwarding(peer->id)) ||
  ...
  auto prepareFlags = Ui::ItemTextOptions(history, _session->user()).flags;
  ...
  while (TextUtilities::CutPart(sending, left, MaxMessageSize) ||
    ...
    const auto ignoreWebPage =
        message.webPage.removed || (exactWebPage && !isLast);
    const auto manualWebPage = exactWebPage && !ignoreWebPage &&
    ...
    if (ignoreWebPage) {
      ...
      media = MTP_messageMediaWebPage(
          MTP_flags(PageFlag() |
                    ...
                    (fields.forceLargeMedia ? PageFlag::f_force_large_media
                                            : PageFlag()) |
                    (fields.forceSmallMedia ? PageFlag::f_force_small_media
                                            : PageFlag())),
          MTP_webPagePending(MTP_flags(PendingFlag::f_url), MTP_long(fields.id),
    ...
    const auto sentEntities = Api::EntitiesToMTP(
        _session, sendingNormalized.entities, Api::ConvertOption::SkipLocal);
    ...
    const auto starsPaid =
        std::min(peer->starsPerMessageChecked(), action.options.starsApproved);
    if (starsPaid) {
      action.options.starsApproved -= starsPaid;
    ...
    lastMessage = history->addNewLocalMessage(
        ...
        sending, media);
    const auto done = [=](const MTPUpdates &result,
                          const MTP::Response &response) {
      if (clearCloudDraft) {
        history->finishSavingCloudDraft(
    ...
    const auto mtpShortcut =
        Data::ShortcutIdToMTP(_session, action.options.shortcutId);
    if (exactWebPage && !ignoreWebPage &&
        (manualWebPage || sending.empty() || message.webPage.previewChanged)) {
      histories.sendPreparedMessage(
          history, action.replyTo, randomId,
          Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
              MTP_flags(mediaFlags), peer->input(),
              Data::Histories::ReplyToPlaceholder(),
              Data::WebPageForMTP(message.webPage, true), msgText,
              MTP_long(randomId), MTPReplyMarkup(), sentEntities,
              MTP_int(action.options.scheduled),
              MTP_int(action.options.scheduleRepeatPeriod),
              (sendAs ? sendAs->input() : MTP_inputPeerEmpty()), mtpShortcut,
              MTP_long(action.options.effectId), MTP_long(starsPaid),
              Api::SuggestToMTP(action.options.suggest)),
          done, fail);
    } else {
      histories.sendPreparedMessage(
          ...
          Data::Histories::PrepareMessage<MTPmessages_SendMessage>(
              MTP_flags(sendFlags), peer->input(),
              Data::Histories::ReplyToPlaceholder(), msgText,
              MTP_long(randomId), MTPReplyMarkup(), sentEntities,
              MTP_int(action.options.scheduled),
              MTP_int(action.options.scheduleRepeatPeriod),
              (sendAs ? sendAs->input() : MTP_inputPeerEmpty()), mtpShortcut,
...
void ApiWrap::sendBotStart(std::shared_ptr<Ui::Show> show,
                           not_null<UserData *> bot, PeerData *chat,
  ...
  if (token.isEmpty()) {
    auto message = MessageToSend(
        Api::SendAction(_session->data().history(chat ? chat : bot.get())));
  ...
  request(MTPmessages_StartBot(bot->inputUser(),
                               chat ? chat->input() : MTP_inputPeerEmpty(),
                               MTP_long(randomId), MTP_string(token)))
      .done([=](const MTPUpdates &result) {
        applyUpdates(result);
...
void ApiWrap::sendInlineResult(not_null<UserData *> bot,
  ...
  const auto newId = FullMsgId(
      peer->id, localMessageId ? (*localMessageId)
                               : _session->data().nextLocalMessageId());
  const auto randomId = base::RandomValue<uint64>();
  const auto topicRootId =
      action.replyTo.messageId ? action.replyTo.topicRootId : 0;
  const auto monoforumPeerId = action.replyTo.monoforumPeerId;
  ...
  data->addToHistory(
      history,
      {
          ...
          .shortcutId = action.options.shortcutId,
          .starsPaid = starsPaid,
          .viaBotId = ((bot && !action.options.hideViaBot) ? peerToUser(bot->id)
                                                           : UserId()),
          .postAuthor = NewMessagePostAuthor(action),
      });
  ...
  histories.sendPreparedMessage(
      ...
      Data::Histories::PrepareMessage<MTPmessages_SendInlineBotResult>(
          MTP_flags(sendFlags), peer->input(),
          Data::Histories::ReplyToPlaceholder(), MTP_long(randomId),
          MTP_long(data->getQueryId()), MTP_string(data->getId()),
          MTP_int(action.options.scheduled),
          (sendAs ? sendAs->input() : MTP_inputPeerEmpty()),
          Data::ShortcutIdToMTP(_session, action.options.shortcutId),
          MTP_long(starsPaid)),
...
void ApiWrap::uploadAlbumMedia(not_null<HistoryItem *> item,
  ...
  request(MTPmessages_UploadMedia(MTP_flags(0),
                                  MTPstring(), // business_connection_id
                                  item->history()->peer->input(), media))
      .done([=](const MTPMessageMedia &result) {
        ...
        case mtpc_messageMediaPhoto: {
          ...
          const auto flags =
              Flag() | (data.vttl_seconds() ? Flag::f_ttl_seconds : Flag()) |
              (spoiler ? Flag::f_spoiler : Flag());
          const auto media = MTP_inputMediaPhoto(
              MTP_flags(flags),
              MTP_inputPhoto(fields.vid(), fields.vaccess_hash(),
        ...
        case mtpc_messageMediaDocument: {
          ...
          const auto media = MTP_inputMediaDocument(
              MTP_flags(flags),
              MTP_inputDocument(fields.vid(), fields.vaccess_hash(),
                                fields.vfile_reference()),
              (cover ? MTP_inputPhoto(cover->vid(), cover->vaccess_hash(),
...
void ApiWrap::sendMedia(not_null<HistoryItem *> item,
                        const MTPInputMedia &media, Api::SendOptions options,
                        Fn<void(bool)> done) {
  const auto randomId = base::RandomValue<uint64>();
...
void ApiWrap::sendMediaWithRandomId(not_null<HistoryItem *> item,
                                    const MTPInputMedia &media,
                                    Api::SendOptions options, uint64 randomId,
                                    Fn<void(bool)> done) {
  ...
  auto sentEntities = Api::EntitiesToMTP(_session, captionNormalized.entities,
                                         Api::ConvertOption::SkipLocal);

  const auto updateRecentStickers = Api::HasAttachedStickers(media);
  const auto starsPaid =
      std::min(peer->starsPerMessageChecked(), options.starsApproved);
  if (starsPaid) {
  ...
  const auto flags =
      Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
      (ShouldSendSilent(history->peer, options) ? Flag::f_silent : Flag(0)) |
  ...
  const auto itemId = item->fullId();
  histories.sendPreparedMessage(
      history, replyTo, randomId,
      Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
          MTP_flags(flags), peer->input(),
          Data::Histories::ReplyToPlaceholder(),
          (options.price
               ? MTPInputMedia(MTP_inputMediaPaidMedia(
                     MTP_flags(0), MTP_long(options.price),
                     MTP_vector<MTPInputMedia>(1, media), MTPstring()))
               : media),
          MTP_string(caption.text), MTP_long(randomId), MTPReplyMarkup(),
          sentEntities, MTP_int(options.scheduled),
          MTP_int(options.scheduleRepeatPeriod),
          (options.sendAs ? options.sendAs->input() : MTP_inputPeerEmpty()),
          Data::ShortcutIdToMTP(_session, options.shortcutId),
          MTP_long(options.effectId), MTP_long(starsPaid),
...
void ApiWrap::sendMultiPaidMedia(not_null<HistoryItem *> item,
  ...
  auto medias = album->items |
                ranges::view::transform([](const SendingAlbum::Item &part) {
  ...
  const auto flags =
      ...
      (!sentEntities.v.isEmpty() ? Flag::f_entities : Flag(0)) |
      (options.scheduled ? Flag::f_schedule_date : Flag(0)) |
      (options.scheduleRepeatPeriod ? Flag::f_schedule_repeat_period
                                    : Flag(0)) |
      (options.sendAs ? Flag::f_send_as : Flag(0)) |
      (options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) |
      (options.effectId ? Flag::f_effect : Flag(0)) |
  ...
  histories.sendPreparedMessage(
      ...
      Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
          MTP_flags(flags), peer->input(),
          Data::Histories::ReplyToPlaceholder(),
          MTP_inputMediaPaidMedia(MTP_flags(0), MTP_long(options.price),
...
void ApiWrap::sendAlbumWithUploaded(not_null<HistoryItem *> item,
                                    const MessageGroupId &groupId,
                                    const MTPInputMedia &media) {
  const auto localId = item->fullId();
  const auto randomId = base::RandomValue<uint64>();
...
void ApiWrap::sendAlbumWithCancelled(not_null<HistoryItem *> item,
                                     const MessageGroupId &groupId) {
  const auto albumIt = _sendingAlbums.find(groupId.raw());
...
void ApiWrap::sendAlbumIfReady(not_null<SendingAlbum *> album) {
  ...
  if (!sample) {
    ...
    album->sent = true;
    sendMediaWithRandomId(sample, single.vmedia(), album->options,
                          single.vrandom_id().v);
    _sendingAlbums.remove(groupId);
    return;
  }
  ...
  const auto starsPaid =
      std::min(history->peer->starsPerMessageChecked() * int(medias.size()),
  ...
  const auto flags =
      Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
      (ShouldSendSilent(history->peer, album->options) ? Flag::f_silent
                                                       : Flag(0)) |
      ...
      (album->options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) |
  ...
  histories.sendPreparedMessage(
      ...
      Data::Histories::PrepareMessage<MTPmessages_SendMultiMedia>(
          ...
          MTP_long(album->options.effectId), MTP_long(starsPaid)),
      [=](const MTPUpdates &result, const MTP::Response &response) {
        _sendingAlbums.remove(groupId);
      },
      [=](const MTP::Error &error, const MTP::Response &response) {
...
void ApiWrap::reloadContactSignupSilent() {
  ...
  const auto requestId = request(MTPaccount_GetContactSignUpNotification())
                             ...
                             .fail([=] { _contactSignupSilentRequestId = 0; })
                             .send();
...
void ApiWrap::saveContactSignupSilent(bool silent) {
  ...
  const auto requestId =
      request(MTPaccount_SetContactSignUpNotification(MTP_bool(silent)))
          .done([=] {
            _contactSignupSilentRequestId = 0;
            _contactSignupSilent = silent;
            _contactSignupSilentChanges.fire_copy(silent);
...
void ApiWrap::requestBotCommonGroups(not_null<UserData *> bot,
                                     Fn<void()> done) {
  ...
  request(MTPmessages_GetCommonChats(bot->inputUser(),
                                     MTP_long(0), // max_id
                                     MTP_int(limit)))
      .done([=](const MTPmessages_Chats &result) {
        const auto chats =
            result.match([](const auto &data) { return &data.vchats().v; });
...
void ApiWrap::checkStatsSessions() {
  for (auto i = begin(_statsRequests); i != end(_statsRequests);) {
    if (i->second.empty()) {
      instance().killSession(MTP::ShiftDcId(i->first, MTP::kStatsDcShift));
...
Api::Authorizations &ApiWrap::authorizations() { return *_authorizations; }

Api::AttachedStickers &ApiWrap::attachedStickers() {
  return *_attachedStickers;
}

Api::BlockedPeers &ApiWrap::blockedPeers() { return *_blockedPeers; }

Api::CloudPassword &ApiWrap::cloudPassword() { return *_cloudPassword; }

Api::SelfDestruct &ApiWrap::selfDestruct() { return *_selfDestruct; }

Api::SensitiveContent &ApiWrap::sensitiveContent() {
  return *_sensitiveContent;
}

Api::GlobalPrivacy &ApiWrap::globalPrivacy() { return *_globalPrivacy; }

Api::ReactionsNotifySettings &ApiWrap::reactionsNotifySettings() {
  return *_reactionsNotifySettings;
}

Api::UserPrivacy &ApiWrap::userPrivacy() { return *_userPrivacy; }

Api::InviteLinks &ApiWrap::inviteLinks() { return *_inviteLinks; }

Api::ChatLinks &ApiWrap::chatLinks() { return *_chatLinks; }

Api::ViewsManager &ApiWrap::views() { return *_views; }

Api::ReadMetrics &ApiWrap::readMetrics() { return *_readMetrics; }

Api::ConfirmPhone &ApiWrap::confirmPhone() { return *_confirmPhone; }

Api::PeerPhoto &ApiWrap::peerPhoto() { return *_peerPhoto; }

Api::Polls &ApiWrap::polls() { return *_polls; }

Api::TodoLists &ApiWrap::todoLists() { return *_todoLists; }

Api::ChatParticipants &ApiWrap::chatParticipants() {
  return *_chatParticipants;
}

Api::UnreadThings &ApiWrap::unreadThings() { return *_unreadThings; }

Api::Ringtones &ApiWrap::ringtones() { return *_ringtones; }

Api::ComposeWithAi &ApiWrap::composeWithAi() { return *_composeWithAi; }

Api::Transcribes &ApiWrap::transcribes() { return *_transcribes; }

Api::Premium &ApiWrap::premium() { return *_premium; }

Api::Usernames &ApiWrap::usernames() { return *_usernames; }

Api::Websites &ApiWrap::websites() { return *_websites; }

Api::PeerColors &ApiWrap::peerColors() { return *_peerColors; }

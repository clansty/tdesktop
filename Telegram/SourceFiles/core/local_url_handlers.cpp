...
void PersonalChannelController::prepare() {
  setDescription(object_ptr<Ui::FlatLabel>(nullptr, tr::lng_contacts_loading(),
                                           computeListSt().about));
      ...
      _window->session()
          ...
          .done([=](const MTPmessages_Chats &result) {
              ...
              if (const auto peer = owner->processChat(chat)) {
                ...
                if (channel && !delegate()->peerListFindRow(rowId)) {
                  ...
                  row->setCustomStatus(tr::lng_chat_status_subscribers(
                      tr::now, lt_count, channel->membersCount()));
            ...
            if (!delegate()->peerListFullRowsCount()) {
              auto none =
                  rpl::combine(tr::lng_settings_channel_no_yet(tr::marked),
                               tr::lng_settings_channel_start()) |
                  rpl::map([](TextWithEntities &&text, const QString &link) {
                    return text.append('\n').append(tr::link(link));
                  });
              auto label = object_ptr<Ui::FlatLabel>(nullptr, std::move(none),
                                                     computeListSt().about);
...
ApplyAccountIndex(not_null<Window::SessionController *> controller,
                  int accountIndex) {
  if (accountIndex <= 0) {
    return nullptr;
  }
...
void SavePersonalChannel(not_null<Window::SessionController *> window,
                         ChannelData *channel) {
  const auto self = window->session().user();
  const auto history =
      channel ? channel->owner().history(channel->id).get() : nullptr;
  const auto item = history ? history->lastServerMessage() : nullptr;
  const auto channelId = channel ? peerToChannel(channel->id) : ChannelId();
  const auto messageId = item ? item->id : MsgId();
  if (self->personalChannelId() != channelId ||
      (messageId && self->personalChannelMessageId() != messageId)) {
    self->setPersonalChannel(channelId, messageId);
    self->session()
        .api()
        .request(MTPaccount_UpdatePersonalChannel(
            channel ? channel->inputChannel() : MTP_inputChannelEmpty()))
        .done(crl::guard(window,
                         [=] {
                           window->showToast(
                               (channel ? tr::lng_settings_channel_saved
                                        : tr::lng_settings_channel_removed)(
                                   tr::now));
                         }))
...
bool JoinGroupByHash(Window::SessionController *controller, const Match &match,
...
bool JoinFilterBySlug(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ShowStickerSet(Window::SessionController *controller, const Match &match,
                    const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  controller->show(Box<StickerSetBox>(
      ...
      (match->captured(1) == "addemoji" ? Data::StickersType::Emoji
                                        : Data::StickersType::Stickers)));
  controller->window().activate();
  return true;
}

bool ShowTheme(Window::SessionController *controller, const Match &match,
               const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  controller->session().data().cloudThemes().resolve(
      &controller->window(), match->captured(1), fromMessageId);
  controller->window().activate();
  return true;
}
...
bool SetLanguage(Window::SessionController *controller, const Match &match,
                 const QVariant &context) {
...
bool ShareUrl(Window::SessionController *controller, const Match &match,
              const QVariant &context) {
  if (!controller) {
    return false;
  }
  const auto params = url_parse_params(match->captured(1),
...
bool ConfirmPhone(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  controller->session().api().confirmPhone().resolve(controller, phone, hash);
  controller->window().activate();
  return true;
}

bool ApplySocksProxy(Window::SessionController *controller, const Match &match,
                     const QVariant &context) {
  auto params = url_parse_params(match->captured(1),
                                 qthelp::UrlParamNameTransform::ToLower);
  ProxiesBoxController::ShowApplyConfirmation(
      controller, MTP::ProxyData::Type::Socks5, params);
  if (controller) {
    controller->window().activate();
  }
  return true;
...
bool ApplyMtprotoProxy(Window::SessionController *controller,
                       const Match &match, const QVariant &context) {
  auto params = url_parse_params(match->captured(1),
                                 qthelp::UrlParamNameTransform::ToLower);
  auto &secret = params[u"secret"_q];
  secret.replace('+', '-').replace('/', '_');
  ProxiesBoxController::ShowApplyConfirmation(
      controller, MTP::ProxyData::Type::Mtproto, params);
  if (controller) {
    controller->window().activate();
  }
  return true;
...
bool ShowPassportForm(Window::SessionController *controller,
  ...
  const auto nonce = params.value(Passport::NonceNameByScope(scope), QString());
  controller->showPassportForm(
      Passport::FormRequest(botId, scope, callback, publicKey, nonce));
  return true;
}

bool ShowPassport(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  return ShowPassportForm(
      controller, url_parse_params(match->captured(1),
                                   qthelp::UrlParamNameTransform::ToLower));
}

bool ShowWallPaper(Window::SessionController *controller, const Match &match,
                   const QVariant &context) {
  if (!controller) {
    return false;
  }
...
[[nodiscard]] ChatAdminRights ParseRequestedAdminRights(const QString &value) {
...
bool ResolveUsernameOrPhone(Window::SessionController *controller,
                            const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  if (params.contains(u"acc"_q)) {
    const auto switched =
        ApplyAccountIndex(controller, params.value(u"acc"_q).toInt());
    if (switched) {
      controller = switched;
    } else {
      controller->showToast(
          u"Could not activate account %1."_q.arg(params.value(u"acc"_q)));
      return false;
    }
  }
  ...
  if (domainParam == u"giftcode"_q && !appnameParam.isEmpty()) {
    ...
    const auto toId = !item                ? PeerId()
                      : (fromId == selfId) ? item->history()->peer->id
                                           : selfId;
  ...
  const auto webChannelPreviewLink =
      (domainParam == u"s"_q) && !appnameParam.isEmpty();
  const auto domain = webChannelPreviewLink ? appnameParam : domainParam;
  const auto phone = params.value(u"phone"_q);
  const auto validDomain = [](const QString &domain) {
    return qthelp::regex_match(u"^[a-zA-Z0-9\\.\\_]+$"_q, domain, {}).valid();
  };
  ...
  auto resolveType = params.contains(u"profile"_q) ? ResolveType::Profile
                                                   : ResolveType::Default;
  ...
  const auto pollOption = PollOptionFromLink(params.value(u"option"_q));
  ...
  controller->showPeerByLink(Window::PeerByLinkInfo{
      ...
      .videoTimestamp = (!videot.isEmpty() ? ParseVideoTimestamp(videot)
                                           : std::optional<TimeId>()),
      .text = params.value(u"text"_q),
      .repliesInfo =
          commentId  ? Window::RepliesByLinkInfo{Window::CommentId{commentId}}
          : threadId ? Window::RepliesByLinkInfo{Window::ThreadId{threadId}}
      ...
      .attachBotToggleCommand =
          (params.contains(u"startattach"_q) ? params.value(u"startattach"_q)
           : (appname.isEmpty() && params.contains(u"startapp"_q))
               ? params.value(u"startapp"_q)
               : std::optional<QString>()),
      .attachBotMainOpen =
          (appname.isEmpty() && params.contains(u"startapp"_q)),
      .attachBotMainCompact =
          (appname.isEmpty() && params.contains(u"startapp"_q) &&
           (params.value(u"mode"_q) == u"compact"_q)),
      .attachBotChooseTypes =
          InlineBots::ParseChooseTypes(params.value(u"choose"_q)),
      ...
      .historyInNewWindow = (params.value(u"tdesktop_target"_q) == u"blank"_q),
  });
  return true;
}

bool ResolvePrivatePost(Window::SessionController *controller,
                        const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  const auto params = url_parse_params(match->captured(1),
                                       qthelp::UrlParamNameTransform::ToLower);
  const auto channelId = ChannelId(params.value(u"channel"_q).toULongLong());
...
bool HandleUnknown(Window::SessionController *controller, const Match &match,
                   const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  const auto callback = crl::guard(
      controller, [=](TextWithEntities message, bool updateRequired) {
        if (updateRequired) {
...
bool HandleOpenMessage(Window::SessionController *controller,
                       const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  const auto peer =
      controller->session().data().peerLoaded(static_cast<PeerId>(userId));
  if (peer != nullptr) {
...
bool HandleUser(Window::SessionController *controller, const Match &match,
                const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool OpenMediaTimestamp(Window::SessionController *controller,
                        const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  if (base.startsWith(u"doc"_q)) {
    ...
    const auto itemId = FullMsgId(PeerId(parts.value(1).toULongLong()),
    ...
    if (document->isVideoFile()) {
      controller->window().openInMediaView(Media::View::OpenRequest(
          controller, document, context,
          context ? context->topicRootId() : MsgId(0),
          context ? context->sublistPeerId() : PeerId(0), false, time));
...
bool ShowInviteLink(Window::SessionController *controller, const Match &match,
                    const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool OpenExternalLink(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  return Ui::Integration::Instance().handleUrlClick(match->captured(1),
                                                    context);
}

bool CopyPeerId(Window::SessionController *controller, const Match &match,
                const QVariant &context) {
...
bool ShowSearchTagsPromo(Window::SessionController *controller,
                         const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ShowEditBirthday(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  if (!controller) {
    return false;
  ...
  if (captured.startsWith(u":suggest:"_q)) {
    ...
    const auto targetUser = controller->session().data().userLoaded(userId);
    if (!targetUser) {
      return false;
    }
    ...
    const auto save = [=](Data::Birthday result) {
      ...
      controller->session()
          .api()
          .request(MTPusers_SuggestBirthday(
              targetUser->inputUser(),
              MTP_birthday(MTP_flags(result.year() ? BFlag::f_year : BFlag()),
                           MTP_int(result.day()), MTP_int(result.month()),
                           MTP_int(result.year()))))
                           ...
                           [=] {
                             ...
                             controller->showToast(
                                 tr::lng_settings_birthday_suggested(
                                     tr::now, lt_user, targetUser->name()));
                           }))
          .fail(crl::guard(controller,
                           [=](const MTP::Error &error) {
                             const auto type = error.type();
                             controller->showToast(
                                 type.startsWith(u"FLOOD_WAIT_"_q)
    ...
    controller->show(Box([=](not_null<Ui::GenericBox *> box) {
      box->setTitle(tr::lng_suggest_birthday_box_title(
          lt_user, Info::Profile::NameValue(targetUser)));
      Ui::EditBirthdayBox(box, Data::Birthday(), save,
  ...
  const auto save = [=](Data::Birthday result) {
    ...
    user->session()
        .api()
        .request(MTPaccount_UpdateBirthday(
        ...
        .done(crl::guard(controller,
                         [=] {
                           controller->showToast(
                               tr::lng_settings_birthday_saved(tr::now));
                         }))
        .fail(crl::guard(controller,
                         [=](const MTP::Error &error) {
                           const auto type = error.type();
  ...
  if (captured.startsWith(u":suggestion_"_q)) {
    ...
    controller->show(Box(Ui::EditBirthdayBox, suggested, save,
      ...
      auto isExactlyContacts =
          ...
          rpl::map([=](const Api::UserPrivacy::Rule &value) {
            return (value.option == Api::UserPrivacy::Option::Contacts) &&
                   value.always.peers.empty() && !value.always.premiums &&
      ...
      Ui::AddDividerText(
          ...
          rpl::conditional(
              ...
              tr::lng_settings_birthday_about(
                  lt_link, tr::lng_settings_birthday_about_link(tr::url(link)),
                  tr::marked)));
    }));
  }
  return true;
...
bool ShowEditBirthdayPrivacy(Window::SessionController *controller,
                             const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  auto syncLifetime =
      ...
      rpl::take(1) | rpl::on_next([=](const Api::UserPrivacy::Rule &value) {
        if (isFromBox) {
          using namespace ::Settings;
          class Controller final : public BirthdayPrivacyController {
            object_ptr<Ui::RpWidget>
            setupAboveWidget(not_null<Window::SessionController *> controller,
          ...
          controller->show(Box<EditPrivacyBox>(
              controller, std::make_unique<Controller>(), value));
          return;
        }
        controller->show(Box<EditPrivacyBox>(
            controller,
            std::make_unique<::Settings::BirthdayPrivacyController>(), value));
      });
  return true;
}

bool ShowEditPersonalChannel(Window::SessionController *controller,
                             const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  } else if (controller->showFrozenError()) {
  ...
  auto listController = std::make_unique<PersonalChannelController>(controller);
  ...
  auto initBox = [=](not_null<PeerListBox *> box) {
    ...
    rawController->chosen() |
        rpl::on_next([=](not_null<ChannelData *> channel) { save(channel); },
                     box->lifetime());
...
bool ShowCollectiblePhone(Window::SessionController *controller,
                          const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  controller->resolveCollectible(peerId,
                                 phone.startsWith('+') ? phone : '+' + phone);
  return true;
}

bool ShowCollectibleUsername(Window::SessionController *controller,
                             const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool CopyUsernameLink(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool CopyUsername(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool EditPaidMessagesFee(Window::SessionController *controller,
                         const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ShowCommonGroups(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  if (const auto id = peerToUser(peerId)) {
    ...
    if (user) {
      controller->showSection(std::make_shared<Info::Memento>(
          user, Info::Section::Type::CommonGroups));
    }
  }
  return true;
}

bool EditPeer(Window::SessionController *controller, const Match &match,
              const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ShowStarsExamples(Window::SessionController *controller,
                       const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ShowPopularAppsAbout(Window::SessionController *controller,
                          const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
void ExportTestChatTheme(not_null<Window::SessionController *> controller,
                         not_null<const Data::CloudTheme *> theme) {
  const auto session = &controller->session();
  const auto show = controller->uiShow();
  const auto inputSettings =
      [&](Data::CloudThemeType type) -> std::optional<MTPInputThemeSettings> {
    ...
    if (!fields.paper || !fields.paper->isPattern() ||
    ...
    const auto color = [](const QColor &color) {
      ...
      return int(((uint32(red) & 0xFFU) << 16) |
                 ((uint32(green) & 0xFFU) << 8) | (uint32(blue) & 0xFFU));
    };
    ...
    const auto settings =
        Setting::f_wallpaper | Setting::f_wallpaper_settings |
        (fields.outgoingAccentColor ? Setting::f_outbox_accent_color
                                    : Setting(0)) |
        (!fields.outgoingMessagesColors.empty() ? Setting::f_message_colors
                                                : Setting(0));
    const auto papers =
        Paper::f_background_color | Paper::f_intensity |
        (bg.size() > 1 ? Paper::f_second_background_color : Paper(0)) |
        (bg.size() > 2 ? Paper::f_third_background_color : Paper(0)) |
        (bg.size() > 3 ? Paper::f_fourth_background_color : Paper(0));
    return MTP_inputThemeSettings(
        MTP_flags(settings),
        ((type == Data::CloudThemeType::Dark) ? MTP_baseThemeTinted()
                                              : MTP_baseThemeClassic()),
        MTP_int(color(fields.accentColor)),
        MTP_int(color(fields.outgoingAccentColor.value_or(Qt::black))),
        MTP_vector<MTPint>(colors(fields.outgoingMessagesColors)),
        MTP_inputWallPaperSlug(MTP_string(slug)),
        MTP_wallPaperSettings(MTP_flags(papers), MTP_int(color(bg[0])),
...
bool ResolveTestChatTheme(Window::SessionController *controller,
                          const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  if (const auto history = controller->activeChatCurrent().history()) {
    ...
    const auto theme = owner->cloudThemes().updateThemeFromLink(
        history->peer->themeToken(), params);
    if (theme) {
      ...
      const auto recache = [&](Data::CloudThemeType type) {
        [[maybe_unused]] auto value =
            theme->settings.contains(type)
                ? controller->cachedChatThemeValue(*theme, Data::WallPaper(0),
                                                   type)
                : nullptr;
      };
...
bool ResolveInvoice(Window::SessionController *controller, const Match &match,
                    const QVariant &context) {
  if (!controller) {
    return false;
  }
  ...
  Payments::CheckoutProcess::Start(
      &controller->session(), slug,
      crl::guard(window, [=](auto) { window->activate(); }),
      Payments::ProcessNonPanelPaymentFormFactory(controller));
  return true;
}

bool ResolvePremiumOffer(Window::SessionController *controller,
                         const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
  const auto params = url_parse_params(match->captured(1).mid(1),
                                       qthelp::UrlParamNameTransform::ToLower);
  const auto refAddition = params.value(u"ref"_q);
  const auto ref =
      "deeplink" + (refAddition.isEmpty() ? QString() : '_' + refAddition);
  ::Settings::ShowPremium(controller, ref);
  controller->window().activate();
  return true;
}

bool ResolvePremiumMultigift(Window::SessionController *controller,
                             const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveLoginCode(Window::SessionController *controller, const Match &match,
                      const QVariant &context) {
  ...
  (controller ? controller->session().account() : domain.active())
...
bool ResolveBoost(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveTopUp(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  if (!controller) {
    return false;
  }
  const auto params = url_parse_params(match->captured(1),
                                       qthelp::UrlParamNameTransform::ToLower);
  const auto amount = std::clamp(params.value(u"balance"_q).toULongLong(),
                                 qulonglong(1), qulonglong(1'000'000));
      ...
      if (const auto strong = weak.get()) {
        ...
        strong->showToast(Ui::Toast::Config{
            .text = tr::lng_credits_enough(
                tr::now, lt_link,
                tr::link(tr::bold(tr::lng_credits_enough_link(tr::now))),
                tr::rich),
            .filter = filter,
  ...
  ::Settings::MaybeRequestBalanceIncrease(
      controller->uiShow(), amount,
      ::Settings::SmallBalanceDeepLink{.purpose = purpose}, done);
  return true;
}

bool ResolveChatLink(Window::SessionController *controller, const Match &match,
                     const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveUniqueGift(Window::SessionController *controller,
                       const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveGiftAuction(Window::SessionController *controller,
                        const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveConferenceCall(Window::SessionController *controller,
                           const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveStarsSettings(Window::SessionController *controller,
                          const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveTonSettings(Window::SessionController *controller,
                        const Match &match, const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool ResolveOAuth(Window::SessionController *controller, const Match &match,
                  const QVariant &context) {
  if (!controller) {
    return false;
  }
...
bool TryRouterForLocalUrl(Window::SessionController *controller,
...
const std::vector<LocalUrlHandler> &LocalUrlHandlers() {
  static auto Result = std::vector<LocalUrlHandler>{
      {u"^join/?\\?invite=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q, JoinGroupByHash},
      {u"^addlist/?\\?slug=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q, JoinFilterBySlug},
      {u"^(addstickers|addemoji)/?\\?set=([a-zA-Z0-9\\.\\_]+)(&|$)"_q,
       ShowStickerSet},
      {u"^addtheme/?\\?slug=([a-zA-Z0-9\\.\\_]+)(&|$)"_q, ShowTheme},
      {u"^addstyle/?\\?slug=([a-zA-Z0-9\\.\\_]+)(&|$)"_q, ShowAiStyle},
      {u"^setlanguage/?(\\?lang=([a-zA-Z0-9\\.\\_\\-]+))?(&|$)"_q, SetLanguage},
      {u"^msg_url/?\\?(.+)(#|$)"_q, ShareUrl},
      {u"^confirmphone/?\\?(.+)(#|$)"_q, ConfirmPhone},
      {u"^socks/?\\?(.+)(#|$)"_q, ApplySocksProxy},
      {u"^proxy/?\\?(.+)(#|$)"_q, ApplyMtprotoProxy},
      {u"^passport/?\\?(.+)(#|$)"_q, ShowPassport},
      {u"^bg/?\\?(.+)(#|$)"_q, ShowWallPaper},
      {u"^resolve/?\\?(.+)(#|$)"_q, ResolveUsernameOrPhone},
      {u"^privatepost/?\\?(.+)(#|$)"_q, ResolvePrivatePost},
      {
          u"^test_chat_theme/?\\?(.+)(#|$)"_q,
          ResolveTestChatTheme,
      },
      {qsl("^openmessage\\?(.+)(#|$)"), HandleOpenMessage},
      {qsl("^user\\?(.+)(#|$)"), HandleUser},
      {
          u"^invoice/?\\?(.+)(#|$)"_q,
          ResolveInvoice,
      ...
      {u"^login/?(\\?code=([0-9]+))(&|$)"_q, ResolveLoginCode},
      {
          u"^boost/?\\?(.+)(#|$)"_q,
          ResolveBoost,
      },
      {u"^message/?\\?slug=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q, ResolveChatLink},
      {u"^stars_topup/?\\?(.+)(#|$)"_q, ResolveTopUp},
      {u"^nft/?\\?slug=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q, ResolveUniqueGift},
      {u"^stargift_auction/?\\?slug=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q,
       ResolveGiftAuction},
      {u"^call/?\\?slug=([a-zA-Z0-9\\.\\_\\-]+)(&|$)"_q, ResolveConferenceCall},
      {u"^stars/?(^\\?.*)?(#|$)"_q, ResolveStarsSettings},
      {u"^ton/?(^\\?.*)?(#|$)"_q, ResolveTonSettings},
      {u"^oauth/?\\?(.+)(#|$)"_q, ResolveOAuth},
      {u"^user\\?(.+)(#|$)"_q, AyuUrlHandlers::ResolveUser},
      {u"^chat\\?(.+)(#|$)"_q, AyuUrlHandlers::ResolveChat},
      {u"^ayusettings/?\\?(.+)(#|$)"_q, AyuUrlHandlers::HandleAyuSettings},
      {u"^ayusettings/?$"_q, AyuUrlHandlers::HandleAyuSettings},
      {u"^ayu(/?.+)?(#|$)"_q, AyuUrlHandlers::HandleAyu},
      {u"^(support)|(donate)$"_q, AyuUrlHandlers::HandleSupport},
      {u"^([^\\?]+)(\\?|#|$)"_q, HandleUnknown},
  };
  return Result;
}
...
const std::vector<LocalUrlHandler> &InternalUrlHandlers() {
  static auto Result = std::vector<LocalUrlHandler>{
      ...
      {u"^copy:(.+)$"_q, CopyPeerId},
      {u"^about_tags$"_q, ShowSearchTagsPromo},
      {
          u"^edit_birthday(.*)$"_q,
          ShowEditBirthday,
      },
...
QString TryConvertUrlToLocal(QString url) {
  ...
  auto subdomainMatch = regex_match(
      u"^(https?://)?([a-zA-Z0-9\\_]+)\\.t\\.me(/\\d+)?/?(\\?.+)?"_q, url,
      matchOptions);
  if (subdomainMatch) {
    const auto name = subdomainMatch->captured(2);
    if (name.size() > 1 && name != "www") {
      const auto result = TryConvertUrlToLocal(
          subdomainMatch->captured(1) + "t.me/" + name +
          subdomainMatch->captured(3) + subdomainMatch->captured(4));
      return result.startsWith("tg://resolve?domain=") ? result : url;
    }
  }
  auto telegramMeMatch =
      regex_match(u"^(https?://)?(www\\.)?(telegram\\.(me|dog)|t\\.me)/(.+)$"_q,
                  url, matchOptions);
  if (telegramMeMatch) {
    const auto query = telegramMeMatch->capturedView(5);
    if (const auto phoneMatch =
            regex_match(u"^\\+([0-9]+)(\\?|$)"_q, query, matchOptions)) {
      const auto params = query.mid(phoneMatch->captured(0).size()).toString();
      return u"tg://resolve?phone="_q + phoneMatch->captured(1) +
             (params.isEmpty() ? QString() : '&' + params);
    } else if (const auto joinChatMatch = regex_match(
                   u"^(joinchat/|\\+|\\%20)([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q,
                   query, matchOptions)) {
      return u"tg://join?invite="_q + url_encode(joinChatMatch->captured(2));
    } else if (const auto joinFilterMatch =
                   regex_match(u"^(addlist/)([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q,
                               query, matchOptions)) {
      return u"tg://addlist?slug="_q + url_encode(joinFilterMatch->captured(2));
    } else if (const auto stickerSetMatch = regex_match(
                   u"^(addstickers|addemoji)/([a-zA-Z0-9\\.\\_]+)(\\?|$)"_q,
                   query, matchOptions)) {
      return u"tg://"_q + stickerSetMatch->captured(1) +
             "?set=" + url_encode(stickerSetMatch->captured(2));
    } else if (const auto themeMatch =
                   regex_match(u"^addtheme/([a-zA-Z0-9\\.\\_]+)(\\?|$)"_q,
                               query, matchOptions)) {
      return u"tg://addtheme?slug="_q + url_encode(themeMatch->captured(1));
    } else if (const auto addStyleMatch =
                   regex_match(u"^addstyle/([a-zA-Z0-9\\.\\_]+)(\\?|$)"_q,
                               query, matchOptions)) {
      return u"tg://addstyle?slug="_q
             + url_encode(addStyleMatch->captured(1));
    } else if (const auto languageMatch =
                   regex_match(u"^setlanguage/([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q,
                               query, matchOptions)) {
      return u"tg://setlanguage?lang="_q +
             url_encode(languageMatch->captured(1));
    } else if (const auto shareUrlMatch = regex_match(u"^share/url/?\\?(.+)$"_q,
                                                      query, matchOptions)) {
      return u"tg://msg_url?"_q + shareUrlMatch->captured(1);
    } else if (const auto confirmPhoneMatch = regex_match(
                   u"^confirmphone/?\\?(.+)"_q, query, matchOptions)) {
      return u"tg://confirmphone?"_q + confirmPhoneMatch->captured(1);
    } else if (const auto ivMatch =
                   regex_match(u"^iv/?\\?(.+)(#|$)"_q, query, matchOptions)) {
      //
      // We need to show our t.me page, not the url directly.
      //
      // auto params = url_parse_params(ivMatch->captured(1),
      // UrlParamNameTransform::ToLower); auto previewedUrl =
      // params.value(u"url"_q); if (previewedUrl.startsWith(u"http://"_q,
      // Qt::CaseInsensitive)
      //	|| previewedUrl.startsWith(u"https://"_q, Qt::CaseInsensitive))
      //{ 	return previewedUrl;
      //}
      return url;
    } else if (const auto socksMatch = regex_match(u"^socks/?\\?(.+)(#|$)"_q,
                                                   query, matchOptions)) {
      return u"tg://socks?"_q + socksMatch->captured(1);
    } else if (const auto proxyMatch = regex_match(u"^proxy/?\\?(.+)(#|$)"_q,
                                                   query, matchOptions)) {
      return u"tg://proxy?"_q + proxyMatch->captured(1);
    } else if (const auto invoiceMatch =
                   regex_match(u"^(invoice/|\\$)([a-zA-Z0-9_\\-]+)(\\?|#|$)"_q,
                               query, matchOptions)) {
      return u"tg://invoice?slug="_q + invoiceMatch->captured(2);
    } else if (const auto bgMatch =
                   regex_match(u"^bg/([a-zA-Z0-9\\.\\_\\-\\~]+)(\\?(.+)?)?$"_q,
                               query, matchOptions)) {
      const auto params = bgMatch->captured(3);
      const auto bg = bgMatch->captured(1);
      const auto type =
          regex_match(u"^[a-fA-F0-9]{6}^"_q, bg) ? "color"
          : (regex_match(u"^[a-fA-F0-9]{6}\\-[a-fA-F0-9]{6}$"_q, bg) ||
             regex_match(u"^[a-fA-F0-9]{6}(\\~[a-fA-F0-9]{6}){1,3}$"_q, bg))
              ? "gradient"
              : "slug";
      return u"tg://bg?"_q + type + '=' + bg +
             (params.isEmpty() ? QString() : '&' + params);
    } else if (const auto chatlinkMatch =
                   regex_match(u"^m/([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q, query,
                               matchOptions)) {
      const auto slug = chatlinkMatch->captured(1);
      return u"tg://message?slug="_q + slug;
    } else if (const auto nftMatch =
                   regex_match(u"^nft/([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q, query,
                               matchOptions)) {
      const auto slug = nftMatch->captured(1);
      return u"tg://nft?slug="_q + slug;
    } else if (const auto auctionMatch =
                   regex_match(u"^auction/([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q,
                               query, matchOptions)) {
      const auto slug = auctionMatch->captured(1);
      return u"tg://stargift_auction?slug="_q + slug;
    } else if (const auto callMatch =
                   regex_match(u"^call/([a-zA-Z0-9\\.\\_\\-]+)(\\?|$)"_q, query,
                               matchOptions)) {
      const auto slug = callMatch->captured(1);
      return u"tg://call?slug="_q + slug;
    } else if (
        const auto newbotMatch = regex_match(
            u"^newbot/([a-zA-Z0-9\\.\\_]+)(/([a-zA-Z0-9\\.\\_]*))?(/?\\?(.+))?$"_q,
            query, matchOptions)) {
      ...
      const auto params =
          query.mid(privateMatch->captured(0).size()).toString();
      ...
      auto added = QString();
      if (const auto threadPostMatch = regex_match(
              u"^/(\\d+)/(\\d+)(/?\\?|/?$)"_q, privateMatch->captured(2))) {
        added = u"&topic=%1&post=%2"_q.arg(threadPostMatch->captured(1),
                                           threadPostMatch->captured(2));
      } else if (const auto postMatch = regex_match(
                     u"^/(\\d+)(/?\\?|/?$)"_q, privateMatch->captured(2))) {
        added = u"&post="_q + postMatch->captured(1);
      }
      return base + added + (params.isEmpty() ? QString() : '&' + params);
    } else if (const auto ayuSettingsMatch =
                   regex_match(u"^(?:ayuSettings|exteraSettings)/?\\?(.+)$"_q,
                               query, matchOptions)) {
      return u"tg://ayusettings?"_q + ayuSettingsMatch->captured(1);
    } else if (const auto usernameMatch =
                   regex_match(u"^"
                               "([a-zA-Z0-9\\.\\_]+)"
                               "("
                               "/?\\?|"
      ...
      const auto params =
          query.mid(usernameMatch->captured(0).size()).toString();
      if (params.indexOf("boost", 0, Qt::CaseInsensitive) >= 0 &&
          params.toLower().split('&').contains(u"boost"_q)) {
        return u"tg://boost?domain="_q + domain;
      } else if (domain == u"boost"_q) {
        if (const auto domainMatch =
                regex_match(u"^/([a-zA-Z0-9\\.\\_]+)(/?\\?|/?$)"_q,
                            usernameMatch->captured(2))) {
      ...
      const auto base =
          u"tg://resolve?domain="_q + url_encode(usernameMatch->captured(1));
      auto added = QString();
      if (const auto threadPostMatch = regex_match(
              u"^/(\\d+)/(\\d+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
        added = u"&topic=%1&post=%2"_q.arg(threadPostMatch->captured(1),
                                           threadPostMatch->captured(2));
      } else if (const auto postMatch = regex_match(
                     u"^/(\\d+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
        added = u"&post="_q + postMatch->captured(1);
      } else if (const auto storyMatch =
                     regex_match(u"^/s/(\\d+|live)(/?\\?|/?$)"_q,
                                 usernameMatch->captured(2))) {
        added = u"&story="_q + storyMatch->captured(1);
      } else if (const auto albumMatch = regex_match(
                     u"^/a/(\\d+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
        added = u"&album="_q + albumMatch->captured(1);
      } else if (const auto collectionMatch = regex_match(
                     u"^/c/(\\d+)(/?\\?|/?$)"_q, usernameMatch->captured(2))) {
        added = u"&collection="_q + collectionMatch->captured(1);
      } else if (const auto appNameMatch =
                     regex_match(u"^/([a-zA-Z0-9\\.\\_\\-]+)(/?\\?|/?$)"_q,
                                 usernameMatch->captured(2))) {
...
[[nodiscard]] InternalLinkCheckResult InternalLinkCheck(const QString &url) {
  ...
  const auto usernameMatch =
      regex_match(u"^resolve/?\\?(.+)(#|$)"_q, command, matchOptions);
  auto username = QString();
  if (usernameMatch->hasMatch()) {
    const auto params = url_parse_params(usernameMatch->captured(1),
...
bool InternalPassportLink(const QString &url) {
  ...
  using namespace qthelp;
  const auto matchOptions = RegExOption::CaseInsensitive;
  const auto authMatch =
      regex_match(u"^passport/?\\?(.+)(#|$)"_q, result.command, matchOptions);
...
bool InternalPassportOrOAuthLink(const QString &url) {
  ...
  using namespace qthelp;
  const auto matchOptions = RegExOption::CaseInsensitive;
  const auto oauthMatch =
      regex_match(u"^oauth/?\\?(.+)(#|$)"_q, result.command, matchOptions);
  const auto oauthLegacy = (result.username == u"oauth"_q);
  return InternalPassportLink(url) || oauthMatch->hasMatch() || oauthLegacy;
}
...
void ResolveAndShowUniqueGift(std::shared_ptr<ChatHelpers::Show> show,
                              const QString &slug,
  ...
  request.id = session->api()
                   .request(MTPpayments_GetUniqueStarGift(MTP_string(slug)))
...
TimeId ParseVideoTimestamp(QStringView value) {
  ...
  return m.hasMatch()
             ? (m.capturedView(1).toInt() * 3600 +
                m.capturedView(2).toInt() * 60 + m.capturedView(3).toInt())
             : value.toInt();
}

...
void SetCrashAnnotationsGL() {
  ...
  CrashReports::SetAnnotation("OpenGL ANGLE", [] {
    if (Core::App().settings().disableOpenGL()) {
      ...
      switch (Ui::GL::CurrentANGLE()) {
      case Ui::GL::ANGLE::Auto:
        return "Auto";
      case Ui::GL::ANGLE::D3D11:
        return "Direct3D 11";
      case Ui::GL::ANGLE::D3D9:
        return "Direct3D 9";
      case Ui::GL::ANGLE::D3D11on12:
        return "D3D11on12";
  ...
  CrashReports::SetAnnotation("OpenGL", Core::App().settings().disableOpenGL()
                                            ? "Disabled"
                                            : "Enabled");
...
Application::Application()
    : QObject(), _private(std::make_unique<Private>()),
  ...
  passcodeLockChanges() | rpl::on_next(
                              [=](bool locked) {
                                _shouldLockAt = 0;
                                if (locked) {
  ...
  settings().windowTitleContentChanges() |
      rpl::on_next([=] { updateWindowTitles(); }, _lifetime);
...
void Application::run() {
  ...
  rpl::combine(_batterySaving->value(), settings().ignoreBatterySavingValue()) |
      rpl::on_next(
          [=](bool saving, bool ignore) {
            PowerSaving::SetForceAll(saving && !ignore);
          },
          _lifetime);

  style::ShortAnimationPlaying() | rpl::on_next(
                                       [=](bool playing) {
                                         if (playing) {
                                           MTP::details::pause();
                                         } else {
  ...
  _domain->activeChanges() |
      rpl::on_next(
          [=](not_null<Main::Account *> account) { showAccount(account); },
          _lifetime);

  (_domain->activeValue() | rpl::to_empty |
   rpl::filter([=] { return _domain->started(); }) | rpl::take(1)) |
      rpl::then(_domain->accountsChanges()) | rpl::map([=] {
            ...
            if (_lastActivePrimaryWindow && it != end(ordered)) {
              ...
              if ((index + 1) > _domain->maxAccounts()) {
                _lastActivePrimaryWindow->show(
                    Box(AccountsLimitBox, &account->session()));
              }
            }
          },
          _lifetime);
  ...
  appDeactivatedValue() | rpl::on_next(
                              [=](bool deactivated) {
                                if (deactivated) {
  ...
  _openInMediaViewRequests.events() |
      rpl::on_next(
          [=](Media::View::OpenRequest &&request) {
            if (_mediaView) {
          ...
          _lifetime);
  {
    const auto countries = std::make_shared<Countries::Manager>(_domain.get());
...
void Application::checkSystemDarkMode() {
  ...
  const auto needToSwitch = darkModeEnabled && maybeDarkMode &&
...
void Application::startSystemDarkModeViewer() {
  ...
  rpl::merge(settings().systemDarkModeChanges() | rpl::to_empty,
             settings().systemDarkModeEnabledChanges() | rpl::to_empty) |
      rpl::on_next([=] { checkSystemDarkMode(); }, _lifetime);
}

void Application::enumerateWindows(
...
void Application::processCreatedWindow(not_null<Window::Controller *> window) {
...
void Application::createTray() {
  ...
  _tray->showFromTrayRequests() | rpl::on_next([=] { activate(); }, _lifetime);

  _tray->hideToTrayRequests() | rpl::on_next(
                                    [=] {
...
bool Application::hideMediaView() {
  if (_mediaView && _mediaView->isFullScreen() && !_mediaView->isMinimized() &&
      !_mediaView->isHidden()) {
    _mediaView->close();
    return true;
  }
...
bool Application::eventFilter(QObject *object, QEvent *e) {
  ...
  case QEvent::Shortcut: {
    ...
    DEBUG_LOG(("Shortcut event caught: %1").arg(event->key().toString()));
  ...
  case QEvent::FileOpen: {
    if (object == QCoreApplication::instance()) {
      if (_urlsToOpen.isEmpty()) {
        InvokeQueued(this, [=] {
          const auto activateRequired =
              ranges::any_of(ranges::views::all(_urlsToOpen) |
      ...
      _urlsToOpen << event->url().toString(QUrl::FullyEncoded).mid(0, 8192);
    }
  } break;
  ...
  case QEvent::ContextMenu: {
    ...
    return (ev->reason() == QContextMenuEvent::Mouse) && _lastMouseIgnored;
  } break;
  }
...
Settings &Application::settings() { return _private->settings; }

const Settings &Application::settings() const { return _private->settings; }
...
void Application::saveSettings() { Local::writeSettings(); }

bool Application::canReadDefaultDownloadPath() const {
  return KSandbox::isInside()
             ? base::CanReadDirectory(QStandardPaths::writableLocation(
...
void Application::refreshFallbackProductionConfig(const MTP::Config &config) {
...
void Application::setCurrentProxy(const MTP::ProxyData &proxy,
...
void Application::badMtprotoConfigurationError() {
  if (settings().proxy().isEnabled() && !_badProxyDisableBox) {
    const auto disableCallback = [=] {
      setCurrentProxy(settings().proxy().selected(),
                      MTP::ProxyData::Settings::System);
    };
    _badProxyDisableBox =
        Ui::show(Ui::MakeInformBox(Lang::Hard::ProxyConfigError()));
    _badProxyDisableBox->boxClosing() |
        rpl::on_next(disableCallback, _badProxyDisableBox->lifetime());
  }
}
...
void Application::startLocalStorage() {
  ...
  settings().saveDelayedRequests() |
      rpl::on_next([=] { saveSettingsDelayed(); }, _lifetime);
}

void Application::startEmojiImageLoader() {
  _emojiImageLoader.with(
      [source = prepareEmojiSourceImages(), large = settings().largeEmoji()](
          Stickers::EmojiImageLoader &loader) mutable {
        loader.init(std::move(source), large);
      });

  settings().largeEmojiChanges() | rpl::on_next(
                                       [=](bool large) {
                                         if (large) {
...
void Application::setScreenIsLocked(bool locked) { _screenIsLocked = locked; }

bool Application::screenIsLocked() const { return _screenIsLocked; }
...
void Application::forceLogOut(not_null<Main::Account *> account,
  ...
  connect(box.get(), &QObject::destroyed,
          [=] { crl::on_main(weak, [=] { account->forcedLogOut(); }); });
}
...
void Application::handleAppDeactivated() {
  enumerateWindows(
      [&](not_null<Window::Controller *> w) { w->updateIsActiveBlur(); });
  const auto session =
      _lastActiveWindow ? _lastActiveWindow->maybeSession() : nullptr;
  if (session) {
...
rpl::producer<bool> Application::appDeactivatedValue() const {
  ...
  return rpl::single(app->applicationState()) |
...
Main::Account &Application::activeAccount() const { return _domain->active(); }
...
bool Application::exportPreventsQuit() {
  if (_exportManager->inProgress()) {
    _exportManager->stopWithConfirmation([] { Quit(); });
    return true;
  }
  return false;
}
...
bool Application::preventsQuit(QuitReason reason) {
  if (exportPreventsQuit() || uploadPreventsQuit() || downloadPreventsQuit()) {
    return true;
  } else if ((!_mediaView || _mediaView->isHidden() ||
...
int Application::unreadBadge() const { return _domain->unreadBadge(); }
...
bool Application::offerLegacyLangPackSwitch() const {
  return (_domain->accounts().size() == 1) && activeAccount().sessionExists();
}
    ...
    cRefStartUrls() = ranges::views::all(cRefStartUrls()) |
                      ranges::views::filter([&](const QUrl &url) {
                        if (url.scheme() == u"tonsite"_q) {
                          ...
                          const auto local =
                              TryConvertUrlToLocal(url.toString());
...
bool Application::openLocalUrl(const QString &url, QVariant context) {
  ...
  if (urlTrimmed.startsWith(protocol, Qt::CaseInsensitive) &&
    ...
    const auto controller = my.sessionWindow.get() ? my.sessionWindow.get()
...
QString Application::changelogLink() const {
  return u"https://github.com/Clansty/tdesktop/releases"_q;
}

bool Application::openCustomUrl(const QString &protocol,
                                const std::vector<LocalUrlHandler> &handlers,
                                const QString &url, const QVariant &context) {
  const auto urlTrimmed = url.trimmed();
  if (!urlTrimmed.startsWith(protocol, Qt::CaseInsensitive) ||
      passcodeLocked()) {
    return false;
  }
  static const auto kTagExp = QRegularExpression(u"^\\~[a-zA-Z0-9_\\-]+\\~:"_q);
...
void Application::lockByPasscode() {
  _passcodeLock = true;
  enumerateWindows(
      [&](not_null<Window::Controller *> w) { w->setupPasscodeLock(); });
  if (_mediaView) {
    _mediaView->close();
  }
}
...
void Application::maybeLockByPasscode() {
  preventOrInvoke([=] { lockByPasscode(); });
}

void Application::unlockPasscode() {
  clearPasscodeLock();
  enumerateWindows(
      [&](not_null<Window::Controller *> w) { w->clearPasscodeLock(); });
}
...
bool Application::passcodeLocked() const { return _passcodeLock.current(); }
...
crl::time Application::lastNonIdleTime() const {
  return std::max(base::Platform::LastUserInputTime().value_or(0),
                  _lastNonIdleTime);
}
...
void Application::lockBySetupEmail() {
  ...
  enumerateWindows(
      [&](not_null<Window::Controller *> w) { w->setupSetupEmailLock(); });
}

void Application::unlockSetupEmail() {
  _setupEmailLock = false;
  enumerateWindows(
      [&](not_null<Window::Controller *> w) { w->clearSetupEmailLock(); });
}
...
void Application::checkAutoLock(crl::time lastNonIdleTime) {
  if (!_domain->local().hasLocalPasscode() || passcodeLocked() ||
      !someSessionExists()) {
    _shouldLockAt = 0;
  ...
  if (checkTimeMs >= shouldLockInMs ||
      (_shouldLockAt > 0 && now > _shouldLockAt + kAutoLockTimeoutLateMs)) {
    _shouldLockAt = 0;
    _autoLockTimer.cancel();
    lockByPasscode();
  } else {
...
Window::Controller *
Application::separateWindowFor(Window::SeparateId id) const {
...
Window::Controller *Application::ensureSeparateWindowFor(Window::SeparateId id,
                                                         MsgId showAtMsgId) {
  ...
  if (const auto existing = separateWindowFor(id)) {
    if (id.thread && id.type == Window::SeparateType::Chat) {
      existing->sessionController()->showThread(
          id.thread, showAtMsgId, Window::SectionShow::Way::ClearStack);
    }
  ...
  const auto result =
      _windows
          .emplace(id, std::make_unique<Window::Controller>(id, showAtMsgId))
          .first->second.get();
...
Window::Controller *
Application::windowForShowingHistory(not_null<PeerData *> peer) const {
...
Window::Controller *
Application::windowForShowingForum(not_null<Data::Forum *> forum) const {
  const auto tabs = forum->bot() || forum->peer()->useSubsectionTabs();
  const auto id = Window::SeparateId(tabs ? Window::SeparateType::Chat
...
Window::Controller *Application::findWindow(not_null<QWidget *> widget) const {
...
bool Application::closeNonLastAsync(not_null<Window::Controller *> window) {
  const auto hasOther = [&] {
    for (const auto &[id, controller] : _windows) {
      if (id.primary() && !_closingAsyncWindows.contains(controller.get()) &&
          controller.get() != window && controller->maybeSession()) {
        return true;
      }
    }
    return false;
...
void Application::setLastActiveWindow(Window::Controller *window) {
  ...
  window->floatPlayerDelegateValue() |
      rpl::on_next(
          [=](Media::Player::FloatDelegate *value) {
            if (!value) {
              _floatPlayers = nullptr;
              ...
              _floatPlayers =
                  std::make_unique<Media::Player::FloatController>(value);
            }
...
void Application::closeWindow(not_null<Window::Controller *> window) {
  ...
  const auto nextFromStack =
      _windowStack.empty() ? nullptr
      : (stackIt == end(_windowStack) || stackIt + 1 != end(_windowStack))
          ? _windowStack.back().get()
      : (_windowStack.size() > 1) ? (stackIt - 1)->get()
                                  : nullptr;
  const auto next = nextFromStack ? nextFromStack
  ...
  const auto account = domain().started() ? &domain().active() : nullptr;
  if (account && !_windows.contains(Window::SeparateId(account)) &&
      _lastActiveWindow) {
...
bool Application::minimizeActiveWindow() {
  if (_mediaView && _mediaView->isActive()) {
    _mediaView->minimize();
    return true;
  } else if (_iv->minimizeActive() || calls().minimizeCurrentActiveCall()) {
    return true;
  } else {
...
bool Application::isSharingScreen() const { return _calls->isSharingScreen(); }

// macOS Qt bug workaround, sometimes no leaveEvent() gets to the nested
// widgets.
void Application::registerLeaveSubscription(not_null<QWidget *> widget) {
  ...
  if (const auto window = widget->window()) {
    ...
    if (i == end(_leaveFilters)) {
      ...
      i = _leaveFilters.emplace(window, LeaveFilter{.filter = filter.get()})
              .first;
    }
...
void Application::refreshApplicationIcon(Main::Session *session) {
  ...
  Platform::SetApplicationIcon(Window::CreateIcon(session, Platform::IsMac()));
}
...
void Application::startShortcuts() {
  ...
  _domain->activeSessionChanges() |
      rpl::on_next(
          [=](Main::Session *session) { refreshApplicationIcon(session); },
          _lifetime);
  ...
  Shortcuts::Requests() |
      rpl::on_next(
          [=](not_null<Shortcuts::Request *> request) {
            ...
            request->check(Command::Minimize) &&
                request->handle([=] { return minimizeActiveWindow(); });
            request->check(Command::Close) &&
                request->handle([=] { return closeActiveWindow(); });
          },
          _lifetime);
}
...
bool IsAppLaunched() { return (Application::Instance != nullptr); }
...
void Restart() {
  const auto updateReady = !UpdaterDisabled() && (UpdateChecker().state() ==

  ...
  if (const auto parsedUrl = QUrl(url); parsedUrl.isValid()) {
    if (const auto host = parsedUrl.host(); !host.isEmpty()) {
      if (const auto hostPos = url.indexOf(host); hostPos != -1) {
        auto boldEntity =
            EntityInText(EntityType::Bold, hostPos, host.length());

        if (host.startsWith("www.")) {
          boldEntity =
              EntityInText(EntityType::Bold, hostPos + 4, host.length() - 4);
        }
...
void SearchByHashtag(ClickContext context, const QString &tag) {
  ...
  if (const auto delegate =
          my.elementDelegate ? my.elementDelegate() : nullptr) {
  ...
  const auto inPeer = my.peer ? my.peer
                      : my.itemId
                          ? data.message(my.itemId)->history()->peer.get()
                          : nullptr;
  controller->content()->searchMessages(tag,
...
void ExportToCalendar(TimeId date, const QString &messageText) {
  const auto start = QDateTime::fromSecsSinceEpoch(date, Qt::UTC);
  const auto end = QDateTime::fromSecsSinceEpoch(date + 3600, Qt::UTC);
  const auto now = QDateTime::currentDateTimeUtc();
  const auto format = u"yyyyMMdd'T'HHmmss'Z'"_q;
  const auto locale = QLocale();
  const auto raw =
      locale.toString(base::unixtime::parse(date), QLocale::LongFormat);
  auto summary = raw;
  ...
  QDir().mkpath(dir);
  const auto path = u"%1/event_%2.ics"_q.arg(dir).arg(date);
...
void DoneSetReminder(std::shared_ptr<ChatHelpers::Show> show) {
  ...
  const auto text = tr::lng_reminder_scheduled_in(
      tr::now, lt_link, tr::link(tr::bold(tr::lng_saved_messages(tr::now))),
      tr::marked);
...
bool UrlRequiresConfirmation(const QUrl &url) {
  ...
  return !regex_match("(^|\\.)("
                      ...
                      "|telesco\\.pe"
                      ")$",
                      url.host(), RegExOption::CaseInsensitive);
}
...
void HiddenUrlClickHandler::Open(QString url, QVariant context) {
  ...
  const auto open = [=] { UrlClickHandler::Open(url, context); };
  if (url.startsWith(u"tg://"_q, Qt::CaseInsensitive) ||
	...
	if (!AyuSettings::getInstance().disableOpenLinkWarning() &&
      ...
      const auto displayed =
          parsedUrl.isValid() ? parsedUrl.toDisplayString() : url;
      const auto displayUrl = !IsSuspicious(displayed) ? displayed
                              : parsedUrl.isValid()
                                  ? QString::fromUtf8(parsedUrl.toEncoded())
                                  : ShowEncoded(displayed);
      const auto controller = my.sessionWindow.get();
      const auto use =
          controller ? &controller->window() : Core::App().activeWindow();
      auto box = Box([=](not_null<Ui::GenericBox *> box) {
        ...
        const auto &st = my.dark ? st::groupCallBoxLabel : st::boxLabel;
        box->addSkip(st.style.lineHeight - st::boxPadding.bottom());
        const auto url = box->addRow(object_ptr<Ui::FlatLabel>(
            box, rpl::single(BoldDomainInUrl(displayUrl)), st));
        url->setContextMenuHook([=](Ui::FlatLabel::ContextMenuRequest request) {
          ...
          if (request.fullSelection) {
            ...
            const auto copySelectedText = [=] {
              TextUtilities::SetClipboardText(
                  TextForMimeData::Simple(displayUrl.mid(
                      selection.from, selection.to - selection.from)));
            };
            request.menu->addAction(tr::lng_context_copy_selected(tr::now),
                                    copySelectedText);
...
void BotGameUrlClickHandler::onClick(ClickContext context) const {
  ...
  const auto openLink = [=] { UrlClickHandler::Open(url, context.other); };
  const auto my = context.other.value<ClickHandlerContext>();
  const auto weakController = my.sessionWindow;
  const auto controller = weakController.get();
  const auto item =
      controller ? controller->session().data().message(my.itemId) : nullptr;
...
void MentionClickHandler::onClick(ClickContext context) const {
  ...
  if (button == Qt::LeftButton || button == Qt::MiddleButton) {
    const auto my = context.other.value<ClickHandlerContext>();
    const auto controller = my.sessionWindow.get();
    const auto use = controller ? controller
...
void BotCommandClickHandler::onClick(ClickContext context) const {
  ...
  if (const auto delegate =
    ...
    auto &data = controller->session().data();
    const auto peer = my.peer ? my.peer
                      : my.itemId
                          ? data.message(my.itemId)->history()->peer.get()
                          : nullptr;
    // Can't find context.
...
MonospaceClickHandler::MonospaceClickHandler(const QString &text,
                                             EntityType type)
    : _text(text), _entity({type}) {}
...
QString MonospaceClickHandler::url() const { return _text; }

FormattedDateClickHandler::FormattedDateClickHandler(int32 date,
                                                     FormattedDateFlags flags)
    : _date(date), _entityData(SerializeFormattedDateData(date, flags)) {}
...
void FormattedDateClickHandler::onClick(ClickContext context) const {
  ...
  const auto menu = Ui::CreateChild<Ui::PopupMenu>(controller->content(),
  ...
  menu->addAction(
      ...
      [date, show] {
        const auto text = QLocale().toString(base::unixtime::parse(date),
                                             QLocale::LongFormat);
  ...
  const auto canForward = item && !item->forbidsForward() &&
...
QString FormattedDateClickHandler::tooltip() const {
  return QLocale().toString(base::unixtime::parse(_date), QLocale::LongFormat);
}

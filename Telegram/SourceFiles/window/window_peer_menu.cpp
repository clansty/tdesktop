...
void ShareBotGame(not_null<UserData *> bot, not_null<Data::Thread *> thread, const QString &shortName) {
	...
	histories.sendPreparedMessage(
		...
		Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
			...
			MTP_inputMediaGame(MTP_inputGameShortName(bot->inputUser(), MTP_string(shortName))),
			MTP_string(),
			MTP_long(randomId),
			MTPReplyMarkup(),
...
const char kOptionViewProfileInChatsListContextMenu[] = "view-profile-in-chats-list-context-menu";
...
[[nodiscard]] QString LookupMemberRank(not_null<PeerData *> peer, not_null<UserData *> user) {
	if (const auto chat = peer->asChat()) {
		const auto i = chat->memberRanks.find(peerToUser(user->id));
		return (i != chat->memberRanks.end()) ? i->second : QString();
	} else if (const auto channel = peer->asChannel()) {
		if (channel->mgInfo) {
			const auto i = channel->mgInfo->memberRanks.find(peerToUser(user->id));
			return (i != channel->mgInfo->memberRanks.end()) ? i->second : QString();
		}
	}
	return QString();
}
...
void SetActionText(not_null<QAction *> action, rpl::producer<QString> &&text) {
	...
	std::move(text) | rpl::on_next([=](const QString &actionText) { action->setText(actionText); }, *lifetime);
}
...
void PeerMenuAddMuteSubmenuAction(not_null<Window::SessionController *> controller,
	...
	if (isMuted) {
		const auto text = tr::lng_context_unmute(tr::now) + '\t' +
			Ui::FormatMuteForTiny(thread->notify().muteUntil().value_or(0) - base::unixtime::now());
		addAction(text,
				  with([=](not_null<Data::Thread *> thread) { notifySettings->update(thread, {.unmute = true}); }),
				  &st::menuIconUnmute);
	} else {
		...
		addAction(PeerMenuCallback::Args{
			...
			.handler = nullptr,
			.icon = (notifySettings->sound(thread).none ? &st::menuIconSilent : &st::menuIconMute),
			.fillSubmenu = [&](not_null<Ui::PopupMenu *> menu) { MuteMenu::FillMuteMenu(menu, thread, show); },
		});
	}
}
...
{
public:
	Filler(not_null<SessionController *> controller, Dialogs::EntryState request, const PeerMenuCallback &addAction);
	void fill();
...
History *FindWastedPin(not_null<Data::Session *> data, Data::Folder *folder) {
	...
	for (const auto &pinned : order) {
		if (const auto history = pinned.history()) {
			if (history->peer->isChat() && history->peer->asChat()->isDeactivated() && !history->inChatList()) {
				return history;
			}
		}
	}
...
void AddChatMembers(not_null<Window::SessionNavigation *> navigation, not_null<ChatData *> chat) {
	AddParticipantsBoxController::Start(navigation, chat);
}

bool PinnedLimitReached(not_null<Window::SessionController *> controller, not_null<Dialogs::Entry *> entry) {
...
bool PinnedLimitReached(not_null<Window::SessionController *> controller,
	...
	controller->show(Box(FilterPinsLimitBox, &history->session(), filterId));
	return true;
}

void TogglePinnedThread(not_null<Window::SessionController *> controller,
	...
	if (const auto history = entry->asHistory()) {
		const auto flags =
			isPinned ? MTPmessages_ToggleDialogPin::Flag::f_pinned : MTPmessages_ToggleDialogPin::Flag(0);
		owner->session()
			.api()
			.request(MTPmessages_ToggleDialogPin(MTP_flags(flags), MTP_inputDialogPeer(history->peer->input())))
			.done(
				[=]
				{
		...
		owner->session()
			.api()
			.request(MTPmessages_UpdatePinnedForumTopic(peer->input(), MTP_int(topic->rootId()), MTP_bool(isPinned)))
			.done(
		...
		const auto flags =
			isPinned ? MTPmessages_ToggleSavedDialogPin::Flag::f_pinned : MTPmessages_ToggleSavedDialogPin::Flag(0);
		owner->session()
			.api()
			.request(MTPmessages_ToggleSavedDialogPin(MTP_flags(flags),
...
Filler::Filler(not_null<SessionController *> controller, Dialogs::EntryState request, const PeerMenuCallback &addAction)
	: _controller(controller), _request(request), _thread(request.key.thread()), _topic(request.key.topic()),
	  _peer(request.key.peer()), _folder(request.key.folder()), _sublist(request.key.sublist()), _addAction(addAction) {
}

void Filler::addHidePromotion() {
	const auto history = _request.key.history();
	if (_topic || !history || !history->useTopPromotion() || history->topPromotionType().isEmpty()) {
		return;
	}
	_addAction(
		tr::lng_context_hide_psa(tr::now),
		[=]
		{
			history->cacheTopPromotion(false, QString(), QString());
			history->session().api().request(MTPhelp_HidePromoData(history->peer->input())).send();
		},
		&st::menuIconRemove);
}
...
void Filler::addToggleTopicClosed() {
	...
	const auto text = closed ? tr::lng_forum_topic_reopen(tr::now) : tr::lng_forum_topic_close(tr::now);
	_addAction(
		text,
		[=]
		{
...
void Filler::addTogglePin() {
	if ((!_sublist && !_peer) || (_topic && !_topic->canTogglePinned())) {
		return;
	} else if (_request.section == Section::SubsectionTabsMenu && !_sublist && !_topic) {
		return;
	...
	const auto pinText = [=]
	{
		return entry->isPinnedDialog(filterId) ? tr::lng_context_unpin_from_top(tr::now)
	...
	_addAction(pinText(), pinToggle, (entry->isPinnedDialog(filterId) ? &st::menuIconUnpin : &st::menuIconPin));
}
...
void Filler::addSupportInfo() {
	...
	const auto controller = _controller;
	_addAction(
		"Edit support info", [=] { user->session().supportHelper().editInfo(controller, user); }, &st::menuIconEdit);
}

void Filler::addInfo() {
	const auto sublist = _thread ? _thread->asSublist() : nullptr;
	const auto infoPeer = sublist ? sublist->sublistPeer().get() : _peer;
	if (infoPeer && (infoPeer->isSelf() || infoPeer->isRepliesChat() || infoPeer->isVerifyCodes())) {
		return;
	} else if (!_thread) {
		return;
		...
		if (thread && !thread->asSublist() && thread == _thread) {
			if (Core::App().settings().thirdSectionInfoEnabled() || Core::App().settings().tabbedReplacedWithInfo()) {
				return;
			}
		}
	}
	...
	const auto text = _thread->asTopic()
		? (_thread->peer()->isBot() ? tr::lng_context_view_thread(tr::now) : tr::lng_context_view_topic(tr::now))
		: (infoPeer->isChat() || infoPeer->isMegagroup()) ? tr::lng_context_view_group(tr::now)
		: infoPeer->isUser()							  ? tr::lng_context_view_profile(tr::now)
		...
		[=]
		{
			if (const auto strong = weak.get()) {
				if (base::IsCtrlPressed()) {
					controller->uiShow()->showBox(PrepareShortInfoBox(infoPeer, controller));
				} else {
...
void Filler::addStoryArchive() {
	...
	const auto weak = base::make_weak(_thread);
	_addAction(
		tr::lng_stories_archive_button(tr::now),
		[=]
		{
			if ([[maybe_unused]] const auto strong = weak.get()) {
				controller->showSection(Info::Stories::Make(channel, Info::Stories::ArchiveId()));
			}
		},
...
void Filler::addToggleFolder() {
	const auto controller = _controller;
	const auto history = _request.key.history();
	if (_topic || !history || !history->owner().chatsFilters().has() || !history->inChatList()) {
		return;
	} else if (_request.section == Section::SubsectionTabsMenu && !_sublist && !_topic) {
		return;
	}
	_addAction(PeerMenuCallback::Args{
		...
		.fillSubmenu = [&](not_null<Ui::PopupMenu *> menu) { FillChooseFilterMenu(controller, menu, history); },
...
void Filler::addToggleUnreadMark() {
	...
	const auto weak = base::make_weak(_thread);
	const auto label = unread ? tr::lng_context_mark_read(tr::now) : tr::lng_context_mark_unread(tr::now);
	_addAction(
		label,
		[=]
		{
...
void Filler::addNewWindow(bool addSeparator) {
	...
	if (_folder) {
		_addAction(
			...
			[=]
			{
				...
				controller->showInNewWindow(SeparateId(SeparateType::Archive, &controller->session()));
			},
		...
		_addAction(
			...
			[=]
			{
				...
				if (const auto sublist = weak.get()) {
					controller->showInNewWindow(SeparateId(SeparateType::SavedSublist, sublist));
				}
			},
			&st::menuIconNewWindow);
		if (addSeparator) {
	...
	const auto history = _request.key.history();
	if (!_peer || (history && history->useTopPromotion() && !history->topPromotionType().isEmpty())) {
		return;
	}
	const auto peer = _peer;
	const auto thread = _topic ? not_null<Data::Thread *>(_topic) : _peer->owner().history(_peer);
	const auto weak = base::make_weak(thread);
	_addAction(
		tr::lng_context_new_window(tr::now),
		[=]
		{
			Ui::PreventDelayedActivation();
			if (const auto strong = weak.get()) {
				const auto forum = !strong->asTopic() && peer->isForum() && !peer->useSubsectionTabs();
				controller->showInNewWindow(SeparateId(forum ? SeparateType::Forum : SeparateType::Chat, strong));
			}
		},
		&st::menuIconNewWindow);
	if (addSeparator) {
...
void Filler::addToggleArchive() {
	if (!_peer || _topic || _request.section == Section::SubsectionTabsMenu) {
		return;
	}
	const auto peer = _peer;
	const auto history = _request.key.history();
	...
	const auto isArchived = [=] { return IsArchived(history); };
	const auto label = [=] { return isArchived() ? tr::lng_archived_remove(tr::now) : tr::lng_archived_add(tr::now); };
	const auto toggle = [=, show = _controller->uiShow()] { ToggleHistoryArchived(show, history, !isArchived()); };
	const auto archiveAction =
		_addAction(label(), toggle, isArchived() ? &st::menuIconUnarchive : &st::menuIconArchive);

	auto actionText =
		history->session().changes().historyUpdates(history, Data::HistoryUpdate::Flag::Folder) | rpl::map(label);
...
void Filler::addClearHistory() {
	...
	if (channel) {
		if (!channel->amIn()) {
			return;
		} else if (!channel->canDeleteMessages() && (!isGroup || channel->isPublic() || channel->isForum())) {
			return;
		}
	}
	_addAction(tr::lng_profile_clear_history(tr::now), ClearHistoryHandler(_controller, _peer), &st::menuIconClear);
}
...
void Filler::addDeleteChat() {
	...
	_addAction({
		.text = ((_peer->isUser() || _sublist) ? tr::lng_profile_delete_conversation(tr::now)
											   : tr::lng_profile_clear_and_exit(tr::now)),
		.handler = (_sublist ? DeleteSublistHandler(_controller, _sublist) : DeleteAndLeaveHandler(_controller, _peer)),
...
void Filler::addLeaveChat() {
	...
	_addAction({
		.text = (_peer->isMegagroup() ? tr::lng_profile_leave_group(tr::now) : tr::lng_profile_leave_channel(tr::now)),
...
void Filler::addJoinChat() {
	...
	const auto label =
		_peer->isMegagroup() ? tr::lng_profile_join_group(tr::now) : tr::lng_profile_join_channel(tr::now);
	_addAction(label, [=] { channel->session().api().joinChannel(channel); }, &st::menuIconAddToFolder);
}

void Filler::addBlockUser() {
	const auto user = _peer->asUser();
	if (!user || user->isInaccessible() || user->isSelf() || user->isRepliesChat() || user->isVerifyCodes()) {
		return;
	}
	const auto window = _controller;
	const auto blockText = [](not_null<UserData *> user)
	{
		return user->isBlocked() ? ((user->isBot() && !user->isSupport()) ? tr::lng_profile_restart_bot(tr::now)
																		  : tr::lng_profile_unblock_user(tr::now))
								 : ((user->isBot() && !user->isSupport()) ? tr::lng_profile_block_bot(tr::now)
																		  : tr::lng_profile_block_user(tr::now));
	};
	const auto blockAction = _addAction(
		blockText(user),
		[=]
		{
			...
			if (show->showFrozenError()) {
				...
				window->show(Box(PeerMenuBlockUserBox, &window->window(), user, v::null, v::null));
			}
		},
		(!user->isBlocked()	 ? &st::menuIconBlock
			 : user->isBot() ? &st::menuIconRestartBot
							 : &st::menuIconUnblock));

	auto actionText = _peer->session().changes().peerUpdates(_peer, Data::PeerUpdate::Flag::IsBlocked) |
...
void Filler::addViewDiscussion() {
	...
	_addAction(
		...
		[=]
		{
			...
			navigation->showPeerHistory(chat, Window::SectionShow::Way::Forward);
		},
...
void Filler::addDirectMessages() {
	...
	_addAction(
		...
		[=] { navigation->showPeerHistory(monoforum, Window::SectionShow::Way::Forward); },
...
void Filler::addExportChat() {
	...
	_addAction(
		tr::lng_profile_export_chat(tr::now), [=] { PeerMenuExportChat(navigation, peer); }, &st::menuIconExport);
}

void Filler::addTranslate() {
	if (_peer->translationFlag() != PeerData::TranslationFlag::Disabled || !_peer->session().premium() ||
		!Core::App().settings().translateChatEnabled()) {
		return;
	}
	const auto history = _peer->owner().historyLoaded(_peer);
	if (!history || !history->translateOfferedFrom() || history->translatedTo()) {
		return;
	}
	_addAction(
...
void Filler::addReport() {
	...
	const auto channel = _peer->asChannel();
	if (_topic || ((!chat || chat->amCreator()) && (!channel || channel->amCreator()))) {
		return;
	}
	const auto peer = _peer;
	const auto navigation = _controller;
...
void Filler::addNewContact() {
	const auto user = _peer->asUser();
	if (!user || user->isContact() || user->isSelf() || user->isInaccessible() || user->isBot()) {
		return;
	}
	const auto controller = _controller;
	const auto edit = [=]
	...
	_addAction(tr::lng_info_add_as_contact(tr::now), edit, &st::menuIconInvite);
}
...
void Filler::addShareContact() {
	...
	const auto controller = _controller;
	_addAction(
		tr::lng_info_share_contact(tr::now), [=] { PeerMenuShareContactBox(controller, user); }, &st::menuIconShare);
}
...
void Filler::addEditContact() {
	...
	_addAction(tr::lng_info_edit_contact(tr::now), edit, &st::menuIconEdit);
}
...
void Filler::addBotToGroup() {
	...
	[[maybe_unused]] const auto lifetime =
		Info::Profile::InviteToChatButton(user) | rpl::take(1) |
		rpl::on_next(
			[=](QString label)
			{
				if (!label.isEmpty()) {
					const auto controller = _controller;
					_addAction(
						label, [=] { AddBotToGroupBoxController::Start(controller, user); }, &st::menuIconInvite);
				}
			});
}
...
void Filler::addNewMembers() {
	const auto chat = _peer->asChat();
	const auto channel = _peer->asChannel();
	if ((!chat || !chat->canAddMembers()) && (!channel || !channel->canAddMembers())) {
		return;
	}
	const auto navigation = _controller;
	const auto callback = chat ? Fn<void()>([=] { AddChatMembers(navigation, chat); }) : [=]
	{ PeerMenuAddChannelMembers(navigation, channel); };
	_addAction(
		((chat || channel->isMegagroup()) ? tr::lng_channel_add_members(tr::now) : tr::lng_channel_add_users(tr::now)),
		callback,
		&st::menuIconInvite);
}
...
void Filler::addTopicLink() {
	...
	const auto weak = base::make_weak(_topic);
	_addAction(
		tr::lng_context_copy_topic_link(tr::now),
		[=]
		{
			if (const auto strong = weak.get()) {
				const auto link = Info::Profile::TopicLink(strong, true);
				QGuiApplication::clipboard()->setText(link);
				controller->showToast(channel->hasUsername() ? tr::lng_channel_public_link_copied(tr::now)
...
void Filler::addManageTopic() {
	...
	const auto navigation = _controller;
	const auto text =
		_topic->forum()->peer()->isBot() ? tr::lng_bot_thread_edit(tr::now) : tr::lng_forum_topic_edit(tr::now);
	_addAction(text, [=] { navigation->show(Box(EditForumTopicBox, navigation, history, rootId)); }, &st::menuIconEdit);
}
...
void Filler::addManageChat() {
	...
	const auto peer = _peer;
	const auto navigation = _controller;
	const auto text = peer->isUser()			  ? tr::lng_manage_bot_title(tr::now)
		: (peer->isChat() || peer->isMegagroup()) ? tr::lng_manage_group_title(tr::now)
												  : tr::lng_manage_channel_title(tr::now);
	_addAction(text, [=] { navigation->showEditPeerBox(peer); }, &st::menuIconManage);
}
...
void Filler::addBoostChat() {
	if (const auto channel = _peer->asChannel()) {
		...
		const auto text =
			channel->isMegagroup() ? tr::lng_boost_group_button(tr::now) : tr::lng_boost_channel_button(tr::now);
...
void Filler::addViewStatistics() {
	if (const auto channel = _peer->asChannel()) {
		...
		const auto canViewCreditsEarn = (channel->flags() & Flag::CanViewCreditsRevenue);
		if (canGetStats) {
			_addAction(
		...
		if (canGetStats || channel->amCreator() || channel->canPostStories()) {
			_addAction(
				tr::lng_boosts_title(tr::now),
				[=]
				{
					if ([[maybe_unused]] const auto strong = weak.get()) {
		...
		if (canViewEarn || canViewCreditsEarn) {
			_addAction(
				tr::lng_channel_earn_title(tr::now),
				[=]
				{
					if ([[maybe_unused]] const auto strong = weak.get()) {
...
bool Filler::skipCreateActions() const {
	...
	const auto isBlocked = [&] { return _peer && _peer->isUser() && _peer->asUser()->isBlocked(); }();
...
void Filler::addCreatePoll() {
	...
	const auto can = _topic ? Data::CanSend(_topic, ChatRestriction::SendPolls) : _peer->canCreatePolls();
	if (!can) {
		return;
	}
	const auto peer = _peer;
	const auto controller = _controller;
	const auto source = (_request.section == Section::Scheduled) ? Api::SendType::Scheduled : Api::SendType::Normal;
	const auto sendMenuType = (_request.section == Section::Scheduled)				? SendMenu::Type::Disabled
		: (_request.section == Section::Replies || _peer->starsPerMessageChecked()) ? SendMenu::Type::SilentOnly
	...
	auto callback = [=]
	{ PeerMenuCreatePoll(controller, peer, replyTo, suggest, chosen, PollData::Flags(), source, {sendMenuType}); };
	_addAction(tr::lng_polls_create(tr::now), std::move(callback), &st::menuIconCreatePoll);
}
...
void Filler::addCreateTodoList() {
	...
	const auto can = _topic ? (_peer->session().premium() && Data::CanSend(_topic, ChatRestriction::SendPolls))
	...
	const auto replyTo = _request.currentReplyTo;
	const auto suggest = _request.currentSuggest;
	auto callback = [=] { PeerMenuCreateTodoList(controller, peer, replyTo, suggest, source, {sendMenuType}); };
	_addAction(tr::lng_todo_create(tr::now), std::move(callback), &st::menuIconCreateTodoList);
}
...
void ShowDisableSharingBox(not_null<SessionController *> controller,
	...
	controller->show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			...
			const auto buttonPadding =
				QMargins(st::showOrBox.buttonPadding.left(), 0, st::showOrBox.buttonPadding.right(), 0);

			auto icon = Settings::CreateLottieIcon(box,
												   {
													   .name = u"stop"_q,
													   .sizeOverride = st::normalBoxLottieSize,
												   },
												   {});
			box->verticalLayout()->add(std::move(icon.widget), st::disableSharingIconPadding, style::al_top);

			Ui::AddSkip(box->verticalLayout());

			box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_disable_sharing_title(), st::boostCenteredTitle),
			...
			for (const auto &feature : features) {
				box->addRow(Ui::MakeFeatureListEntry(box, feature), st::boxRowPadding);
			}
			...
			button->setText(Data::AmPremiumValue(&peer->session()) |
							rpl::map(
								[](bool premium)
								{
									if (premium) {
										return TextWithEntities{tr::lng_disable_sharing_button(tr::now)};
									}
									return Ui::Text::IconEmoji(&st::disableSharingButtonLock)
										.append(' ' + tr::lng_disable_sharing_unlock(tr::now));
								}));

			box->setShowFinishedCallback([animate = std::move(icon.animate)] { animate(anim::repeat::once); });
		}));
}

void Filler::addToggleNoForwards() {
	const auto user = _peer->asUser();
	if (!user || user->isInaccessible() || user->isBot() || user->isServiceUser() || user->isSelf()) {
		return;
	}
	const auto controller = _controller;
	const auto peer = _peer;
	const auto toggleNoForwards = [=](bool enabled)
	{
		...
		peer->session()
			.api()
			.request(MTPmessages_ToggleNoForwards(MTP_flags(Flag()), peer->input(), MTP_bool(enabled), MTPint()))
			.done(
				[=](const MTPUpdates &result)
				{
	...
	_addAction(
		disabledNow ? tr::lng_enable_sharing(tr::now) : tr::lng_disable_sharing(tr::now),
		[=]
		{
			if (controller->showFrozenError()) {
				return;
			} else if (disabledNow) {
				const auto willBeRequest = true && (user->flags() & UserDataFlag::NoForwardsPeerEnabled) &&
					!peer->session().data().sharingRecentlyDisabledByMe(user);
				if (willBeRequest) {
					controller->show(Ui::MakeConfirmBox({
						.text = tr::lng_enable_sharing_request_text(
							tr::now, lt_name, tr::marked(user->shortName()), tr::rich),
						.confirmed =
							[=](Fn<void()> close)
						{
...
void Filler::addTTLSubmenu(bool addSeparator) {
	...
	const auto validator = TTLMenu::TTLValidator(_controller->uiShow(), _peer);
	if (!validator.can()) {
		return;
	}
	const auto text = tr::lng_manage_messages_ttl_menu(tr::now) +
		(_peer->messagesTTL() ? ('\t' + Ui::FormatTTLTiny(_peer->messagesTTL())) : QString());
...
void Filler::addSendGift() {
	...
	if (!user && !channel) {
		return;
			   ...
			   (user->isInaccessible() || user->isSelf() || user->isBot() || user->isServiceUser() ||
				user->isNotificationsUser() || user->isRepliesChat() || user->isVerifyCodes() ||
				!user->session().premiumCanBuy())) {
		return;
	} else if (channel && (channel->isForbidden() || !channel->stargiftsAvailable())) {
		return;
	}
	...
	const auto navigation = _controller;
	_addAction(
		tr::lng_profile_gift_premium(tr::now),
...
void Filler::addPinnedMessages() {
	...
	if (channel->owner().history(channel)->hasPinnedMessages()) {
		...
		if (hasHidden) {
			_addAction(
				tr::lng_pinned_message_show(tr::now), [=] { PeerMenuUnhidePinnedMessage(channel); }, &st::menuIconEdit);
		} else {
			_addAction(
				tr::lng_pinned_message_hide(tr::now), [=] { PeerMenuHidePinnedMessage(channel); }, &st::menuIconEdit);
		}
		_addAction(
...
void Filler::addFirstMessage() {
	...
	_addAction(
		tr::lng_go_to_first_message(tr::now),
		[=] { App::wnd()->sessionController()->showPeerHistory(peer, Window::SectionShow::Way::Forward, 1); },
...
void Filler::addViewChannel() {
	...
	if (const auto chat = peer->discussionLink()) {
		_addAction(
			peer->isMegagroup() ? tr::lng_profile_view_channel(tr::now) : tr::lng_profile_view_discussion(tr::now),
			[=] { App::wnd()->sessionController()->showPeerHistory(chat, Window::SectionShow::Way::Forward); },
			&st::menuIconDiscussion);
	}
}
...
void Filler::addCreateTopic() {
	...
	const auto controller = _controller;
	_addAction(
		tr::lng_forum_create_topic(tr::now),
		[=]
		{
			if (const auto forum = peer->forum()) {
				controller->show(Box(NewForumTopicBox, controller, forum->history()));
			}
		},
		&st::menuIconDiscussion);
	_addAction(PeerMenuCallback::Args{.isSeparator = true});
...
void Filler::addViewAsMessages() {
	...
	const auto filterOutChatPreview = [=]
	{
		if (base::IsAltPressed()) {
			...
			Ui::PreventDelayedActivation();
			controller->showInNewWindow(SeparateId(SeparateType::Chat, peer->owner().history(peer)));
			return true;
		}
		return false;
	};
	...
	{
		auto owned =
			base::make_unique_q<Ui::Menu::Action>(popupMenu->menu(),
												  popupMenu->menu()->st(),
												  Ui::Menu::CreateAction(popupMenu->menu(),
...
void Filler::addViewAsTopics() {
	if (!_peer || !_peer->isForum() || !_peer->isChannel() ||
		(_peer->asChannel()->flags() & ChannelDataFlag::ForumTabs) || !_controller->adaptive().isOneColumn()) {
		return;
	}
	const auto peer = _peer;
	const auto controller = _controller;
	_addAction(
		tr::lng_forum_view_as_topics(tr::now),
		[=]
		{
			if (const auto forum = peer->forum()) {
...
void Filler::addSearchTopics() {
	...
	const auto controller = _controller;
	_addAction(tr::lng_dlg_filter(tr::now), [=] { controller->searchInChat(history); }, &st::menuIconSearch);
}
...
void Filler::addVideoChat() {
	...
	FillVideoChatMenu(_controller, _request, Ui::Menu::CreateAddActionCallback(&test));
	...
	_addAction(PeerMenuCallback::Args{
		...
		{ FillVideoChatMenu(_controller, _request, Ui::Menu::CreateAddActionCallback(menu)); },
	});
}
...
void Filler::fillArchiveActions() {
	...
	if (!inmenu) {
		const auto text = hidden ? tr::lng_context_archive_expand(tr::now) : tr::lng_context_archive_collapse(tr::now);
		_addAction(
			text,
			[=]
			{
	...
	{
		const auto text = inmenu ? tr::lng_context_archive_to_list(tr::now) : tr::lng_context_archive_to_menu(tr::now);
		_addAction(
			text,
			[=]
			{
	...
	MenuAddMarkAsReadChatListAction(controller, [folder = _folder] { return folder->chatsList(); }, _addAction);

	_addAction({.isSeparator = true});

	Settings::PreloadArchiveSettings(&controller->session());
	const auto openSettings = [=] { controller->show(Box(Settings::ArchiveSettingsBox, controller)); };
	_addAction(tr::lng_context_archive_settings(tr::now), openSettings, &st::menuIconManage);
	_addAction(
		tr::lng_context_archive_how_does_it_work(tr::now),
		[=]
		{
			const auto unarchiveOnNewMessage =
				controller->session().api().globalPrivacy().unarchiveOnNewMessageCurrent();
			controller->show(
				Box(ArchiveHintBox, unarchiveOnNewMessage != Api::UnarchiveOnNewMessage::None, openSettings),
...
void Filler::addToggleFee() {
	...
	const auto text = feeRemoved ? tr::lng_context_charge_fee(tr::now) : tr::lng_context_remove_fee(tr::now);
	const auto navigation = _controller;
	...
	_addAction(
		...
		[=]
		{
			...
			PeerMenuConfirmToggleFee(navigation, paidAmount, parent, user, removeFee);
		},
	...
	_addAction({.make = [=](not_null<Ui::PopupMenu *> menuParent)
				{
					...
					const auto text = feeRemoved
						? tr::lng_context_fee_free(tr::now, lt_name, TextWithEntities{user->shortName()}, tr::marked)
						: tr::lng_context_fee_now(
							  tr::now,
							  lt_name,
							  TextWithEntities{user->shortName()},
							  lt_amount,
							  helper.paletteDependent(Ui::Earn::IconCurrencyEmojiSmall())
								  .append(Lang::FormatCountDecimal(user->owner().commonStarsPerMessage(parent))),
							  tr::marked);
					const auto action = Ui::CreateChild<QAction>(actionParent);
					action->setDisabled(true);
					auto result = base::make_unique_q<Ui::Menu::Action>(
						actionParent, st::windowFeeItem, action, nullptr, nullptr);
					result->setMarkedText(text, QString(), Core::TextContext({.session = &user->session()}));
					return result;
				}});
}
...
void Filler::addSetPersonalChannel() {
	const auto channel = _peer->asChannel();
	if (!channel || _peer->session().user()->personalChannelId() || channel->isMegagroup() || !channel->amCreator() ||
		!channel->isPublic()) {
		return;
	}
	_addAction(
		tr::lng_edit_channel_personal_channel(tr::now),
		[=] { UrlClickHandler::Open(u"internal:edit_personal_channel:"_q + QString::number(channel->id.value)); },
		&st::menuIconProfile);
}
...
void PeerMenuExportChat(not_null<Window::SessionController *> controller, not_null<PeerData *> peer) {
	base::call_delayed(st::defaultPopupMenu.showDuration, [=] { Core::App().exportManager().start(peer); });
}

void PeerMenuExportTopic(not_null<Window::SessionNavigation *> navigation,
						 not_null<PeerData *> peer,
						 MsgId topicRootId) {
...
void PeerMenuDeleteContact(not_null<Window::SessionController *> controller, not_null<UserData *> user) {
	if (controller->showFrozenError()) {
		return;
	}
	const auto text = tr::lng_sure_delete_contact(tr::now, lt_contact, user->name());
	const auto deleteSure = [=](Fn<void()> &&close)
	{
		close();
		user->session()
			.api()
			.request(MTPcontacts_DeleteContacts(MTP_vector<MTPInputUser>(1, user->inputUser())))
			.done([=](const MTPUpdates &result) { user->session().api().applyUpdates(result); })
			.send();
	};
	auto box = Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			Ui::AddSkip(box->verticalLayout());
			Ui::IconWithTitle(box->verticalLayout(),
							  Ui::CreateChild<Ui::UserpicButton>(box, user, st::mainMenuUserpic),
							  Ui::CreateChild<Ui::FlatLabel>(
								  box, tr::lng_info_delete_contact(tr::bold), box->getDelegate()->style().title));
			Ui::ConfirmBox(box,
						   {
							   .text = text,
...
void PeerMenuDeleteTopicWithConfirmation(not_null<Window::SessionNavigation *> navigation,
	...
	controller->show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			Ui::AddSkip(box->verticalLayout());
			Ui::IconWithTitle(box->verticalLayout(),
							  Ui::CreateChild<Info::Profile::TopicIconButton>(box, controller, topic),
							  Ui::CreateChild<Ui::FlatLabel>(box, topic->title(), box->getDelegate()->style().title));
			Ui::AddSkip(box->verticalLayout());
			Ui::AddSkip(box->verticalLayout());
			Ui::ConfirmBox(box,
						   {
...
void PeerMenuDeleteTopic(not_null<Window::SessionNavigation *> navigation, not_null<PeerData *> peer, MsgId rootId) {
	const auto api = &peer->session().api();
	api->request(MTPmessages_DeleteTopicHistory(peer->input(), MTP_int(rootId)))
		.done(
...
void PeerMenuDeleteTopic(not_null<Window::SessionNavigation *> navigation, not_null<Data::ForumTopic *> topic) {
	PeerMenuDeleteTopic(navigation, topic->peer(), topic->rootId());
}

void PeerMenuShareContactBox(not_null<Window::SessionNavigation *> navigation, not_null<UserData *> user) {
	...
	auto callback = [=](not_null<Data::Thread *> thread)
	{
		...
		if (!Data::CanSend(thread, ChatRestriction::SendOther)) {
			navigation->parentController()->show(Ui::MakeInformBox(tr::lng_forward_share_cant()));
			return;
		} else if (peer->isSelf()) {
		...
		const auto title = thread->asTopic() ? thread->asTopic()->title() : peer->name();
		auto recipient = peer->isUser() ? title : ('\xAB' + title + '\xBB');
		struct State
		{
		...
		state->share = [=](Api::SendOptions options)
		{
			...
			const auto checked = state->sendPayment.check(navigation, peer, action.options, 1, withPaymentApproved);
			if (!checked) {
				return;
			}
			navigation->showThread(strong, ShowAtTheEndMsgId, Window::SectionShow::Way::ClearStack);
			strong->session().api().shareContact(user, action);
			state->share = nullptr;
		};

		navigation->parentController()->show(Ui::MakeConfirmBox({
			.text = tr::lng_forward_share_contact(tr::now, lt_recipient, recipient),
			.confirmed =
				[state](Fn<void()> &&close)
			{
				state->share({});
	...
	*weak = navigation->parentController()->show(Box<PeerListBox>(
		...
		[](not_null<PeerListBox *> box) { box->addButton(tr::lng_cancel(), [=] { box->closeBox(); }); }));
}

void PeerMenuCreatePoll(not_null<Window::SessionController *> controller,
						not_null<PeerData *> peer,
						FullReplyTo replyTo,
	...
	auto starsRequired = peer->session().changes().peerFlagsValue(
							 peer, Data::PeerUpdate::Flag::FullInfo | Data::PeerUpdate::Flag::StarsPerMessage) |
		rpl::map([=] { return peer->starsPerMessageChecked(); });
	auto box =
		Box<CreatePollBox>(controller, peer, chosen, disabled, std::move(starsRequired), sendType, sendMenuDetails);
	struct State
	{
	...
	{
		auto action = Api::SendAction(peer->owner().history(peer), result.options);
		...
		const auto checked = state->sendPayment.check(controller, peer, action.options, 1, withPaymentApproved);
		if (!checked || std::exchange(state->lock, true)) {
			return;
		}

		const auto local = action.history->localDraft(replyTo.topicRootId, replyTo.monoforumPeerId);
		if (local) {
	...
	box->submitRequests() | rpl::on_next(state->create, box->lifetime());
	controller->show(std::move(box), Ui::LayerOption::CloseOther);
}
...
void PeerMenuTodoWantsPremium(TodoWantsPremium type) {
	...
	const auto link = tr::link(tr::semibold(tr::lng_todo_premium_link(tr::now)));
	const auto text = [&]
	{
		switch (type) {
	...
	window->uiShow()->showToast(Ui::Toast::Config{
		.text = text(tr::now, lt_link, link, tr::marked),
		.filter = filter,
		.duration = kToastDuration,
	});
}

void PeerMenuCreateTodoList(not_null<Window::SessionController *> controller,
							not_null<PeerData *> peer,
							FullReplyTo replyTo,
							SuggestOptions suggest,
							Api::SendType sendType,
	...
	auto starsRequired = peer->session().changes().peerFlagsValue(
							 peer, Data::PeerUpdate::Flag::FullInfo | Data::PeerUpdate::Flag::StarsPerMessage) |
		rpl::map([=] { return peer->starsPerMessageChecked(); });
	auto box = Box<EditTodoListBox>(controller, std::move(starsRequired), sendType, sendMenuDetails);
	struct State
	{
	...
	state->create = [=](const EditTodoListBox::Result &result)
	{
		...
		const auto api = &peer->session().api();
		api->todoLists().create(result.todolist,
								action,
								crl::guard(weak,
										   [=]
										   {
...
void PeerMenuEditTodoList(not_null<Window::SessionController *> controller, not_null<HistoryItem *> item) {
	...
	box->submitRequests() |
		rpl::on_next(
			[=](const EditTodoListBox::Result &result)
			{
				const auto api = &item->history()->session().api();
				api->todoLists().edit(item,
									  result.todolist,
									  result.options,
									  crl::guard(weak, [=] { weak->closeBox(); }),
									  crl::guard(weak, [=](const QString &error) { weak->submitFailed(error); }));
			},
			box->lifetime());
	controller->show(std::move(box), Ui::LayerOption::CloseOther);
}
...
bool PeerMenuShowAddTodoListTasks(not_null<HistoryItem *> item) {
	...
	return item->isRegular() && !item->Has<HistoryMessageForwarded>() && todolist &&
		(todolist->items.size() < appConfig->todoListItemsLimit()) &&
		(item->out() || item->history()->peer->isSelf() || todolist->othersCanAppend());
}

void PeerMenuAddTodoListTasks(not_null<Window::SessionController *> controller, not_null<HistoryItem *> item) {
	...
	box->submitRequests() |
		rpl::on_next(
			[=](const AddTodoListTasksBox::Result &result)
			{
				const auto show = raw->uiShow();
				raw->closeBox();
				session->api().todoLists().add(
					item, result.items, [] {}, [=](const QString &error) { show->showToast(error); });
			},
			box->lifetime());
	controller->show(std::move(box), Ui::LayerOption::CloseOther);
}

void PeerMenuBlockUserBox(not_null<Ui::GenericBox *> box,
	...
	const auto reportNeeded =
		v::is_null(suggestReport) ? ((settings & PeerBarSetting::ReportSpam) != 0) : v::get<bool>(suggestReport);
	...
	if (user) {
		box->addRow(object_ptr<Ui::FlatLabel>(
			box,
			tr::lng_blocked_list_confirm_text(lt_name, rpl::single(tr::bold(name)), tr::marked),
			st::blockUserConfirmation));

		box->addSkip(st::boxMediumSkip);
	}
	const auto report = reportNeeded
		? box->addRow(object_ptr<Ui::Checkbox>(box, tr::lng_report_spam(tr::now), true, st::defaultBoxCheckbox))
		: nullptr;
	...
	const auto clear = v::is<ClearChat>(suggestClear)
		? box->addRow(
			  object_ptr<Ui::Checkbox>(box, tr::lng_blocked_list_confirm_clear(tr::now), true, st::defaultBoxCheckbox))
		: v::is<ClearReply>(suggestClear)
		? box->addRow(object_ptr<Ui::Checkbox>(box, tr::lng_context_delete_msg(tr::now), true, st::defaultBoxCheckbox))
		: nullptr;
	if (clear) {
		box->addSkip(st::boxMediumSkip);
	}
	const auto allFromUser = v::is<ClearReply>(suggestClear)
		? box->addRow(object_ptr<Ui::Checkbox>(
			  box,
			  tr::lng_delete_all_from_user(tr::now, lt_user, tr::bold(peer->name()), tr::marked),
			  true,
	...
	box->setTitle(tr::lng_blocked_list_confirm_title(lt_name, rpl::single(name)));

	box->addButton(
		tr::lng_blocked_list_confirm_ok(),
		[=]
		{
			...
			if (const auto clearReply = std::get_if<ClearReply>(&suggestClear)) {
				...
				peer->session()
					.api()
					.request(
						MTPcontacts_BlockFromReplies(MTP_flags((clearChecked ? Flag::f_delete_message : Flag(0)) |
															   (fromUserChecked ? Flag::f_delete_history : Flag(0)) |
															   (reportChecked ? Flag::f_report_spam : Flag(0))),
													 MTP_int(clearReply->replyId.msg)))
					.done([=](const MTPUpdates &result) { peer->session().updates().applyUpdates(result); })
					.send();
			} else {
				peer->session().api().blockedPeers().block(peer);
				if (reportChecked) {
					peer->session().api().request(MTPmessages_ReportSpam(peer->input())).send();
				}
				if (clearChecked) {
					crl::on_main(&peer->session(), [=] { peer->session().api().deleteConversation(peer, false); });
					window->sessionController()->showBackFromStack();
				}
			}

			window->showToast(tr::lng_new_contact_block_done(tr::now, lt_user, name));
		},
...
void PeerMenuUnblockUserWithBotRestart(std::shared_ptr<Ui::Show> show, not_null<UserData *> user) {
...
void BlockSenderFromRepliesBox(not_null<Ui::GenericBox *> box, not_null<SessionController *> controller, FullMsgId id) {
	const auto item = controller->session().data().message(id);
	Assert(item != nullptr);

	PeerMenuBlockUserBox(box, &controller->window(), item->originalSender(), true, Window::ClearReply{id});
}

object_ptr<Ui::BoxContent>
PrepareChooseRecipientBox(not_null<Main::Session *> session,
						  FnMut<bool(not_null<Data::Thread *>)> &&chosen,
						  rpl::producer<QString> titleOverride,
						  FnMut<void()> &&successCallback,
						  InlineBots::PeerTypes typesRestriction,
						  Fn<void(std::vector<not_null<Data::Thread *>>, Api::SendOptions)> sendMany) {
	...
	public:
		...
		base::unique_qptr<Ui::PopupMenu> rowContextMenu(QWidget *parent, not_null<PeerListRow *> row) override final {
			if (!_selectable) {
				return ChooseRecipientBoxController::rowContextMenu(parent, row);
			}
			if (!row->checked() && !row->peer()->isForum() && !row->peer()->isMonoforum()) {
				auto menu = base::make_unique_q<Ui::PopupMenu>(parent, st::popupMenuWithIcons);
				menu->addAction(
					tr::lng_bot_choose_chat(tr::now),
					[=]
					{
		...
		[[nodiscard]] rpl::producer<> selectionChanges() const { return _selectionChanges.events_starting_with({}); }
		[[nodiscard]] bool hasSelected() const { return delegate()->peerListSelectedRowsCount() > 0; }

		[[nodiscard]] rpl::producer<Chosen> singleChosen() const { return _singleChosen.events(); }
	...
	auto callback = [chosen = std::move(chosen), success = std::move(successCallback), weak](
	...
	auto filter = typesRestriction ? [=](not_null<Data::Thread *> thread) -> bool
	{
	...
	auto controller = std::make_unique<Controller>(session, std::move(callback), std::move(filter), selectable);
				...
				[=]
				{
					...
					if (shown) {
						...
						state->submit = [=](Api::SendOptions options)
						{
							...
							if (!waiting.empty()) {
								session->changes().peerUpdates(Data::PeerUpdate::Flag::FullInfo) |
									rpl::on_next(
								...
								if (!session->credits().loaded()) {
									session->credits().loadedValue() | rpl::filter(rpl::mappers::_1) | rpl::take(1) |
										rpl::on_next([=] { withPaymentApproved(alreadyApproved); },
													 state->submitLifetime);
								}
								return;
							} else if (totalStars > alreadyApproved) {
								ShowSendPaidConfirm(show,
													paid,
													SendPaymentDetails{
														.messages = 1,
														.stars = totalStars,
							...
							sendMany(ranges::views::all(peers) |
										 ranges::views::transform([&](not_null<PeerData *> peer) -> Controller::Chosen
						...
						send->setText(PaidSendButtonText(state->starsToSend.value(), tr::lng_send_button()));
					}
					box->addButton(tr::lng_cancel(), [=] { box->closeBox(); });
				},
				box->lifetime());
	...
	auto result = Box<PeerListBox>(std::move(controller), std::move(initBox));
	*weak = result.data();

	return result;
}

base::weak_qptr<Ui::BoxContent> ShowChooseRecipientBox(not_null<Window::SessionNavigation *> navigation,
													   FnMut<bool(not_null<Data::Thread *>)> &&chosen,
													   rpl::producer<QString> titleOverride,
													   FnMut<void()> &&successCallback,
													   InlineBots::PeerTypes typesRestriction) {
	return navigation->parentController()->show(PrepareChooseRecipientBox(&navigation->session(),
																		  std::move(chosen),
...
Api::SendAction prepareSendAction(History *history, Api::SendOptions options) {
...
QPointer<Ui::BoxContent> ShowNewForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
	...
	const auto isGame = item->getMessageBot() && item->media() && (item->media()->game() != nullptr);

	const auto items = owner->idsToItems(msgIds);
	const auto hasCaptions = ranges::any_of(
		items,
		[](auto item)
		{ return item->media() && !item->originalText().text.isEmpty() && item->media()->allowsEditCaption(); });
	const auto hasOnlyForcedForwardedInfo = hasCaptions
		? false
		: ranges::all_of(items, [](auto item) { return item->media() && item->media()->forceForwardedInfo(); });
	...
	auto filterCallback = [=](not_null<Data::Thread *> thread)
	{
		...
		return Data::CanSend(thread, requiredRight) &&
			(!requiresInline || Data::CanSend(thread, ChatRestriction::SendInline)) &&
	...
	*weak = Ui::show(Box<ShareBox>(ShareBox::Descriptor{
						 .session = session,
						 .submitCallback = ShareBox::DefaultForwardCallback(navigation->parentController()->uiShow(),
																			history,
																			msgIds,
																			TimeId(0),
																			no_quote,
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(std::shared_ptr<ChatHelpers::Show> show,
	...
	{
	public:
		ListBox(QWidget *parent, std::unique_ptr<PeerListController> controller, Fn<void(not_null<ListBox *>)> init)
			: PeerListBox(parent,
						  std::move(controller),
						  [=](not_null<PeerListBox *> box) { init(static_cast<ListBox *>(box.get())); }) {}

		void setBottomSkip(int bottomSkip) { PeerListBox::setInnerBottomSkip(bottomSkip); }

		[[nodiscard]] rpl::producer<> focusRequests() const { return _focusRequests.events(); }

		[[nodiscard]] Data::ForwardOptions forwardOptionsData() const {
			return (_forwardOptions.captionsCount && _forwardOptions.dropCaptions)
				? Data::ForwardOptions::NoNamesAndCaptions
				: _forwardOptions.dropNames ? Data::ForwardOptions::NoSenderNames
											: Data::ForwardOptions::PreserveInfo;
		}
		[[nodiscard]] Ui::ForwardOptions forwardOptions() const { return _forwardOptions; }
		void setForwardOptions(Ui::ForwardOptions forwardOptions) { _forwardOptions = forwardOptions; }

		not_null<PeerListContent *> peerListContent() const { return PeerListBox::content(); }

		void setFilterId(FilterId filterId) { _filterId = filterId; }
		[[nodiscard]] FilterId filterId() const { return _filterId; }
	...
	public:
		...
		Controller(not_null<Main::Session *> session)
			: ChooseRecipientBoxController({
				  .session = session,
				  .callback = [=](Chosen thread) { _singleChosen.fire_copy(thread); },
				  .moneyRestrictionError = WriteMoneyRestrictionError,
			  }) {}

		std::unique_ptr<PeerListRow> createRestoredRow(not_null<PeerData *> peer) override final {
			return ChooseRecipientBoxController::createRow(peer->owner().history(peer));
		}
	...
	const auto state = [&]
	{
		...
		{
			controllerRaw->setSearchNoResultsText(tr::lng_bot_chats_not_found(tr::now));
			...
			chatsFilters->lower();
			rpl::combine(chatsFilters->heightValue(),
						 rpl::producer<bool>(
							 [=](auto consumer)
							 {
								 auto lifetime = rpl::lifetime();
								 consumer.put_next(false);
								 box->appendQueryChangedCallback(
									 [=](const QString &q)
									 {
				...
				rpl::on_next([box](int h, bool hasQuery) { box->setAddedTopScrollSkip(hasQuery ? 0 : h); },
							 box->lifetime());
			box->multiSelectHeightValue() |
				rpl::on_next([=](int h) { chatsFilters->moveToLeft(0, h); }, chatsFilters->lifetime());
		};
	...
	{ // Chosen a single.
		...
		{
			const auto peer = thread->peer();
			if (peer->isSelf() && !draft.ids.empty() && draft.ids.front().peer != peer->id) {
				ForwardToSelf(show, draft);
				return true;
			}
			const auto id = SeparateId(
				((peer->isForum() && !peer->useSubsectionTabs()) ? SeparateType::Forum : SeparateType::Chat), thread);
		...
		auto callback = [=, chosen = std::move(chosen)](Controller::Chosen thread) mutable
		{
		...
		state->controller->singleChosen() | rpl::on_next(std::move(callback), state->box->lifetime());
	}

	const auto comment = Ui::CreateChild<Ui::SlideWrap<Ui::InputField>>(
		state->box.get(),
		object_ptr<Ui::InputField>(
			state->box, st::shareComment, Ui::InputField::Mode::MultiLine, tr::lng_photos_comment()),
		st::shareCommentPadding);

	const auto history = session->data().message(msgIds.front())->history();
	const auto send = ShareBox::DefaultForwardCallback(show, history, msgIds);
	const auto countMessages = ShareBox::DefaultForwardCountMessages(history, msgIds);
	...
	state->submit = [=](Api::SendOptions options)
	{
		...
		const auto checkPaid = [=]
		{
			...
			if (!waiting.empty()) {
				...
				if (!session->credits().loaded()) {
					session->credits().loadedValue() | rpl::filter(rpl::mappers::_1) | rpl::take(1) |
						rpl::on_next([=] { withPaymentApproved(alreadyApproved); }, state->submitLifetime);
				}
				return false;
			} else if (totalStars > alreadyApproved) {
				ShowSendPaidConfirm(show,
	...
	const auto sendMenuType = [=]
	{
		...
		return hasPaid														? SendMenu::Type::SilentOnly
			: ranges::all_of(selected, HistoryView::CanScheduleUntilOnline) ? SendMenu::Type::ScheduledToUser
			: ((selected.size() == 1) && selected.front()->isSelf())		? SendMenu::Type::Reminder
																			: SendMenu::Type::Scheduled;
	};
	...
	const auto showMenu = [=](not_null<Ui::RpWidget *> parent)
	{
		...
		if (showForwardOptions) {
			...
			{
				auto item =
					base::make_unique_q<Menu::ItemWithCheck>(state->menu->menu(),
															 st::popupMenuWithIcons.menu,
															 Ui::CreateChild<QAction>(state->menu->menu().get()),
															 nullptr,
															 nullptr);
				std::move(text) |
					rpl::on_next([action = item->action()](QString text) { action->setText(text); }, item->lifetime());
				item->init(checked);
			...
			Ui::FillForwardOptions(
				...
				[=](Ui::ForwardOptions o) { state->box->setForwardOptions(o); },
				state->menu->lifetime());

			state->menu->addSeparator();
		}
		state->menu->setForcedVerticalOrigin(Ui::PopupMenu::VerticalOrigin::Bottom);
		SendMenu::FillSendMenu(state->menu.get(),
							   show,
							   SendMenu::Details{sendMenuType()},
							   SendMenu::DefaultCallback(show,
														 crl::guard(parent,
	...
	state->refreshStarsToSend = [=]
	{
		...
		state->starsToSend = perMessage * countMessages(field->getTextWithTags());
	};

	comment->hide(anim::type::instant);
	comment->toggleOn(state->controller->selectionChanges() |
					  rpl::map([=] { return state->controller->hasSelected(); }));

	rpl::combine(state->box->sizeValue(), comment->heightValue()) |
		rpl::on_next(
			[=](const QSize &size, int commentHeight)
			{
	...
	InitMessageFieldHandlers({
		...
		.show = show,
		.field = field,
		.customEmojiPaused = [=] { return show->paused(GifPauseReason::Layer); },
	});
	field->setSubmitSettings(Core::App().settings().sendSubmitWay());
	field->changes() | rpl::on_next([=] { state->refreshStarsToSend(); }, field->lifetime());
	...
	state->controller->selectionChanges() |
		rpl::on_next(
			[=]
			{
				...
				state->refreshStarsToSend();
				if (shown) {
					const auto send = state->box->addButton(tr::lng_send_button(),
															[=]
															{
																if (const auto onstack = state->submit) {
																	onstack({});
				...
				state->box->addButton(tr::lng_cancel(), [=] { state->box->closeBox(); });
			},
			state->box->lifetime());
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
													   Data::ForwardDraft &&draft,
													   Fn<void()> &&successCallback) {
	return ShowForwardMessagesBox(navigation->uiShow(), std::move(draft), std::move(successCallback));
}

base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
													   MessageIdsList &&items,
													   Fn<void()> &&successCallback) {
	return ShowForwardMessagesBox(navigation->uiShow(), Data::ForwardDraft{.ids = items}, std::move(successCallback));
}

base::weak_qptr<Ui::BoxContent>
ShowShareGameBox(not_null<Window::SessionNavigation *> navigation, not_null<UserData *> bot, QString shortName) {
	const auto weak = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
	auto chosen = [=](not_null<Data::Thread *> thread) mutable
	{
		...
		auto send = crl::guard(thread,
							   [=]
							   {
								   ...
								   navigation->showThread(thread, ShowAtUnreadMsgId, SectionShow::Way::ClearStack);
							   });
		const auto confirmText = thread->peer()->isUser()
			? tr::lng_bot_sure_share_game(tr::now, lt_user, thread->chatListName())
			: tr::lng_bot_sure_share_game_group(tr::now, lt_group, thread->chatListName());
		*confirm = navigation->parentController()->show(Ui::MakeConfirmBox({
			.text = confirmText,
			.confirmed = std::move(send),
		}));
	};
	auto filter = [](not_null<Data::Thread *> thread)
	{ return !thread->peer()->isSelf() && (Data::CanSend(thread, ChatRestriction::SendGames) || thread->asForum()); };
	auto initBox = [](not_null<PeerListBox *> box) { box->addButton(tr::lng_cancel(), [box] { box->closeBox(); }); };
	*weak = navigation->parentController()->show(
		Box<PeerListBox>(std::make_unique<ChooseRecipientBoxController>(ChooseRecipientArgs{
							 .session = &navigation->session(),
...
base::weak_qptr<Ui::BoxContent> ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
												 std::shared_ptr<QMimeData> data,
												 not_null<Data::Forum *> forum,
												 FnMut<void()> &&successCallback) {
	const auto weak = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
	auto chosen = [data = std::move(data), callback = std::move(successCallback), weak, navigation](
					  not_null<Data::Thread *> thread) mutable
	{
	...
	auto initBox = [=](not_null<PeerListBox *> box)
	{
		...
		forum->destroyed() | rpl::on_next([=] { box->closeBox(); }, box->lifetime());
	};
	*weak = navigation->parentController()->show(
		Box<PeerListBox>(std::make_unique<ChooseTopicBoxController>(forum, std::move(chosen)), std::move(initBox)));
	return weak->get();
}
...
base::weak_qptr<Ui::BoxContent> ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
	...
	auto initBox = [=](not_null<PeerListBox *> box)
	{
		...
		monoforum->destroyed() | rpl::on_next([=] { box->closeBox(); }, box->lifetime());
	};
	*weak = navigation->parentController()->show(Box<PeerListBox>(
		std::make_unique<ChooseSublistBoxController>(monoforum, std::move(chosen)), std::move(initBox)));
	return weak->get();
}

base::weak_qptr<Ui::BoxContent> ShowSendNowMessagesBox(not_null<Window::SessionNavigation *> navigation,
													   not_null<History *> history,
													   MessageIdsList &&items,
													   Fn<void()> &&successCallback) {
	const auto session = &navigation->session();
	const auto text = (items.size() > 1) ? tr::lng_scheduled_send_now_many(tr::now, lt_count, items.size())
										 : tr::lng_scheduled_send_now(tr::now);

	const auto list = session->data().idsToItems(items);
	const auto error = GetErrorForSending(history->peer, {.forward = &list});
	if (error) {
		Data::ShowSendErrorToast(navigation, history->peer, error);
		return {nullptr};
	}
	auto done = [=, list = std::move(items), callback = std::move(successCallback)](Fn<void()> &&close)
	{
		close();
		...
		session->api()
			.request(MTPmessages_SendScheduledMessages(history->peer->input(), MTP_vector<MTPint>(ids)))
			.done([=](const MTPUpdates &result) { session->api().applyUpdates(result); })
			.fail([=](const MTP::Error &error) { session->api().sendMessageFail(error, history->peer); })
			.send();
		if (callback) {
			callback();
		}
...
void PeerMenuAddChannelMembers(not_null<Window::SessionNavigation *> navigation, not_null<ChannelData *> channel) {
	if (!channel->isMegagroup() && (channel->membersCount() >= channel->session().serverConfig().chatSizeMax)) {
		navigation->parentController()->show(Box<MaxInviteBox>(channel));
		return;
	}
	const auto api = &channel->session().api();
	api->chatParticipants().requestForAdd(
		channel,
		crl::guard(navigation,
				   [=](const Api::ChatParticipants::TLMembers &data)
				   {
					   const auto &[availableCount, list] = Api::ChatParticipants::Parse(channel, data);
					   const auto already =
						   (list) |
						   ranges::views::transform(
							   [&](const Api::ChatParticipant &p)
							   { return p.isUser() ? channel->owner().userLoaded(p.userId()) : nullptr; }) |
						   ranges::views::filter([](UserData *user) { return (user != nullptr); }) | ranges::to_vector;

					   AddParticipantsBoxController::Start(navigation, channel, {already.begin(), already.end()});
				   }));
}

void ToggleMessagePinned(not_null<Window::SessionNavigation *> navigation, FullMsgId itemId, bool pin) {
	const auto item = navigation->session().data().message(itemId);
	if (!item || !item->canPin()) {
		return;
	}
	if (pin) {
		navigation->parentController()->show(Box(PinMessageBox, item), Ui::LayerOption::CloseOther);
	} else {
		const auto peer = item->history()->peer;
		const auto session = &peer->session();
		const auto callback = crl::guard(
			session,
			[=](Fn<void()> &&close)
			{
				close();
				session->api()
					.request(MTPmessages_UpdatePinnedMessage(
						MTP_flags(MTPmessages_UpdatePinnedMessage::Flag::f_unpin), peer->input(), MTP_int(itemId.msg)))
					.done([=](const MTPUpdates &result) { session->api().applyUpdates(result); })
					.send();
			});
		navigation->parentController()->show(Ui::MakeConfirmBox({
...
void HidePinnedBar(not_null<Window::SessionNavigation *> navigation,
				   not_null<PeerData *> peer,
				   MsgId topicRootId,
	...
	const auto callback = crl::guard(
		...
		[=](Fn<void()> &&close)
		{
			...
			const auto migrated = (topicRootId || monoforumPeerId) ? nullptr : peer->migrateFrom();
			const auto top = Data::ResolveTopPinnedId(peer, topicRootId, monoforumPeerId, migrated);
			const auto universal = !top					 ? MsgId(0)
				: (migrated && !peerIsChannel(top.peer)) ? (top.msg - ServerMaxMsgId)
														 : top.msg;
			if (universal) {
				session.settings().setHiddenPinnedMessageId(peer->id, topicRootId, monoforumPeerId, universal);
...
void UnpinAllMessages(not_null<Window::SessionNavigation *> navigation, not_null<Data::Thread *> thread) {
	const auto weak = base::make_weak(thread);
	const auto callback = crl::guard(
		navigation,
		[=](Fn<void()> &&close)
		{
			...
			const auto sendRequest = [=](auto self) -> void
			{
				...
				api->request(MTPmessages_UnpinAllMessages(MTP_flags((topicRootId ? Flag::f_top_msg_id : Flag()) |
...
void MenuAddMarkAsReadAllChatsAction(not_null<Main::Session *> session,
		...
		show->show(Box(
					   [=](not_null<Ui::GenericBox *> box)
					   {
						   ...
						   const auto userpic = Ui::CreateChild<Ui::UserpicButton>(
							   box->verticalLayout(), session->user(), st::mainMenuUserpic);
						   Ui::IconWithTitle(box->verticalLayout(),
											 userpic,
											 Ui::CreateChild<Ui::FlatLabel>(box->verticalLayout(),
																			Info::Profile::NameValue(session->user()),
																			box->getDelegate()->style().title));
						   auto text = rpl::combine(tr::lng_context_mark_read_all_sure(),
													tr::lng_context_mark_read_all_sure_2(tr::rich)) |
							   rpl::map(
										   [](QString t1, TextWithEntities t2)
										   {
	...
	addAction(tr::lng_context_mark_read_all(tr::now), std::move(callback), &st::menuIconMarkRead);
}

void MenuAddMarkAsReadChatListAction(not_null<Window::SessionController *> controller,
									 Fn<not_null<Dialogs::MainList *>()> &&list,
									 const PeerMenuCallback &addAction,
									 Fn<Dialogs::UnreadState()> customUnreadState) {
	// There is no async to make weak from controller.
	const auto unreadState = customUnreadState ? customUnreadState() : list()->unreadState();
	...
	auto callback = [=]
	{
		if (unreadState.messages > kMaxUnreadWithoutConfirmation) {
			...
			controller->show(Ui::MakeConfirmBox({tr::lng_context_mark_read_sure(), std::move(boxCallback)}),
							 Ui::LayerOption::CloseOther);
		} else {
			MarkAsReadChatList(list());
		}
	};
	addAction(tr::lng_context_mark_read(tr::now), std::move(callback), &st::menuIconMarkRead);
}

void ToggleHistoryArchived(std::shared_ptr<ChatHelpers::Show> show, not_null<History *> history, bool archived) {
	const auto callback = [=]
	{
		show->showToast(Ui::Toast::Config{
			.text = {(archived ? tr::lng_archived_added(tr::now) : tr::lng_archived_removed(tr::now))},
			.st = &st::windowArchiveToast,
			.duration = (archived ? kArchivedToastDuration : Ui::Toast::kDefaultDuration),
		});
	};
	history->session().api().toggleHistoryArchived(history, archived, callback);
}

Fn<void()> ClearHistoryHandler(not_null<Window::SessionController *> controller, not_null<PeerData *> peer) {
	return [=]
	{
		if (!controller->showFrozenError()) {
...
Fn<void()> DeleteAndLeaveHandler(not_null<Window::SessionController *> controller, not_null<PeerData *> peer) {
	...
	if (isCreator) {
		...
		return [=]
		{
			...
			*requestId = peer->session()
							 .api()
							 .request(MTPmessages_GetFutureChatCreatorAfterLeave(peer->input()))
							 .done(
								 [=](const MTPUser &result)
								 {
									 *requestId = 0;
...
Fn<void()> DeleteSublistHandler(not_null<Window::SessionController *> controller,
...
void FillDialogsEntryMenu(not_null<SessionController *> controller,
						  Dialogs::EntryState request,
						  const PeerMenuCallback &callback) {
	Filler(controller, request, callback).fill();
}

bool FillVideoChatMenu(not_null<SessionController *> controller,
					   Dialogs::EntryState request,
					   const PeerMenuCallback &addAction) {
	...
	const auto rtmpCallback = [=] { Core::App().calls().showStartWithRtmp(controller->uiShow(), peer); };
	...
	if (has) {
		addAction(tr::lng_menu_start_group_call_join(tr::now), [=] { callback({}); }, &st::menuIconVideoChat);
	} else if (manager) {
		addAction((livestream ? tr::lng_menu_start_group_call_channel : tr::lng_menu_start_group_call)(tr::now),
				  [=] { callback({}); },
				  &st::menuIconStartStream);
	}
	if (!has && manager) {
		addAction((livestream ? tr::lng_menu_start_group_call_scheduled_channel
							  : tr::lng_menu_start_group_call_scheduled)(tr::now),
				  [=] { callback({.scheduleNeeded = true}); },
				  &st::menuIconReschedule);
		addAction(
			(livestream ? tr::lng_menu_start_group_call_with_channel : tr::lng_menu_start_group_call_with)(tr::now),
			rtmpCallback,
...
void FillSenderUserpicMenu(not_null<SessionController *> controller,
						   not_null<PeerData *> peer,
						   PeerData *groupPeer,
	...
	const auto viewProfileText = group ? tr::lng_context_view_group(tr::now)
		: channel					   ? tr::lng_context_view_channel(tr::now)
	...
	const auto showHistoryText = group ? tr::lng_context_open_group(tr::now)
		: channel					   ? tr::lng_context_open_channel(tr::now)
	...
	if (const auto guard = mention ? fieldForMention : nullptr) {
		addAction(tr::lng_context_mention(tr::now),
				  crl::guard(guard,
							 [=]
							 {
								 if (!username.isEmpty()) {
									 ...
									 fieldForMention->insertTag(peer->shortName(), PrepareMentionTag(peer->asUser()));
								 }
							 }),
				  &st::menuIconUsername);
	}
	...
	if (const auto user = peer->asUser()) {
		if (groupPeer) {
			...
			const auto isDiscussionGroup =
				groupChannel && groupChannel->isMegagroup() && groupChannel->discussionLink();
			const auto canEditTarget = [&]
			{
				if (const auto chat = groupPeer->asChat()) {
					...
					if (channel->mgInfo &&
						(channel->mgInfo->lastAdmins.contains(user) || channel->mgInfo->creator == user)) {
			...
			if (!isDiscussionGroup && canEditTarget && groupPeer->canManageRanks() && !user->isSelf()) {
				const auto currentRank = LookupMemberRank(groupPeer, user);
				addAction(
					(currentRank.isEmpty() ? tr::lng_context_add_member_tag(tr::now)
										   : tr::lng_context_edit_member_tag(tr::now)),
					[=]
					{
						controller->show(
							Box(EditCustomRankBox, controller->uiShow(), groupPeer, user, currentRank, false, nullptr));
					},
					&st::menuIconEdit);
			}
		}
...
void AddSenderUserpicModerateAction(not_null<SessionController *> controller,
									HistoryItem *moderateItem,
									const PeerMenuCallback &addAction) {
	const auto moderateChannel = moderateItem ? moderateItem->history()->peer->asChannel() : nullptr;
	const auto moderateFrom = moderateItem ? moderateItem->from().get() : nullptr;
	const auto moderateUser = moderateFrom ? moderateFrom->asUser() : nullptr;
	const auto canDeleteAndBan = moderateItem && moderateChannel && moderateChannel->isMegagroup() && moderateFrom &&
		(!moderateUser || !moderateChannel->isGroupAdmin(moderateUser)) && moderateItem->suggestBanReport() &&
		moderateItem->suggestDeleteAllReport() &&
		CanCreateModerateMessagesBox(HistoryItemsList{not_null<HistoryItem *>(moderateItem)});
	if (canDeleteAndBan) {
		...
		addAction({
			...
			.handler =
				[=]
			{
				controller->show(Box(CreateModerateMessagesBox,
...
bool IsUnreadThread(not_null<Data::Thread *> thread) { return thread->chatListBadgesState().unread; }

void MarkAsReadThread(not_null<Data::Thread *> thread) {
	const auto readHistory = [&](not_null<History *> history) { history->owner().histories().readInbox(history); };
	if (!IsUnreadThread(thread)) {
		return;
	} else if (const auto forum = thread->asForum()) {
		forum->enumerateTopics([](not_null<Data::ForumTopic *> topic) { MarkAsReadThread(topic); });
...
void AddSeparatorAndShiftUp(const PeerMenuCallback &addAction) {
	...
	const auto shift = st::popupMenuExpandedSeparator.scrollPadding.top() + st.itemPadding.top() +
		st.itemStyle.font->height + st.itemPadding.bottom() + st.separator.padding.top() + st.separator.width / 2;
...
void TogglePinnedThread(not_null<Window::SessionController *> controller,
	...
	if (!ranges::contains((&owner->session())->data().chatsFilters().list(), filterId, &Data::ChatFilter::id)) {
...
bool IsArchived(not_null<History *> history) { return (history->folder() != nullptr); }
...
void PeerMenuConfirmToggleFee(not_null<Window::SessionNavigation *> navigation,
	...
	const auto exception = [=](bool refund)
	{
		...
		api->request(MTPaccount_ToggleNoPaidMessagesException(MTP_flags((refund ? Flag::f_refund_charged : Flag()) |
	...
	navigation->uiShow()->show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			...
			Ui::ConfirmBox(
				box,
				{
					.text = tr::lng_payment_refund_text(tr::now, lt_name, tr::bold(user->shortName()), tr::marked),
					.confirmed =
						[=](Fn<void()> close)
					{
			...
			const auto paid = box->lifetime().make_state<rpl::variable<int>>();
			*paid = paidAmount->value();
					...
					[=](int already)
					{
						if (!already) {
							...
							*refund =
								box->addRow(object_ptr<Ui::Checkbox>(
												box,
												tr::lng_payment_refund_also(lt_count, paid->value() | tr::to_count()),
												false,
												st::defaultCheckbox),
			...
			user->session()
				.api()
				.request(MTPaccount_GetPaidMessagesRevenue(MTP_flags(parent ? Flag::f_parent_peer : Flag()),
														   parent ? parent->input() : MTPInputPeer(),
														   user->inputUser()))
...
void ForwardToSelf(std::shared_ptr<Main::SessionShow> show, const Data::ForwardDraft &draft) {
	const auto session = &show->session();
	...
	if (!resolved.items.empty()) {
		...
		session->api().forwardMessages(std::move(resolved),
									   action,
									   [=]
									   {
										   auto phrase =
											   rpl::variable<TextWithEntities>(ChatHelpers::ForwardedMessagePhrase({
																				   .toCount = 1,

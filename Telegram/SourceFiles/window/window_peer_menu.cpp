...
void ShareBotGame(not_null<UserData *> bot, not_null<Data::Thread *> thread,
  ...
  histories.sendPreparedMessage(
      ...
      Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
          MTP_flags(flags), history->peer->input(),
          Data::Histories::ReplyToPlaceholder(),
          MTP_inputMediaGame(
              MTP_inputGameShortName(bot->inputUser(), MTP_string(shortName))),
          MTP_string(), MTP_long(randomId), MTPReplyMarkup(),
...
[[nodiscard]] QString LookupMemberRank(not_null<PeerData *> peer,
...
void SetActionText(not_null<QAction *> action, rpl::producer<QString> &&text) {
  ...
  std::move(text) |
      rpl::on_next(
          [=](const QString &actionText) { action->setText(actionText); },
          *lifetime);
}
...
void PeerMenuAddMuteSubmenuAction(
    ...
    not_null<Data::Thread *> thread, const PeerMenuCallback &addAction) {
  ...
  if (isMuted) {
    ...
    addAction(PeerMenuCallback::Args{
        ...
        .handler = nullptr,
        .icon = (notifySettings->sound(thread).none ? &st::menuIconSilent
                                                    : &st::menuIconMute),
        .fillSubmenu =
...
class Filler {
public:
  Filler(not_null<SessionController *> controller, Dialogs::EntryState request,
...
History *FindWastedPin(not_null<Data::Session *> data, Data::Folder *folder) {
  ...
  for (const auto &pinned : order) {
    if (const auto history = pinned.history()) {
      if (history->peer->isChat() && history->peer->asChat()->isDeactivated() &&
          !history->inChatList()) {
        return history;
      }
    }
...
void AddChatMembers(not_null<Window::SessionNavigation *> navigation,
...
bool PinnedLimitReached(not_null<Window::SessionController *> controller,
                        not_null<History *> history, FilterId filterId) {
...
void TogglePinnedThread(not_null<Window::SessionController *> controller,
  ...
  if (const auto history = entry->asHistory()) {
    const auto flags = isPinned ? MTPmessages_ToggleDialogPin::Flag::f_pinned
                                : MTPmessages_ToggleDialogPin::Flag(0);
    owner->session()
        .api()
        .request(MTPmessages_ToggleDialogPin(
            MTP_flags(flags), MTP_inputDialogPeer(history->peer->input())))
        .done([=] {
    ...
    owner->session()
        .api()
        .request(MTPmessages_UpdatePinnedForumTopic(
            peer->input(), MTP_int(topic->rootId()), MTP_bool(isPinned)))
    ...
    owner->session()
        .api()
        .request(MTPmessages_ToggleSavedDialogPin(
            MTP_flags(flags),
...
Filler::Filler(not_null<SessionController *> controller,
               Dialogs::EntryState request, const PeerMenuCallback &addAction)
    : _controller(controller), _request(request), _thread(request.key.thread()),
      _topic(request.key.topic()), _peer(request.key.peer()),
      _folder(request.key.folder()), _sublist(request.key.sublist()),
      _addAction(addAction) {}

void Filler::addHidePromotion() {
  const auto history = _request.key.history();
  if (_topic || !history || !history->useTopPromotion() ||
  ...
  _addAction(
      ...
      [=] {
        ...
        history->session()
            .api()
            .request(MTPhelp_HidePromoData(history->peer->input()))
            .send();
      },
      &st::menuIconRemove);
}
...
void Filler::addToggleTopicClosed() {
  ...
  const auto text = closed ? tr::lng_forum_topic_reopen(tr::now)
...
void Filler::addTogglePin() {
  if ((!_sublist && !_peer) || (_topic && !_topic->canTogglePinned())) {
    return;
  } else if (_request.section == Section::SubsectionTabsMenu && !_sublist &&
             !_topic) {
    return;
  ...
  _addAction(pinText(), pinToggle,
             (entry->isPinnedDialog(filterId) ? &st::menuIconUnpin
                                              : &st::menuIconPin));
}
...
void Filler::addInfo() {
  ...
  if (infoPeer && (infoPeer->isSelf() || infoPeer->isRepliesChat() ||
  ...
  const auto text =
      _thread->asTopic()
          ? (_thread->peer()->isBot() ? tr::lng_context_view_thread(tr::now)
      ...
      [=] {
        if ([[maybe_unused]] const auto strong = weak.get()) {
          controller->showSection(
              Info::Stories::Make(channel, Info::Stories::ArchiveId()));
        }
      },
...
void Filler::addToggleUnreadMark() {
  ...
  const auto weak = base::make_weak(_thread);
  const auto label = unread ? tr::lng_context_mark_read(tr::now)
...
void Filler::addNewWindow(bool addSeparator) {
    ...
    _addAction(
        ...
        [=] {
          ...
          controller->showInNewWindow(
              SeparateId(SeparateType::Archive, &controller->session()));
        },
    ...
    _addAction(
        ...
        [=] {
          ...
          if (const auto sublist = weak.get()) {
            controller->showInNewWindow(
                SeparateId(SeparateType::SavedSublist, sublist));
          }
        },
        &st::menuIconNewWindow);
    if (addSeparator) {
  ...
  const auto history = _request.key.history();
  if (!_peer || (history && history->useTopPromotion() &&
                 !history->topPromotionType().isEmpty())) {
    return;
  }
  const auto peer = _peer;
  const auto thread =
      _topic ? not_null<Data::Thread *>(_topic) : _peer->owner().history(_peer);
  const auto weak = base::make_weak(thread);
  _addAction(
      tr::lng_context_new_window(tr::now),
      [=] {
        ...
        if (const auto strong = weak.get()) {
          ...
          controller->showInNewWindow(SeparateId(
              forum ? SeparateType::Forum : SeparateType::Chat, strong));
        }
      },
      &st::menuIconNewWindow);
  if (addSeparator) {
...
void Filler::addToggleArchive() {
  ...
  const auto label = [=] {
    return isArchived() ? tr::lng_archived_remove(tr::now)
...
void Filler::addClearHistory() {
  ...
  _addAction(tr::lng_profile_clear_history(tr::now),
             ClearHistoryHandler(_controller, _peer), &st::menuIconClear);
}
...
void Filler::addDeleteChat() {
  ...
  _addAction({
      ...
      .handler = (_sublist ? DeleteSublistHandler(_controller, _sublist)
...
void Filler::addLeaveChat() {
  ...
  _addAction({
      .text = (_peer->isMegagroup() ? tr::lng_profile_leave_group(tr::now)
...
void Filler::addJoinChat() {
  ...
  _addAction(
      label, [=] { channel->session().api().joinChannel(channel); },
      &st::menuIconAddToFolder);
}

void Filler::addBlockUser() {
  const auto user = _peer->asUser();
  if (!user || user->isInaccessible() || user->isSelf() ||
      user->isRepliesChat() || user->isVerifyCodes()) {
    return;
  }
  const auto window = _controller;
  const auto blockText = [](not_null<UserData *> user) {
    return user->isBlocked() ? ((user->isBot() && !user->isSupport())
  ...
  const auto blockAction = _addAction(
      ...
      [=] {
        ...
        if (show->showFrozenError()) {
          ...
          window->show(Box(PeerMenuBlockUserBox, &window->window(), user,
                           v::null, v::null));
        }
      },
  ...
  auto actionText = _peer->session().changes().peerUpdates(
                        _peer, Data::PeerUpdate::Flag::IsBlocked) |
...
void Filler::addDirectMessages() {
  ...
  _addAction(
      ...
      [=] {
        navigation->showPeerHistory(monoforum,
...
void Filler::addExportChat() {
  ...
  _addAction(
      ...
      [=] { PeerMenuExportChat(navigation, peer); }, &st::menuIconExport);
}
...
void Filler::addReport() {
  ...
  const auto channel = _peer->asChannel();
  if (_topic ||
      ((!chat || chat->amCreator()) && (!channel || channel->amCreator()))) {
    return;
  }
  const auto peer = _peer;
  const auto navigation = _controller;
...
void Filler::addNewContact() {
  const auto user = _peer->asUser();
  if (!user || user->isContact() || user->isSelf() || user->isInaccessible() ||
      user->isBot()) {
    return;
  }
  const auto controller = _controller;
...
void Filler::addShareContact() {
  ...
  _addAction(
      ...
      [=] { PeerMenuShareContactBox(controller, user); }, &st::menuIconShare);
}
...
void Filler::addManageChat() {
  ...
  _addAction(
      text, [=] { navigation->showEditPeerBox(peer); }, &st::menuIconManage);
}
...
void Filler::addCreatePoll() {
  ...
  const auto can = _topic ? Data::CanSend(_topic, ChatRestriction::SendPolls)
  ...
  auto callback = [=] {
    PeerMenuCreatePoll(controller, peer, replyTo, suggest, chosen,
                       PollData::Flags(), source, {sendMenuType});
  };
...
void Filler::addCreateTodoList() {
  ...
  const auto can = _topic ? (_peer->session().premium() &&
  ...
  const auto suggest = _request.currentSuggest;
  auto callback = [=] {
    PeerMenuCreateTodoList(controller, peer, replyTo, suggest, source,
                           {sendMenuType});
  };
  _addAction(tr::lng_todo_create(tr::now), std::move(callback),
...
void ShowDisableSharingBox(not_null<SessionController *> controller,
  ...
  controller->show(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    const auto buttonPadding = QMargins(st::showOrBox.buttonPadding.left(), 0,
                                        st::showOrBox.buttonPadding.right(), 0);

    auto icon =
        Settings::CreateLottieIcon(box,
                                   {
                                       .name = u"stop"_q,
    ...
    box->verticalLayout()->add(std::move(icon.widget),
                               st::disableSharingIconPadding, style::al_top);

    Ui::AddSkip(box->verticalLayout());

    box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_disable_sharing_title(),
    ...
    button->setText(
        Data::AmPremiumValue(&peer->session()) | rpl::map([](bool premium) {
          if (premium) {
    ...
    box->setShowFinishedCallback(
        [animate = std::move(icon.animate)] { animate(anim::repeat::once); });
  }));
}

void Filler::addToggleNoForwards() {
  const auto user = _peer->asUser();
  if (!user || user->isInaccessible() || user->isBot() ||
  ...
  const auto toggleNoForwards = [=](bool enabled) {
    ...
    peer->session()
        .api()
        .request(MTPmessages_ToggleNoForwards(MTP_flags(Flag()), peer->input(),
                                              MTP_bool(enabled), MTPint()))
        .done([=](const MTPUpdates &result) {
  ...
  _addAction(disabledNow ? tr::lng_enable_sharing(tr::now)
							: tr::lng_disable_sharing(tr::now), [=] {
    if (controller->showFrozenError()) {
      return;
    } else if (disabledNow) {
      const auto willBeRequest =
          true && (user->flags() & UserDataFlag::NoForwardsPeerEnabled) &&
...
void Filler::addTTLSubmenu(bool addSeparator) {
    ...
    const auto text =
        tr::lng_manage_messages_ttl_menu(tr::now) +
        (_peer->messagesTTL() ? ('\t' + Ui::FormatTTLTiny(_peer->messagesTTL()))
                              : QString());
...
void Filler::addSendGift() {
    ...
    if (!user && !channel) {
      return;
               ...
               (user->isInaccessible() || user->isSelf() || user->isBot() ||
                user->isServiceUser() || user->isNotificationsUser() ||
                user->isRepliesChat() || user->isVerifyCodes() ||
...
void Filler::addFirstMessage() {
    const auto peer =
        _peer->isMegagroup() ? _peer->asMegagroup() : _peer->asChannel();
    if (!peer) {
      return;
    }
    _addAction(
        ...
        [=] {
          App::wnd()->sessionController()->showPeerHistory(
              peer, Window::SectionShow::Way::Forward, 1);
        },
...
void Filler::addViewChannel() {
    ...
    if (const auto chat = peer->discussionLink()) {
      _addAction(
          peer->isMegagroup() ? tr::lng_profile_view_channel(tr::now)
                              : tr::lng_profile_view_discussion(tr::now),
          [=] {
            App::wnd()->sessionController()->showPeerHistory(
                chat, Window::SectionShow::Way::Forward);
          },
          &st::menuIconDiscussion);
    }
}
...
void Filler::fill() {
    if (_folder) {
      ...
      case Section::ChatsList:
        fillChatsListActions();
        break;
      ...
      case Section::Scheduled:
        fillScheduledActions();
        break;
      case Section::ContextMenu:
      case Section::SubsectionTabsMenu:
        fillContextMenuActions();
        break;
      case Section::SavedSublist:
        fillMonoforumPeerActions();
        break;
      default:
        Unexpected("_request.section in Filler::fill.");
      }
}
    ...
    _addAction(
        ...
        [=] {
          if (const auto forum = peer->forum()) {
            controller->show(
                Box(NewForumTopicBox, controller, forum->history()));
          }
        },
        &st::menuIconDiscussion);
    _addAction(PeerMenuCallback::Args{.isSeparator = true});
...
void Filler::addViewAsMessages() {
    ...
    auto make = [=](not_null<Ui::PopupMenu *> popupMenu) {
      auto owned = base::make_unique_q<Ui::Menu::Action>(
          popupMenu->menu(), popupMenu->menu()->st(),
          ...
          &st::menuIconAsMessages, &st::menuIconAsMessages);
...
void Filler::addSearchTopics() {
    ...
    const auto controller = _controller;
    _addAction(
        tr::lng_dlg_filter(tr::now), [=] { controller->searchInChat(history); },
        &st::menuIconSearch);
}
...
void Filler::addVideoChat() {
    ...
    FillVideoChatMenu(_controller, _request,
...
void Filler::fillArchiveActions() {
    ...
    if (!inmenu) {
      const auto text = hidden ? tr::lng_context_archive_expand(tr::now)
    ...
    {
      const auto text = inmenu ? tr::lng_context_archive_to_list(tr::now)
    ...
    MenuAddMarkAsReadChatListAction(
        controller, [folder = _folder] { return folder->chatsList(); },
        _addAction);
    ...
    _addAction(tr::lng_context_archive_settings(tr::now), openSettings,
               &st::menuIconManage);
    _addAction(
...
void Filler::addToggleFee() {
    ...
    const auto text = feeRemoved ? tr::lng_context_charge_fee(tr::now)
    ...
    _addAction(
        ...
        [=] {
          ...
          PeerMenuConfirmToggleFee(navigation, paidAmount, parent, user,
                                   removeFee);
        },
    ...
    _addAction({.make = [=](not_null<Ui::PopupMenu *> menuParent) {
      ...
      const auto text =
          feeRemoved
              ? tr::lng_context_fee_free(tr::now, lt_name,
                                         TextWithEntities{user->shortName()},
                                         tr::marked)
              : tr::lng_context_fee_now(
                    tr::now, lt_name, TextWithEntities{user->shortName()},
                    lt_amount,
      ...
      result->setMarkedText(text, QString(),
...
void Filler::addSetPersonalChannel() {
    const auto channel = _peer->asChannel();
    if (!channel || _peer->session().user()->personalChannelId() ||
...
void PeerMenuExportChat(not_null<Window::SessionController *> controller,
                        not_null<PeerData *> peer) {
...
void PeerMenuExportTopic(not_null<Window::SessionNavigation *> navigation,
                         not_null<PeerData *> peer, MsgId topicRootId) {
  base::call_delayed(st::defaultPopupMenu.showDuration, [=] {
...
void PeerMenuDeleteContact(not_null<Window::SessionController *> controller,
                           not_null<UserData *> user) {
  if (controller->showFrozenError()) {
    return;
  }
  const auto text =
      tr::lng_sure_delete_contact(tr::now, lt_contact, user->name());
  const auto deleteSure = [=](Fn<void()> &&close) {
    close();
    user->session()
        .api()
        .request(MTPcontacts_DeleteContacts(
  ...
  auto box = Box([=](not_null<Ui::GenericBox *> box) {
    ...
    Ui::IconWithTitle(
        ...
        Ui::CreateChild<Ui::FlatLabel>(box,
...
void PeerMenuDeleteTopicWithConfirmation(
  ...
  controller->show(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    Ui::IconWithTitle(
        ...
        Ui::CreateChild<Ui::FlatLabel>(box, topic->title(),
                                       box->getDelegate()->style().title));
    Ui::AddSkip(box->verticalLayout());
    Ui::AddSkip(box->verticalLayout());
    Ui::ConfirmBox(box, {
...
void PeerMenuDeleteTopic(not_null<Window::SessionNavigation *> navigation,
                         not_null<PeerData *> peer, MsgId rootId) {
...
void PeerMenuShareContactBox(not_null<Window::SessionNavigation *> navigation,
                             not_null<UserData *> user) {
  ...
  auto callback = [=](not_null<Data::Thread *> thread) {
    ...
    const auto title =
        thread->asTopic() ? thread->asTopic()->title() : peer->name();
    ...
    state->share = [=](Api::SendOptions options) {
      ...
      const auto checked = state->sendPayment.check(
          navigation, peer, action.options, 1, withPaymentApproved);
      if (!checked) {
        return;
      }
      navigation->showThread(strong, ShowAtTheEndMsgId,
...
void PeerMenuCreatePoll(not_null<Window::SessionController *> controller,
                        not_null<PeerData *> peer, FullReplyTo replyTo,
                        SuggestOptions suggest, PollData::Flags chosen,
                        PollData::Flags disabled, Api::SendType sendType,
  ...
  auto starsRequired = peer->session().changes().peerFlagsValue(
                           peer, Data::PeerUpdate::Flag::FullInfo |
                                     Data::PeerUpdate::Flag::StarsPerMessage) |
                       rpl::map([=] { return peer->starsPerMessageChecked(); });
  auto box =
      Box<CreatePollBox>(controller, peer, chosen, disabled,
                         std::move(starsRequired), sendType, sendMenuDetails);
  struct State {
  ...
  state->create = [=](const CreatePollBox::Result &result) {
    ...
    const auto checked = state->sendPayment.check(
        controller, peer, action.options, 1, withPaymentApproved);
    if (!checked || std::exchange(state->lock, true)) {
      return;
    }

    const auto local = action.history->localDraft(replyTo.topicRootId,
    ...
    const auto api = &peer->session().api();
    api->polls().create(result.poll, result.text, action,
                        crl::guard(weak,
                                   [=] {
...
void PeerMenuTodoWantsPremium(TodoWantsPremium type) {
  ...
  const auto text = [&] {
    switch (type) {
    case TodoWantsPremium::Create:
      return tr::lng_todo_create_premium;
    case TodoWantsPremium::Add:
      return tr::lng_todo_add_premium;
    case TodoWantsPremium::Mark:
      return tr::lng_todo_mark_premium;
    }
...
void PeerMenuCreateTodoList(not_null<Window::SessionController *> controller,
                            not_null<PeerData *> peer, FullReplyTo replyTo,
                            SuggestOptions suggest, Api::SendType sendType,
                            SendMenu::Details sendMenuDetails) {
  if (!peer->session().premium()) {
  ...
  auto starsRequired = peer->session().changes().peerFlagsValue(
                           peer, Data::PeerUpdate::Flag::FullInfo |
                                     Data::PeerUpdate::Flag::StarsPerMessage) |
                       rpl::map([=] { return peer->starsPerMessageChecked(); });
  auto box = Box<EditTodoListBox>(controller, std::move(starsRequired),
                                  sendType, sendMenuDetails);
  struct State {
  ...
  state->create = [=](const EditTodoListBox::Result &result) {
    ...
    const auto api = &peer->session().api();
    api->todoLists().create(result.todolist, action,
                            crl::guard(weak,
                                       [=] {
                                         state->create = nullptr;
                                         weak->closeBox();
...
void PeerMenuEditTodoList(not_null<Window::SessionController *> controller,
  ...
  box->submitRequests() |
      rpl::on_next(
          [=](const EditTodoListBox::Result &result) {
            ...
            api->todoLists().edit(item, result.todolist, result.options,
...
void PeerMenuAddTodoListTasks(not_null<Window::SessionController *> controller,
                              not_null<HistoryItem *> item) {
...
void PeerMenuBlockUserBox(
    not_null<Ui::GenericBox *> box, not_null<Window::Controller *> window,
    not_null<PeerData *> peer, std::variant<v::null_t, bool> suggestReport,
  ...
  if (user) {
    box->addRow(object_ptr<Ui::FlatLabel>(
        box,
        tr::lng_blocked_list_confirm_text(lt_name, rpl::single(tr::bold(name)),
                                          tr::marked),
        st::blockUserConfirmation));

    box->addSkip(st::boxMediumSkip);
  }
  const auto report = reportNeeded ? box->addRow(object_ptr<Ui::Checkbox>(
                                         box, tr::lng_report_spam(tr::now),
                                         true, st::defaultBoxCheckbox))
                                   : nullptr;
  ...
  const auto clear = v::is<ClearChat>(suggestClear)
                         ? box->addRow(object_ptr<Ui::Checkbox>(
                               box, tr::lng_blocked_list_confirm_clear(tr::now),
                               true, st::defaultBoxCheckbox))
                     : v::is<ClearReply>(suggestClear)
                         ? box->addRow(object_ptr<Ui::Checkbox>(
                               box, tr::lng_context_delete_msg(tr::now), true,
                               st::defaultBoxCheckbox))
                         : nullptr;
  if (clear) {
    box->addSkip(st::boxMediumSkip);
  }
  const auto allFromUser =
      v::is<ClearReply>(suggestClear)
          ? box->addRow(object_ptr<Ui::Checkbox>(
                box,
                tr::lng_delete_all_from_user(
                    tr::now, lt_user, tr::bold(peer->name()), tr::marked),
                true, st::defaultBoxCheckbox))
          : nullptr;
  ...
  box->addButton(
      ...
      [=] {
        ...
        if (const auto clearReply = std::get_if<ClearReply>(&suggestClear)) {
          ...
          peer->session()
              .api()
              .request(MTPcontacts_BlockFromReplies(
                  MTP_flags(
                      (clearChecked ? Flag::f_delete_message : Flag(0)) |
          ...
          if (reportChecked) {
            peer->session()
                .api()
                .request(MTPmessages_ReportSpam(peer->input()))
                .send();
          }
          if (clearChecked) {
...
void PeerMenuUnblockUserWithBotRestart(std::shared_ptr<Ui::Show> show,
                                       not_null<UserData *> user) {
...
void BlockSenderFromRepliesBox(not_null<Ui::GenericBox *> box,
                               not_null<SessionController *> controller,
                               FullMsgId id) {
  const auto item = controller->session().data().message(id);
  Assert(item != nullptr);

  PeerMenuBlockUserBox(box, &controller->window(), item->originalSender(), true,
...
object_ptr<Ui::BoxContent> PrepareChooseRecipientBox(
    ...
    rpl::producer<QString> titleOverride, FnMut<void()> &&successCallback,
    InlineBots::PeerTypes typesRestriction,
    Fn<void(std::vector<not_null<Data::Thread *>>, Api::SendOptions)>
        sendMany) {
  ...
  public:
    ...
    Controller(not_null<Main::Session *> session, FnMut<void(Chosen)> callback,
               Fn<bool(Chosen)> filter, bool selectable)
    ...
    rowContextMenu(QWidget *parent,
      ...
      if (!row->checked() && !row->peer()->isForum() &&
        ...
        auto menu =
            base::make_unique_q<Ui::PopupMenu>(parent, st::popupMenuWithIcons);
        menu->addAction(
  ...
  auto callback = [chosen = std::move(chosen),
                   ...
                   weak](not_null<Data::Thread *> thread) mutable {
    if (!chosen(thread)) {
      return;
  ...
  auto filter =
      typesRestriction ? [=](not_null<Data::Thread *> thread) -> bool {
  ...
  auto controller = std::make_unique<Controller>(session, std::move(callback),
                                                 std::move(filter), selectable);
            ...
            [=] {
              ...
              if (shown) {
                ...
                state->submit = [=](Api::SendOptions options) {
                  ...
                  const auto withPaymentApproved =
                      crl::guard(weak, [=](int approved) {
                        auto copy = options;
                ...
                send->setText(PaidSendButtonText(state->starsToSend.value(),
                                                 tr::lng_send_button()));
              }
              box->addButton(tr::lng_cancel(), [=] { box->closeBox(); });
            },
...
ShowChooseRecipientBox(not_null<Window::SessionNavigation *> navigation,
                       FnMut<bool(not_null<Data::Thread *>)> &&chosen,
                       rpl::producer<QString> titleOverride,
                       FnMut<void()> &&successCallback,
                       InlineBots::PeerTypes typesRestriction) {
  return navigation->parentController()->show(PrepareChooseRecipientBox(
      &navigation->session(), std::move(chosen), std::move(titleOverride),
      std::move(successCallback), typesRestriction));
}
...
QPointer<Ui::BoxContent>
ShowNewForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
                          MessageIdsList &&msgIds, bool no_quote,
                          FnMut<void()> &&successCallback) {
  const auto item = navigation->session().data().message(msgIds[0]);
  const auto history = item->history();
  const auto owner = &history->owner();
  const auto session = &history->session();
  const auto isGame = item->getMessageBot() && item->media() &&
                      (item->media()->game() != nullptr);

  const auto items = owner->idsToItems(msgIds);
  const auto hasCaptions = ranges::any_of(items, [](auto item) {
    return item->media() && !item->originalText().text.isEmpty() &&
           item->media()->allowsEditCaption();
  });
  const auto hasOnlyForcedForwardedInfo =
      hasCaptions ? false : ranges::all_of(items, [](auto item) {
  ...
  *weak =
      Ui::show(Box<ShareBox>(ShareBox::Descriptor{
                   ...
                   .submitCallback = ShareBox::DefaultForwardCallback(
                       navigation->parentController()->uiShow(), history,
                       msgIds, TimeId(0), no_quote, std::move(successCallback)),
                   .filterCallback = std::move(filterCallback),
                   .title = no_quote ? tr::lng_title_forward_as_copy()
...
base::weak_qptr<Ui::BoxContent>
ShowForwardMessagesBox(std::shared_ptr<ChatHelpers::Show> show,
  ...
  public:
    ListBox(QWidget *parent, std::unique_ptr<PeerListController> controller,
            Fn<void(not_null<ListBox *>)> init)
        : PeerListBox(parent, std::move(controller),
                      [=](not_null<PeerListBox *> box) {
  ...
  public:
    ...
    createRestoredRow(not_null<PeerData *> peer) override final {
  ...
  const auto state = [&] {
    ...
    auto init = [=](not_null<ListBox *> box) {
      ...
      rpl::combine(chatsFilters->heightValue(),
          ...
          rpl::on_next(
              [box](int h, bool hasQuery) {
                box->setAddedTopScrollSkip(hasQuery ? 0 : h);
              },
              box->lifetime());
      box->multiSelectHeightValue() |
          rpl::on_next([=](int h) { chatsFilters->moveToLeft(0, h); },
  ...
  { // Chosen a single.
    auto chosen = [show, draft = std::move(draft)](
      ...
      const auto peer = thread->peer();
      if (peer->isSelf() && !draft.ids.empty() &&
          draft.ids.front().peer != peer->id) {
        ForwardToSelf(show, draft);
        return true;
      }
      const auto id =
          SeparateId(((peer->isForum() && !peer->useSubsectionTabs())
                          ? SeparateType::Forum
                          : SeparateType::Chat),
                     thread);
    ...
    auto callback =
        [=, chosen = std::move(chosen)](Controller::Chosen thread) mutable {
  ...
  const auto comment = Ui::CreateChild<Ui::SlideWrap<Ui::InputField>>(
      state->box.get(),
      object_ptr<Ui::InputField>(state->box, st::shareComment,
  ...
  const auto countMessages =
      ShareBox::DefaultForwardCountMessages(history, msgIds);
  ...
  state->submit = [=](Api::SendOptions options) {
    ...
    const auto checkPaid = [=] {
      ...
      if (!waiting.empty()) {
        ...
        if (!session->credits().loaded()) {
          session->credits().loadedValue() | rpl::filter(rpl::mappers::_1) |
              rpl::take(1) |
    ...
    send(ranges::views::all(peers) |
             ...
             ranges::to_vector,
         checkPaid, std::move(comment), options,
    ...
    auto ayuForwarding = AyuForward::isAyuForwardNeeded(items) ||
  ...
  const auto showMenu = [=](not_null<Ui::RpWidget *> parent) {
    ...
    if (showForwardOptions) {
      auto createView = [&](rpl::producer<QString> &&text, bool checked) {
        auto item = base::make_unique_q<Menu::ItemWithCheck>(
            state->menu->menu(), st::popupMenuWithIcons.menu,
            Ui::CreateChild<QAction>(state->menu->menu().get()), nullptr,
            nullptr);
        std::move(text) |
            rpl::on_next([action = item->action()](
                             QString text) { action->setText(text); },
                         item->lifetime());
        item->init(checked);
      ...
      Ui::FillForwardOptions(
          std::move(createView), state->box->forwardOptions(),
    ...
    SendMenu::FillSendMenu(
        state->menu.get(), show, SendMenu::Details{sendMenuType()},
        SendMenu::DefaultCallback(
            show, crl::guard(parent, [=](Api::SendOptions options) {
              if (const auto onstack = state->submit) {
                onstack(options);
              }
            })));
  ...
  field->changes() |
      rpl::on_next([=] { state->refreshStarsToSend(); }, field->lifetime());

  Ui::SendPendingMoveResizeEvents(comment);

  state->box->focusRequests() | rpl::on_next(
                                    [=] {
                                      if (!comment->isHidden()) {
  ...
  state->controller->selectionChanges() |
      rpl::on_next(
          [=] {
            ...
            if (shown) {
              const auto send =
                  state->box->addButton(tr::lng_send_button(), [=] {
                    if (const auto onstack = state->submit) {
                      onstack({});
                    }
                  });
              send->setAcceptBoth();
              send->clicks() | rpl::on_next(
...
base::weak_qptr<Ui::BoxContent>
ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
                       Data::ForwardDraft &&draft,
                       Fn<void()> &&successCallback) {
  return ShowForwardMessagesBox(navigation->uiShow(), std::move(draft),
                                std::move(successCallback));
}

base::weak_qptr<Ui::BoxContent>
ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
                       MessageIdsList &&items, Fn<void()> &&successCallback) {
  return ShowForwardMessagesBox(navigation->uiShow(),
                                Data::ForwardDraft{.ids = items},
                                std::move(successCallback));
}

base::weak_qptr<Ui::BoxContent>
ShowShareGameBox(not_null<Window::SessionNavigation *> navigation,
                 not_null<UserData *> bot, QString shortName) {
  const auto weak = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
  auto chosen = [=](not_null<Data::Thread *> thread) mutable {
    ...
    auto send = crl::guard(thread, [=] {
      ...
      navigation->showThread(thread, ShowAtUnreadMsgId,
                             SectionShow::Way::ClearStack);
    });
    const auto confirmText =
        thread->peer()->isUser()
            ? tr::lng_bot_sure_share_game(tr::now, lt_user,
                                          thread->chatListName())
            : tr::lng_bot_sure_share_game_group(tr::now, lt_group,
                                                thread->chatListName());
...
base::weak_qptr<Ui::BoxContent>
ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
                 std::shared_ptr<QMimeData> data, not_null<Data::Forum *> forum,
                 FnMut<void()> &&successCallback) {
  const auto weak = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
  auto chosen = [data = std::move(data), callback = std::move(successCallback),
                 weak, navigation](not_null<Data::Thread *> thread) mutable {
  ...
  *weak = navigation->parentController()->show(Box<PeerListBox>(
      std::make_unique<ChooseTopicBoxController>(forum, std::move(chosen)),
      std::move(initBox)));
  return weak->get();
}
...
ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
  ...
  auto chosen = [data = std::move(data), callback = std::move(successCallback),
                 weak,
                 navigation](not_null<Data::SavedSublist *> sublist) mutable {
    const auto content = navigation->parentController()->content();
  ...
  *weak = navigation->parentController()->show(
      Box<PeerListBox>(std::make_unique<ChooseSublistBoxController>(
                           monoforum, std::move(chosen)),
                       std::move(initBox)));
  return weak->get();
}

base::weak_qptr<Ui::BoxContent>
ShowSendNowMessagesBox(not_null<Window::SessionNavigation *> navigation,
                       not_null<History *> history, MessageIdsList &&items,
                       Fn<void()> &&successCallback) {
  ...
  auto done = [=, list = std::move(items),
               callback = std::move(successCallback)](Fn<void()> &&close) {
    close();
    ...
    session->api()
        .request(MTPmessages_SendScheduledMessages(history->peer->input(),
...
void PeerMenuAddChannelMembers(not_null<Window::SessionNavigation *> navigation,
  ...
  api->chatParticipants().requestForAdd(
      channel,
      crl::guard(navigation, [=](const Api::ChatParticipants::TLMembers &data) {
        const auto &[availableCount, list] =
            Api::ChatParticipants::Parse(channel, data);
        const auto already =
            (list) |
            ranges::views::transform([&](const Api::ChatParticipant &p) {
              return p.isUser() ? channel->owner().userLoaded(p.userId())
                                : nullptr;
            }) |
            ranges::views::filter(
                [](UserData *user) { return (user != nullptr); }) |
            ranges::to_vector;
...
void ToggleMessagePinned(not_null<Window::SessionNavigation *> navigation,
                         FullMsgId itemId, bool pin) {
  const auto item = navigation->session().data().message(itemId);
  if (!item || !item->canPin()) {
    return;
  }
  if (pin) {
    navigation->parentController()->show(Box(PinMessageBox, item),
                                         Ui::LayerOption::CloseOther);
  } else {
...
void HidePinnedBar(not_null<Window::SessionNavigation *> navigation,
                   not_null<PeerData *> peer, MsgId topicRootId,
                   PeerId monoforumPeerId, Fn<void()> onHidden) {
  const auto callback = crl::guard(navigation, [=](Fn<void()> &&close) {
    ...
    const auto top =
        Data::ResolveTopPinnedId(peer, topicRootId, monoforumPeerId, migrated);
    const auto universal = !top ? MsgId(0)
                           : (migrated && !peerIsChannel(top.peer))
                               ? (top.msg - ServerMaxMsgId)
                               : top.msg;
    if (universal) {
      session.settings().setHiddenPinnedMessageId(peer->id, topicRootId,
                                                  monoforumPeerId, universal);
...
void UnpinAllMessages(not_null<Window::SessionNavigation *> navigation,
  ...
  const auto callback = crl::guard(navigation, [=](Fn<void()> &&close) {
    ...
    const auto sendRequest = [=](auto self) -> void {
      ...
      api
          ->request(MTPmessages_UnpinAllMessages(
              MTP_flags((topicRootId ? Flag::f_top_msg_id : Flag()) |
                        (sublist ? Flag::f_saved_peer_id : Flag())),
              history->peer->input(), MTP_int(topicRootId.bare),
...
void MenuAddMarkAsReadAllChatsAction(not_null<Main::Session *> session,
  ...
  auto callback = [=, owner = &session->data()] {
    ...
    show->show(
        Box([=](not_null<Ui::GenericBox *> box) {
          ...
          Ui::IconWithTitle(box->verticalLayout(), userpic,
                            Ui::CreateChild<Ui::FlatLabel>(
                                box->verticalLayout(),
                                Info::Profile::NameValue(session->user()),
                                box->getDelegate()->style().title));
          auto text =
              rpl::combine(tr::lng_context_mark_read_all_sure(),
  ...
  addAction(tr::lng_context_mark_read_all(tr::now), std::move(callback),
            &st::menuIconMarkRead);
}
...
void MenuAddMarkAsReadChatListAction(
  ...
  // There is no async to make weak from controller.
  const auto unreadState =
      customUnreadState ? customUnreadState() : list()->unreadState();
  ...
  auto callback = [=] {
    if (unreadState.messages > kMaxUnreadWithoutConfirmation) {
      ...
      controller->show(Ui::MakeConfirmBox({tr::lng_context_mark_read_sure(),
  ...
  addAction(tr::lng_context_mark_read(tr::now), std::move(callback),
            &st::menuIconMarkRead);
}

void ToggleHistoryArchived(std::shared_ptr<ChatHelpers::Show> show,
                           not_null<History *> history, bool archived) {
  const auto callback = [=] {
    show->showToast(Ui::Toast::Config{
        .text = {(archived ? tr::lng_archived_added(tr::now)
                           : tr::lng_archived_removed(tr::now))},
        .st = &st::windowArchiveToast,
        .duration =
            (archived ? kArchivedToastDuration : Ui::Toast::kDefaultDuration),
    });
  };
  history->session().api().toggleHistoryArchived(history, archived, callback);
}

Fn<void()> ClearHistoryHandler(not_null<Window::SessionController *> controller,
                               not_null<PeerData *> peer) {
  return [=] {
    if (!controller->showFrozenError()) {
...
Fn<void()>
DeleteAndLeaveHandler(not_null<Window::SessionController *> controller,
                      not_null<PeerData *> peer) {
    ...
    return [=] {
      ...
      *requestId =
          peer->session()
              .api()
              .request(
                  MTPmessages_GetFutureChatCreatorAfterLeave(peer->input()))
...
Fn<void()>
DeleteSublistHandler(not_null<Window::SessionController *> controller,
...
bool FillVideoChatMenu(not_null<SessionController *> controller,
  ...
  if (has) {
    addAction(
        tr::lng_menu_start_group_call_join(tr::now), [=] { callback({}); },
        &st::menuIconVideoChat);
  } else if (manager) {
    addAction((livestream ? tr::lng_menu_start_group_call_channel
                          : tr::lng_menu_start_group_call)(tr::now),
              [=] { callback({}); }, &st::menuIconStartStream);
  }
  if (!has && manager) {
    ...
    addAction((livestream ? tr::lng_menu_start_group_call_with_channel
                          : tr::lng_menu_start_group_call_with)(tr::now),
              rtmpCallback, &st::menuIconStartStreamWith);
  }
  return has || manager;
}

void FillSenderUserpicMenu(not_null<SessionController *> controller,
                           not_null<PeerData *> peer, PeerData *groupPeer,
    ...
    if (groupPeer) {
      ...
      const auto canEditTarget = [&] {
        if (const auto chat = groupPeer->asChat()) {
          ...
          if (channel->mgInfo && (channel->mgInfo->lastAdmins.contains(user) ||
      ...
      if (!isDiscussionGroup && canEditTarget && groupPeer->canManageRanks() &&
          !user->isSelf()) {
        const auto currentRank = LookupMemberRank(groupPeer, user);
        addAction((currentRank.isEmpty()
                       ? tr::lng_context_add_member_tag(tr::now)
                       : tr::lng_context_edit_member_tag(tr::now)),
                  [=] {
                    controller->show(Box(EditCustomRankBox,
                                         controller->uiShow(), groupPeer, user,
                                         currentRank, false, nullptr));
                  },
                  &st::menuIconEdit);
      }
    }
...
void AddSenderUserpicModerateAction(not_null<SessionController *> controller,
                                    ...
                                    const PeerMenuCallback &addAction) {
  const auto moderateChannel =
      moderateItem ? moderateItem->history()->peer->asChannel() : nullptr;
  ...
  const auto canDeleteAndBan =
      moderateItem && moderateChannel && moderateChannel->isMegagroup() &&
      moderateFrom &&
...
void MarkAsReadThread(not_null<Data::Thread *> thread) {
  ...
  if (!IsUnreadThread(thread)) {
    ...
    forum->enumerateTopics(
        [](not_null<Data::ForumTopic *> topic) { MarkAsReadThread(topic); });
...
void AddSeparatorAndShiftUp(const PeerMenuCallback &addAction) {
  ...
  const auto shift = st::popupMenuExpandedSeparator.scrollPadding.top() +
                     st.itemPadding.top() + st.itemStyle.font->height +
                     st.itemPadding.bottom() + st.separator.padding.top() +
                     st.separator.width / 2;
  addAction({.addTopShift = -shift});
}

void TogglePinnedThread(not_null<Window::SessionController *> controller,
                        not_null<Dialogs::Entry *> entry, FilterId filterId,
                        Fn<void()> onToggled) {
  if (!filterId) {
  ...
  if (!ranges::contains((&owner->session())->data().chatsFilters().list(),
                        filterId, &Data::ChatFilter::id)) {
...
void PeerMenuConfirmToggleFee(not_null<Window::SessionNavigation *> navigation,
                              ...
                              not_null<PeerData *> peer,
                              not_null<UserData *> user, bool removeFee) {
  ...
  navigation->uiShow()->show(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    Ui::ConfirmBox(
        box, {
                 .text = tr::lng_payment_refund_text(
                     tr::now, lt_name, tr::bold(user->shortName()), tr::marked),
                 .confirmed =
                     [=](Fn<void()> close) {
    ...
    *paid = paidAmount->value();
                ...
                *refund = box->addRow(
                    object_ptr<Ui::Checkbox>(
                        ...
                        tr::lng_payment_refund_also(
                            lt_count, paid->value() | tr::to_count()),
                        false, st::defaultCheckbox),
    ...
    user->session()
        .api()
        .request(MTPaccount_GetPaidMessagesRevenue(
...
void ForwardToSelf(std::shared_ptr<Main::SessionShow> show,

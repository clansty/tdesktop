...
FilterIconPanel::FilterIconPanel(QWidget *parent)
    : RpWidget(parent), _inner(Ui::CreateChild<Ui::RpWidget>(this)),
      _innerBg(ImageRoundRadius::Large, st::dialogsBg),
      _shadow(st::emojiPanAnimation.shadow) {
  setup();
}

FilterIconPanel::~FilterIconPanel() { hideFast(); }
...
void FilterIconPanel::setupInner() {
  ...
  const auto rows = (count / kIconsPerRow) + ((count % kIconsPerRow) ? 1 : 0);
  const auto single = st::windowFilterIconSingle;
  const auto size =
      QSize(single.width() * kIconsPerRow, single.height() * rows);
  const auto full =
      QRect(QPoint(), size).marginsAdded(st::windowFilterIconPadding).size();
  _inner->resize(full);
  ...
  _inner->paintRequest() |
      rpl::on_next(
          [=](QRect clip) {
            ...
            p.drawTextLeft(st::windowFilterIconHeaderPosition.x(),
...
QRect FilterIconPanel::countRect(int index) const {
  ...
  const auto single = st::windowFilterIconSingle;
  const auto rect =
      QRect(QPoint(column * single.width(), row * single.height()), single);
...
void FilterIconPanel::otherEnter() { showAnimated(); }
...
void FilterIconPanel::startShowAnimation() {
  if (!_a_show.animating()) {
    ...
    _showAnimation = std::make_unique<Ui::PanelAnimation>(
        st::emojiPanAnimation, Ui::PanelAnimation::Origin::TopRight);

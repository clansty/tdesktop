...
Widget::Widget(QWidget *parent,
    ...
    : RpWidget(parent), _controller(controller),
      ...
      _contentHeight(_contentMaxHeight), _shadow(st::emojiPanAnimation.shadow),
      _scroll(this, st::inlineBotsScroll),
      _innerRounding(
          Ui::PrepareCornerPixmaps(ImageRoundRadius::Large, st::emojiPanBg)),
      _inlineRequestTimer([=] { onInlineRequest(); }) {
  resize(QRect(0, 0, st::emojiPanWidth, _contentHeight)
             .marginsAdded(innerPadding())
             .size());
  _width = width();
  _height = height();
  ...
  _scroll->scrolls() | rpl::on_next([=] { onScroll(); }, lifetime());

  _inner->inlineRowsCleared() | rpl::on_next(
                                    [=] {
                                      hideAnimated();
  ...
  style::PaletteChanged() | rpl::on_next(
                                [=] {
                                  _innerRounding = Ui::PrepareCornerPixmaps(
                                      ImageRoundRadius::Large, st::emojiPanBg);
                                },
                                lifetime());

  macWindowDeactivateEvents() | rpl::filter([=] { return !isHidden(); }) |
      rpl::on_next([=] { leaveEvent(nullptr); }, lifetime());

  // Inner widget has OpaquePaintEvent attribute so it doesn't repaint on
  // scroll. But we should force it to repaint so that GIFs will continue to
  // animate without update() calls. We do that by creating a transparent widget
  // above our _inner.
  auto forceRepaintOnScroll = object_ptr<RpWidget>(this);
  forceRepaintOnScroll->setGeometry(innerRect().x() + st::roundRadiusSmall,
                                    innerRect().y() + st::roundRadiusSmall,
...
void Widget::updateContentHeight() {
  ...
  auto wantedContentHeight =
      qRound(st::emojiPanHeightRatio * _bottom) - addedHeight;
  auto contentHeight =
      std::clamp(wantedContentHeight, st::inlineResultsMinHeight,
  ...
  resize(QRect(0, 0, innerRect().width(), _contentHeight)
             .marginsAdded(innerPadding())
             .size());
  _height = height();
...
void Widget::paintEvent(QPaintEvent *e) {
  ...
  if (showAnimating) {
    ...
    if (!_cache.isNull())
      _cache = QPixmap();
    if (!_inPanelGrab)
      _shadow.paint(p, innerRect(), st::roundRadiusSmall);
    paintContent(p);
  }
}
...
void Widget::paintContent(QPainter &p) {
  ...
  Ui::FillRoundRect(p, inner.x(), inner.y(), inner.width(), radius,
                    st::emojiPanBg, top);

  const auto bottom = Ui::CornersPixmaps{
      .p = {QPixmap(), QPixmap(), _innerRounding.p[2], _innerRounding.p[3]},
  };
  Ui::FillRoundRect(p, inner.x(), inner.y() + inner.height() - radius,
                    inner.width(), radius, st::emojiPanBg, bottom);

  auto horizontal = horizontalRect();
  auto sidesTop = horizontal.y();
  auto sidesHeight = horizontal.height();
  p.fillRect(myrtlrect(inner.x() + inner.width() - st::emojiScroll.width,
                       sidesTop, st::emojiScroll.width, sidesHeight),
             st::emojiPanBg);
  p.fillRect(myrtlrect(inner.x(), sidesTop, st::roundRadiusSmall, sidesHeight),
             st::emojiPanBg);
}

void Widget::moveByBottom() { updateContentHeight(); }
...
void Widget::startOpacityAnimation(bool hiding) {
  ...
  _hiding = hiding;
  hideChildren();
  _a_opacity.start([this] { opacityAnimationCallback(); }, _hiding ? 1. : 0.,
                   _hiding ? 0. : 1., st::emojiPanDuration);
}
...
void Widget::startShowAnimation() {
  if (!_a_show.animating()) {
    ...
    _showAnimation = std::make_unique<Ui::PanelAnimation>(
        st::emojiPanAnimation, Ui::PanelAnimation::Origin::BottomLeft);
    auto inner = rect().marginsRemoved(st::emojiPanMargins);
    _showAnimation->setFinalImage(
        std::move(image),
        QRect(inner.topLeft() * style::DevicePixelRatio(),
...
QImage Widget::grabForPanelAnimation() {
  ...
  auto result = QImage(size() * style::DevicePixelRatio(),
...
void Widget::showAnimated() { showStarted(); }
...
style::margins Widget::innerPadding() const { return st::emojiPanMargins; }
...
QRect Widget::horizontalRect() const {
  return innerRect().marginsRemoved(
      style::margins(0, st::roundRadiusSmall, 0, st::roundRadiusSmall));
}

QRect Widget::verticalRect() const {
  return innerRect().marginsRemoved(
      style::margins(st::roundRadiusSmall, 0, st::roundRadiusSmall, 0));
}

void Widget::clearInlineBot() { inlineBotChanged(); }

bool Widget::overlaps(const QRect &globalRect) const {
  if (isHidden() || !_cache.isNull())
    return false;
  ...
  return inner
             .marginsRemoved(
                 QMargins(st::roundRadiusSmall, 0, st::roundRadiusSmall, 0))
             .contains(testRect) ||
         inner
             .marginsRemoved(
                 QMargins(0, st::roundRadiusSmall, 0, st::roundRadiusSmall))
             .contains(testRect);
}
...
void Widget::inlineResultsDone(const MTPmessages_BotResults &result) {
  ...
  if (result.type() == mtpc_messages_botResults) {
    ...
    if (it == _inlineCache.cend()) {
      it = _inlineCache.emplace(_inlineQuery, std::make_unique<CacheEntry>())
               .first;
    }
    ...
    for (const auto &res : v) {
      auto result =
          InlineBots::Result::Create(&_controller->session(), queryId, res);
      if (result) {
        ++added;
...
void Widget::onInlineRequest() {
  if (_inlineRequestId || !_inlineBot || !_inlineQueryPeer)
    return;
  ...
  _inlineRequestId =
      _api.request(MTPmessages_GetInlineBotResults(
                       MTP_flags(0), _inlineBot->inputUser(),
                       _inlineQueryPeer->input(), MTPInputGeoPoint(),
                       MTP_string(_inlineQuery), MTP_string(nextOffset)))
...
bool Widget::refreshInlineRows(int *added) {
  ...
  if (!entry)
    prepareCache();
  auto result =
      _inner->refreshInlineRows(_inlineQueryPeer, _inlineBot, entry, false);
  if (added)
    *added = result;

...
void StickerSetBox::updateButtons() {
	...
	if (_inner->reorderState()) {
		...
		const auto addPackIdActions = [=](not_null<Ui::PopupMenu*> menu) {
				...
				[weak, session, innerId] {
					...
					searchUserById(
						...
						[session, weak, innerId](
							...
							const auto mainWidget
								= window->widget()->sessionController();
							if (mainWidget) {
		...
		const auto popupTopMenu = [](auto top, not_null<Ui::PopupMenu*> menu) {
			menu->setForcedOrigin(Ui::PanelAnimation::Origin::TopRight);
			top->setForceRippled(true);
			menu->setDestroyedCallback([top] {
				crl::on_main(top, [top] {
					const auto strong = top.data();
					if (strong) {
						strong->setForceRippled(false);
					}
				});
			});
			menu->popup(top->mapToGlobal(QPoint(
				top->width(),

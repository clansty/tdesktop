...
bool SendFilesBox::Block::isSingleMedia() const { return _isSingleMedia; }
...
void SendFilesBox::setReplyTo(FullReplyTo replyTo) {
	...
	_replyHeader->setRoundedShapeBelow(
		!_blocks.empty() && !_blocks.front().isSingleMedia());
	_replyHeader->show();
		...
		if (canEditFileData) {
			state->menu->addAction(
				tr::lng_rename_file(tr::now),
					[=]
					{
						const auto displayName = _list.files[fileIndex].displayName;

...
[[nodiscard]] InnerWidget::ChatsFilterTagsKey
SerializeFilterTagsKey(FilterId filterId, uint8 more, bool active) {
  return (filterId & 0xFFFFFFFF) | (static_cast<int64_t>(more) << 32) |
         (static_cast<int64_t>(active) << 40);
}

[[nodiscard]] int
FixedOnTopDialogsCount(not_null<Dialogs::IndexedList *> list) {
  auto result = 0;
...
[[nodiscard]] int PinnedDialogsCount(FilterId filterId,
...
[[nodiscard]] object_ptr<SearchEmpty>
MakeSearchEmpty(QWidget *parent, SearchState state,
  ...
  const auto fromPeer =
      (state.tab == ChatSearchTab::MyMessages ||
       state.tab == ChatSearchTab::PublicPosts || !state.inChat.peer() ||
       !(state.inChat.peer()->isChat() || state.inChat.peer()->isMegagroup()))
          ? nullptr
          : state.fromPeer;
  const auto waiting = trimmed.isEmpty() && state.tags.empty() && !fromPeer;
  const auto suggestAllChats = !waiting &&
                               state.tab == ChatSearchTab::MyMessages &&
                               state.filter != ChatTypeFilter::All;
  const auto icon =
      waiting ? SearchEmptyIcon::Search : SearchEmptyIcon::NoResults;
  ...
  if (waiting) {
    ...
    text.append(tr::lng_search_tab_no_results(tr::now, tr::bold));
    if (!trimmed.isEmpty()) {
      const auto preview =
          (trimmed.size() > kQueryPreviewLimit + 3)
              ? (trimmed.mid(0, kQueryPreviewLimit) + Ui::kQEllipsis)
              : trimmed;
      text.append("\n").append(tr::lng_search_tab_no_results_text(
          tr::now, lt_query, trimmed.mid(0, kQueryPreviewLimit)));
      if (suggestAllChats) {
        text.append("\n\n").append(
            tr::link(tr::lng_search_tab_try_in_all(tr::now)));
      } else if (hashtag) {
        text.append("\n").append(tr::lng_search_tab_no_results_retry(tr::now));
      }
    }
  }
  auto result =
      object_ptr<SearchEmpty>(parent, icon, rpl::single(std::move(text)));
  if (suggestAllChats) {
...
Key InnerWidget::FilterResult::key() const { return row->key(); }

int InnerWidget::FilterResult::bottom() const { return top + row->height(); }
...
InnerWidget::InnerWidget(QWidget *parent,
    ...
    : RpWidget(parent), _controller(controller),
      ...
      _st(&st::defaultDialogRow), _pinnedShiftAnimation([=](crl::time now) {
        return pinnedShiftAnimationCallback(now);
      }),
      _narrowWidth(st::defaultDialogRow.padding.left() +
                   st::defaultDialogRow.photoSize +
                   st::defaultDialogRow.padding.left()),
      _childListShown(std::move(childListShown)), _freezeTimer([=] {
        _shownList->unfreeze();
        update();
      }) {
  ...
  session().data().contactsLoaded().changes() | rpl::on_next(
                                                    [=] {
                                                      refresh();
                                                      refreshEmpty();
                                                    },
  ...
  session().data().itemRemoved() |
      rpl::on_next(
          [=](not_null<const HistoryItem *> item) { itemRemoved(item); },
          lifetime());

  session().data().dialogsRowReplacements() |
      rpl::on_next(
          [=](Data::Session::DialogsRowReplacement r) {
      ...
      rpl::on_next(
          [=](const Data::SendActionManager::AnimationUpdate &update) {
            const auto updateRect = Ui::RowPainter::SendActionAnimationRect(
                update.thread, _filterId, update.rect, width(),
                update.textUpdated);
            updateDialogRow(
                RowDescriptor(update.thread, FullMsgId()), updateRect,
  ...
  rpl::merge(session().data().chatsListChanges(),
             session().data().chatsListLoadedEvents()) |
      rpl::filter([=](Data::Folder *folder) {
        return !_savedSublists && !_openedForum && (folder == _openedFolder);
      }) |
      rpl::on_next([=] { refresh(); }, lifetime());

  rpl::merge(session().settings().archiveCollapsedChanges() |
                 rpl::map_to(false),
      ...
      rpl::on_next(
          [=](bool tags) {
            ...
            session().data().chatsFilters().tagColorChanged() |
                rpl::on_next(
                    [=](Data::TagColorChanged data) {
                      const auto filterId = data.filterId;
                      const auto key =
                          SerializeFilterTagsKey(filterId, 0, false);
                      const auto activeKey =
                          SerializeFilterTagsKey(filterId, 0, true);
                      {
                        auto &tags = _chatsFilterTags;
                        if (const auto it = tags.find(key); it != tags.end()) {
                          tags.erase(it);
                        }
                        if (const auto it = tags.find(activeKey);
                            it != tags.end()) {
                          tags.erase(it);
                        }
                      }
  ...
  session().changes().historyUpdates(Data::HistoryUpdate::Flag::IsPinned |
  ...
  session().changes().peerUpdates(UpdateFlag::Name | UpdateFlag::Photo |
                                  UpdateFlag::IsContact | UpdateFlag::FullInfo |
                                  UpdateFlag::EmojiStatus) |
      rpl::on_next(
          [=](const Data::PeerUpdate &update) {
            if (update.flags &
                (UpdateFlag::Name | UpdateFlag::Photo | UpdateFlag::FullInfo |
                 UpdateFlag::EmojiStatus)) {
  ...
  session().changes().messageUpdates(
      ...
      rpl::on_next(
          [=](const Data::MessageUpdate &update) {
            refreshDialogRow({update.item->history(), update.item->fullId()});
          },
          lifetime());

  session().changes().entryUpdates(Data::EntryUpdate::Flag::Repaint |
                                   Data::EntryUpdate::Flag::Height) |
      rpl::on_next(
          [=](const Data::EntryUpdate &update) {
            ...
            const auto repaintId =
                (_state == WidgetState::Default) ? _filterId : 0;
  ...
  _controller->activeChatEntryValue() | rpl::combine_previous() |
      rpl::on_next(
          [=](RowDescriptor previous, RowDescriptor next) {
  ...
  _controller->activeChatsFilter() |
      rpl::on_next([=](FilterId filterId) { switchToFilter(filterId); },
                   lifetime());
...
void InnerWidget::refreshWithCollapsedRows(bool toTop) {
  ...
  const auto archive =
      !_shownList->empty() ? _shownList->begin()->get()->folder() : nullptr;
...
int InnerWidget::collapsedRowsOffset() const { return 0; }
...
int InnerWidget::filteredIndex(int y) const {
  return ranges::lower_bound(_filterResults, y, ranges::less(),
                             &FilterResult::bottom) -
         begin(_filterResults);
}

int InnerWidget::filteredHeight(int till) const {
  return (!till || _filterResults.empty()) ? 0
         : (till > 0 && till < _filterResults.size())
             ? _filterResults[till].top
             : (_filterResults.back().top +
                _filterResults.back().row->height());
}

int InnerWidget::idSearchOffset() const {
  return filteredOffset() + filteredHeight() + st::searchedBarHeight;
}
...
int InnerWidget::searchedOffset() const {
  ...
  if (!_previewResults.empty()) {
    result +=
        (_previewResults.size() * st::dialogsRowHeight) + st::searchedBarHeight;
  }
  return result;
}
...
void InnerWidget::changeOpenedForum(Data::Forum *forum) {
  ...
  _filterId = forum ? 0 : _controller->activeChatsFilterCurrent();
  if (_openedForum) {
  ...
  if (forum) {
    rpl::merge(forum->chatsListChanges(), forum->chatsListLoadedEvents()) |
        rpl::on_next([=] { refresh(); }, _openedForumLifetime);
  }
...
void InnerWidget::paintEvent(QPaintEvent *e) {
  ...
  const auto paintRow = [&](not_null<Row *> row, bool selected,
                            bool mayBeActive) {
    ...
    const auto expanding =
        (forum || monoforum) && (history->peer->id == childListShown.peerId);
    context.rightButton = maybeCacheRightButton(row);
    if (history) {
      if (_activeQuickAction &&
          (_activeQuickAction->data.msgBareId == history->peer->id.value)) {
        ...
        while (it != _inactiveQuickActions.end()) {
          ...
          if (raw->finishedAt && (ms - raw->finishedAt >
    ...
    if (context.narrow) {
      ...
      const auto a = active;
      context.st = (forum || monoforum) ? &st::taggedForumDialogRow
                                        : &st::taggedDialogRow;
      auto availableWidth = context.width - context.st->padding.right() -
                            st::dialogsUnreadPadding - context.st->nameLeft;
      auto more = uint8(0);
      ...
      for (const auto &filter : list) {
        ...
        if (active && (filter.flags() & Data::ChatFilter::Flag::NoRead) &&
        ...
        if (const auto tag = cacheChatsFilterTag(filter, 0, a)) {
          ...
          const auto tagWidth = tag->width() / style::DevicePixelRatio();
          if (availableWidth < tagWidth) {
            more++;
          } else {
            chatsFilterTags.push_back(tag);
            availableWidth -= tagWidth + st::dialogRowFilterTagSkip;
          }
        }
      }
      if (more) {
    ...
    context.topicsExpanded = (expanding && !active) ? childListShown.shown : 0.;
    context.active = active;
    context.selected = _menuRow.key ? (row->key() == _menuRow.key)
                       : _chatPreviewRow.key
                           ? (row->key() == _chatPreviewRow.key)
                           : selected;
    context.topicJumpSelected =
        selected && _selectedTopicJump && (!_pressed || _pressedTopicJump);
  ...
  if (_state == WidgetState::Default) {
    ...
    const auto selected = isPressed() ? (_pressed ? _pressed->key() : Key())
                                      : (_selected ? _selected->key() : Key());
    if (shownBottom) {
      const auto skip = dialogsOffset();
      const auto promoted = fixedOnTopCount();
      const auto reorderingPinned = (_aboveIndex >= 0) && !_pinnedRows.empty();
      const auto reorderingIndex =
          reorderingPinned ? (promoted + _aboveIndex) : -1;
      const auto reorderingRow =
          (reorderingIndex >= 0 && reorderingIndex < list.size())
      ...
      if (i != list.cend()) {
        ...
        // If we're reordering pinned chats we need to fill this area background
        // first.
        if (reorderingPinned) {
          const auto pinnedBottom = shownHeight(promoted + _pinnedRows.size());
          const auto pinnedTop = shownHeight(promoted);
          p.fillRect(0, pinnedTop - skippedTop, fullWidth,
                     pinnedBottom - pinnedTop, currentBg());
        }
    ...
    if (!_hashtagResults.empty()) {
      ...
      auto from = floorclamp(r.y() - skip, st::mentionHeight, 0,
                             _hashtagResults.size());
      auto to = ceilclamp(r.y() + r.height() - skip, st::mentionHeight, 0,
                          _hashtagResults.size());
      p.translate(0, from * st::mentionHeight);
      if (from < _hashtagResults.size()) {
        const auto htagleft = st::defaultDialogRow.padding.left();
        auto htagwidth =
            fullWidth - htagleft - st::defaultDialogRow.padding.right();

        p.setFont(st::mentionFont);
        for (; from < to; ++from) {
          auto &result = _hashtagResults[from];
          bool selected =
              (from == (isPressed() ? _hashtagPressed : _hashtagSelected));
          p.fillRect(0, 0, fullWidth, st::mentionHeight,
                     selected ? st::mentionBgOver : currentBg());
          result->row.paintRipple(p, 0, 0, fullWidth);
          auto &tag = result->tag;
          if (selected) {
            int skip =
                (st::mentionHeight - st::smallCloseIconOver.height()) / 2;
            st::smallCloseIconOver.paint(
                p,
                QPoint(fullWidth - st::smallCloseIconOver.width() - skip, skip),
                width());
          }
          auto first = (_hashtagFilter.size() < 2)
                           ? QString()
                           : ('#' + tag.mid(0, _hashtagFilter.size() - 1));
          auto second = (_hashtagFilter.size() < 2)
                            ? ('#' + tag)
                            : tag.mid(_hashtagFilter.size() - 1);
          ...
          if (!first.isEmpty()) {
            ...
            p.drawText(htagleft, st::mentionTop + st::mentionFont->ascent,
                       first);
          }
          if (!second.isEmpty()) {
            p.setPen(selected ? st::mentionFgOver : st::mentionFg);
            p.drawText(htagleft + firstwidth,
                       st::mentionTop + st::mentionFont->ascent, second);
          }
    ...
    if (!_filterResults.empty()) {
      ...
      auto to = std::min(filteredIndex(r.y() + r.height() - skip) + 1,
                         int(_filterResults.size()));
      const auto height = filteredHeight(from);
      p.translate(0, height);
      for (; from < to; ++from) {
        const auto selected = isPressed() ? (from == _filteredPressed)
    ...
    if (!_idSearchResults.empty()) {
      ...
      p.drawTextLeft(st::searchedBarPosition.x(), st::searchedBarPosition.y(),
                     width(), tr::ayu_SearchByIDResults(tr::now));
      p.translate(0, st::searchedBarHeight);

      auto skip = idSearchOffset();
      auto from = floorclamp(r.y() - skip, st::dialogsRowHeight, 0,
                             _idSearchResults.size());
      auto to = ceilclamp(r.y() + r.height() - skip, st::dialogsRowHeight, 0,
                          _idSearchResults.size());
      ...
      if (from < _idSearchResults.size()) {
        ...
        for (; from < to; ++from) {
          ...
          const auto active =
              !activeEntry.fullId && activePeer &&
              ((peer == activePeer) || (peer->migrateTo() == activePeer));
          const auto selected =
              (from == (isPressed() ? _idSearchPressed : _idSearchSelected));
    ...
    if (!_peerSearchResults.empty()) {
      ...
      p.setPen(st::searchedBarFg);
      p.drawTextLeft(st::searchedBarPosition.x(), st::searchedBarPosition.y(),
                     width(), tr::lng_search_global_results(tr::now));
      p.translate(0, st::searchedBarHeight);
      ...
      if (from < _peerSearchResults.size()) {
        ...
        for (; from < to; ++from) {
          ...
          if (result->sponsored &&
              ...
              r.y() + r.height() >=
                  (skip + (from + 1) * st::dialogsRowHeight)) {
          ...
          const auto active =
              ...
              ((peer == activePeer) || (peer->migrateTo() == activePeer));
          const auto selected =
              (from == ((_peerSearchMenu >= 0) ? _peerSearchMenu
                        : isPressed()          ? _peerSearchPressed
                                               : _peerSearchSelected));
          if (result->sponsored && result->sponsored->button.text.isEmpty()) {
            fillRightButton(
                result->sponsored->button,
                tr::lng_search_sponsored_button(tr::now, tr::marked),
          ...
          paintPeerSearchResult(
              p, result.get(),
              {
                  .rightButton = (result->sponsored ? &result->sponsored->button
                                                    : nullptr),
                  .st = &st::defaultDialogRow,
                  .currentBg = currentBg(),
                  .now = ms,
        ...
        if (to < _peerSearchResults.size()) {
          p.translate(0,
                      (_peerSearchResults.size() - to) * st::dialogsRowHeight);
        }
      }
    }

    const auto showUnreadInSearchResults = uniqueSearchResults();
    if (_idSearchResults.empty() && _previewResults.empty() &&
    ...
    if (!_previewResults.empty()) {
                                ...
                                : st::searchedBarFont;
      {
        ...
        p.setFont(moreFont);
        p.drawTextLeft(width() - st::searchedBarPosition.x() - _morePostsWidth,
                       st::searchedBarPosition.y(), width(), text);
        p.translate(0, st::searchedBarHeight);
      }
      auto skip = previewOffset();
      auto from =
          floorclamp(r.y() - skip, _st->height, 0, _previewResults.size());
      auto to = ceilclamp(r.y() + r.height() - skip, _st->height, 0,
                          _previewResults.size());
      p.translate(0, from * _st->height);
      if (from < _previewResults.size()) {
        const auto searchLowerText =
        ...
        for (; from < to; ++from) {
          ...
          const auto selected =
              _menuRow.key ? isSearchResultActive(result.get(), _menuRow)
              : _chatPreviewRow.key
                  ? isSearchResultActive(result.get(), _chatPreviewRow)
                  : (from ==
                     (isPressed() ? _previewPressed : _previewSelected));
          Ui::RowPainter::Paint(
              p, result.get(),
              {
                  .st = _st,
    ...
    if (!_searchResults.empty()) {
      const auto text =
          showUnreadInSearchResults ? u"Search results"_q
          : (_searchState.tab == ChatSearchTab::PublicPosts && !_searchIn)
              ...
              : tr::lng_search_found_results(
                    tr::now, lt_count, _searchedMigratedCount + _searchedCount);
      ...
      p.drawTextLeft(st::searchedBarPosition.x(), st::searchedBarPosition.y(),
                     width(), text);
      const auto filterOver = _selectedChatTypeFilter || _pressedChatTypeFilter;
      const auto filterFont =
          filterOver ? st::searchedBarFont->underline() : st::searchedBarFont;
      ...
      auto from =
          floorclamp(r.y() - skip, _st->height, 0, _searchResults.size());
      auto to = ceilclamp(r.y() + r.height() - skip, _st->height, 0,
                          _searchResults.size());
      p.translate(0, from * _st->height);
        ...
        for (; from < to; ++from) {
          ...
          const auto selected =
              ...
              : _chatPreviewRow.key
                  ? isSearchResultActive(result.get(), _chatPreviewRow)
                  : (from ==
                     (isPressed() ? _searchedPressed : _searchedSelected));
          Ui::RowPainter::Paint(
              p, result.get(),
              {
                  .st = _st,
...
void InnerWidget::fillRightButton(RightButton &button,
                                  const TextWithEntities &text,
                                  const style::DialogRightButton &st) {
  button.st = &st;
  button.text.setMarkedText(st.button.style, text);
  const auto size =
      QSize(button.text.maxWidth() + button.text.minHeight(), st.button.height);
  const auto generateBg = [&](const style::color &c) {
    auto bg = QImage(style::DevicePixelRatio() * size,
...
[[nodiscard]] RightButton *InnerWidget::maybeCacheRightButton(Row *row) {
  if (const auto user = MaybeBotWithApp(row)) {
    ...
    if (it == _rightButtons.end()) {
      ...
      fillRightButton(rightButton,
                      tr::lng_profile_open_app_short(tr::now, tr::marked),
                      st::dialogRowOpenBot);
      return &(_rightButtons.emplace(user->id, std::move(rightButton))
                   .first->second);
    } else {
      return &(it->second);
    }
...
Ui::VideoUserpic *
InnerWidget::validateVideoUserpic(not_null<History *> history) {
  const auto peer = history->peer;
  if (!peer->isPremium() || peer->userpicPhotoUnknown() ||
  ...
  return _videoUserpics
      .emplace(peer, std::make_unique<Ui::VideoUserpic>(peer, repaint))
      .first->second.get();
}
...
void InnerWidget::paintCollapsedRows(Painter &p, QRect clip) const {
  ...
  for (const auto &row : _collapsedRows) {
    ...
    const auto selected =
        (index == _collapsedSelected) || (index == _collapsedPressed);
    paintCollapsedRow(p, row.get(), selected);
  }
}

void InnerWidget::paintCollapsedRow(Painter &p,
  ...
  Ui::PaintCollapsedRow(
      p, row->row, row->folder, text, unread,
      {
          .st = _st,
          .currentBg = currentBg(),
          .width = fullWidth,
...
bool InnerWidget::isRowActive(not_null<Row *> row,
...
bool InnerWidget::isSearchResultActive(not_null<FakeRow *> result,
                                       const RowDescriptor &entry) const {
  const auto item = result->item();
  const auto peer = item->history()->peer;
  return (item->fullId() == entry.fullId) ||
         (peer->migrateTo() && (peer->migrateTo()->id == entry.fullId.peer) &&
          (item->id == -entry.fullId.msg)) ||
         (uniqueSearchResults() && peer == entry.key.peer());
}

void InnerWidget::paintPeerSearchResult(
    Painter &p, not_null<const PeerSearchResult *> result,
  ...
  const auto badgeWidth = result->badge.drawGetWidth(
      p, {
             ...
             .verified = (context.active     ? &st::dialogsVerifiedIconActive
                          : context.selected ? &st::dialogsVerifiedIconOver
                                             : &st::dialogsVerifiedIcon),
             .premium = &ThreeStateIcon(st::dialogsPremiumIcon, context.active,
                                        context.selected),
             .scam = (context.active     ? &st::dialogsScamFgActive
                      : context.selected ? &st::dialogsScamFgOver
                                         : &st::dialogsScamFg),
             .direct = (context.active     ? &st::dialogsDraftFgActive
                        : context.selected ? &st::windowSubTextFgOver
                                           : &st::windowSubTextFg),
             .premiumFg = (context.active     ? &st::dialogsVerifiedIconBgActive
                           : context.selected ? &st::dialogsVerifiedIconBgOver
  ...
  QRect tr(context.st->textLeft, context.st->textTop, namewidth,
           st::dialogsTextFont->height);
  p.setFont(st::dialogsTextFont);
  QString username = peer->username();
  if (username.isEmpty()) {
    const auto idText =
        QString("ID: ") + QString::number(abs(getDialogIdFromPeer(peer)));
    p.setPen(context.active ? st::dialogsTextFgActive
                            : st::dialogsTextFgService);
    p.drawText(tr.left(), tr.top() + st::dialogsTextFont->ascent,
               st::dialogsTextFont->elided(idText, tr.width()));
    ...
    if (w >= tr.width()) {
      ...
      p.drawText(tr.left(), tr.top() + st::dialogsTextFont->ascent,
                 st::dialogsTextFont->elided(first, tr.width()));
    } else {
      p.setPen(st::dialogsTextFgService);
      p.drawText(tr.left(), tr.top() + st::dialogsTextFont->ascent, first);
      p.setPen(st::dialogsTextFg);
      p.drawText(tr.left() + w, tr.top() + st::dialogsTextFont->ascent,
                 st::dialogsTextFont->elided(second, tr.width() - w));
    }
  } else {
    p.setPen(context.active ? st::dialogsTextFgActive
                            : st::dialogsTextFgService);
    p.drawText(tr.left(), tr.top() + st::dialogsTextFont->ascent,
               st::dialogsTextFont->elided('@' + username, tr.width()));
  }

  p.setPen(context.active ? st::dialogsTextFgActive : st::dialogsNameFg);
  result->name.drawElided(p, rectForName.left(), rectForName.top(),
                          rectForName.width());
}

QBrush InnerWidget::currentBg() const {
  return anim::brush(st::dialogsBg, st::dialogsBgOver,
                     _childListShown.current().shown);
}

void InnerWidget::paintSearchTags(Painter &p,
...
void InnerWidget::showPeerMenu() {
  ...
  const auto pos =
      QPoint(width() - padding.right(),
             _selected->top() + _selected->height() + padding.bottom());
  auto event =
      QContextMenuEvent(QContextMenuEvent::Keyboard, pos, mapToGlobal(pos));
...
void InnerWidget::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (!_lastMousePosition) {
    ...
    return;
  } else if (!_mouseSelection && *_lastMousePosition == globalPosition) {
    return;
  }
...
bool InnerWidget::skipChatsListFreeze() const { return _dragging != nullptr; }
...
void InnerWidget::performDrag() {
  ...
  mimeData->setData(u"application/x-telegram-dialog"_q, std::move(byteArray));

  if (const auto u = history->peer->username(); !u.isEmpty()) {
    mimeData->setText(history->peer->session().createInternalLinkFull(u));
    mimeData->setData(u"application/x-telegram-input-field"_q,
                      ('@' + u).toUtf8());
  }
...
bool InnerWidget::lookupIsInBotAppButton(Row *row, QPoint localPosition) {
  if (_narrowRatio) {
    return false;
  }
  if (const auto user = MaybeBotWithApp(row)) {
...
bool InnerWidget::lookupIsInRightButton(const RightButton &button,
  ...
  const auto r = QRect(width() - s.width() - button.st->margin.right(),
                       button.st->margin.top(), s.width(), s.height());
...
void InnerWidget::selectByMouse(QPoint globalPosition) {
  ...
  const auto tagBase = QPoint(_searchTagsLeft, st::dialogsSearchTagBottom / 2);
  const auto tagPoint = local - tagBase;
  const auto inTags =
      _searchTags && QRect(tagBase, QSize(width() - 2 * _searchTagsLeft,
                                          _searchTags->height()))
                         .contains(local);
  const auto tagLink = inTags ? _searchTags->lookupHandler(tagPoint) : nullptr;
  ...
  if (_state == WidgetState::Default) {
    ...
    const auto collapsedSelected =
        (mouseY >= 0 &&
         mouseY < _collapsedRows.size() * st::dialogsImportantBarHeight)
            ? (mouseY / st::dialogsImportantBarHeight)
            : -1;
    const auto selected = (collapsedSelected >= 0) ? nullptr
                          : (mouseY >= offset)
                              ? _shownList->rowAtY(mouseY - offset)
                              : nullptr;
    const auto mappedY = selected ? mouseY - offset - selected->top() : 0;
    const auto selectedTopicJump =
        selected && selected->lookupIsInTopicJump(local.x(), mappedY);
    const auto selectedRightButton =
        selected &&
        lookupIsInBotAppButton(selected, QPoint(local.x(), mappedY));
    if (_collapsedSelected != collapsedSelected || _selected != selected ||
      ...
      updateSelectedRow();
      setCursor((_selected || _collapsedSelected >= 0) ? style::cur_pointer
                                                       : style::cur_default);
    }
  } else if (_state == WidgetState::Filtered) {
    ...
    if (_hashtagResults.empty()) {
      ...
      auto hashtagSelected =
          (mouseY >= skip) ? ((mouseY - skip) / st::mentionHeight) : -1;
      ...
      _hashtagDeleteSelected =
          (_hashtagSelected >= 0) && (local.x() >= w - st::mentionHeight);
    }
    if (!_filterResults.empty()) {
      auto skip = filteredOffset();
      auto filteredSelected =
          (mouseY >= skip) ? filteredIndex(mouseY - skip) : -1;
      ...
      const auto selectedTopicJump =
          (filteredSelected >= 0) &&
          _filterResults[filteredSelected].row->lookupIsInTopicJump(local.x(),
                                                                    mappedY);
      const auto selectedRightButton =
          (filteredSelected >= 0)
              ? lookupIsInBotAppButton(_filterResults[filteredSelected].row,
    ...
    if (!_idSearchResults.empty()) {
      ...
      auto idSearchSelected =
          (mouseY >= skip) ? ((mouseY - skip) / st::dialogsRowHeight) : -1;
    ...
    if (!_peerSearchResults.empty()) {
      ...
      auto peerSearchSelected =
          (mouseY >= skip) ? ((mouseY - skip) / st::dialogsRowHeight) : -1;
      if (peerSearchSelected < 0 ||
          peerSearchSelected >= _peerSearchResults.size()) {
        peerSearchSelected = -1;
      }
      const auto mappedY =
          (peerSearchSelected >= 0)
    ...
    if (!_previewResults.empty()) {
      auto skip = previewOffset();
      auto previewSelected =
          (mouseY >= skip) ? ((mouseY - skip) / _st->height) : -1;
      ...
      if (mouseY <= skip && mouseY >= from) {
        const auto left =
            width() - _morePostsWidth - 2 * st::searchedBarPosition.x();
    ...
    if (!_searchResults.empty()) {
      ...
      if (hasChatTypeFilter() && mouseY <= skip && mouseY >= from) {
        const auto left =
            width() - _chatTypeFilterWidth - 2 * st::searchedBarPosition.x();
...
RowDescriptor InnerWidget::computeChatPreviewRow() const {
  ...
  if (const auto peer = result.key.peer()) {
    const auto topicId = _pressedTopicJump ? _pressedTopicJumpRootId : MsgId();
    const auto sublistPeerId =
        _pressedTopicJump ? _pressedSublistJumpPeerId : PeerId();
...
void InnerWidget::mousePressEvent(QMouseEvent *e) {
  ...
  if (base::in_range(_collapsedSelected, 0, _collapsedRows.size())) {
    ...
    row->addRipple(e->pos(), QSize(width(), st::dialogsImportantBarHeight),
                   [this, index = _collapsedSelected] {
                     update(0, (index * st::dialogsImportantBarHeight), width(),
    ...
    const auto origin = e->pos() - QPoint(0, dialogsOffset() + _pressed->top());
    if ((_pressButton == Qt::MiddleButton) &&
      ...
      row->addTopicJumpRipple(origin, _topicJumpCache.get(), updateCallback);
    } else {
      row->clearTopicJumpRipple();
      row->addRipple(origin, QSize(width(), _pressed->height()),
                     updateCallback);
    }
    _dragStart = e->pos();
  } else if (base::in_range(_hashtagPressed, 0, _hashtagResults.size()) &&
             !_hashtagDeletePressed) {
    auto row = &_hashtagResults[_hashtagPressed]->row;
    const auto origin =
        e->pos() -
        QPoint(0, hashtagsOffset() + _hashtagPressed * st::mentionHeight);
    row->addRipple(origin, QSize(width(), st::mentionHeight),
                   [this, index = _hashtagPressed] {
                     update(0, hashtagsOffset() + index * st::mentionHeight,
                            width(), st::mentionHeight);
                   });
    ...
    const auto origin = e->pos() - QPoint(0, filteredOffset() + result.top);
    ...
    if (addRightButtonRipple(origin, updateCallback)) {
      ...
      row->clearTopicJumpRipple();
      row->addRipple(origin, QSize(width(), row->height()), updateCallback);
    }
    ...
    row->addRipple(origin, QSize(width(), st::dialogsRowHeight),
                   updateCallback);
  } else if (base::in_range(_peerSearchPressed, 0, _peerSearchResults.size())) {
    auto &result = _peerSearchResults[_peerSearchPressed];
    const auto row = &result->row;
    const auto origin =
        e->pos() - QPoint(0, peerSearchOffset() +
                                 _peerSearchPressed * st::dialogsRowHeight);
    const auto updateCallback = [this, peer = result->peer] {
      updateSearchResult(peer);
    };
    if (addRightButtonRipple(origin, updateCallback)) {
    ...
    row->addRipple(
        ...
        QSize(width(), _st->height), row->repaint());
  }
  ClickHandler::pressed();
  if (anim::Disabled() && !_chatPreviewScheduled &&
      (!_pressed || !_pressed->entry()->isPinnedDialog(_filterId))) {
    mousePressReleased(e->globalPos(), e->button(), e->modifiers());
  }
}

bool InnerWidget::addRightButtonRipple(QPoint origin,
                                       Fn<void()> updateCallback) {
  if (!(_pressedRightButton && _pressedRightButtonData)) {
    return false;
  }
  const auto size =
      _pressedRightButtonData->bg.size() / style::DevicePixelRatio();
  ...
  const auto shift = QPoint(width() - size.width() -
                                _pressedRightButtonData->st->margin.right(),
...
bool InnerWidget::addQuickActionRipple(not_null<Row *> row,
  ...
  context->icon->jumpTo(context->icon->framesCount() - 1, [=] {
    const auto size = QSize(st::dialogsQuickActionRippleSize, rowHeight);
    if (!context->ripple) {
      context->ripple = std::make_unique<Ui::RippleAnimation>(
          st::defaultRippleAnimation, Ui::RippleAnimation::RectMask(size),
          updateCallback);
    }
    if (!context->rippleFg) {
      context->rippleFg = std::make_unique<Ui::RippleAnimation>(
          ...
          Ui::RippleAnimation::MaskByDrawer(
              ...
              [&](QPainter &p) {
                ...
                DrawQuickAction(
                    p, Rect(size), context->icon.get(),
                    ResolveQuickDialogLabel(history, action, _filterId));
              }),
          std::move(updateCallback));
    }
...
const std::vector<Key> &InnerWidget::pinnedChatsOrder() const {
  ...
  return _savedSublists ? owner->pinnedChatsOrder(_savedSublists)
         : _openedForum ? owner->pinnedChatsOrder(_openedForum)
         : _filterId    ? owner->pinnedChatsOrder(_filterId)
...
void InnerWidget::startReorderPinned(QPoint localPosition) {
  ...
  if (updateReorderIndexGetCount() < 2) {
    ...
    _pinnedOnDragStart = base::flat_set<Key>{order.begin(), order.end()};
    _pinnedRows[_draggingIndex].yadd =
        anim::value(0, localPosition.y() - _dragStart.y());
...
bool InnerWidget::updateReorderPinned(QPoint localPosition) {
  ...
  if (_dragStart.y() > localPosition.y() && _draggingIndex > 0) {
    shift =
        -floorclamp(_dragStart.y() - localPosition.y() + (draggingHeight / 2),
                    draggingHeight, 0, _draggingIndex);

    for (auto from = _draggingIndex, to = _draggingIndex + shift; from > to;
         --from) {
      _shownList->movePinned(_dragging, -1);
      std::swap(_pinnedRows[from], _pinnedRows[from - 1]);
      _pinnedRows[from].yadd =
          anim::value(_pinnedRows[from].yadd.current() - draggingHeight, 0);
    ...
    shift =
        floorclamp(localPosition.y() - _dragStart.y() + (draggingHeight / 2),
                   draggingHeight, 0, pinnedCount - _draggingIndex - 1);
  ...
  _pinnedRows[_draggingIndex].yadd =
      anim::value(yaddWas - shiftHeight, localPosition.y() - _dragStart.y());
  ...
  const auto delta = [&] {
    if (localPosition.y() < _visibleTop) {
      return localPosition.y() - _visibleTop;
               ...
               (_savedSublists || _openedFolder || _openedForum || _filterId)) {
...
bool InnerWidget::pinnedShiftAnimationCallback(crl::time now) {
  ...
  for (auto i = 0, l = static_cast<int>(_pinnedRows.size()); i != l; ++i) {
    ...
    if (start) {
      ...
      if (start + st::stickersRowDuration > now && now >= start) {
        _pinnedRows[i].yadd.update(
            float64(now - start) / st::stickersRowDuration, anim::sineInOut);
        animating = true;
      } else {
  ...
  if (_draggingIndex >= 0) {
    ...
    if (updateMax < _draggingIndex)
      updateMax = _draggingIndex;
  }
  if (updateMin >= 0) {
    ...
    if (base::in_range(_aboveIndex, 0, _pinnedRows.size())) {
      ...
      accumulate_max(updateHeight,
                     (aboveRowBottom - updateFrom) + _aboveTopShift);
      accumulate_max(updateHeight,
                     (aboveRowBottom - updateFrom) + aboveTopShift);
...
void InnerWidget::mousePressReleased(QPoint globalPosition,
  ...
  if ((pressButton == Qt::MiddleButton) && _activeQuickAction && pressed &&
    ...
    if (const auto history = pressed->history()) {
      ...
      if (pressed == _selected) {
        PerformQuickDialogAction(_controller, history->peer, raw->action,
                                 _filterId);
      }
  ...
  if (!wasDragging && button == Qt::LeftButton) {
    if ((collapsedPressed >= 0 && collapsedPressed == _collapsedSelected) ||
        ...
        (hashtagPressed >= 0 && hashtagPressed == _hashtagSelected &&
         hashtagDeletePressed == _hashtagDeleteSelected) ||
        (filteredPressed >= 0 && filteredPressed == _filteredSelected &&
         pressedRightButton == _selectedRightButton) ||
        (idSearchPressed >= 0 && idSearchPressed == _idSearchSelected) ||
        (peerSearchPressed >= 0 && peerSearchPressed == _peerSearchSelected &&
         pressedRightButton == _selectedRightButton) ||
        (previewPressed >= 0 && previewPressed == _previewSelected) ||
        (searchedPressed >= 0 && searchedPressed == _searchedSelected) ||
        (pressedMorePosts && pressedMorePosts == _selectedMorePosts) ||
        (pressedChatTypeFilter &&
         pressedChatTypeFilter == _selectedChatTypeFilter)) {
      if (pressedRightButton && (pressed || filteredPressed >= 0)) {
        const auto &row =
            pressed ? pressed : _filterResults[filteredPressed].row.get();
        if (const auto user = MaybeBotWithApp(row)) {
        ...
        chooseRow(modifiers, pressedTopicRootId, pressedSublistPeerId);
      }
    }
  }
  if (auto activated = ClickHandler::unpressed()) {
    ActivateClickHandler(
        window(), activated,
        ClickContext{button, QVariant::fromValue(ClickHandlerContext{
...
void InnerWidget::setPressed(Row *pressed, bool pressedTopicJump,
  ...
  if ((_pressed != pressed) ||
    ...
    if (pressed || !pressedTopicJump || !pressedRightButton) {
      ...
      _pressedSublistJumpPeerId = item ? item->sublistPeerId() : PeerId();
    }
  }
}

void InnerWidget::clearPressed() { setPressed(nullptr, false, false); }
...
void InnerWidget::setFilteredPressed(int pressed, bool pressedTopicJump,
  ...
  if (_filteredPressed != pressed ||
    ...
    if (pressed >= 0 || !pressedTopicJump || !pressedRightButton) {
      ...
      const auto history =
          pressedTopicJump ? _filterResults[pressed].row->history() : nullptr;
      const auto item = history ? history->chatListMessage() : nullptr;
      _pressedTopicJumpRootId = item ? item->topicRootId() : MsgId();
      _pressedSublistJumpPeerId = item ? item->sublistPeerId() : PeerId();
    }
...
void InnerWidget::moveSearchIn() {
  ...
  const auto searchInWidth =
      std::max(width(), st::columnMinimalWidthLeft - _narrowWidth);
  _searchIn->resizeToWidth(searchInWidth);
  _searchIn->moveToLeft(0, searchInChatOffset());
}

void InnerWidget::dialogRowReplaced(Row *oldRow, Row *newRow) {
  auto found = false;
  if (_state == WidgetState::Filtered) {
    auto top = 0;
    for (auto i = _filterResults.begin(); i != _filterResults.end();) {
      if (i->row ==
          oldRow) { // this row is shown in filtered and maybe is in contacts!
        found = true;
        top = i->top;
        if (!newRow) {
      ...
      rpl::on_next(
          [=](const Event &event) {
            ...
            if (!_dragging && (from != to) &&
                ...
                (key.topic()     ? (key.topic()->forum() == _openedForum)
                 : key.sublist() ? (key.sublist()->parent() == _savedSublists)
            ...
            if (event.existenceChanged) {
              if (!entry->inChatList()) {
                ...
                const auto i =
                    ranges::find(_filterResults, key, &FilterResult::key);
              ...
              update(0, std::min(from, to), width(),
...
void InnerWidget::repaintCollapsedFolderRow(not_null<Data::Folder *> folder) {
  for (auto i = 0, l = int(_collapsedRows.size()); i != l; ++i) {
    if (_collapsedRows[i]->folder == folder) {
      update(0, i * st::dialogsImportantBarHeight, width(),
             st::dialogsImportantBarHeight);
      return;
    }
  }
...
void InnerWidget::repaintDialogRow(FilterId filterId, not_null<Row *> row) {
  if (_state == WidgetState::Default) {
    ...
    if (!filterId) {
      for (auto i = 0, l = int(_filterResults.size()); i != l; ++i) {
        ...
        if (result.key() == row->key()) {
          update(0, filteredOffset() + result.top, width(),
                 result.row->height());
          break;
        }
      }
...
void InnerWidget::repaintDialogRow(RowDescriptor row) { updateDialogRow(row); }
...
void InnerWidget::updateSearchResult(not_null<PeerData *> peer) {
  if (_state == WidgetState::Filtered) {
    const auto i =
        ranges::find(_peerSearchResults, peer, &PeerSearchResult::peer);
    if (i != end(_peerSearchResults)) {
      const auto top = peerSearchOffset();
      const auto index = (i - begin(_peerSearchResults));
      rtlupdate(0, top + index * st::dialogsRowHeight, width(),
                st::dialogsRowHeight);
    }
  }
}

void InnerWidget::updateDialogRow(RowDescriptor row, QRect updateRect,
    ...
    if (const auto peer = row.key.peer()) {
      if (const auto from = peer->migrateFrom()) {
        if (const auto migrated = from->owner().historyLoaded(from)) {
          row = RowDescriptor(migrated, FullMsgId(from->id, -row.fullId.msg));
        }
      }
    }
  }
  ...
  if (_state == WidgetState::Default) {
    ...
    if ((sections & UpdateRowSection::Filtered) && !_filterResults.empty()) {
      for (const auto &result : _filterResults) {
        if (result.key() == row.key) {
          updateRow(filteredOffset() + result.top, result.row->height());
          break;
        }
      }
    }
    if ((sections & UpdateRowSection::PeerSearch) &&
      ...
      if (const auto peer = row.key.peer()) {
        ...
        for (const auto &result : _peerSearchResults) {
          if (result->peer == peer) {
            updateRow(peerSearchOffset() + index * rowHeight, rowHeight);
            break;
          }
          ++index;
        }
...
void InnerWidget::enterEventHook(QEnterEvent *e) { setMouseTracking(true); }
...
void InnerWidget::updateSelectedRow(Key key) {
  if (_state == WidgetState::Default) {
    if (key) {
      ...
      update(0, dialogsOffset() + _selected->top(), width(),
             _selected->height());
    } else if (_collapsedSelected >= 0) {
      update(0, _collapsedSelected * st::dialogsImportantBarHeight, width(),
             st::dialogsImportantBarHeight);
    }
  } else if (_state == WidgetState::Filtered) {
    if (key) {
      ...
      update(0, hashtagsOffset() + _hashtagSelected * st::mentionHeight,
             width(), st::mentionHeight);
      ...
      update(0, idSearchOffset() + _idSearchSelected * st::dialogsRowHeight,
             width(), st::dialogsRowHeight);
    } else if (_peerSearchSelected >= 0) {
      update(0, peerSearchOffset() + _peerSearchSelected * st::dialogsRowHeight,
             width(), st::dialogsRowHeight);
    } else if (_previewSelected >= 0) {
      update(0, previewOffset() + _previewSelected * _st->height, width(),
             _st->height);
    } else if (_searchedSelected >= 0) {
      update(0, searchedOffset() + _searchedSelected * _st->height, width(),
             _st->height);
    }
  }
}
...
void InnerWidget::refreshShownList() {
  const auto list =
      _savedSublists ? _savedSublists->chatsList()->indexed()
      : _openedForum ? _openedForum->topicsList()->indexed()
      : _filterId
...
FilterId InnerWidget::filterId() const { return _filterId; }
...
void InnerWidget::clearSelection() {
  ...
  if (isSelected()) {
    ...
    _selected = nullptr;
    _filteredSelected = _searchedSelected = _previewSelected =
        _peerSearchSelected = _hashtagSelected = -1;
...
void InnerWidget::fillArchiveSearchMenu(not_null<Ui::PopupMenu *> menu) {
  ...
  const auto globalSearch = (_searchState.tab == ChatSearchTab::MyMessages) ||
                            (_searchState.tab == ChatSearchTab::PublicPosts);
  if (!folder || !folder->chatsList()->fullSize().current() ||
      (!globalSearch && _searchState.inChat) ||
      (_searchState.tab == ChatSearchTab::PublicPosts)) {
    return;
  }
  const auto skip = session().settings().skipArchiveInSearch();
  const auto text = skip ? tr::lng_dialogs_show_archive_in_search(tr::now)
...
bool InnerWidget::showChatPreview() {
  ...
  const auto callback =
      crl::guard(this, [=](bool shown) { chatPreviewShown(shown, row); });
...
bool InnerWidget::scheduleChatPreview(QPoint positionOverride) {
  ...
  _chatPreviewScheduled = _controller->scheduleChatPreview(
      row, callback, nullptr, positionOverride);
...
void InnerWidget::contextMenuEvent(QContextMenuEvent *e) {
  ...
  _menu = base::make_unique_q<Ui::PopupMenu>(
      this, row.fullId ? st::defaultPopupMenu : st::popupMenuExpandedSeparator);
  if (row.fullId) {
    if (session().supportMode()) {
...
void InnerWidget::showSponsoredMenu(int peerSearchIndex, QPoint globalPos) {
  ...
  _menu =
      base::make_unique_q<Ui::PopupMenu>(this, st::popupMenuExpandedSeparator);
  const auto peer = entry->peer;
  const auto remove = crl::guard(this, [=] {
    _sponsoredRemoved.emplace(peer);
    if (_pressedRightButtonData) {
      for (const auto &result : _peerSearchResults) {
        if (result->peer == peer && result->sponsored &&
    ...
    _peerSearchResults.erase(
        ranges::remove(_peerSearchResults, peer, &PeerSearchResult::peer),
        end(_peerSearchResults));
    refresh();
  });
  Menu::FillSponsored(
      Ui::Menu::CreateAddActionCallback(_menu), _controller->uiShow(),
      ...
      session().sponsoredMessages().createReportCallback(
          entry->sponsored->data.randomId, remove));
  QObject::connect(_menu.get(), &QObject::destroyed, [=] {
    if (_peerSearchMenu >= 0 && _peerSearchMenu < _peerSearchResults.size()) {
...
void InnerWidget::applySearchState(SearchState state) {
  ...
  if (peer && peer->isSelf()) {
    ...
    _searchTags = std::make_unique<SearchTags>(
        &peer->owner(), reactions->myTagsValue(sublist), state.tags);
        ...
        rpl::on_next(
            [=](Data::ReactionId id) {
              HistoryView::ShowTagInListMenu(
                  &_menu, _lastMousePosition.value_or(QCursor::pos()), this, id,
                  _controller);
            },
            _searchTags->lifetime());

    _searchTags->heightValue() | rpl::skip(1) |
        rpl::on_next(
            [=] {
              refresh();
              moveSearchIn();
  ...
  _searchFromShown = ignoreInChat ? nullptr
                     : sublist    ? sublist->sublistPeer().get()
                                  : state.fromPeer;
  if (state.inChat) {
  ...
  if (newFilter != _filter || otherChanged) {
    _filter = newFilter;
    if (_filter.isEmpty() && !_searchState.fromPeer &&
      ...
      const auto append = [&](not_null<IndexedList *> list) {
        ...
        auto i = _filterResults.insert(end(_filterResults), begin(results),
                                       end(results));
  ...
  if (_state != WidgetState::Default) {
    _searchWaiting = true;
    _searchRequests.fire(otherChanged ? SearchRequestDelay::Instant
...
void InnerWidget::onHashtagFilterUpdate(QStringView newFilter) {
  if (newFilter.isEmpty() || newFilter.at(0) != '#' || _searchState.inChat) {
  ...
  if (!recent.isEmpty()) {
    ...
    for (const auto &tag : recent) {
      if (tag.first.startsWith(base::StringViewMid(_hashtagFilter, 1),
                               Qt::CaseInsensitive) &&
          tag.first.size() + 1 != newFilter.size()) {
        _hashtagResults.push_back(std::make_unique<HashtagResult>(tag.first));
        if (_hashtagResults.size() == kHashtagResultsLimit)
          break;
      }
    }
  }
          ...
          [=] {
            ...
            for (auto i = begin(_filterResultsGlobal);
              ...
              if (const auto topic = i->first.topic()) {
                if (topic->peer() == peer) {
                  removed = true;
                  _filterResults.erase(ranges::remove(_filterResults, i->first,
                                                      &FilterResult::key),
                                       end(_filterResults));
    ...
    forum->topicDestroyed() |
        rpl::on_next(
            [=](not_null<Data::ForumTopic *> topic) {
              auto removed = false;
              const auto sfrom =
                  ranges::remove(_searchResults, topic.get(), &FakeRow::topic);
              if (sfrom != end(_searchResults)) {
                _searchResults.erase(sfrom, end(_searchResults));
                removed = true;
              }
              const auto ffrom = ranges::remove(_filterResults, Key(topic),
                                                &FilterResult::key);
...
Data::Thread *InnerWidget::updateFromParentDrag(QPoint globalPosition) {
  ...
  const auto fromRow = [](Row *row) { return row ? row->thread() : nullptr; };
  if (_state == WidgetState::Default) {
    return fromRow(_selected);
  } else if (_state == WidgetState::Filtered) {
    if (base::in_range(_filteredSelected, 0, _filterResults.size())) {
      return fromRow(_filterResults[_filteredSelected].row);
    } else if (base::in_range(_peerSearchSelected, 0,
                              _peerSearchResults.size())) {
...
rpl::producer<> InnerWidget::updated() const { return _updated.events(); }
...
void InnerWidget::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
  ...
  preloadRowsData();
  const auto loadTill =
      _visibleTop + PreloadHeightsCount * (_visibleBottom - _visibleTop);
  ...
  for (auto i = _searchResults.begin(); i != _searchResults.end();) {
    if ((*i)->item() == item) {
      ...
      if (item->history() == _searchInMigrated) {
        if (_searchedMigratedCount > 0)
          --_searchedMigratedCount;
      } else {
        if (_searchedCount > 0)
          --_searchedCount;
      }
...
bool InnerWidget::hasHistoryInResults(not_null<History *> history) const {
  ...
  const auto inSearchResults =
      ranges::find(_searchResults, history, [](const Result &result) {
        return result->item()->history();
      }) != end(_searchResults);
  if (inSearchResults) {
    return true;
  }
  const auto inFilteredResults =
      ranges::find(_filterResults, Key(history), &FilterResult::key) !=
      end(_filterResults);
  if (inFilteredResults) {
    return true;
  }
  const auto inPeerSearchResults =
      ranges::find(_peerSearchResults, history->peer, [](const auto &result) {
        return result->peer;
      }) != end(_peerSearchResults);
...
void InnerWidget::searchReceived(std::vector<not_null<HistoryItem *>> messages,
                                 HistoryItem *inject, SearchRequestType type,
                                 int fullCount) {
  _searchWaiting = false;
  ...
  const auto globalSearch = (_searchState.tab == ChatSearchTab::MyMessages) ||
                            (_searchState.tab == ChatSearchTab::PublicPosts);
  const auto key = globalSearch ? Key()
                   : (!_openedForum || _searchState.inChat.topic())
                       ? _searchState.inChat
                       : Key(_openedForum->history());
  if (inject && (globalSearch || !_searchState.inChat ||
    ...
    _searchResults.push_back(std::make_unique<FakeRow>(
        key, inject, [=] { repaintSearchResult(index); }));
  ...
  for (const auto &item : messages) {
    ...
    if (toPreview || !uniquePeers || !hasHistoryInResults(history)) {
      ...
      results.push_back(std::make_unique<FakeRow>(key, item, repaint));
...
void InnerWidget::peerSearchReceived(Api::PeerSearchResult result) {
  ...
  clearPeerSearchResults();
  _peerSearchResults.reserve(result.peers.size() + result.sponsored.size());
...
void InnerWidget::idSearchReceived(
  ...
  for (const auto &peer : results) {
    _idSearchResults.push_back(std::make_unique<PeerSearchResult>(peer));
  }
  refresh();
}

Data::Folder *InnerWidget::shownFolder() const { return _openedFolder; }

Data::Forum *InnerWidget::shownForum() const { return _openedForum; }

bool InnerWidget::needCollapsedRowsRefresh() const {
  const auto archive =
      !_shownList->empty() ? _shownList->begin()->get()->folder() : nullptr;
  const auto collapsedHasArchive =
      !_collapsedRows.empty() && (_collapsedRows.back()->folder != nullptr);
  const auto archiveIsCollapsed =
      (archive != nullptr) && session().settings().archiveCollapsed();
  const auto archiveIsInMainMenu =
      (archive != nullptr) && session().settings().archiveInMainMenu();
  return archiveIsInMainMenu  ? (collapsedHasArchive || !_skipTopDialog)
...
void InnerWidget::refresh(bool toTop) {
  ...
  refreshEmpty();
  if (_searchTags) {
    _searchTagsLeft = st::dialogsFilterSkip + st::dialogsFilterPadding.x();
    _searchTags->resizeToWidth(width() - 2 * _searchTagsLeft);
  }
  auto h = 0;
  if (_state == WidgetState::Default) {
...
void InnerWidget::refreshEmpty() {
  if (_state == WidgetState::Filtered) {
    const auto empty = _filterResults.empty() && _idSearchResults.empty() &&
                       _searchResults.empty() && _peerSearchResults.empty() &&
    ...
    if ((!_searchLoading && !_searchWaiting) || !empty) {
      ...
      _loadingAnimation = Ui::CreateLoadingDialogRowWidget(this, *_st, 2);
  ...
  const auto state =
      !_shownList->empty() ? EmptyState::None
      : _savedSublists     ? (_savedSublists->chatsList()->loaded()
                                  ? EmptyState::EmptySavedSublists
                                  : EmptyState::Loading)
      : _openedForum
          ? (_openedForum->topicsList()->loaded() ? EmptyState::EmptyForum
                                                  : EmptyState::Loading)
      : (!_filterId && data->contactsLoaded().current())
          ? EmptyState::NoContacts
      : (_filterId > 0) && data->chatsList()->loaded() ? EmptyState::EmptyFolder
                                                       : EmptyState::Loading;
  if (state == EmptyState::None) {
    ...
    const auto isEmptyVisible =
        (_state == WidgetState::Default) && !_openedFolder;
  ...
  _emptyState = state;
  auto phrase = (state == EmptyState::NoContacts)    ? tr::lng_no_chats()
                : (state == EmptyState::EmptyFolder) ? tr::lng_no_chats_filter()
                : (state == EmptyState::EmptyForum)  ? tr::lng_forum_no_topics()
                : (state == EmptyState::EmptySavedSublists)
                    ? tr::lng_no_saved_sublists()
                    : tr::lng_contacts_loading();
  auto link = (state == EmptyState::NoContacts) ? tr::lng_add_contact_button()
              : (state == EmptyState::EmptyFolder)
                  ? tr::lng_filters_context_edit()
              : (state == EmptyState::EmptyForum) ? tr::lng_forum_create_topic()
                                                  : rpl::single(QString());
  auto full = rpl::combine(std::move(phrase), std::move(link)) |
  ...
  if (state == EmptyState::NoContacts) {
    ...
    _emptyList->add(object_ptr<Ui::FlatLabel>(_emptyList,
    ...
    _emptyButton.create(this, tr::lng_no_conversations_button(),
    ...
    geometryValue() |
        rpl::on_next(
            [=](const QRect &r) {
              const auto top = r.height() - _emptyButton->height() -
        ...
        rpl::on_next(
            [=](const QRect &r) {
              ...
              _emptyList->moveToLeft(
                  0, ((r.height() - bottom) - _emptyList->height()) / 2);
            },
            _emptyList->lifetime());
...
void InnerWidget::clearMouseSelection(bool clearSelection) {
  ...
  if (clearSelection) {
    if (_state == WidgetState::Default) {
      ...
      _filteredSelected = _peerSearchSelected = _previewSelected =
          _searchedSelected = _hashtagSelected = -1;
    }
    setCursor(style::cur_default);
  }
}

WidgetState InnerWidget::state() const { return _state; }
...
auto InnerWidget::searchTagsChanges() const
  ...
  return _searchTags ? _searchTags->selectedChanges()
                     : rpl::never<std::vector<Data::ReactionId>>();
}

void InnerWidget::updateSearchIn() {
  if (!_searchState.inChat && _searchHashOrCashtag == HashOrCashtag::None) {
    _searchIn = nullptr;
    return;
  } else if (!_searchIn) {
    _searchIn = std::make_unique<ChatSearchIn>(this);
    _searchIn->show();
    _searchIn->changeFromRequests() |
        rpl::start_to_stream(_changeSearchFromRequests, _searchIn->lifetime());
    _searchIn->cancelFromRequests() |
        rpl::start_to_stream(_cancelSearchFromRequests, _searchIn->lifetime());
    _searchIn->cancelInRequests() |
        rpl::start_to_stream(_cancelSearchRequests, _searchIn->lifetime());
    _searchIn->tabChanges() |
        rpl::start_to_stream(_changeSearchTabRequests, _searchIn->lifetime());
  }
  ...
  const auto textFg = [] { return st::windowSubTextFg->c; };
  const auto topicIcon =
      !topic ? nullptr
      : topic->iconId()
          ? Ui::MakeEmojiThumbnail(
                &topic->owner(), Data::SerializeCustomEmojiId(topic->iconId()),
                paused, textFg)
          : Ui::MakeEmojiThumbnail(
                &topic->owner(),
                Data::TopicIconEmojiEntity({
                    .title = (topic->isGeneral() ? Data::ForumGeneralIconTitle()
                                                 : topic->title()),
                    .colorId =
                        (topic->isGeneral() ? Data::ForumGeneralIconColor(
                                                  st::windowSubTextFg->c)
                                            : topic->colorId()),
                }),
                paused, textFg);
  const auto peerIcon = peer ? Ui::MakeUserpicThumbnail(peer)
                        : sublist
  ...
  const auto fromImage =
      _searchFromShown ? Ui::MakeUserpicThumbnail(_searchFromShown) : nullptr;
  const auto fromName =
      _searchFromShown ? _searchFromShown->shortName() : QString();
  _searchIn->apply(
      {
...
void InnerWidget::repaintSearchResult(int index) {
  rtlupdate(0, searchedOffset() + index * _st->height, width(), _st->height);
}

void InnerWidget::repaintPreviewResult(int index) {
  rtlupdate(0, previewOffset() + index * _st->height, width(), _st->height);
}
...
void InnerWidget::setState(WidgetState state) { _state = state; }
  ...
  if (_state == WidgetState::Default) {
    ...
    if (_collapsedSelected < 0 && !_selected) {
      ...
      auto cur =
          (_collapsedSelected >= 0)
              ? _collapsedSelected
              : int(_collapsedRows.size() + (_shownList->cfind(_selected) -
                                             _shownList->cbegin() - skip));
      cur = std::clamp(cur + direction, 0,
                       static_cast<int>(_collapsedRows.size() +
                                        _shownList->size() - skip - 1));
      if (cur < _collapsedRows.size()) {
        ...
        _collapsedSelected = -1;
        _selected =
            *(_shownList->cbegin() + skip + cur - _collapsedRows.size());
      }
    }
    scrollToDefaultSelected();
  } else if (_state == WidgetState::Filtered) {
    if (_hashtagResults.empty() && _filterResults.empty() &&
        _idSearchResults.empty() && _peerSearchResults.empty() &&
        _previewResults.empty() && _searchResults.empty()) {
      return;
    }
    if ((_hashtagSelected < 0 || _hashtagSelected >= _hashtagResults.size()) &&
        (_filteredSelected < 0 || _filteredSelected >= _filterResults.size()) &&
        (_idSearchSelected < 0 ||
         _idSearchSelected >= _idSearchResults.size()) &&
        (_peerSearchSelected < 0 ||
         _peerSearchSelected >= _peerSearchResults.size()) &&
      ...
      int32 cur =
          base::in_range(_hashtagSelected, 0, _hashtagResults.size())
              ? _hashtagSelected
          ...
          : base::in_range(_idSearchSelected, 0, _idSearchResults.size())
              ? (_idSearchSelected + _filterResults.size() +
                 _hashtagResults.size())
          : base::in_range(_peerSearchSelected, 0, _peerSearchResults.size())
              ? (_peerSearchSelected + _idSearchResults.size() +
                 _filterResults.size() + _hashtagResults.size())
          : base::in_range(_previewSelected, 0, _previewResults.size())
              ? (_previewSelected + _peerSearchResults.size() +
                 _idSearchResults.size() + _filterResults.size() +
                 _hashtagResults.size())
              : (_searchedSelected + _previewResults.size() +
                 _peerSearchResults.size() + _idSearchResults.size() +
                 _filterResults.size() + _hashtagResults.size());
      cur = std::clamp(
          cur + direction, 0,
          static_cast<int>(_hashtagResults.size() + _filterResults.size() +
                           _idSearchResults.size() + _peerSearchResults.size() +
                           _previewResults.size() + _searchResults.size()) -
              1);
      if (cur < _hashtagResults.size()) {
        _hashtagSelected = cur;
        _filteredSelected = _idSearchSelected = _peerSearchSelected =
        ...
        _hashtagSelected = _idSearchSelected = _peerSearchSelected =
            _previewSelected = _searchedSelected = -1;
      } else if (cur < _hashtagResults.size() + _filterResults.size() +
                           _idSearchResults.size()) {
        _idSearchSelected =
            cur - _hashtagResults.size() - _filterResults.size();
        _hashtagSelected = _filteredSelected = _peerSearchSelected =
            _previewSelected = _searchedSelected = -1;
      } else if (cur < _hashtagResults.size() + _filterResults.size() +
                           _idSearchResults.size() +
                           _peerSearchResults.size()) {
        _peerSearchSelected = cur - _hashtagResults.size() -
                              _filterResults.size() - _idSearchResults.size();
        _hashtagSelected = _filteredSelected = _idSearchSelected =
            _previewSelected = _searchedSelected = -1;
      } else if (cur < _hashtagResults.size() + _filterResults.size() +
                           _idSearchResults.size() + _peerSearchResults.size() +
                           _previewResults.size()) {
        _previewSelected = cur - _hashtagResults.size() -
                           _filterResults.size() - _idSearchResults.size() -
                           _peerSearchResults.size();
        _hashtagSelected = _filteredSelected = _idSearchSelected =
            _peerSearchSelected = _searchedSelected = -1;
      } else {
        _searchedSelected = cur - _hashtagResults.size() -
                            _filterResults.size() - _idSearchResults.size() -
                            _peerSearchResults.size() - _previewResults.size();
        _hashtagSelected = _filteredSelected = _idSearchSelected =
            _peerSearchSelected = _previewSelected = -1;
      }
    }
    if (base::in_range(_hashtagSelected, 0, _hashtagResults.size())) {
      ...
      const auto from = previewOffset() + _previewSelected * _st->height +
                        (_previewSelected ? 0 : -st::searchedBarHeight);
      const auto height =
          _st->height + (_previewSelected ? 0 : st::searchedBarHeight);
      scrollToItem(from, height);
    } else {
      const auto from = searchedOffset() + _searchedSelected * _st->height +
...
void InnerWidget::preloadRowsData() {
  ...
  auto yTo =
      _visibleTop + (_visibleBottom - _visibleTop) * (PreloadHeightsCount + 1);
  if (_state == WidgetState::Default) {
    ...
    if (from < _filterResults.size()) {
      const auto to = std::min(((yTo - filteredOffset()) / _st->height) + 1,
                               int(_filterResults.size()));
      for (; from < to; ++from) {
    ...
    if (from < _peerSearchResults.size()) {
      const auto to =
          std::min(((yTo - peerSearchOffset()) / st::dialogsRowHeight) + 1,
    ...
    if (from < 0)
      from = 0;
    if (from < _previewResults.size()) {
      const auto to = std::min(((yTo - previewOffset()) / _st->height) + 1,
    ...
    if (from < 0)
      from = 0;
    if (from < _searchResults.size()) {
      const auto to = std::min(((yTo - searchedOffset()) / _st->height) + 1,
...
void InnerWidget::switchToFilter(FilterId filterId) {
  ...
  const auto &list = session().data().chatsFilters().list();
  const auto filterIt =
      filterId ? ranges::find(list, filterId, &Data::ChatFilter::id)
               : end(list);
...
void InnerWidget::jumpToTop() { _mustScrollTo.fire({0, -1}); }
...
QImage *InnerWidget::cacheChatsFilterTag(const Data::ChatFilter &filter,
                                         uint8 more, bool active) {
  if (!filter.id() && !more) {
    return nullptr;
  }
  ...
  if (_state == WidgetState::Default) {
    ...
    if (base::in_range(_filteredSelected, 0, _filterResults.size())) {
      ...
      return {
          ...
          .message = Data::UnreadMessagePosition,
          .sponsoredRandomId =
              (row->sponsored ? row->sponsored->data.randomId : QByteArray()),
      };
    } else if (base::in_range(_previewSelected, 0, _previewResults.size())) {
      const auto result = _previewResults[_previewSelected].get();
      const auto topic = result->topic();
      const auto item = result->item();
      return {.key = (topic ? (Entry *)topic : (Entry *)item->history()),
...
bool InnerWidget::isUserpicPress() const {
  return (_lastRowLocalMouseX >= 0) && (_lastRowLocalMouseX < _st->nameLeft) &&
         (_collapsedSelected < 0 ||
          _collapsedSelected >= _collapsedRows.size());
}
...
bool InnerWidget::chooseRow(Qt::KeyboardModifiers modifiers,
  ...
  if (chooseHashtag()) {
    ...
    for (const auto tab : {
      ...
      _menu->addAction(
          ChatTypeFilterLabel(tab),
          [=] { _changeSearchFilterRequests.fire_copy(tab); },
          (tab == _searchState.filter) ? &st::mediaPlayerMenuCheck : nullptr);
    }
    _menu->popup(QCursor::pos());
    return true;
  }
  const auto modifyChosenRow = [&](ChosenRow row,
                                   Qt::KeyboardModifiers modifiers) {
...
RowDescriptor
InnerWidget::chatListEntryBefore(const RowDescriptor &which) const {
  if (!which.key) {
    return RowDescriptor();
  }
  if (_state == WidgetState::Default) {
  ...
  if (!_searchResults.empty()) {
    for (auto b = _searchResults.cbegin(), i = b + 1, e = _searchResults.cend();
         i != e; ++i) {
      if (isSearchResultActive(i->get(), which)) {
        const auto j = i - 1;
        return RowDescriptor((*j)->item()->history(), (*j)->item()->fullId());
      }
    }
    if (isSearchResultActive(_searchResults[0].get(), which)) {
      if (_peerSearchResults.empty()) {
        ...
        return RowDescriptor(_filterResults.back().key(),
                             FullMsgId(PeerId(), ShowAtUnreadMsgId));
      }
      return RowDescriptor(
  ...
  if (const auto history = whichThread->asHistory()) {
    ...
    if (!_peerSearchResults.empty()) {
      for (auto b = _peerSearchResults.cbegin(), i = b + 1,
                e = _peerSearchResults.cend();
           i != e; ++i) {
        if ((*i)->peer == history->peer) {
          return RowDescriptor(session().data().history((*(i - 1))->peer),
                               FullMsgId(PeerId(), ShowAtUnreadMsgId));
        }
      }
    }
  ...
  for (auto b = _filterResults.cbegin(), i = b + 1, e = _filterResults.cend();
       i != e; ++i) {
    if (i->key() == which.key) {
      return RowDescriptor((i - 1)->key(),
                           FullMsgId(PeerId(), ShowAtUnreadMsgId));
...
RowDescriptor
InnerWidget::chatListEntryAfter(const RowDescriptor &which) const {
  if (!which.key) {
    return RowDescriptor();
  }
  if (_state == WidgetState::Default) {
  ...
  for (auto i = _searchResults.cbegin(), e = _searchResults.cend(); i != e;
       ++i) {
    if (isSearchResultActive(i->get(), which)) {
      if (++i != e) {
        return RowDescriptor((*i)->item()->history(), (*i)->item()->fullId());
      }
      return RowDescriptor();
    }
  }
  if (const auto history = whichThread->asHistory()) {
    for (auto i = _peerSearchResults.cbegin(), e = _peerSearchResults.cend();
         i != e; ++i) {
      if ((*i)->peer == history->peer) {
        ++i;
        if (i != e) {
          return RowDescriptor(session().data().history((*i)->peer),
                               FullMsgId(PeerId(), ShowAtUnreadMsgId));
        } else if (!_searchResults.empty()) {
          return RowDescriptor(_searchResults.front()->item()->history(),
  ...
  for (auto i = _filterResults.cbegin(), e = _filterResults.cend(); i != e;
       ++i) {
    if ((*i).key() == which.key) {
      ++i;
      if (i != e) {
...
RowDescriptor InnerWidget::chatListEntryFirst() const {
  if (_state == WidgetState::Default) {
    ...
    if (i != _shownList->cend()) {
      return RowDescriptor((*i)->key(), FullMsgId(PeerId(), ShowAtUnreadMsgId));
    }
    return RowDescriptor();
  } else if (!_filterResults.empty()) {
    return RowDescriptor(_filterResults.front().key(),
                         FullMsgId(PeerId(), ShowAtUnreadMsgId));
  } else if (!_peerSearchResults.empty()) {
    return RowDescriptor(
        session().data().history(_peerSearchResults.front()->peer),
...
RowDescriptor InnerWidget::chatListEntryLast() const {
  if (_state == WidgetState::Default) {
    ...
    return RowDescriptor(_searchResults.back()->item()->history(),
...
void InnerWidget::setupOnlineStatusCheck() {
  session().changes().peerUpdates(Data::PeerUpdate::Flag::OnlineStatus |
...
void InnerWidget::repaintDialogRowCornerStatus(not_null<History *> history) {
  ...
  const auto size =
      user ? st::dialogsOnlineBadgeSize : st::dialogsCallBadgeSize;
  const auto stroke = st::dialogsOnlineBadgeStroke;
  const auto updateRect =
      AyuUserpic::OnlineBadgeRect(_st->photoSize, size, stroke)
          .marginsAdded({stroke, stroke, stroke, stroke})
          .translated(st::defaultDialogRow.padding.left(),
                      st::defaultDialogRow.padding.top());
  const auto ttlUpdateRect =
      !history->peer->messagesTTL()
          ? QRect()
          : Dialogs::CornerBadgeTTLRect(_st->photoSize)
                .translated(st::defaultDialogRow.padding.left(),
                            st::defaultDialogRow.padding.top());
...
void InnerWidget::updateRowCornerStatusShown(not_null<History *> history) {
  const auto repaint = [=] { repaintDialogRowCornerStatus(history); };
  repaint();

  const auto findRow =
      [&](not_null<History *> history) -> std::pair<Row *, int> {
  ...
  if (const auto &[row, top] = findRow(history); row != nullptr) {
    const auto visible =
        (top < _visibleBottom) && (top + _st->height > _visibleTop);
    row->updateCornerBadgeShown(history->peer,
                                visible ? Fn<void()>(crl::guard(this, repaint))
                                        : nullptr);
  }
}
...
RowDescriptor InnerWidget::resolveChatNext(RowDescriptor from) const {
  ...
  return row.key ? computeJump(chatListEntryAfter(row), JumpSkip::NextOrEnd)
                 : row;
}
...
RowDescriptor InnerWidget::resolveChatPrevious(RowDescriptor from) const {
  const auto row = from.key ? from : _controller->activeChatEntryCurrent();
  return row.key
             ? computeJump(chatListEntryBefore(row), JumpSkip::PreviousOrBegin)
             : row;
}

void InnerWidget::setupShortcuts() {
  Shortcuts::Requests() | rpl::filter([=] {
    return isActiveWindow() && !_controller->isLayerShown() &&
           !_controller->window().locked() &&
           !_childListShown.current().shown && !_chatPreviewRow.key;
  }) |
      rpl::on_next(
          [=](not_null<Shortcuts::Request *> request) {
            ...
            const auto previous = row.key
                                      ? computeJump(chatListEntryBefore(row),
                                                    JumpSkip::PreviousOrBegin)
                                      : row;
            const auto next = row.key ? computeJump(chatListEntryAfter(row),
                                                    JumpSkip::NextOrEnd)
                                      : row;
            const auto first = [&] {
            ...
            if (row.key) {
              request->check(Command::ChatPrevious) &&
                  request->handle([=] { return jumpToDialogRow(previous); });
              request->check(Command::ChatNext) &&
                  request->handle([=] { return jumpToDialogRow(next); });
            } else if (_state == WidgetState::Default
                           ? !_shownList->empty()
                           : !_filterResults.empty()) {
              request->check(Command::ChatNext) &&
                  request->handle([=] { return jumpToDialogRow(first); });
            }
            request->check(Command::ChatFirst) &&
                request->handle([=] { return jumpToDialogRow(first); });
            request->check(Command::ChatLast) &&
                request->handle([=] { return jumpToDialogRow(last); });
            request->check(Command::ChatSelf) && request->handle([=] {
              _controller->showThread(
                  session().data().history(session().user()), ShowAtUnreadMsgId,
                  Window::SectionShow::Way::ClearStack);
              return true;
            });
            request->check(Command::ShowArchive) && request->handle([=] {
              const auto folder =
                  session().data().folderLoaded(Data::Folder::kId);
            ...
            if (session().data().chatsFilters().has()) {
              ...
              for (const auto &[command, index] : folders) {
                const auto select =
                    (command == Command::ShowFolderLast)
                        ? (filtersCount - 1)
            ...
            static const auto kPinned = {
                Command::ChatPinned1, Command::ChatPinned2,
                Command::ChatPinned3, Command::ChatPinned4,
                Command::ChatPinned5, Command::ChatPinned6,
                Command::ChatPinned7, Command::ChatPinned8,
            };
            auto &&pinned = ranges::views::zip(
                kPinned, ranges::views::ints(0, ranges::unreachable));
            ...
            (!_openedForum) && request->check(Command::ArchiveChat) &&
                request->handle([=] {
                  const auto thread = _selected ? _selected->thread() : nullptr;
                  if (!thread) {
                    return false;
                  ...
                  Window::ToggleHistoryArchived(_controller->uiShow(), history,
                                                !isArchived);
                  return true;
                });
            ...
            if (session().supportMode() && row.key.history()) {
              request->check(Command::SupportScrollToCurrent) &&
                  request->handle([=] {
                    scrollToEntry(row);
                    return true;
                  });
...
RowDescriptor InnerWidget::computeJump(const RowDescriptor &to,
                                       JumpSkip skip) const {
  auto result = to;
  if (result.key) {
    const auto down =
        (skip == JumpSkip::NextOrEnd) || (skip == JumpSkip::NextOrOriginal);
    const auto needSkip = [&] {
    ...
    while (needSkip()) {
      const auto next =
          down ? chatListEntryAfter(result) : chatListEntryBefore(result);
      if (next.key) {
        result = next;
      } else {
...
void InnerWidget::setSwipeContextData(
    int64 key, std::optional<Ui::Controls::SwipeContextData> data) {
  if (!key) {
    return;
  }
  if (!data) {
  ...
  if (context->data.msgBareId) {
    ...
    if (context->data.ratio > kStartAnimateThreshold) {
      if (context->icon && !context->icon->frameIndex() &&
          !context->icon->animating()) {
        context->icon->animate([=] { update(); }, 0,
...
void InnerWidget::prepareQuickAction(int64 key,
                                     Dialogs::Ui::QuickDialogAction action) {
  Expects(key != 0);

  const auto context = ensureQuickAction(key);
  auto name = ResolveQuickDialogLottieIconName(ResolveQuickDialogLabel(
      session().data().history(PeerId(key)), action, _filterId));
  context->icon = Lottie::MakeIcon({
      .name = std::move(name),
      .sizeOverride = Size(st::dialogsQuickActionSize),
  });
...
void InnerWidget::clearQuickActions() { _inactiveQuickActions.clear(); }

...
[[nodiscard]] QImage UpdateIcon() {
  ...
  auto result =
      QImage(Size(iconSize) * ratio, QImage::Format_ARGB32_Premultiplied);
  ...
  result.fill(Qt::transparent);
  {
    ...
    const auto rect =
        fullRect - Margins(st::dialogsInstallUpdateIconInnerMargin);
    p.setPen(Qt::NoPen);
    p.setBrush(Qt::white);
...
public:
  BottomButton(QWidget *parent, const QString &text,
               const style::FlatButton &st, const style::icon &icon,
               const style::icon &iconOver, bool hasTextIcon);
...
Widget::BottomButton::BottomButton(QWidget *parent, const QString &text,
                                   const style::FlatButton &st,
                                   const style::icon &icon,
                                   const style::icon &iconOver,
                                   bool hasTextIcon)
    : RippleButton(parent, st.ripple), _text(text), _st(st), _icon(icon),
      _iconOver(iconOver), _hasTextIcon(hasTextIcon) {
...
void Widget::BottomButton::onStateChanged(State was, StateChangeSource source) {
  ...
  if ((was & StateFlag::Disabled) != (state() & StateFlag::Disabled)) {
    _loading = isDisabled() ? std::make_unique<Ui::InfiniteRadialAnimation>(
...
void Widget::BottomButton::paintEvent(QPaintEvent *e) {
  ...
  if (width() >= st::columnMinimalWidthLeft) {
    ...
    if (_hasTextIcon) {
      ...
      const auto rect =
          QRect((width() - (iconSize.width() + textWidth + skip)) / 2, r.y(),
                textWidth, r.height());
      p.drawText(rect.translated(iconSize.width() + skip, 0), _text,
                 style::al_top);
    ...
    if (_hasTextIcon) {
      ...
      p.drawImage((width() - size.width()) / 2, (height() - size.height()) / 2,
                  _textIcon);
    } else {
...
Widget::Widget(QWidget *parent,
               not_null<Window::SessionController *> controller, Layout layout)
  ...
  _scroll->setOverscrollTypes(_stories ? OverscrollType::Virtual
                                       : OverscrollType::Real,
                              OverscrollType::Real);
  _innerList = _scroll->setOwnedWidget(object_ptr<Ui::VerticalLayout>(this));
  _inner = _innerList->add(object_ptr<InnerWidget>(
      _innerList, controller,
      rpl::combine(_childListPeerId.value(), _childListShown.value(),
                   makeChildListShown)));
  rpl::combine(_scroll->heightValue(),
      ...
      rpl::on_next(
          ...
          _innerList->lifetime());
  _scroll->widthValue() |
      rpl::on_next([=](int width) { _innerList->resizeToWidth(width); },
                   _innerList->lifetime());
  _scrollToTop->raise();
  ...
  rpl::combine(session().api().dialogsLoadMayBlockByDate(),
               session().api().dialogsLoadBlockedByDate()) |
      rpl::on_next(
          [=](bool mayBlock, bool isBlocked) {
  ...
  session().changes().historyUpdates(Data::HistoryUpdate::Flag::MessageSent) |
      ...
      rpl::on_next([=](const Data::HistoryUpdate &update) { jumpToTop(true); },
                   lifetime());
  ...
  _inner->mustScrollTo() | rpl::on_next(
                               ...
                               lifetime());
  _inner->dialogMoved() | rpl::on_next(
                              [=](const Ui::ScrollToRequest &data) {
  ...
  _inner->searchRequests() |
      rpl::on_next([=](SearchRequestDelay delay) { searchRequested(delay); },
                   lifetime());
  _inner->completeHashtagRequests() |
      rpl::on_next([=](const QString &tag) { completeHashtag(tag); },
                   lifetime());
  _inner->refreshHashtagsRequests() |
      rpl::on_next([=] { searchCursorMoved(); }, lifetime());
  _inner->changeSearchTabRequests() |
      rpl::filter([=](ChatSearchTab tab) { return _searchState.tab != tab; }) |
      rpl::on_next(
          [=](ChatSearchTab tab) {
  ...
  _inner->changeSearchFilterRequests() |
      ...
      rpl::on_next(
          ...
          lifetime());
  _inner->cancelSearchRequests() | rpl::on_next(
                                       [=] {
                                         cancelSearch({
  ...
  _inner->changeSearchFromRequests() |
      rpl::on_next([=] { showSearchFrom(); }, lifetime());
  _inner->chosenRow() |
      rpl::on_next([=](const ChosenRow &row) { chosenRow(row); }, lifetime());
  ...
  _scroll->geometryChanged() |
      rpl::on_next(crl::guard(_inner, [=] { _inner->parentGeometryChanged(); }),
                   lifetime());
  _scroll->scrolls() | rpl::on_next([=] { listScrollUpdated(); }, lifetime());

  session().data().chatsListChanges() | rpl::filter([=](Data::Folder *folder) {
  ...
  controller->widget()->imeCompositionStarts() |
      rpl::filter([=] { return redirectImeToSearch(); }) |
      rpl::on_next([=] { _search->setFocusFast(); }, lifetime());

  _search->changes() |
      rpl::on_next([=] { crl::on_main(this, [=] { applySearchUpdate(); }); },
                   _search->lifetime());
  ...
  QObject::connect(
      _search->rawTextEdit().get(), &QTextEdit::cursorPositionChanged, this,
      [=] { searchCursorMoved(); },
      Qt::QueuedConnection); // So getLastText() works already.

  if (!Core::UpdaterDisabled()) {
    Core::UpdateChecker checker;
    rpl::merge(rpl::single(rpl::empty), checker.isLatest(), checker.failed(),
               checker.ready()) |
  ...
  rpl::single(rpl::empty) |
      rpl::then(session().domain().local().localPasscodeChanged()) |
  ...
  lockUnlock->setClickedCallback([=] {
    lockUnlock->setIconOverride(&st::dialogsUnlockIcon,
  ...
  _inner->setLoadMoreFilteredCallback([=] {
    ...
    if (state == WidgetState::Filtered && !_topicSearchFull &&
  ...
  _inner->setLoadMoreCallback([=] {
    ...
    if (state == WidgetState::Filtered &&
        (!process->full || (_searchInMigrated && !_migratedProcess.full))) {
      searchMore();
  ...
  _inner->listBottomReached() |
      rpl::on_next([=] { loadMoreBlockedByDate(); }, lifetime());
  ...
  const auto overscrollBg = [=] {
    return anim::color(st::dialogsBg, st::dialogsBgOver,
                       _childListShown.current());
  };
  _scroll->setOverscrollBg(overscrollBg());
  style::PaletteChanged() |
      rpl::on_next([=] { _scroll->setOverscrollBg(overscrollBg()); },
                   lifetime());

  if (_layout != Layout::Child) {
    setupConnectingWidget();

    changeOpenedFolder(controller->openedFolder().current(),
                       anim::type::instant);
    ...
    _childListShown.changes() | rpl::on_next(
                                    [=] {
                                      _scroll->setOverscrollBg(overscrollBg());
      ...
      if (data.translation < 0 && _inner &&
        ...
        if (!_swipeBackData.callback) {
          _swipeBackData = Ui::Controls::SetupSwipeBack(
              ...
              _swipeBackMirrored, _swipeBackIconMirrored);
        }
              ...
              .callback =
                  [=, session = &session()] {
                    auto callback = [=, peerId = PeerId(key)] {
                      PerformQuickDialogAction(controller(),
                                               session->data().peer(peerId),
                                               action, _inner->filterId());
                    };
                    base::call_delayed(st::slideWrapDuration, session,
                                       std::move(callback));
                  },
              .msgBareId = key,
              .speedRatio = 1.,
...
void Widget::chosenRow(const ChosenRow &row) {
  ...
  const auto topicJump =
      history ? history->peer->forumTopicFor(row.topicJumpRootId) : nullptr;
  const auto sublistJump =
      history ? history->peer->monoforumSublistFor(row.sublistJumpPeerId)
              : nullptr;
  ...
  if (topicJump) {
    if (controller()->shownForum().current() == topicJump->forum()) {
      ...
      if (!controller()->adaptive().isOneColumn() &&
        ...
        controller()->showForum(topicJump->forum(),
                                Window::SectionShow().withChildColumn());
      }
      controller()->showThread(topicJump, ShowAtUnreadMsgId,
    ...
    if (row.newWindow) {
      ...
      controller()->showThread(sublistJump, ShowAtUnreadMsgId,
                               Window::SectionShow::Way::ClearStack);
    }
    return;
    ...
    if (controller()->shownForum().current() == forum) {
      ...
      if (controller()->shownForum().current() == forum &&
        ...
        controller()->showThread(history, ShowAtUnreadMsgId,
                                 Window::SectionShow::Way::ClearStack);
      }
    }
    return;
    ...
    if (row.newWindow) {
      ...
      if (const auto active = monoforum->activeSubsectionThread()) {
        controller()->showThread(active, ShowAtUnreadMsgId,
                                 Window::SectionShow::Way::ClearStack);
      } else {
    ...
    const auto showAtMsgId = controller()->uniqueChatsInSearchResults(_searchState)
                                 ? ShowAtUnreadMsgId
                                 : row.message.fullId.msg;
    auto params = Window::SectionShow(Window::SectionShow::Way::ClearStack);
    params.highlight = Window::SearchHighlightId(_searchState.query);
    ...
    if (row.newWindow) {
      controller()->showInNewWindow(
          Window::SeparateId(Window::SeparateType::Archive, &session()));
      return;
    }
  ...
  void Widget::setupFrozenAccountBar() {
    session().frozenValue() | rpl::on_next(
                                  [=] {
  ...
  void Widget::setupTopBarSuggestions() {
    ...
    crl::on_main(_innerList, [=] {
      ...
      (owner->chatsListLoaded(nullptr) ? rpl::single<Data::Folder *>(nullptr)
              ...
              [=](Ui::SlideWrap<Ui::RpWidget> *raw) {
                if (raw) {
                  _topBarSuggestion = _innerList->insert(
                      0, object_ptr<Ui::SlideWrap<Ui::RpWidget>>::fromRaw(raw));
                  ...
                  rpl::combine(
                      ...
                      rpl::on_next(
                          [=](int desiredHeight, float64 shown) {
                            ...
                            _topBarSuggestion->entity()->setMaximumHeight(
                                newHeight);
                            _topBarSuggestion->entity()->setMinimumWidth(
                                (shown > 0) ? width() : 0);
                            _topBarSuggestion->entity()->resize(width(),
                                                                newHeight);
                          },
  ...
  void Widget::updateFrozenAccountBar() {
    if (_layout == Layout::Child || _openedForum || _openedFolder ||
        !session().frozen()) {
      _frozenAccountBar = nullptr;
    } else if (!_frozenAccountBar) {
      _frozenAccountBar =
          FrozenWriteRestriction(this, controller()->uiShow(),
        ...
        rpl::on_next(
            [=](FilterId id) {
              ...
              _moreChatsBar = std::make_unique<Ui::MoreChatsBar>(
                  this, filters->moreChatsContent(id));
              ...
              _moreChatsBar->heightValue() |
                  rpl::on_next([=] { updateControlsGeometry(); },
                               _moreChatsBar->lifetime());
            },
            lifetime());
  }
            ...
            [=](Ui::DownloadBarContent &&content) {
              ...
              if (create) {
                _downloadBar = std::make_unique<Ui::DownloadBar>(
                    this, Data::MakeDownloadBarProgress());
              }
              if (_downloadBar) {
                        ...
                        [=] {
                          ...
                          for (const auto id : list) {
                            if (!first) {
                              ...
                              controller()->showSection(Info::Downloads::Make(
  ...
  void Widget::setupShortcuts(
    ...
    Shortcuts::Requests() | rpl::filter([=] {
      return isActiveWindow() && !controller->isLayerShown() &&
  ...
  void Widget::startScrollUpButtonAnimation(bool shown) {
    const auto smallColumn =
        (width() < st::columnMinimalWidthLeft) || _childList;
    shown &= !smallColumn;
  ...
  void Widget::updateScrollUpPosition() {
    ...
    auto top = anim::interpolate(
        0, _scrollToTop->height() + st::connectingMargin.top(),
        _scrollToTopShown.value(_scrollToTopIsShown ? 1. : 0.));
    _scrollToTop->moveToRight(st::historyToDownPosition.x(),
                              _scroll->height() - top);
  ...
  void Widget::setupStories() {
    ...
    _stories->verticalScrollEvents() |
        rpl::on_next(
            [=](not_null<QWheelEvent *> e) { _scroll->viewportEvent(e); },
            _stories->lifetime());

    _storiesContents.fire(Stories::ContentForSession(
        &controller()->session(), Data::StorySourcesList::NotHidden));
    ...
    rpl::combine(_scroll->positionValue(), _scroll->movementValue(),
                 _storiesExplicitExpandValue.value()) |
        rpl::on_next(
            [=](Ui::ElasticScrollPosition position,
                Ui::ElasticScrollMovement movement, int explicitlyExpanded) {
              if (_stories->isHidden()) {
                return;
              }
              const auto overscrollTop = std::max(-position.overscroll, 0);
              if (overscrollTop > 0 && _storiesExplicitExpand) {
                _scroll->setOverscrollDefaults(-st::dialogsStoriesFull.height,
                                               0, true);
              }
              if (explicitlyExpanded > 0 &&
                  explicitlyExpanded < overscrollTop) {
              ...
              _stories->setExpandedHeight(_aboveScrollAdded,
                                          ((movement == Phase::Momentum ||
                                            movement == Phase::Returning) &&
                                           (explicitlyExpanded < above)));
              if (position.overscroll > 0 ||
                  (position.value >
                   (_storiesExplicitExpandScrollTop + st::dialogsRowHeight))) {
                storiesToggleExplicitExpand(false);
              }
    ...
    _stories->collapsedGeometryChanged() |
        rpl::on_next([=] { updateLockUnlockPosition(); }, lifetime());
    ...
    _stories->showMenuRequests() |
        rpl::on_next(
            [=](const Stories::ShowMenuRequest &request) {
    ...
    _stories->emptyValue() | rpl::skip(1) |
        rpl::on_next([=] { updateStoriesVisibility(); }, lifetime());
  ...
  void Widget::storiesToggleExplicitExpand(bool expand) {
    ...
    _storiesExplicitExpandAnimation.start(
        [=](float64 value) {
  ...
  void Widget::setupShortcuts() {
    Shortcuts::Requests() | rpl::filter([=] {
      return isActiveWindow() && Ui::InFocusChain(this) && !_childList &&
             !controller()->isLayerShown() && !controller()->window().locked();
    }) |
        rpl::on_next(
            [=](not_null<Shortcuts::Request *> request) {
              using Command = Shortcuts::Command;
              ...
              if (!controller()->activeChatCurrent()) {
                request->check(Command::Search) && request->handle([=] {
                  if (const auto forum = _openedForum) {
                    ...
                    controller()->searchInChat(history);
                    return true;
                  } else if (!_openedFolder && !_childList &&
                             _search->isVisible()) {
                    _search->setFocus();
                    return true;
                  }
  ...
  void Widget::updateControlsVisibility(bool fast) {
    ...
    if (_openedFolder || _openedForum) {
      ...
      updateLockUnlockVisibility(fast ? anim::type::instant
                                      : anim::type::normal);
  ...
  void Widget::toggleFiltersMenu(bool enabled) {
    ...
    if (!enabled == !_chatFilters) {
      ...
      const auto inner = Ui::AddChatFiltersTabsStrip(
          ...
          Window::GifPauseReason::Any, controller(), true);
      ...
      inner->sizeValue() |
          rpl::on_next(
              [=, this](const QSize &s) {
                raw->resize(s);
                shadow->setGeometry(0, s.height() - shadow->height(), s.width(),
                                    shadow->height());
                updateControlsGeometry();
              },
  ...
  bool Widget::cancelSearchByMouseBack() {
    return _searchHasFocus && !_searchSuggestionsLocked &&
           !_searchState.inChat &&
           cancelSearch({.jumpBackToSearchedChat = true});
  }
  ...
  void Widget::updateSuggestions(anim::type animated) {
    ...
    if (!suggest && _suggestions) {
      if (animated == anim::type::normal) {
        ...
        _suggestions->hide(animated, [=, raw = _suggestions.get()] {
          ...
          _hidingSuggestions.erase(
              ranges::remove(_hidingSuggestions, raw,
      ...
      _suggestions = std::make_unique<Suggestions>(
          this, controller(), TopPeersContent(&session()),
          RecentPeersContent(&session()));
      _suggestions->clearSearchQueryRequests() |
          rpl::on_next([=] { setSearchQuery(QString()); },
      ...
      rpl::merge(_suggestions->openBotMainAppRequests(),
      ...
      _suggestions->popularAppChosen() | rpl::on_next(
                                             [=](not_null<PeerData *> peer) {
                                               controller()->showPeerInfo(peer);
                                             },
                                             _suggestions->lifetime());

      _suggestions->closeRequests() |
          rpl::on_next([=] { closeSuggestions(); }, _suggestions->lifetime());
  ...
  void Widget::changeOpenedSubsection(FnMut<void()> change, bool fromRight,
                                      anim::type animated) {
    if (isHidden()) {
      animated = anim::type::instant;
    }
    auto oldContentCache = QPixmap();
    const auto showDirection = fromRight ? Window::SlideDirection::FromRight
  ...
  void Widget::storiesExplicitCollapse() {
    if (_storiesExplicitExpand) {
      ...
      _scroll->setOverscrollTypes(
          _stories->isHidden() ? Type::Real : Type::Virtual, Type::Real);
    }
    _storiesExplicitExpandAnimation.stop();
    _storiesExplicitExpandValue = 0;
    ...
    _storiesContents.fire(Stories::ContentForSession(
        &session(), _openedFolder ? List::Hidden : List::NotHidden));
  }
  ...
  void Widget::changeOpenedForum(Data::Forum * forum, anim::type animated) {
    ...
    changeOpenedSubsection(
        [&] {
          ...
          _openedForum = forum;
          _searchState.tab =
              forum ? ChatSearchTab::ThisPeer : ChatSearchTab::MyMessages;
  ...
  void Widget::refreshTopBars() {
    if (_openedFolder || _openedForum) {
      if (!_subsectionTopBar) {
        ...
        _subsectionTopBar->searchCancelled() |
            rpl::on_next([=] { escape(); }, _subsectionTopBar->lifetime());
        _subsectionTopBar->searchSubmitted() |
            rpl::on_next([=] { submit(); }, _subsectionTopBar->lifetime());
        _subsectionTopBar->searchQuery() |
            rpl::on_next([=](QString query) { applySearchUpdate(); },
                         _subsectionTopBar->lifetime());
        _subsectionTopBar->jumpToDateRequest() |
            rpl::on_next([=] { showCalendar(); },
                         _subsectionTopBar->lifetime());
        _subsectionTopBar->chooseFromUserRequest() |
            rpl::on_next([=] { showSearchFrom(); },
                         _subsectionTopBar->lifetime());
        updateControlsGeometry();
      }
      const auto history =
          _openedForum ? _openedForum->history().get() : nullptr;
      _subsectionTopBar->setActiveChat(
          HistoryView::TopBarWidget::ActiveChat{
              .key = (_openedForum ? Dialogs::Key(history)
    ...
    if (_openedForum && _openedForum->peer()->isChannel()) {
      ...
      _forumReportBar = std::make_unique<HistoryView::ContactStatus>(
          controller(), this, peer, true);
      _forumRequestsBar = std::make_unique<Ui::RequestsBar>(
          this, HistoryView::RequestsBarContentByPeer(
                    peer, st::historyRequestsUserpics.size, true));
      _forumGroupCallBar = std::make_unique<Ui::GroupCallBar>(
          this,
          HistoryView::GroupCallBarContentByPeer(
              peer, st::historyGroupCallUserpics.size, true),
      ...
      rpl::merge(_forumGroupCallBar->barClicks(),
      ...
      rpl::combine(_forumGroupCallBar->heightValue(),
  ...
  void Widget::checkUpdateStatus() {
    ...
    if (Checker().state() == Checker::State::Ready) {
      ...
      _updateTelegram.create(this, tr::ayu_UpdateAyuGram(tr::now),
                             st::dialogsUpdateButton, st::dialogsInstallUpdate,
                             st::dialogsInstallUpdateOver, true);
      _updateTelegram->show();
  ...
  void Widget::setInnerFocus(bool unfocusSearch) {
    if (_childList) {
      ...
      return;
               ...
               (!_search->getLastText().isEmpty() || _searchState.inChat ||
  ...
  void Widget::scrollToDefault(bool verytop) {
    ...
    _scrollToAnimation.start(scroll, scrollTop, scrollTo, st::slideDuration,
                             anim::sineInOut);
  }
  ...
  [[nodiscard]] QPixmap Widget::grabNonNarrowScrollFrame() {
    ...
    auto wideGeometry =
        QRect(scrollGeometry.x(), scrollGeometry.y(),
    ...
    const auto grabSize =
        QSize(wideGeometry.width(), skip + wideGeometry.height());
    auto image = QImage(grabSize * style::DevicePixelRatio(),
                        QImage::Format_ARGB32_Premultiplied);
    image.setDevicePixelRatio(style::DevicePixelRatio());
    image.fill(Qt::transparent);
    {
      QPainter p(&image);
      Ui::RenderWidget(p, this, QPoint(),
                       QRect(0, top, wideGeometry.width(), skip));
      if (_chatFilters) {
        Ui::RenderWidget(p, _chatFilters,
  ...
  void Widget::updateStoriesVisibility() {
    ...
    const auto suggestionsAnimation =
        widthAnimation && (!_suggestions || !_hidingSuggestions.empty());
    const auto hiddenInstant =
        _showAnimation || _openedForum ||
        (widthAnimation && !suggestionsAnimation) || _childList ||
        _stories->empty() ||
        (_scroll->position().overscroll < -st::dialogsFilterSkip);
    const auto hiddenAnimated = _searchHasFocus || _searchSuggestionsLocked ||
                                !_searchState.query.isEmpty() ||
  ...
  void Widget::showAnimated(Window::SlideDirection direction,
    ...
    startSlideAnimation(std::move(oldContentCache), std::move(newContentCache),
                        direction);
  }

  void Widget::startSlideAnimation(QPixmap oldContentCache,
  ...
  void Widget::submit() {
    ...
    if (state == WidgetState::Default ||
        (state == WidgetState::Filtered && _inner->hasFilteredResults())) {
      _inner->selectSkip(1);
      _inner->chooseRow();
    } else {
      search();
  ...
  void Widget::refreshLoadMoreButton(bool mayBlock, bool isBlocked) {
    ...
    if (!_loadMoreChats) {
      _loadMoreChats.create(this, "Load more", st::dialogsLoadMoreButton,
                            st::dialogsLoadMore, st::dialogsLoadMore, false);
      _loadMoreChats->show();
  ...
  bool Widget::search(bool inCache, SearchRequestDelay delay) {
    ...
    if (trimmed.isEmpty() && !fromPeer && inTags.empty()) {
      ...
      const auto success =
          _singleMessageSearch.lookup(query, [=] { searchRequested(delay); });
      if (!success) {
        return false;
      }
      const auto process = currentSearchProcess();
    ...
    } else if (_searchQuery != query || _searchQueryFrom != fromPeer ||
               _searchQueryTags != inTags || _searchQueryTab != tab ||
               _searchQueryFilter != filter) {
      const auto process = currentSearchProcess();
      _searchQuery = query;
      _searchQueryFrom = fromPeer;
      ...
      if (inPeer) {
        ...
        const auto sublist =
            _openedForum ? nullptr : _searchState.inChat.sublist();
    ...
    if (peerSearchRequired()) {
      const auto requestType =
          inCache ? Api::PeerSearch::RequestType::CacheOnly
                  : Api::PeerSearch::RequestType::CacheOrRemote;
      _peerSearch.request(
          query,
          [=](Api::PeerSearchResult result) { peerSearchReceived(result); },
          requestType);
    } else {
      _peerSearch.clear();
      peerSearchReceived({});
    ...
    const auto shouldIdSearch =
        isGlobalSearch && (idQuery.type != IdSearchType::None);
  ...
  void Widget::searchTopics() {
    ...
    _topicSearchRequest =
        _api
            .request(MTPmessages_GetForumTopics(
                MTP_flags(MTPmessages_GetForumTopics::Flag::f_q),
                _openedForum->peer()->input(), MTP_string(_topicSearchQuery),
                MTP_int(_topicSearchOffsetDate), MTP_int(_topicSearchOffsetId),
                MTP_int(_topicSearchOffsetTopicId), MTP_int(kSearchPerPage)))
            .done([=](const MTPmessages_ForumTopics &result) {
              ...
              _openedForum->applyReceivedTopics(
                  result, [&](not_null<Data::ForumTopic *> topic) {
                      ...
                      .request(MTPmessages_Search(
                          MTP_flags(
                              ...
                              (_searchQueryTags.empty()
                                   ? Flag()
                                   : Flag::f_saved_reaction)),
                          peer->input(), MTP_string(_searchQuery),
                          (fromPeer ? fromPeer->input() : MTP_inputPeerEmpty()),
                          (savedPeer ? savedPeer->input()
                                     : MTP_inputPeerEmpty()),
                          MTP_vector_from_range(
          ...
          histories.sendRequest(history, type, [=](Fn<void()> finish) {
            ...
            _migratedProcess.requestId =
                session()
                    .api()
                    .request(MTPmessages_Search(
                        flags, _searchInMigrated->peer->input(),
                        MTP_string(_searchQuery),
                        (_searchQueryFrom ? _searchQueryFrom->input()
                                          : MTP_inputPeerEmpty()),
  ...
  void Widget::requestMessages(bool fromStart) {
    ...
    const auto flags =
        Flag() |
        (session().settings().skipArchiveInSearch() ? Flag::f_folder_id
                                                    : Flag()) |
    ...
    _searchProcess.requestId =
        session()
            ...
            .request(MTPmessages_SearchGlobal(
                MTP_flags(flags), MTP_int(folderId), MTP_string(_searchQuery),
                MTP_inputMessagesFilterEmpty(),
                MTP_int(0), // min_date
                MTP_int(0), // max_date
  ...
  auto Widget::currentSearchProcess() -> not_null<SearchProcessState *> {
    return (_searchState.tab == ChatSearchTab::PublicPosts) ? &_postsProcess
                                                            : &_searchProcess;
  }
  ...
  void Widget::searchReceived(
      SearchRequestType type, const MTPmessages_Messages &result,
      not_null<SearchProcessState *> process, bool cacheResults) {
    const auto state = _inner->state();
    ...
    auto messages = result.match(
        ...
        [&](const MTPDmessages_messagesSlice &data) {
          ...
          const auto rateUpdated =
              nextRate && (nextRate->v != process->nextRate);
  ...
  void Widget::searchApplyEmpty(SearchRequestType type,
                                not_null<SearchProcessState *> process) {
    process->full = true;
    searchReceived(type,
                   MTP_messages_messages(
                       MTP_vector<MTPMessage>(), MTP_vector<MTPForumTopic>(),
                       MTP_vector<MTPChat>(), MTP_vector<MTPUser>()),
                   process);
  }

  void Widget::searchFailed(SearchRequestType type, const MTP::Error &error,
                            not_null<SearchProcessState *> process) {
  ...
  void Widget::dropEvent(QDropEvent * e) {
    ...
    if (_scroll->geometry().contains(e->pos())) {
      ...
      const auto thread =
          _suggestions ? _suggestions->updateFromParentDrag(globalPosition)
  ...
  void Widget::updateCancelSearch() {
    const auto shown =
        !_searchState.query.isEmpty() ||
        (!_searchState.inChat && (_searchHasFocus || _searchSuggestionsLocked));
  ...
  QString Widget::validateSearchQuery() {
    ...
    if (!_subsectionTopBar && _suggestions &&
      ...
      const auto fixed = FixHashtagSearchQuery(
          query, currentSearchQueryCursorPosition(), _searchHashOrCashtag);
      if (fixed.text != query) {
  ...
  void Widget::applySearchUpdate() {
    ...
    if (_chooseFromUser->toggled() || _searchState.fromPeer ||
  ...
  void Widget::updateForceDisplayWide() {
    ...
    controller()->setChatsForceDisplayWide(
        ...
        _searchSuggestionsLocked || !_searchState.query.isEmpty() ||
        _searchState.inChat ||
        !session().api().authorizations().unreviewed().empty());
  }

  void Widget::showForum(not_null<Data::Forum *> forum,
    ...
    if (!params.childColumn ||
        ...
        (_layout != Layout::Main) || OptionForumHideChatsList.value()) {
  ...
  void Widget::openChildList(not_null<Data::Forum *> forum,
                             const Window::SectionShow &params) {
    auto slide = Window::SectionSlideParams();
    const auto animated =
        !_childList && (params.animated == anim::type::normal);
    if (animated) {
      destroyChildListCanvas();
      slide.oldContentCache = Ui::GrabWidget(
          this, QRect(_narrowWidth, 0, width() - _narrowWidth, height()));
    }
    auto copy = params;
  ...
  void Widget::closeChildList(anim::type animated) {
    ...
    if (animated == anim::type::normal) {
      ...
      animation =
          _hideChildListCanvas->lifetime().make_state<Window::SlideAnimation>();
  ...
  bool Widget::applySearchState(SearchState state) {
    ...
    if (state.tab == ChatSearchTab::PublicPosts &&
      ...
      state.tab = (_openedForum && !state.inChat) ? ChatSearchTab::ThisPeer
                                                  : ChatSearchTab::MyMessages;
    } else if (!state.inChat && _searchHashOrCashtag == HashOrCashtag::None) {
      state.tab = (forum || _openedForum) ? ChatSearchTab::ThisPeer
                                          : ChatSearchTab::MyMessages;
    }
    if (!state.tags.empty()) {
    ...
    const auto queryEmptyChanged =
        queryChanged ? (_searchState.query.isEmpty() != state.query.isEmpty())
                     : false;
    ...
    if ((state.tab == ChatSearchTab::ThisTopic && !state.inChat.topic()) ||
        (state.tab == ChatSearchTab::ThisPeer && !state.inChat &&
         !_openedForum) ||
    ...
    const auto migrateFrom =
        (peer && !topic && state.tab == ChatSearchTab::ThisPeer)
            ? peer->migrateFrom()
            : nullptr;
    _searchInMigrated =
        migrateFrom ? peer->owner().history(migrateFrom).get() : nullptr;
    _searchState = state;
    if (_chatFilters && (queryEmptyChanged || inChatChanged)) {
      _chatFilters->setVisible(_searchState.query.isEmpty() && !_openedForum &&
                               !searchInPeer());
      updateControlsGeometry();
    }
    ...
    const auto searchCleared =
        state.query.isEmpty() && !state.fromPeer && state.tags.empty();
    if (searchCleared || inChatChanged || fromPeerChanged || filterChanged ||
        tagsChanged || tabChanged) {
    ...
    if (_subsectionTopBar) {
      _subsectionTopBar->searchEnableJumpToDate(_openedForum &&
                                                _searchState.inChat);
    }
  ...
  void Widget::completeHashtag(QString tag) {
    ...
    for (int start = cur; start > 0;) {
      ...
      if (t.size() <= start) {
        ...
        if (cur == start + 1 ||
          ...
          if (cur - start - 1 == tag.size() && cur < t.size() &&
              t.at(cur) == ' ') {
            ++cur;
          }
    ...
    setSearchQuery(t.mid(0, cur) + '#' + tag + ' ' + t.mid(cur),
                   cur + 1 + tag.size() + 1);
    applySearchUpdate();
  }
  ...
  void Widget::updateSearchFromVisibility(bool fast) {
    ...
    _chooseFromUser->toggle(visible,
                            fast ? anim::type::instant : anim::type::normal);
    if (_subsectionTopBar) {
  ...
  void Widget::updateControlsGeometry() {
    ...
    const auto ratiow =
        anim::interpolate(width(), _narrowWidth, _childListShown.current());
    const auto smallw = st::columnMinimalWidthLeft - _narrowWidth;
    const auto narrowRatio =
        (ratiow < smallw) ? ((smallw - ratiow) / float64(smallw - _narrowWidth))
                          : 0.;
    ...
    if (_subsectionTopBar) {
      _subsectionTopBar->setGeometryWithNarrowRatio(_searchControls->geometry(),
                                                    _narrowWidth, narrowRatio);
    }

    auto filterTop = (filterAreaHeight - _search->height()) / 2;
    filterLeft = anim::interpolate(filterLeft, _narrowWidth, narrowRatio);
    _search->setGeometryToLeft(filterLeft, filterTop, filterWidth,
                               _search->height());

    auto mainMenuLeft = anim::interpolate(
        st::dialogsFilterPadding.x(),
        (_narrowWidth - _mainMenu.toggle->width()) / 2, narrowRatio);
    _mainMenu.toggle->moveToLeft(mainMenuLeft, st::dialogsFilterPadding.y());
    _mainMenu.under->setGeometry(0, 0, filterLeft,
                                 _mainMenu.toggle->y() +
                                     _mainMenu.toggle->height() +
                                     st::dialogsFilterPadding.y());
    const auto searchLeft = anim::interpolate(
        -_searchForNarrowLayout->width(),
        (_narrowWidth - _searchForNarrowLayout->width()) / 2, narrowRatio);
    _searchForNarrowLayout->moveToLeft(searchLeft,
                                       st::dialogsFilterPadding.y());
    ...
    const auto storiesHeight =
        2 * st::dialogsStories.photoTop + st::dialogsStories.photo;
    const auto added = (st::dialogsFilter.heightMin - storiesHeight) / 2;
    if (_stories) {
      _stories->setLayoutConstraints(
          {filterLeft + filterWidth, filterTop + added}, style::al_right,
    ...
    _updateScrollGeometryCached = [=] {
      const auto frozenBarTop =
          expandedStoriesTop +
          ((!_stories || _stories->isHidden()) ? 0 : _aboveScrollAdded);
      if (_frozenAccountBar) {
        _frozenAccountBar->move(0, frozenBarTop);
      }
      const auto moreChatsBarTop =
          frozenBarTop + (_frozenAccountBar ? _frozenAccountBar->height() : 0);
      if (_moreChatsBar) {
        _moreChatsBar->move(0, moreChatsBarTop);
      }
      const auto forumGroupCallTop =
          moreChatsBarTop + (_moreChatsBar ? _moreChatsBar->height() : 0);
      if (_forumGroupCallBar) {
      ...
      const auto scrollTop =
          chatFiltersTop + ((_chatFilters && _searchState.query.isEmpty() &&
    ...
    if (_suggestions) {
      _suggestions->setGeometry(0, expandedStoriesTop, scrollWidth,
    ...
    if (_childList) {
      ...
      _childList->setGeometryWithTopMoved({childx, 0, childw, childh},
                                          _topDelta);
  ...
  void Widget::keyPressEvent(QKeyEvent * e) {
    if (e->key() == Qt::Key_Escape) {
      ...
      // }
               ...
               _searchHasFocus && !_searchState.inChat &&
               _searchState.query.isEmpty()) {
      escape();
  ...
  bool Widget::redirectToSearchPossible() const {
    return !_openedFolder && !_openedForum && !_childList &&
           _search->isVisible() && !_search->hasFocus() && hasFocus();
  }
  ...
  bool Widget::redirectKeyToSearch(QKeyEvent * e) const {
    ...
    const auto useSelectionMode =
        (e->key() == Qt::Key_Insert) &&
        (e->modifiers() == (Qt::CTRL | Qt::SHIFT)) &&
        QGuiApplication::clipboard()->supportsSelection();
    const auto pasteMode =
        useSelectionMode ? QClipboard::Selection : QClipboard::Clipboard;
  ...
  void Widget::paintEvent(QPaintEvent * e) {
    ...
    const auto bg = anim::brush(st::dialogsBg, st::dialogsBgOver,
                                _childListShown.current());
    ...
    if (!_widthAnimationCache.isNull()) {
      const auto suggestionsShown =
          _suggestions ? _suggestions->shownOpacity()
          : !_hidingSuggestions.empty()
              ? _hidingSuggestions.back()->shownOpacity()
              : 0.;
      const auto suggestionsSkip =
          suggestionsShown * (st::topPeers.height + st::searchedBarHeight);
      const auto top =
          _searchControls->y() + _searchControls->height() + suggestionsSkip;
      p.drawPixmapLeft(0, top, width(), _widthAnimationCache);
      belowTop =
          top + (_widthAnimationCache.height() / style::DevicePixelRatio());
    }
  ...
  QString Widget::currentSearchQuery() const {
    return _subsectionTopBar ? _subsectionTopBar->searchQueryCurrent()
                             : _search->getLastText();
  }

  int Widget::currentSearchQueryCursorPosition() const {
    return _subsectionTopBar ? _subsectionTopBar->searchQueryCursorPosition()
  ...
  bool Widget::cancelSearch(CancelSearchOptions options) {
    const auto clearingSuggestionsQuery =
        _suggestions && _suggestions->consumeSearchQuery(QString());
    ...
    const auto clearingQuery =
        clearingSuggestionsQuery || !updatedState.query.isEmpty();
    ...
    if (clearingInChat) {
      if (options.jumpBackToSearchedChat && updatedState.inChat &&
          controller()->adaptive().isOneColumn()) {
    ...
    if (!clearingQuery && _subsectionTopBar &&

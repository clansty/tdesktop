...
Storage::Cache::Key
AudioAlbumThumbCacheKey(const AudioAlbumThumbLocation &location);
...
struct MessageGroupId {
  ...
  static MessageGroupId FromRaw(PeerId peer, uint64 value, bool scheduled) {
    auto result = MessageGroupId();
    result.peerAndScheduledFlag = peer.value | (scheduled ? (1ULL << 55) : 0);
    result.value = value;
    return result;
  }
  ...
  uint64 raw() const { return value; }

  friend inline constexpr auto operator<=>(MessageGroupId,
...
[[nodiscard]] BusinessShortcutId
BusinessShortcutIdFromMessage(const MTPmessage &message);
...
struct EmojiStatusId {
  ...
  explicit operator bool() const { return documentId || collectible; }

  friend inline auto operator<=>(const EmojiStatusId &,
                                 const EmojiStatusId &) = default;
  friend inline bool operator==(const EmojiStatusId &,
                                const EmojiStatusId &) = default;
};
...
struct MessageCursor {
  ...
  MessageCursor(int position, int anchor, int scroll)
      : position(position), anchor(anchor), scroll(scroll) {}
  MessageCursor(not_null<const Ui::InputField *> field) { fillFrom(field); }
...
inline bool operator==(const MessageCursor &a, const MessageCursor &b) {
  return (a.position == b.position) && (a.anchor == b.anchor) &&
         (a.scroll == b.scroll);
}
...
struct StickerSetIdentifier {
  ...
  [[nodiscard]] bool empty() const { return !id && shortName.isEmpty(); }
...
struct ForwardDraft {
  ...
  friend inline auto operator<=>(const ForwardDraft &,

...
[[nodiscard]] rpl::producer<TextWithEntities>
UsernamesSubtext(not_null<PeerData *> peer, rpl::producer<QString> fallback) {
  return rpl::combine(UsernamesValue(peer), std::move(fallback)) |
         rpl::map([](std::vector<TextWithEntities> usernames, QString text) {
           if (usernames.size() < 2) {
             ...
             result.append(' ');
             auto &&subrange =
                 ranges::make_subrange(begin(usernames) + 1, end(usernames));
             for (auto &username : std::move(subrange)) {
               const auto isLast = (usernames.back() == username);
               result.append(tr::link('@' + base::take(username.text),
...
[[nodiscard]] rpl::producer<TextWithEntities>
TopicSubtext(not_null<PeerData *> peer) {
  return rpl::conditional(
      UsernamesValue(peer) |
          rpl::map([](std::vector<TextWithEntities> v) { return !v.empty(); }),
      tr::lng_filters_link_subtitle(tr::marked),
      tr::lng_info_link_topic_label(tr::marked));
}

[[nodiscard]] Fn<void(QString)>
UsernamesLinkCallback(not_null<PeerData *> peer,
  ...
  return [=](QString link) {
    ...
    if (link.startsWith(u"internal:"_q)) {
      Core::App().openInternalUrl(link, QVariant::fromValue(ClickHandlerContext{
                                            .sessionWindow = weak,
                                        }));
      return;
...
[[nodiscard]] object_ptr<Ui::RpWidget>
CreateSkipWidget(not_null<Ui::RpWidget *> parent) {
  return Ui::CreateSkipWidget(parent, st::infoProfileSkip);
}

[[nodiscard]] object_ptr<Ui::SlideWrap<>>
CreateSlideSkipWidget(not_null<Ui::RpWidget *> parent) {
  auto result = Ui::CreateSlideSkipWidget(parent, st::infoProfileSkip);
  result->setDuration(st::infoSlideDuration);
  return result;
}

[[nodiscard]] rpl::producer<TextWithEntities>
AboutWithAdvancedValue(not_null<PeerData *> peer) {

  return AboutValue(peer) | rpl::map([=](TextWithEntities &&value) {
               ...
               if (!channel->amCreator() && channel->inviteDate) {
                 ...
                 value.append(
                     (channel->isMegagroup()
                          ...
                          : tr::lng_action_you_joined)(tr::now, tr::italic));
                 value.append(Italic(": "));
                 const auto raw = channel->inviteDate;
                 value.append(
                     Link(Italic(langDateTimeFull(base::unixtime::parse(raw))),
...
[[nodiscard]] TimeId OpensIn(const Data::WorkingIntervals &intervals,
                             TimeId now) {
...
[[nodiscard]] rpl::producer<QString>
OpensInText(rpl::producer<TimeId> in, rpl::producer<bool> hoursExpanded,
            rpl::producer<QString> fallback) {
  return rpl::combine(std::move(in), std::move(hoursExpanded),
                      std::move(fallback)) |
         rpl::map([](TimeId in, bool hoursExpanded, QString fallback) {
           return (!in || hoursExpanded) ? std::move(fallback)
                  : (in >= 86400)        ? tr::lng_info_hours_opens_in_days(
                                               tr::now, lt_count, in / 86400)
                  : (in >= 3600)         ? tr::lng_info_hours_opens_in_hours(
                                               tr::now, lt_count, in / 3600)
                                         : tr::lng_info_hours_opens_in_minutes(
                                               tr::now, lt_count, std::max(in / 60, 1));
         });
}
...
[[nodiscard]] QString FormatDayHours(const Data::WorkingHours &hours,
...
[[nodiscard]] Data::WorkingIntervals
ShiftedIntervals(Data::WorkingIntervals intervals, int delta) {
...
[[nodiscard]] object_ptr<Ui::SlideWrap<>>
CreateWorkingHours(not_null<QWidget *> parent, not_null<UserData *> user) {
  using namespace Data;

  auto result = object_ptr<Ui::SlideWrap<Ui::RoundButton>>(
      parent,
      object_ptr<Ui::RoundButton>(parent, rpl::single(QString()),
                                  st::infoHoursOuter),
  ...
  const auto recount = [=] { state->recounts.fire({}); };

  state->hours = user->session().changes().peerFlagsValue(
                     user, PeerUpdate::Flag::BusinessDetails) |
                 rpl::map([=] { return user->businessDetails().hours; });
  ...
  state->time =
      ...
      rpl::then(base::timer_once(firstTick * crl::time(1000))) |
      rpl::then(base::timer_each(60 * crl::time(1000))) | rpl::map([] {
  ...
  state->day =
      state->time.value() | rpl::map([](TimeId time) { return time / kDay; });
  state->timezoneDelta =
      rpl::combine(state->hours.value(), info->timezonesValue()) |
      rpl::filter([](const WorkingHours &hours, const Timezones &timezones) {
        return ranges::contains(timezones.list, hours.timezoneId,
                                &Timezone::id);
      }) |
  ...
  state->mine =
      rpl::combine(state->hours.value(), state->timezoneDelta.value()) |
      rpl::map([](WorkingHours &&hours, int delta) {
        return ShiftedIntervals(hours.intervals, delta);
      });

  state->opensIn = rpl::combine(state->mine.value(), state->time.value()) |
      ...
      rpl::map([](WorkingHours &&hours, int delta) {
        ...
        for (auto i = 0; i != 7; ++i) {
          if (IsFullOpen(ExtractDayIntervals(hours.intervals, i))) {
            full[i] = true;
            withoutFullDays = ReplaceDayIntervals(withoutFullDays, i,
        ...
        for (auto i = 0; i != 7; ++i) {
          if (full[i]) {
            result = ReplaceDayIntervals(result, i,
  ...
  const auto dayHoursText = [=](int day) {
    return rpl::combine(state->hours.value(), state->mineByDays.value(),
                        state->myTimezone.value()) |
           rpl::map(
               [=](const WorkingHours &hours, const WorkingIntervals &mine,
                   bool my) { return FormatDayHours(hours, mine, my, day); });
  };
  const auto dayHoursTextValue = [=](rpl::producer<int> day) {
    return std::move(day) | rpl::map(dayHoursText) | rpl::flatten_latest();
  };
  ...
  const auto timing = Ui::CreateChild<Ui::FlatLabel>(
      openedWrap,
      OpensInText(state->opensIn.value(), state->expanded.value(),
  ...
  state->opened.value() |
      rpl::on_next(
          [=](bool value) {
            opened->setTextColorOverride(value ? st::boxTextFgGood->c
                                               : st::boxTextFgError->c);
          },
          opened->lifetime());

  rpl::combine(openedWrap->widthValue(), opened->heightValue(),
               timing->sizeValue()) |
      rpl::on_next(
          [=](int width, int h1, QSize size) {
  ...
  rpl::combine(state->opened.value(), state->opensIn.value(),
               state->expanded.value(), dayHoursTextValue(state->day.value())) |
      rpl::on_next(
          [=](bool opened, TimeId opensIn, bool expanded,
              const QString &timing) {
            const auto status = (opened ? tr::lng_info_work_open
                                        : tr::lng_info_work_closed)(tr::now);
            const auto when =
                (!opensIn || expanded) ? timing
                : (opensIn >= 86400)   ? tr::lng_info_hours_opens_in_days(
                                             tr::now, lt_count, opensIn / 86400)
                : (opensIn >= 3600)
                    ? tr::lng_info_hours_opens_in_hours(tr::now, lt_count,
                                                        opensIn / 3600)
                    : tr::lng_info_hours_opens_in_minutes(
                          tr::now, lt_count, std::max(opensIn / 60, 1));
            button->setAccessibleName(tr::lng_info_hours_label(tr::now) + ": " +
                                      status + ", " + when);
          },
          inner->lifetime());
  ...
  const auto label = Ui::CreateChild<Ui::FlatLabel>(
      labelWrap, tr::lng_info_hours_label(), st::infoLabel);
  label->setAttribute(Qt::WA_TransparentForMouseEvents);
  auto linkText = rpl::combine(state->nonTrivial.value(), state->hours.value(),
                               state->mine.value(), state->myTimezone.value()) |
                  rpl::map([=](bool complex, const WorkingHours &hours,
                               const WorkingIntervals &mine, bool my) {
                    return (!complex || hours.intervals == mine)
                               ? rpl::single(QString())
                           : my ? tr::lng_info_hours_my_time()
                                : tr::lng_info_hours_local_time();
                  }) |
                  rpl::flatten_latest();
  const auto link = Ui::CreateChild<Ui::RoundButton>(
      labelWrap, std::move(linkText), st::defaultTableSmallButton);
  link->setClickedCallback([=] {
    state->myTimezone = !state->myTimezone.current();
    state->expanded = true;
  });

  rpl::combine(labelWrap->widthValue(), label->heightValue(),
               link->sizeValue()) |
      rpl::on_next(
          [=](int width, int h1, QSize size) {
  ...
  const auto other = inner->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
      inner, object_ptr<Ui::VerticalLayout>(inner)));
  ...
  other->setDuration(kSlideDuration);
  {
    ...
    timingArrow->paintRequest() |
        rpl::on_next(
            [=] {
              auto p = QPainter(timingArrow);
              const auto progress =
                  other->animating()
                      ? (crl::now() - arrowAnimation->started()) /
                            kSlideDuration
                      : 1.;

              const auto path = Ui::ToggleUpDownArrowPath(
                  timingArrow->width() / 2, timingArrow->height() / 2,
                  st::infoHoursArrowSize, st::mainMenuToggleFourStrokes,
    ...
    state->expanded.value() |
        rpl::on_next([=] { arrowAnimation->start(); }, other->lifetime());
  }
  ...
  for (auto i = 1; i != 7; ++i) {
    const auto dayWrap = days->add(object_ptr<Ui::RpWidget>(other),
                                   QMargins(0, st::infoHoursDaySkip, 0, 0));
    auto label = state->day.value() | rpl::map([=](int day) {
                   switch ((day + i) % 7) {
                   case 0:
                     return tr::lng_hours_monday();
                   case 1:
                     return tr::lng_hours_tuesday();
                   case 2:
                     return tr::lng_hours_wednesday();
                   case 3:
                     return tr::lng_hours_thursday();
                   case 4:
                     return tr::lng_hours_friday();
                   case 5:
                     return tr::lng_hours_saturday();
                   case 6:
                     return tr::lng_hours_sunday();
                   }
                   Unexpected("Index in working hours.");
                 }) |
                 rpl::flatten_latest();
    const auto dayLabel = Ui::CreateChild<Ui::FlatLabel>(
        dayWrap, std::move(label), st::infoHoursDayLabel);
    ...
    rpl::combine(dayWrap->widthValue(), dayLabel->heightValue(),
                 dayHours->sizeValue()) |
        rpl::on_next(
            [=](int width, int h1, QSize size) {
  ...
  result->toggleOn(
      state->hours.value() |
      rpl::map([](const WorkingHours &data) { return bool(data); }));

  return result;
}

void DeleteContactNote(not_null<UserData *> user,
                       Fn<void(const QString &)> showError = nullptr) {
  user->session()
      .api()
      .request(MTPcontacts_UpdateContactNote(
          user->inputUser(),
          MTP_textWithEntities(MTP_string(), MTP_vector<MTPMessageEntity>())))
      .done([=] { user->setNote(TextWithEntities()); })
...
[[nodiscard]] object_ptr<Ui::SlideWrap<>>
CreateNotes(not_null<QWidget *> parent,
            not_null<Window::SessionController *> controller,
            not_null<UserData *> user) {
  auto allNotesText = user->session().changes().peerFlagsValue(
                          user, Data::PeerUpdate::Flag::FullInfo) |
                      rpl::map([=] { return user->note(); });

  auto notesText = rpl::duplicate(allNotesText) |
                   rpl::filter([](const TextWithEntities &note) {
                     return !note.text.isEmpty();
                   });

  auto result = object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
      parent, object_ptr<Ui::VerticalLayout>(parent));
  result->toggleOn(rpl::duplicate(allNotesText) |
  ...
  auto notesLine = CreateTextWithLabel(
      notesContainer, tr::lng_info_notes_label(TextWithEntities::Simple),
      rpl::duplicate(notesText), st::infoLabel, st::infoLabeled,
      st::infoProfileLabeledPadding);

  std::move(notesText) |
      rpl::on_next(
          [=, raw = notesLine.text](TextWithEntities note) {
            TextUtilities::ParseEntities(note, TextParseLinks);
            raw->setMarkedText(note, context);
          },
          notesLine.text->lifetime());

  notesLine.text->setContextMenuHook(
      [=, raw = notesLine.text](Ui::FlatLabel::ContextMenuRequest request) {
        raw->fillContextMenu(request);
        const auto addAction = Ui::Menu::CreateAddActionCallback(request.menu);
        addAction({
  ...
  rpl::merge(notesLine.wrap->events(), notesLine.subtext->events()) |
      rpl::on_next(
          [=, raw = notesLine.text](not_null<QEvent *> e) {
            if (e->type() == QEvent::ContextMenu) {
              const auto ce = static_cast<QContextMenuEvent *>(e.get());
              QCoreApplication::postEvent(
                  raw, new QContextMenuEvent(ce->reason(), ce->pos(),
                                             ce->globalPos()));
            }
          },
  ...
  const auto subtextLabel = Ui::CreateChild<Ui::FlatLabel>(
      notesLine.wrap->entity(), tr::lng_info_notes_private(tr::now),
      st::infoLabel);
  subtextLabel->setAttribute(Qt::WA_TransparentForMouseEvents);

  rpl::combine(notesLine.wrap->entity()->widthValue(),
               notesLine.subtext->geometryValue()) |
      rpl::on_next(
          [=, skip = st::lineWidth * 5](int width,
                                        const QRect &subtextGeometry) {
            subtextLabel->moveToRight(0, subtextGeometry.y() + skip, width);
          },
...
[[nodiscard]] object_ptr<Ui::SlideWrap<>>
CreateBirthday(not_null<QWidget *> parent,
               not_null<Window::SessionController *> controller,
               not_null<UserData *> user) {
  using namespace Data;
  ...
  auto outer = Ui::CreateChild<Ui::SlideWrap<Ui::VerticalLayout>>(
      button, object_ptr<Ui::VerticalLayout>(button), st::infoHoursOuterMargin);
  const auto layout = outer->entity();
  layout->setAttribute(Qt::WA_TransparentForMouseEvents);

  auto birthday = BirthdayValue(user) | rpl::start_spawning(result->lifetime());

  auto label = BirthdayLabelText(rpl::duplicate(birthday));
  auto text =
      BirthdayValueText(rpl::duplicate(birthday)) | rpl::map(tr::marked);
  ...
  layout->sizeValue() |
      rpl::on_next(
          [=](QSize size) {
            giftIcon->moveToRight(0, (size.height() - giftIcon->height()) / 2,
                                  size.width());
          },
          giftIcon->lifetime());
  giftIcon->paintRequest() |
      rpl::on_next(
          [=] {
  ...
  rpl::duplicate(birthday) | rpl::map([](Data::Birthday value) {
  ...
  BirthdayValueText(rpl::duplicate(birthday), true) |
      rpl::on_next(
          [=](const QString &accessibleText) {
            button->setAccessibleName(tr::lng_info_birthday_label(tr::now) +
                                      ": " + accessibleText);
          },
          button->lifetime());
  ...
  auto nonEmptyText =
      std::move(text) |
      rpl::before_next([slide = result.data()](const TextWithEntities &value) {
        if (value.text.isEmpty()) {
          slide->hide(anim::type::normal);
        }
      }) |
      rpl::filter(
          [](const TextWithEntities &value) { return !value.text.isEmpty(); }) |
      rpl::after_next([slide = result.data()](const TextWithEntities &value) {
        slide->show(anim::type::normal);
      });
  layout->add(object_ptr<Ui::FlatLabel>(layout, std::move(nonEmptyText),
                                        st::birthdayLabeled));
  layout->add(Ui::CreateSkipWidget(layout, st::infoLabelSkip));
  layout->add(object_ptr<Ui::FlatLabel>(
      layout, std::move(label) | rpl::after_next([=] {
...
auto AddActionButton(
    not_null<Ui::VerticalLayout *> parent, Text &&text, ToggleOn &&toggleOn,
    Callback &&callback, const style::icon *icon,
    const style::SettingsButton &st = st::infoSharedMediaButton) {
  auto result = parent->add(object_ptr<Ui::SlideWrap<Ui::SettingsButton>>(
      parent, object_ptr<Ui::SettingsButton>(parent, std::move(text), st)));
  result->setDuration(st::infoSlideDuration)
      ->toggleOn(std::move(toggleOn))
      ->entity()
      ->addClickHandler(std::move(callback));
  result->finishAnimating();
  if (icon) {
    object_ptr<Profile::FloatingIcon>(result, *icon,
...
template <typename Text, typename ToggleOn, typename Callback>
[[nodiscard]] auto
AddMainButton(not_null<Ui::VerticalLayout *> parent, Text &&text,
              ToggleOn &&toggleOn, Callback &&callback,
              Ui::MultiSlideTracker &tracker,
              Ui::MultiSlideTracker *buttonTracker,
              const style::SettingsButton &st = st::infoMainButton) {
  const auto button =
      AddActionButton(parent, std::move(text) | rpl::map(tr::upper),
                      std::move(toggleOn), std::move(callback), nullptr, st);
...
rpl::producer<CreditsAmount>
AddCurrencyAction(not_null<UserData *> user,
  ...
  const auto wrapButton = AddActionButton(
      wrap, tr::lng_manage_peer_bot_balance_currency(),
      state->balance.value() | rpl::map(rpl::mappers::_1 > CreditsAmount(0)),
  ...
  const auto icon = state->helper.paletteDependent(
      {.factory =
           [=] {
             return Ui::Earn::IconCurrencyColored(st.rightLabel.style.font,
                                                  st.rightLabel.textFg->c);
           },
       .margin = st::channelEarnCurrencyCommonMargins});
  name->show();
  rpl::combine(button->widthValue(), tr::lng_manage_peer_bot_balance_currency(),
               state->balance.value()) |
      rpl::on_next(
          [=, &st](int width, const QString &button, CreditsAmount balance) {
            const auto available = width - rect::m::sum::h(st.padding) -
...
rpl::producer<CreditsAmount>
AddCreditsAction(not_null<UserData *> user, not_null<Ui::VerticalLayout *> wrap,
                 not_null<Controller *> controller) {
  struct State final {
    rpl::variable<CreditsAmount> balance;
  };
  ...
  const auto parentController = controller->parentController();
  const auto wrapButton = AddActionButton(
      wrap, tr::lng_manage_peer_bot_balance_credits(),
      state->balance.value() | rpl::map(rpl::mappers::_1 > CreditsAmount(0)),
  ...
  {
    const auto api = wrap->lifetime().make_state<Api::CreditsStatus>(user);
  ...
  name->show();
  rpl::combine(button->widthValue(), tr::lng_manage_peer_bot_balance_credits(),
               state->balance.value()) |
      rpl::on_next(
          [=, &st](int width, const QString &button, CreditsAmount balance) {
            const auto available = width - rect::m::sum::h(st.padding) -
...
class DetailsFiller {
public:
  DetailsFiller(not_null<Controller *> controller,
                not_null<Ui::RpWidget *> parent, not_null<PeerData *> peer,
                Origin origin, Ui::MultiSlideTracker &mainTracker,
...
private:
  ...
  void addShowTopicsListButton(Ui::MultiSlideTracker &tracker,
                               not_null<Data::Forum *> forum,
                               Ui::MultiSlideTracker *buttonTracker);
  void addViewChannelButton(Ui::MultiSlideTracker &tracker,
  ...
  template <typename Widget, typename = std::enable_if_t<
                                 std::is_base_of_v<Ui::RpWidget, Widget>>>
  Widget *add(object_ptr<Widget> &&child,
              const style::margins &margin = style::margins()) {
    return _wrap->add(std::move(child), margin);
  }
...
class ActionsFiller {
public:
  ActionsFiller(not_null<Controller *> controller,
                not_null<Ui::RpWidget *> parent, not_null<PeerData *> peer);
...
void ReportReactionBox(not_null<Ui::GenericBox *> box,
                       not_null<Window::SessionController *> controller,
                       not_null<PeerData *> participant,
                       GroupReactionOrigin data, bool ban, Fn<void()> sent) {
  box->setTitle(tr::lng_report_reaction_title());
  box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_report_reaction_about(),
                                        st::boxLabel));
  const auto check =
      ban ? box->addRow(object_ptr<Ui::Checkbox>(
                            box, tr::lng_report_and_ban_button(tr::now), true),
                        st::boxRowPadding +
                            QMargins{0, st::boxLittleSkip, 0, 0})
          : nullptr;
  box->addButton(
      tr::lng_report_button(),
      [=] {
        const auto chat = data.group->asChat();
        const auto channel = data.group->asMegagroup();
        if (check && check->checked()) {
          if (chat) {
            chat->session().api().chatParticipants().kick(chat, participant);
          } else if (channel) {
...
DetailsFiller::DetailsFiller(not_null<Controller *> controller,
                             not_null<Ui::RpWidget *> parent,
                             not_null<PeerData *> peer, Origin origin,
                             Ui::MultiSlideTracker &mainTracker,
                             rpl::variable<bool> &dividerOverridden)
    : _controller(controller), _parent(parent), _peer(peer), _origin(origin),
      _mainTracker(mainTracker), _dividerOverridden(dividerOverridden),
      _wrap(_parent) {}
...
DetailsFiller::DetailsFiller(not_null<Controller *> controller,
                             ...
                             Ui::MultiSlideTracker &mainTracker,
                             rpl::variable<bool> &dividerOverridden)
    : _controller(controller), _parent(parent), _peer(sublist->sublistPeer()),
      _sublist(sublist), _mainTracker(mainTracker),
      _dividerOverridden(dividerOverridden), _wrap(_parent) {}
...
DetailsFiller::DetailsFiller(not_null<Controller *> controller,
                             ...
                             Ui::MultiSlideTracker &mainTracker,
                             rpl::variable<bool> &dividerOverridden)
    : _controller(controller), _parent(parent), _peer(topic->peer()),
      _topic(topic), _mainTracker(mainTracker),
      _dividerOverridden(dividerOverridden), _wrap(_parent) {}

template <typename T>
bool SetClickContext(const ClickHandlerPtr &handler,
...
object_ptr<Ui::RpWidget> DetailsFiller::setupInfo() {
  ...
  add(CreateSlideSkipWidget(wrap))
      ->toggleOn(tracker.atLeastOneShownValueLater());

  // Fill context for a mention / hashtag / bot command link.
  const auto infoClickFilter =
      [=, peer = _peer.get(), window = _controller->parentController()](
          const ClickHandlerPtr &handler, Qt::MouseButton button) {
        const auto context =
        ...
        if (SetClickContext<BotCommandClickHandler>(handler, context)) {
          ...
          const auto joinDate =
              handler->url().split(u"show:"_q, Qt::SkipEmptyParts).last();
          if (!joinDate.isEmpty()) {
  ...
  const auto addTranslateToMenu = [&, peer = _peer.get(),
                                   controller =
    ...
    label->setContextMenuHook([=](Ui::FlatLabel::ContextMenuRequest request) {
      ...
      request.menu->addAction(std::move(item), [=] {
        controller->window().show(Box(
            Ui::TranslateBox, peer, MsgId(),
            request.selection.empty()
                ? state->labelText.current()
                : Ui::Text::Mid(state->labelText.current(),
  ...
  const auto addInfoLineGeneric =
      [&](v::text::data &&label, rpl::producer<TextWithEntities> &&text,
          const style::FlatLabel &textSt = st::infoLabeled,
          const style::margins &padding = st::infoProfileLabeledPadding) {
        auto line = CreateTextWithLabel(
            result, v::text::take_marked(std::move(label)), std::move(text),
            st::infoLabel, textSt, padding);
  ...
  const auto addInfoLine =
      [&](v::text::data &&label, rpl::producer<TextWithEntities> &&text,
          const style::FlatLabel &textSt = st::infoLabeled,
          const style::margins &padding = st::infoProfileLabeledPadding) {
        return addInfoLineGeneric(std::move(label), std::move(text), textSt,
                                  padding);
      };
  const auto addInfoOneLine =
      [&](v::text::data &&label, rpl::producer<TextWithEntities> &&text,
          ...
          const style::margins &padding = st::infoProfileLabeledPadding) {
        auto result = addInfoLine(std::move(label), std::move(text),
                                  st::infoLabeledOneLine, padding);
        result.text->setDoubleClickSelectsParagraph(true);
        result.text->setContextCopyText(contextCopyText);
        return result;
      };
  const auto fitLabelToButton = [&](not_null<Ui::RpWidget *> button,
                                    not_null<Ui::FlatLabel *> label,
                                    int rightSkip) {
    const auto parent = label->parentWidget();
    const auto container = result;
    rpl::combine(container->widthValue(), label->geometryValue(),
                 button->sizeValue(), button->shownValue()) |
        rpl::on_next(
            [=](int width, QRect, QSize buttonSize, bool buttonShown) {
              button->moveToRight(rightSkip,
                                  (parent->height() - buttonSize.height()) / 2);
              const auto x = Ui::MapFrom(container, label, QPoint(0, 0)).x();
              const auto s =
                  buttonShown ? Ui::MapFrom(container, button, QPoint(0, 0)).x()
                              : width;
  ...
  const auto lnkHook = [=](Ui::FlatLabel::ContextMenuRequest request) {
    ...
    if (url.startsWith(u"https://")) {
      request.menu->addAction(tr::lng_context_copy_link(tr::now), [=] {
        TextUtilities::SetClipboardText({url});
        if (const auto strong = weak.get()) {
          strong->showToast(tr::lng_channel_public_link_copied(tr::now));
        }
      });
      request.menu->addAction(tr::lng_group_invite_share(tr::now), [=] {
        if (const auto strong = weak.get()) {
    ...
    static const auto kPrefix = QRegularExpression(
        u"^internal:"
    ...
    request.menu->addAction(tr::lng_context_copy_mention(tr::now), [=] {
  ...
  if (const auto user = _peer->asUser()) {
    if (user->session().supportMode()) {
      addInfoLineGeneric(user->session().supportHelper().infoLabelValue(user),
                         user->session().supportHelper().infoTextValue(user));
    }

    {
      const auto phoneLabel =
          addInfoOneLine(tr::lng_info_mobile_label(), PhoneOrHiddenValue(user),
      ...
      const auto hook = [=](Ui::FlatLabel::ContextMenuRequest request) {
        if (request.selection.empty()) {
          const auto callback = [=] {
            auto phone =
                rpl::variable<TextWithEntities>(PhoneOrHiddenValue(user))
                    .current()
                    .text;
            phone.replace(' ', QString()).replace('-', QString());
            TextUtilities::SetClipboardText({phone});
          };
          request.menu->addAction(tr::lng_profile_copy_phone(tr::now),
                                  callback);
        } else {
    ...
    auto label =
        user->isBot() ? tr::lng_info_about_label() : tr::lng_info_bio_label();
    addTranslateToMenu(
    ...
    const auto usernameLine = addInfoOneLine(
        ...
        QString(), st::infoProfileLabeledUsernamePadding);
    const auto callback = UsernamesLinkCallback(_peer, controller, QString());
    ...
    const auto qrButton = Ui::CreateChild<Ui::IconButton>(
        usernameLine.text->parentWidget(), st::infoProfileLabeledButtonQr);
    qrButton->setAccessibleName(tr::lng_group_invite_context_qr(tr::now));
    UsernamesValue(_peer) |
        rpl::on_next([=](const auto &u) { qrButton->setVisible(!u.empty()); },
                     qrButton->lifetime());
    ...
    if (!user->isBot()) {
      tracker.track(result->add(CreateBirthday(result, controller, user), {},
                                style::al_justify));
      tracker.track(
          result->add(CreateWorkingHours(result, user), {}, style::al_justify));

      tracker.track(result->add(CreateNotes(result, controller, user), {},
                                style::al_justify));

      auto locationText =
          user->session().changes().peerFlagsValue(
              user, Data::PeerUpdate::Flag::BusinessDetails) |
          rpl::map([=] {
            ...
            return tr::link(TextUtilities::SingleLine(details.location.address),
                            LocationClickHandler::Url(*details.location.point));
          });
      addInfoOneLine(tr::lng_info_location_label(), std::move(locationText),
                     QString())
    ...
    }
    {
      ...
      auto idDrawableText =
          IDValue(user) | rpl::map([](TextWithEntities &&text) {
            return Ui::Text::Code(text.text);
          });
      auto idInfo =
          addInfoOneLine(rpl::single(idLabel), std::move(idDrawableText),
      ...
      AddRegistrationOrCreationButton(controller, _peer, idInfo,
                                      fitLabelToButton);
    }
  } else {
    const auto topicRootId = _topic ? _topic->rootId() : 0;
    const auto addToLink =
        topicRootId ? ('/' + QString::number(topicRootId.bare)) : QString();
    if (_peer->isMegagroup()) {
      addInfoLine(tr::lng_chat_restriction_reason(),
    ...
    auto linkText =
        LinkValue(_peer, true, topicRootId) |
        rpl::map([=](const LinkWithUrl &link) {
          const auto text = link.text;
          return text.isEmpty()
                     ? TextWithEntities()
                     : tr::link((text.startsWith(u"https://"_q)
                                     ? text.mid(u"https://"_q.size())
                                     : text) +
                                    addToLink,
                                (addToLink.isEmpty() ? link.url
                                                     : (text + addToLink)));
        });
    const auto linkLine = addInfoOneLine(
        (topicRootId ? TopicSubtext(_peer)
                     : UsernamesSubtext(_peer, tr::lng_info_link_label())),
        std::move(linkText), QString());
    const auto controller = _controller->parentController();
    const auto linkCallback =
        UsernamesLinkCallback(_peer, controller, addToLink);
    ...
    if (!topicRootId || !_peer->username().isEmpty()) {
      const auto qr = Ui::CreateChild<Ui::IconButton>(
          linkLine.text->parentWidget(), st::infoProfileLabeledButtonQr);
      qr->setAccessibleName(tr::lng_group_invite_context_qr(tr::now));
      UsernamesValue(_peer) |
          rpl::on_next([=](const auto &u) { qr->setVisible(!u.empty()); },
                       qr->lifetime());
      const auto rightSkip = st::infoProfileLabeledButtonQrRightSkip;
    ...
    if (const auto channel = _topic ? nullptr : _peer->asChannel()) {
      auto locationText =
          LocationValue(channel) |
          rpl::map([](const ChannelLocation *location) {
            return location
                       ? tr::link(TextUtilities::SingleLine(location->address),
    ...
    const auto about = addInfoLine(tr::lng_info_about_label(),
                                   _topic ? rpl::single(TextWithEntities())
    ...
    if (!_topic) {
      ...
      auto idDrawableText =
          IDValue(_peer) | rpl::map([](TextWithEntities &&text) {
            return Ui::Text::Code(text.text);
          });
      auto idInfo = addInfoOneLine(idLabel, std::move(idDrawableText),
                                   tr::ayu_ContextCopyID(tr::now));
    ...
    if (_topic) {
      auto idDrawableText =
          IDValue(_peer->forumTopicFor(topicRootId)->topicRootId()) |
          rpl::map([](TextWithEntities &&text) {
            return Ui::Text::Code(text.text);
          });
      auto idInfo =
          addInfoOneLine(rpl::single(QString("ID")), std::move(idDrawableText),
                         tr::ayu_ContextCopyID(tr::now));

      idInfo.text->setClickHandlerFilter([=, peer = _peer](auto &&...) {
        const auto idText =
            IDString(peer->forumTopicFor(topicRootId)->topicRootId());
        if (!idText.isEmpty()) {
          QGuiApplication::clipboard()->setText(idText);
          controller->showToast(tr::ayu_IDCopiedToast(tr::now));
        }
...
DetailsFiller::setupPersonalChannel(not_null<UserData *> user) {
  auto result = object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
      _wrap, object_ptr<Ui::VerticalLayout>(_wrap));
  ...
  result->toggleOn(PersonalChannelValue(user) |
                   rpl::map(rpl::mappers::_1 != nullptr));
  result->finishAnimating();

  auto channel =
      PersonalChannelValue(user) | rpl::start_spawning(result->lifetime());

  const auto channelLabelFactory = [=](rpl::producer<ChannelData *> c) {
    return rpl::combine(tr::lng_info_personal_channel_label(tr::marked),
                        std::move(c)) |
...

  {
    ...
    auto text = rpl::duplicate(channel) | rpl::map([=](ChannelData *channel) {
    ...
    auto line = CreateTextWithLabel(
        result, channelLabelFactory(rpl::duplicate(channel)), std::move(text),
        st::infoLabel, st::infoLabeled, st::infoProfilePersonalChannelPadding);
    ...
    object_ptr<FloatingIcon>(onlyChannelWrap, st::infoIconMediaChannel,
...

  {
    ...
    messageChannelWrap->toggledValue() | rpl::filter(rpl::mappers::_1) |
        rpl::on_next(
            [=] {
    ...
    const auto rebuild = [=](not_null<HistoryItem *> item,
                             anim::type animated) {
      ...
      const auto line = inner->add(object_ptr<Ui::FixedHeightWidget>(
          inner, stUserpic.photoSize + rect::m::sum::v(stLabeled)));
      const auto userpic = Ui::CreateChild<Ui::UserpicButton>(
          line, item->history()->peer, st::infoPersonalChannelUserpic);

      userpic->moveToLeft(-st::infoPersonalChannelUserpicSkip +
      ...
      const auto name =
          Ui::CreateChild<Ui::FlatLabel>(line, NameValue(item->history()->peer),
          ...
          rpl::on_next(
              [=] {
                ...
                if (!item) {
                  ...
                  p.drawText(preview->rect(), tr::lng_deleted_message(tr::now),
                             style::al_left);
                  return;
                }
                if (!state->view.prepared(item, nullptr, nullptr)) {
                  const auto repaint = [=] { preview->update(); };
                  state->view.prepare(item, nullptr, nullptr, repaint, {});
                }
      ...
      line->sizeValue() | rpl::filter_size() |
          rpl::on_next(
              [=](const QSize &size) {
                ...
                name->resizeToWidth(size.width() - left - date->width() -
                                    st::defaultVerticalListSkip - right);
                name->moveToLeft(left, top);

                preview->resize(size.width() - left - right,
                                st::infoLabeled.style.font->height);
                preview->moveToLeft(left, size.height() - stLabeled.bottom() -
                                              preview->height());
              },
              preview->lifetime());
      ...
      {
        inner->add(
            object_ptr<Ui::FlatLabel>(inner,
                                      channelLabelFactory(rpl::single(
                                          item->history()->peer->asChannel())),
                                      st::infoLabel),
            QMargins(st::infoProfilePersonalChannelPadding.left(), 0,
      ...
      }
      {
        const auto button = Ui::CreateSimpleRectButton(
            messageChannelWrap->entity(), st::defaultRippleAnimation);
        inner->geometryValue() |
            rpl::on_next([=](const QRect &rect) { button->setGeometry(rect); },
                         button->lifetime());
        button->setClickedCallback([=, msg = item->fullId().msg] {
          window->showPeerHistory(item->history()->peer,
                                  Window::SectionShow::Way::Forward, msg);
        });
        button->lower();
        inner->lifetime().make_state<base::unique_qptr<Ui::RpWidget>>(button);
...
void DetailsFiller::setupMainApp(bool suppressBottom) {
  const auto button =
      _wrap->add(object_ptr<Ui::RoundButton>(_wrap, tr::lng_profile_open_app(),
                                             st::infoOpenApp),
                 st::infoOpenAppMargin, style::al_justify);
  ...
  const auto parts =
      suppressBottom ? RectPart::Top : (RectPart::Top | RectPart::Bottom);
  const auto divider = Ui::AddDividerText(
      _wrap,
      rpl::combine(tr::lng_profile_open_app_about(
                       lt_terms, tr::lng_profile_open_app_terms(tr::url(url)),
                       tr::marked),
                   user->session().changes().peerFlagsValue(
                       user, Data::PeerUpdate::Flag::VerifyInfo)) |
      ...
      st::defaultBoxDividerLabelPadding, st::defaultDividerLabel, parts);
...
void DetailsFiller::setupBotPermissions() {
  ...
  const auto emoji = _wrap->add(object_ptr<Ui::SettingsButton>(
      _wrap, tr::lng_profile_bot_emoji_status_access(),
      st::infoSharedMediaButton));
  object_ptr<Profile::FloatingIcon>(emoji, st::infoIconEmojiStatusAccess,
                                    st::infoSharedMediaButtonIconPosition);

  const auto user = _peer->asUser();
  emoji->toggleOn(rpl::single(bool(user->botInfo->canManageEmojiStatus)))
          ->toggledValue() |
      rpl::filter([=](bool allowed) {
      ...
      rpl::on_next(
          [=](bool allowed) {
            ...
            session->api()
                .request(MTPbots_ToggleUserEmojiStatusPermission(
                    user->inputUser(), MTP_bool(allowed)))
                .send();
          },
          emoji->lifetime());
  AddSkip(_wrap);
...
void DetailsFiller::addReportReaction(GroupReactionOrigin data, bool ban,
  ...
  auto shown = user ? rpl::combine(Info::Profile::IsContactValue(user),
                                   forceHidden->value(),
                                   !rpl::mappers::_1 && !rpl::mappers::_2) |
                          rpl::type_erased
                    : (forceHidden->value() | rpl::map(!rpl::mappers::_1));
  const auto sent = [=] { *forceHidden = true; };
  ...
  AddMainButton(
      ...
      tracker, buttonTracker, st::infoMainButtonAttention);
}

void DetailsFiller::addViewChannelButton(Ui::MultiSlideTracker &tracker,
                                         not_null<ChannelData *> channel,
                                         Ui::MultiSlideTracker *buttonTracker) {
  ...
  auto activePeerValue = window->activeChatValue() |
                         rpl::map([](Dialogs::Key key) { return key.peer(); });
  auto viewChannelVisible =
      rpl::combine(_controller->wrapValue(), std::move(activePeerValue),
                   (_1 != Wrap::Side) || (_2 != channel));
  auto viewChannel = [=] {
    window->showPeerHistory(channel, Window::SectionShow::Way::Forward);
  };
  wrap->toggleOn(rpl::duplicate(viewChannelVisible));
  AddMainButton(wrap->entity(), tr::lng_profile_view_channel(),
                std::move(viewChannelVisible), std::move(viewChannel), tracker,
                buttonTracker);
}

void DetailsFiller::addShowTopicsListButton(
    Ui::MultiSlideTracker &tracker, not_null<Data::Forum *> forum,
    Ui::MultiSlideTracker *buttonTracker) {
  using namespace rpl::mappers;

  const auto window = _controller->parentController();
  const auto peer = forum->peer();
  auto showTopicsVisible =
      rpl::combine(window->adaptive().oneColumnValue(),
                   window->shownForum().value(), _1 || (_2 != forum));
  const auto callback = [=] {
  ...
  AddMainButton(_wrap,
                (forum->peer()->isBot() ? tr::lng_bot_show_threads_list()
                                        : tr::lng_forum_show_topics_list()),
                std::move(showTopicsVisible), callback, tracker, buttonTracker);
}
...
object_ptr<Ui::RpWidget> DetailsFiller::fill() {
  ...
  if (const auto user = _peer->asUser()) {
    {
      const auto wrap =
          _wrap->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
              _wrap.data(), object_ptr<Ui::VerticalLayout>(_wrap.data())));
      Ui::AddSkip(wrap->entity());
      AddMainButton(
          wrap->entity(), tr::lng_info_add_as_contact(),
          CanAddContactValue(user),
          [=, controller = _controller->parentController()] {
            controller->uiShow()->show(Box(EditContactBox, controller, user));
          },
          _mainTracker, &lastButtonTracker);
      wrap->toggleOn(CanAddContactValue(user));
    }
    if (const auto info = user->botInfo.get()) {
      if (info->hasMainApp) {
        _dividerOverridden.force_assign(true);
        const auto managedBotFollows =
            user->botManagerId() && !info->canManageEmojiStatus &&
      ...
      if (user->botManagerId()) {
        if (const auto managerUser =
                user->owner().userLoaded(user->botManagerId())) {
          if (!info->hasMainApp) {
            _dividerOverridden.force_assign(true);
          }
          const auto botUsername = managerUser->username();
          const auto linkText = botUsername.isEmpty() ? managerUser->name()
                                                      : (u"@"_q + botUsername);
          ...
          const auto divider = Ui::AddDividerText(
              ...
              tr::lng_managed_bot_label(
                  ...
                  lt_bot, rpl::single(tr::link(linkText)), tr::marked),
              st::defaultBoxDividerLabelPadding, st::defaultDividerLabel,
              parts);
    ...
    if (const auto forum = channel->forum()) {
      _dividerOverridden.force_assign(false);
      addShowTopicsListButton(_mainTracker, forum, &lastButtonTracker);
    }
  }
  add(CreateSlideSkipWidget(_wrap))
      ->toggleOn(lastButtonTracker.atLeastOneShownValueLater());

  return std::move(_wrap);
}

ActionsFiller::ActionsFiller(not_null<Controller *> controller,
                             not_null<Ui::RpWidget *> parent,
                             not_null<PeerData *> peer)
    : _controller(controller), _parent(parent), _peer(peer) {}
...
void ActionsFiller::addAffiliateProgram(not_null<UserData *> user) {
  ...
  auto program = user->session().changes().peerFlagsValue(
                     user, Data::PeerUpdate::Flag::StarRefProgram) |
                 rpl::map([=] { return user->botInfo->starRefProgram; }) |
                 rpl::start_spawning(inner->lifetime());
  auto commission =
      rpl::duplicate(program) | rpl::filter([=](StarRefProgram program) {
  ...
  recipients->open = [=] {
    if (!recipients->list.empty()) {
      ...
      show->show(Info::BotStarRef::JoinStarRefBox(
          {user, {program}}, user->session().user(), recipients->list));
  ...
  Ui::AddDividerText(
      inner, tr::lng_manage_peer_bot_star_ref_about(
                 lt_bot, rpl::single(TextWithEntities{user->name()}), lt_amount,
                 rpl::duplicate(commission) | rpl::map(tr::marked), tr::rich));
  Ui::AddSkip(inner);

  wrap->toggleOn(std::move(program) | rpl::map([](StarRefProgram program) {
                   return program.commission > 0;
                 }));
  wrap->finishAnimating();
}
...
void ActionsFiller::addBalanceActions(not_null<UserData *> user) {
  ...
  Ui::AddSkip(inner);
  wrap->toggleOn(
      rpl::combine(std::move(currencyBalance), std::move(creditsBalance)) |
...
void ActionsFiller::addInviteToGroupAction(not_null<UserData *> user) {
  const auto notEmpty = [](const QString &value) { return !value.isEmpty(); };
  const auto controller = _controller->parentController();
  AddActionButton(
      _wrap, InviteToChatButton(user) | rpl::filter(notEmpty),
      InviteToChatButton(user) | rpl::map(notEmpty),
      [=] { AddBotToGroupBoxController::Start(controller, user); },
      &st::infoIconAddMember);
  const auto about = _wrap->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
      _wrap.data(), object_ptr<Ui::VerticalLayout>(_wrap.data())));
  about->toggleOn(InviteToChatAbout(user) | rpl::map(notEmpty));
  Ui::AddSkip(about->entity());
  Ui::AddDividerText(about->entity(),
...
void ActionsFiller::addShareContactAction(not_null<UserData *> user) {
  const auto controller = _controller->parentController();
  AddActionButton(
      _wrap, tr::lng_info_share_contact(), CanShareContactValue(user),
...
void ActionsFiller::addEditContactAction(not_null<UserData *> user) {
  ...
  AddActionButton(_wrap, tr::lng_info_edit_contact(), IsContactValue(user),
                  edit, &st::infoIconEdit);
}

void ActionsFiller::addDeleteContactAction(not_null<UserData *> user) {
  const auto controller = _controller->parentController();
  AddActionButton(
      _wrap, tr::lng_info_delete_contact(), IsContactValue(user),
...
void ActionsFiller::addFastButtonsMode(not_null<UserData *> user) {
  ...
  const auto button = _wrap->add(object_ptr<Ui::SettingsButton>(
      _wrap, rpl::single(u"Fast buttons mode"_q), st::infoSharedMediaButton));
  object_ptr<Info::Profile::FloatingIcon>(
      button, st::infoIconMediaBot, st::infoSharedMediaButtonIconPosition);
  ...
  button->toggledValue() |
      rpl::filter([=](bool value) { return value != bots->enabled(user); }) |
      rpl::on_next([=](bool value) { bots->setEnabled(user, value); },
                   button->lifetime());
}
...
void ActionsFiller::addBotCommandActions(not_null<UserData *> user) {
  ...
  const auto findBotCommand = [user](const QString &command) {
    ...
    for (const auto &data : user->botInfo->commands) {
      const auto isSame = !data.command.compare(command, Qt::CaseInsensitive);
      if (isSame) {
        return data.command;
      }
    }
  ...
  const auto hasBotCommandValue = [=](const QString &command) {
    return user->session().changes().peerFlagsValue(
               user, Data::PeerUpdate::Flag::BotCommands) |
           rpl::map([=] { return !findBotCommand(command).isEmpty(); });
  };
  ...
  const auto sendBotCommand = [=](const QString &command) {
    ...
    BotCommandClickHandler('/' + original)
        .onClick(ClickContext{Qt::LeftButton, makeOtherContext()});
    return true;
  };
  const auto addBotCommand = [=](rpl::producer<QString> text,
                                 const QString &command,
                                 const style::icon *icon = nullptr) {
    AddActionButton(
        _wrap, std::move(text), hasBotCommandValue(command),
        [=] { sendBotCommand(command); }, icon);
  };
  addBotCommand(tr::lng_profile_bot_help(), u"help"_q,
  ...
  const auto openUrl = [=](const QString &url) {
    Core::App().iv().openWithIvPreferred(&user->session(), url,
                                         makeOtherContext());
  };
  ...
  AddActionButton(_wrap, tr::lng_profile_bot_privacy(), rpl::single(true),
                  openPrivacyPolicy, &st::infoIconPrivacyPolicy);
}
...
void ActionsFiller::addReportAction() {
  ...
  AddActionButton(_wrap, tr::lng_profile_report(), rpl::single(true), report,
                  &st::infoIconReport, st::infoBlockButton);
}
...
void ActionsFiller::addBlockAction(not_null<UserData *> user) {
  ...
  auto text = user->session().changes().peerFlagsValue(
                  user, Data::PeerUpdate::Flag::IsBlocked) |
              rpl::map([=] {
  ...
  auto toggleOn = rpl::duplicate(text) |
                  rpl::map([](const QString &text) { return !text.isEmpty(); });
  auto callback = [=] {
    if (user->isBlocked()) {
  ...
  AddActionButton(_wrap, rpl::duplicate(text), std::move(toggleOn),
                  std::move(callback), &st::infoIconBlock, st::infoBlockButton);
}
...
void ActionsFiller::addLeaveChannelAction(not_null<ChannelData *> channel) {
  ...
  AddActionButton(
      _wrap, tr::lng_profile_leave_channel(), AmInChannelValue(channel),
      Window::DeleteAndLeaveHandler(_controller->parentController(), channel),
      &st::infoIconLeave);
}

void ActionsFiller::addJoinChannelAction(not_null<ChannelData *> channel) {
  using namespace rpl::mappers;
  auto joinVisible = AmInChannelValue(channel) | rpl::map(!_1) |
                     rpl::start_spawning(_wrap->lifetime());
  AddActionButton(
      _wrap, tr::lng_profile_join_channel(), rpl::duplicate(joinVisible),
      [=] { channel->session().api().joinChannel(channel); },
      &st::infoIconAddMember);
  _wrap
      ->add(object_ptr<Ui::SlideWrap<Ui::FixedHeightWidget>>(
          _wrap, CreateSkipWidget(_wrap, st::infoBlockButtonSkip)))
      ->setDuration(st::infoSlideDuration)
...
void ActionsFiller::fillChannelActions(not_null<ChannelData *> channel) {
  using namespace rpl::mappers;
...
object_ptr<Ui::RpWidget> ActionsFiller::fill() {
  ...
  if (auto user = _peer->asUser()) {
    return wrapResult([=] { fillUserActions(user); });
  } else if (auto channel = _peer->asChannel()) {
    if (channel->isMegagroup()) {
      return {nullptr};
    }
    return wrapResult([=] { fillChannelActions(channel); });
  }
  return {nullptr};
}
...
const char kOptionShowChannelJoinedBelowAbout[] =
    "show-channel-joined-below-about";

object_ptr<Ui::RpWidget> SetupDetails(not_null<Controller *> controller,
                                      not_null<Ui::RpWidget *> parent,
                                      not_null<PeerData *> peer, Origin origin,
                                      Ui::MultiSlideTracker &mainTracker,
                                      rpl::variable<bool> &dividerOverridden) {
  DetailsFiller filler(controller, parent, peer, origin, mainTracker,
                       dividerOverridden);
  return filler.fill();
}
...
object_ptr<Ui::RpWidget> SetupDetails(not_null<Controller *> controller,
                                      ...
                                      Ui::MultiSlideTracker &mainTracker,
                                      rpl::variable<bool> &dividerOverridden) {
  DetailsFiller filler(controller, parent, sublist, mainTracker,
                       dividerOverridden);
  return filler.fill();
}
...
object_ptr<Ui::RpWidget> SetupDetails(not_null<Controller *> controller,
                                      ...
                                      Ui::MultiSlideTracker &mainTracker,
                                      rpl::variable<bool> &dividerOverridden) {
  DetailsFiller filler(controller, parent, topic, mainTracker,
                       dividerOverridden);
  return filler.fill();
}

object_ptr<Ui::RpWidget> SetupActions(not_null<Controller *> controller,
                                      not_null<Ui::RpWidget *> parent,
                                      not_null<PeerData *> peer) {
...
void SetupAddChannelMember(not_null<Window::SessionNavigation *> navigation,
                           not_null<Ui::RpWidget *> parent,
                           not_null<ChannelData *> channel) {
  auto add =
      Ui::CreateChild<Ui::IconButton>(parent.get(), st::infoMembersAddMember);
  ...
  parent->widthValue() |
      rpl::on_next(
          [add](int newWidth) {
            ...
            add->moveToLeft(availableWidth - add->width(),
                            st::infoMembersButtonPosition.y(), newWidth);
          },
          add->lifetime());
}

object_ptr<Ui::RpWidget>
SetupChannelMembersAndManage(not_null<Controller *> controller,
                             not_null<Ui::RpWidget *> parent,
                             not_null<PeerData *> peer) {
  using namespace rpl::mappers;
  ...
  auto membersShown = rpl::combine(
      ...
      Data::PeerFlagValue(channel, ChannelDataFlag::CanViewParticipants),
      (_1 > 0) && _2);
  auto membersText = tr::lng_chat_status_subscribers(
      lt_count_decimal, MembersCountValue(channel) | tr::to_count());
  auto membersCallback = [=] {
    controller->showSection(
        std::make_shared<Info::Memento>(channel, Section::Type::Members));
  };

  const auto membersWrap =
      result->entity()->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
          result->entity(), object_ptr<Ui::VerticalLayout>(result->entity())));
  membersWrap->setDuration(st::infoSlideDuration)
      ->toggleOn(rpl::duplicate(membersShown));

  const auto members = membersWrap->entity();
  {
    auto button =
        AddActionButton(members, std::move(membersText), rpl::single(true),
                        std::move(membersCallback), nullptr)
            ->entity();

    SetupAddChannelMember(controller, button, channel);
  }

  object_ptr<FloatingIcon>(members, st::infoIconMembers,
                           st::infoChannelMembersIconPosition);

  auto adminsShown = peer->session().changes().peerFlagsValue(
                         channel, Data::PeerUpdate::Flag::Rights) |
                     rpl::map([=] { return channel->canViewAdmins(); });
  auto adminsText = tr::lng_profile_administrators(
      lt_count_decimal, Info::Profile::MigratedOrMeValue(channel) |
                            rpl::map(Info::Profile::AdminsCountValue) |
                            rpl::flatten_latest() | tr::to_count());
  auto adminsCallback = [=] {
    ParticipantsBoxController::Start(controller, channel,
                                     ParticipantsBoxController::Role::Admins);
  };

  const auto adminsWrap =
      result->entity()->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
          result->entity(), object_ptr<Ui::VerticalLayout>(result->entity())));
  adminsWrap->setDuration(st::infoSlideDuration)
      ->toggleOn(rpl::duplicate(adminsShown));

  const auto admins = adminsWrap->entity();
  AddActionButton(admins, std::move(adminsText), rpl::single(true),
                  std::move(adminsCallback), nullptr);

  object_ptr<FloatingIcon>(admins, st::menuIconAdmin,
                           st::infoChannelAdminsIconPosition);

  const auto canViewBalance =
      false || (channel->flags() & ChannelDataFlag::CanViewRevenue) ||
      (channel->flags() & ChannelDataFlag::CanViewCreditsRevenue) ||
      (channel->loadedStatus() != ChannelData::LoadedStatus::Full);
  if (canViewBalance) {
    const auto balanceWrap =
        result->entity()->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
            result->entity(),
            object_ptr<Ui::VerticalLayout>(result->entity())));
    auto refreshed =
        channel->session().credits().refreshedByPeerId(channel->id);
    auto creditsValue =
        rpl::single(rpl::empty_value()) | rpl::then(rpl::duplicate(refreshed)) |
        rpl::map(
    ...
    balanceWrap->toggleOn(rpl::combine(rpl::duplicate(creditsValue),
    ...
    auto customEmojiFactory =
        [height = st.style.font->height, font = st.rightLabel.style.font,
    ...
    const auto button = AddActionButton(
        balance, tr::lng_manage_peer_bot_balance(), rpl::single(true),
        [=] { controller->showSection(Info::ChannelEarn::Make(peer)); },
        nullptr);

    ::Settings::CreateRightLabel(
        button->entity(),
        rpl::combine(std::move(creditsValue), std::move(currencyValue)) |
            rpl::map([](CreditsAmount credits, CreditsAmount currency) {
              auto creditsText =
                  (credits > CreditsAmount(0))
                      ...
                      : TextWithEntities();
              auto currencyText =
                  (currency > CreditsAmount(0))
                      ? Ui::Text::SingleCustomEmoji("_")
                            .append(QChar(' '))
                            .append(Info::ChannelEarn::MajorPart(currency))
                            .append(Info::ChannelEarn::MinorPart(currency))
                      : TextWithEntities();
              return currencyText.append(QChar(' '))
                  .append(std::move(creditsText));
            }),
        st, tr::lng_manage_peer_bot_balance(), context);

    object_ptr<FloatingIcon>(balance, st::menuIconEarn,
                             st::infoChannelAdminsIconPosition);
  }

  result->setDuration(st::infoSlideDuration)
      ->toggleOn(rpl::combine(std::move(membersShown), std::move(adminsShown)) |
...
void AddDetails(not_null<Ui::VerticalLayout *> container,
                not_null<Controller *> controller, not_null<PeerData *> peer,
                Data::ForumTopic *topic, Data::SavedSublist *sublist,
                Origin origin, Ui::MultiSlideTracker &mainTracker,
                rpl::variable<bool> &dividerOverridden) {
  if (topic) {
    container->add(SetupDetails(controller, container, topic, mainTracker,
                                dividerOverridden));
  } else if (sublist) {
    container->add(SetupDetails(controller, container, sublist, mainTracker,
                                dividerOverridden));
  } else {
    container->add(SetupDetails(controller, container, peer, origin,
                                mainTracker, dividerOverridden));
  }
}

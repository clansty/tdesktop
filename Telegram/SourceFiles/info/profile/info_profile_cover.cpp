...
TopicIconView::TopicIconView(not_null<Data::ForumTopic *> topic,
                             Fn<bool()> paused, Fn<void()> update)
    : TopicIconView(topic, std::move(paused), std::move(update),
                    st::windowSubTextFg) {}

TopicIconView::TopicIconView(not_null<Data::ForumTopic *> topic,
                             Fn<bool()> paused, Fn<void()> update,
                             const style::color &generalIconFg)
    : _topic(topic), _generalIconFg(generalIconFg), _paused(std::move(paused)),
...
void TopicIconView::paintInRect(QPainter &p, QRect rect, QColor textColor) {
  const auto paint = [&](const QImage &image) {
    ...
    p.drawImage(QRect(rect.x() + (rect.width() - size.width()) / 2,
                      rect.y() + (rect.height() - size.height()) / 2,
                      size.width(), size.height()),
                image);
  };
  if (_player && _player->ready()) {
    const auto colored = (textColor.alpha() > 0) ? textColor
                         : _playerUsesTextColor  ? st::windowFg->c
                                                 : QColor(0, 0, 0, 0);
    paint(_player
              ->frame(st::infoTopicCover.photo.size, colored, false, crl::now(),
                      _paused())
              .image);
  ...
  IconIdValue(topic) |
      ...
      rpl::map([=](DocumentData *document)
        ...
        return rpl::single() |
               ...
               rpl::filter([=] { return media->loaded(); }) | rpl::take(1) |
               rpl::map([=] {
                 ...
                 if (sticker->isLottie()) {
                   ...
                   result = std::make_shared<HistoryView::WebmPlayer>(
                       media->owner()->location(), media->bytes(),
                       st::infoTopicCover.photo.size);
                 } else {
...
void TopicIconView::setupImage(not_null<Data::ForumTopic *> topic) {
  ...
  if (topic->isGeneral()) {
    rpl::single(rpl::empty) | rpl::then(style::PaletteChanged()) |
        rpl::on_next(
            [=] {
              _image = ForumTopicGeneralIconFrame(st::infoForumTopicIcon.size,
                                                  _generalIconFg->c);
              _update();
            },
            _lifetime);
  ...
  rpl::combine(TitleValue(topic), ColorIdValue(topic)) |
...
TopicIconButton::TopicIconButton(QWidget *parent,
                                 not_null<Data::ForumTopic *> topic,
                                 Fn<bool()> paused)
    : AbstractButton(parent), _view(topic, paused, [=] { update(); }) {
...
Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller,
             not_null<PeerData *> peer,
             Fn<not_null<QWidget *>()> parentForTooltip)
    : Cover(parent, controller, peer, nullptr, Role::Info, NameValue(peer),
            parentForTooltip) {}

Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller,
             not_null<Data::ForumTopic *> topic)
    : Cover(parent, controller, topic->peer(), topic, Role::Info,
            TitleValue(topic), nullptr) {}

Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller,
             not_null<PeerData *> peer, Role role, rpl::producer<QString> title)
    : Cover(parent, controller, peer, nullptr, role, std::move(title),
            nullptr) {}

Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller,
             not_null<PeerData *> peer, Data::ForumTopic *topic, Role role,
             rpl::producer<QString> title,
             Fn<not_null<QWidget *>()> parentForTooltip)
    : FixedHeightWidget(parent, CoverStyle(peer, topic, role).height),
      _st(CoverStyle(peer, topic, role)), _role(role), _controller(controller),
      _peer(peer),
      _emojiStatusPanel(peer->isSelf() ? std::make_unique<EmojiStatusPanel>()
                                       : nullptr),
      _botVerify(role == Role::EditContact
                     ? nullptr
                     : std::make_unique<Badge>(
                           this, st::infoBotVerifyBadge, &peer->session(),
                           BotVerifyBadgeForPeer(peer), nullptr,
                           [=] {
      ...
      _badge(role == Role::EditContact
                 ? nullptr
                 : std::make_unique<Badge>(
                       this, st::infoPeerBadge, &peer->session(),
                       _badgeContent.value(), _emojiStatusPanel.get(),
                       [=] {
                         return controller->isGifPausedAtLeastFor(
                             Window::GifPauseReason::Layer);
                       })),
      _verified(role == Role::EditContact
                    ...
                    : std::make_unique<Badge>(
                          this, st::infoPeerBadge, &peer->session(),
                          VerifiedContentForPeer(peer), _emojiStatusPanel.get(),
                          [=] {
                            return controller->isGifPausedAtLeastFor(
                                Window::GifPauseReason::Layer);
                          })),
      ...
      _userpic(topic ? nullptr
                     : object_ptr<Ui::UserpicButton>(
                           this, controller, _peer->userpicPaintingPeer(),
                           Ui::UserpicButton::Role::OpenPhoto,
                           Ui::UserpicButton::Source::PeerPhoto, _st.photo,
                           _peer->userpicShape())),
      _changePersonal(
          (role == Role::Info || role == Role::EditContact || topic ||
           !_peer->isUser() || _peer->isSelf() || _peer->asUser()->isBot())
              ? nullptr
              : CreateUploadSubButton(this, _peer->asUser(), controller).get()),
      _iconButton(topic ? object_ptr<TopicIconButton>(this, controller, topic)
                        : nullptr),
      _name(this, _st.name),
      _starsRating(
          _peer->isUser() && _role != Role::EditContact
              ? std::make_unique<Ui::StarsRating>(
                    this, _controller->uiShow(),
  ...
  if (!_peer->isMegagroup()) {
    ...
    if (const auto rating = _starsRating.get()) {
      ...
      _statusShift.changes() |
          rpl::on_next([=] { refreshStatusGeometry(width()); },
                       _status->lifetime());
      rating->raise();
    }
  }
  ...
  if (_badge) {
    badgeUpdates = rpl::merge(std::move(badgeUpdates), _badge->updated());
  }
  if (_verified) {
    badgeUpdates = rpl::merge(std::move(badgeUpdates), _verified->updated());
  }
  if (_botVerify) {
    badgeUpdates = rpl::merge(std::move(badgeUpdates), _botVerify->updated());
  }
  if (_exteraBadge) {
    ...
    badgeUpdates = rpl::merge(std::move(badgeUpdates), _exteraBadge->updated());
  }
  std::move(badgeUpdates) |
      rpl::on_next([=] { refreshNameGeometry(width()); }, _name->lifetime());
  ...
  if (_userpic) {
    ...
    _iconButton->setClickedCallback([=] {
      _controller->show(Box(EditForumTopicBox, _controller, topic->history(),
                            topic->rootId()));
    });
  } else {
...
void Cover::setupShowLastSeen() {
  ...
  if (_st.showLastSeenVisible && user && !user->isSelf() && !user->isBot() &&
      !user->isServiceUser() && user->session().premiumPossible()) {
    ...
    rpl::combine(user->session().changes().peerFlagsValue(
        ...
        rpl::on_next(
            [=](auto, bool premium) {
              ...
              const auto shown =
                  ...
                  !premium && user->session().premiumPossible();
  ...
  _showLastSeen->setClickedCallback([=] {
    ...
    auto box = Box(
        Ui::ShowOrPremiumBox, type, user->shortName(),
        [=] {
  ...
  widthValue() |
      rpl::on_next(
          [this](int newWidth) {
            ...
            if (_changePersonal) {
              _changePersonal->moveToLeft((_st.photoLeft + _st.photo.photoSize -
                                           _changePersonal->width() +
                                           st::infoEditContactPersonalLeft),
                                          (_userpic->y() + _userpic->height() -
...
void Cover::initViewers(rpl::producer<QString> title) {
  ...
  _peer->session().changes().peerFlagsValue(_peer, Flag::OnlineStatus |
                                                       Flag::Members) |
      rpl::on_next(
          [=] {
  ...
  _peer->session().changes().peerFlagsValue(
      _peer, (_peer->isUser() ? Flag::IsContact : Flag::Rights)) |
      rpl::on_next([=] { refreshUploadPhotoOverlay(); }, lifetime());
...
void Cover::refreshUploadPhotoOverlay() {
  ...
  const auto canChange = [&] {
    if (const auto chat = _peer->asChat()) {
      ...
      return channel->canEditInformation() && !channel->isMonoforum();
    } else if (const auto user = _peer->asUser()) {
      return user->isSelf() || (user->isContact() && !user->isInaccessible() &&
                                !user->isServiceUser());
    }
  ...
  _userpic->switchChangePhotoOverlay(
      canChange, [=](Ui::UserpicButton::ChosenImage chosen) {
        ...
        case ChosenType::Set:
          ...
          _peer->session().api().peerPhoto().upload(_peer, std::move(result));
          break;
        case ChosenType::Suggest:
          _peer->session().api().peerPhoto().suggest(_peer, std::move(result));
          break;
        }
      });
  ...
  const auto canReport = [=, peer = _peer] {
    ...
    } else if (user->hasPersonalPhoto() || user->isSelf() ||
               user->isInaccessible() || user->isRepliesChat() ||
               user->isVerifyCodes() ||
  ...
  const auto contextMenu =
      _userpic->lifetime().make_state<base::unique_qptr<Ui::PopupMenu>>();
        ...
        [=] {
          controller->show(
              ReportProfilePhotoBox(
                  peer, peer->owner().photo(peer->userpicPhotoId())),
  ...
  base::install_event_filter(
      _userpic, [showMenu, raw = _userpic.data()](not_null<QEvent *> e) {
  ...
  _changePersonal->resetPersonalRequests() |
      rpl::on_next(
          [=] {
            ...
            _userpic->showSource(Ui::UserpicButton::Source::NonPersonalPhoto);
...
void Cover::refreshNameGeometry(int newWidth) {
  ...
  if (_verified) {
    _verified->move(badgeLeft + (badgeWidget ? badgeWidget->width() : 0),
                    badgeTop, badgeBottom);
  }
  if (_exteraBadge) {
    const auto exteraBadgeLeft =
        badgeLeft + (badgeWidget ? badgeWidget->width() : 0) +
        (badgeWidget && verifiedWidget ? st::infoVerifiedCheckPosition.x()
                                       : 0) +
...
void Cover::refreshStatusGeometry(int newWidth) {
  ...
  _showLastSeen->moveToLeft(left + _st.showLastSeenPosition.x(),
                            _st.showLastSeenPosition.y(), newWidth);
}
...
void Cover::hideBadgeTooltip() {
  ...
  if (auto old = base::take(_badgeTooltip)) {
    ...
    raw->fade(false);
    raw->shownValue() | rpl::filter(!rpl::mappers::_1) |
        rpl::on_next(
            [=] {
              const auto i = ranges::find(_badgeOldTooltips, raw,
...
void Cover::setupUniqueBadgeTooltip() {
  ...
  base::timer_once(kWaitBeforeGiftBadge) | rpl::then(_badge->updated()) |
      rpl::on_next(
          [=] {
            ...
            const auto id =
                (collectible && widget && premium) ? collectible->id : uint64();
            ...
            _badgeTooltip =
                std::make_unique<BadgeTooltip>(parent, collectible, widget);

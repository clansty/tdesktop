...
Memento::Memento(not_null<UserData *> self, Type type)
    : ContentMemento(Tag{self}), _type(type) {}

Section Memento::section() const { return Section(_type); }

object_ptr<ContentWidget>
Memento::createWidget(QWidget *parent, not_null<Controller *> controller,
...
Widget::Widget(QWidget *parent, not_null<Controller *> controller)
    : ContentWidget(parent, controller),
      ...
      _inner(setupFlexibleInnerWidget(
          _type->create(this, controller->parentController(), scroll(),
                        controller->wrapValue() | rpl::map([](Wrap wrap) {
  ...
  _inner->sectionShowOther() |
      rpl::on_next([=](Type type) { controller->showSettings(type); },
                   _inner->lifetime());

  _inner->sectionShowBack() |
      rpl::on_next([=] { controller->showBackFromStack(); },
                   _inner->lifetime());
  ...
  _removesFromStack.events() |
      rpl::on_next(
          [=](const std::vector<Type> &types) {
            const auto sections = ranges::views::all(types) |
                                  ranges::views::transform(
                                      [](Type type) { return Section(type); }) |
                                  ranges::to_vector;
  ...
  if (_pinnedToTop) {
    ...
    _pinnedToTop->heightValue() |
        rpl::on_next([=](int h) { setScrollTopSkip(h); },
                     _pinnedToTop->lifetime());
  }
  ...
  if (_pinnedToBottom) {
    const auto processHeight = [=] {
      ...
      _pinnedToBottom->moveToLeft(_pinnedToBottom->x(),
                                  height() - _pinnedToBottom->height());
    };

    _inner->sizeValue() | rpl::on_next(
                              [=](const QSize &s) {
    ...
    rpl::combine(_pinnedToBottom->heightValue(), heightValue()) |
        rpl::on_next(processHeight, _pinnedToBottom->lifetime());
  }

  if (_pinnedToTop && _pinnedToTop->minimumHeight() &&
      _inner->hasFlexibleTopBar()) {
    const auto heightDiff = [=] {
      return _pinnedToTop->maximumHeight() - _pinnedToTop->minimumHeight();
    };

    rpl::combine(_pinnedToTop->heightValue(), _inner->heightValue()) |
        rpl::on_next(
            [=](int, int h) {
    ...
    _flexibleScroll.fillerWidthValue.events() |
        rpl::on_next([=](int w) { _inner->resizeToWidth(w); },
                     _inner->lifetime());

    setPaintPadding({0, _pinnedToTop->minimumHeight(), 0, 0});

    setViewport(_pinnedToTop->events() | rpl::filter([](not_null<QEvent *> e) {
...
not_null<UserData *> Widget::self() const { return _self; }
...
void Widget::setInternalState(const QRect &geometry,
...
void Widget::setInnerFocus() { _inner->setInnerFocus(); }
...
rpl::producer<QString> Widget::title() { return _inner->title(); }
...
void Widget::enableBackButton() { _flexibleScroll.backButtonEnables.fire({}); }

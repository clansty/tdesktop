...
std::optional<SendPaymentDetails> ComputePaymentDetails(not_null<PeerData *> peer, int messagesCount) {
	...
	const auto has = (user && user->hasStarsPerMessage()) || (channel && channel->hasStarsPerMessage());
	if (!has) {
	...
	const auto known2 = user ? user->messageMoneyRestrictionsKnown() : channel->starsPerMessageKnown();
	if (!known2) {
		peer->updateFull();
	}
...
bool SuggestPaymentDataReady(not_null<PeerData *> peer, SuggestOptions suggest) {
...
object_ptr<Ui::BoxContent> MakeSendErrorBox(const Data::SendErrorWithThread &error, bool withTitle) {
	...
	if (withTitle) {
		text.append(tr::bold(error.thread->chatListName())).append("\n\n");
	}
...
void ShowSendPaidConfirm(not_null<Window::SessionNavigation *> navigation,
	...
	return ShowSendPaidConfirm(navigation->uiShow(), peer, details, confirmed, styles, suggestStarsPrice);
}
...
void ShowSendPaidConfirm(std::shared_ptr<Main::SessionShow> show,
						 ...
						 int suggestStarsPrice) {
	ShowSendPaidConfirm(
		std::move(show), std::vector<not_null<PeerData *>>{peer}, details, confirmed, styles, suggestStarsPrice);
}
...
void ShowSendPaidConfirm(std::shared_ptr<Main::SessionShow> show,
	...
	const auto check = [=]
	{
		...
		MaybeRequestBalanceIncrease(show,
									required,
									(suggestStarsPrice ? SmallBalanceSource(SmallBalanceForSuggest{singlePeerId})
				...
				box,
				{
					.text = (singlePeer ? tr::lng_payment_confirm_text(tr::now,
										...
										: (usersOnly ? tr::lng_payment_confirm_users : tr::lng_payment_confirm_chats)(
											  tr::now, lt_count, int(peers.size()), tr::rich))
								.append(' ')
								.append(tr::lng_payment_confirm_sure(
									...
									messages,
									lt_amount,
									tr::lng_payment_confirm_amount(tr::now, lt_count, stars, tr::rich),
									tr::rich)),
					.confirmed = proceed,
...
bool SendPaymentHelper::check(not_null<Window::SessionNavigation *> navigation,
							  ...
							  Fn<void(int)> resend,
							  PaidConfirmStyles styles) {
	return check(navigation->uiShow(), peer, options, messagesCount, std::move(resend), styles);
}
...
bool SendPaymentHelper::check(std::shared_ptr<Main::SessionShow> show,
	...
	const auto checkSuggestPriceStars = (admin || suggest.ton) ? 0 : int(base::SafeRound(suggest.price().value()));
	const auto checkSuggestPriceTon = (!admin && suggest.ton) ? suggest.price() : CreditsAmount();
	...
	if (!details || !suggestDetails) {
		...
		if ((!details || !suggest.ton) && !peer->session().credits().loaded()) {
		...
		if ((!suggestDetails && suggest.ton) && !peer->session().credits().tonLoaded()) {
			peer->session().credits().tonLoadedValue() | rpl::filter(rpl::mappers::_1) | rpl::take(1) |
				rpl::on_next(
					[=]
					{
						if (const auto callback = base::take(_resend)) {
		...
		ShowSendPaidConfirm(show, peer, *details, [=] { resend(stars); }, styles, checkSuggestPriceStars);
		return false;
	} else if (checkSuggestPriceStars &&
			   (CreditsAmount(details->stars + checkSuggestPriceStars) > peer->session().credits().balance())) {
		using namespace Settings;
		...
		{
			if (result == SmallBalanceResult::Success || result == SmallBalanceResult::Already) {
				resend(forMessages);
			}
		};
		MaybeRequestBalanceIncrease(show, required, SmallBalanceForSuggest{broadcastId}, done);
		return false;
	}
	const auto session = &peer->session();
	if (checkSuggestPriceTon && checkSuggestPriceTon > session->credits().tonBalance()) {
		using namespace HistoryView;
		show->show(Box(Ui::InsufficientTonBox, session, checkSuggestPriceTon));
		return false;
	}
	return true;
}
...
PeerId NewMessageFromId(const Api::SendAction &action) {
	return action.options.sendAs				   ? action.options.sendAs->id
		: action.history->peer->amMonoforumAdmin() ? action.history->peer->monoforumBroadcast()->id
...
ClickHandlerPtr
JumpToMessageClickHandler(not_null<HistoryItem *> item, FullMsgId returnToId, MessageHighlightId highlight) {
	return JumpToMessageClickHandler(item->history()->peer, item->id, returnToId, std::move(highlight));
}

ClickHandlerPtr
JumpToMessageClickHandler(not_null<PeerData *> peer, MsgId msgId, FullMsgId returnToId, MessageHighlightId highlight) {
		...
		[=](ClickContext context)
		{
			...
			if (const auto controller = my.sessionWindow.get()) {
				...
				if (session.premium()) {
					...
					session.sponsoredMessages().createReportCallback(my.itemId).callback(Result::Id("-1"),
																						 [](const auto &) {});
				} else {
...
MessageFlags FlagsFromMTP(MsgId id, MTPDmessage::Flags flags, MessageFlags localFlags) {
	...
	return localFlags | (IsServerMsgId(id) ? Flag::HistoryEntry : Flag()) |
		...
		((flags & MTP::f_paid_suggested_post_ton)		  ? Flag::TonPaidSuggested
			 : (flags & MTP::f_paid_suggested_post_stars) ? Flag::StarsPaidSuggested
														  : Flag()) |
		((flags & MTP::f_summary_from_language) ? Flag::CanBeSummarized : Flag()) |
		((flags & MTP::f_guestchat_via_from) ? Flag::GuestChatViaFrom : Flag());
}
...
MTPMessageReplyHeader NewMessageReplyHeader(const Api::SendAction &action) {
	if (const auto replyTo = action.replyTo) {
		...
		const auto topicPost = replyTo.topicRootId && (replyTo.topicRootId != Data::ForumTopic::kGeneralId);
		const auto quoteNormalized = reverseLocalPremiumEmoji(replyTo.quote, action.history, true);
		auto quoteEntities =
			Api::EntitiesToMTP(&action.history->session(), quoteNormalized.entities, Api::ConvertOption::SkipLocal);
		return MTP_messageReplyHeader(
			MTP_flags(Flag::f_reply_to_msg_id | (replyToTop ? Flag::f_reply_to_top_id : Flag()) |
					  ...
					  (quoteNormalized.empty() ? Flag() : (Flag::f_quote | Flag::f_quote_text | Flag::f_quote_offset)) |
					  (quoteEntities.v.empty() ? Flag() : Flag::f_quote_entities) |
					  (replyTo.todoItemId ? Flag::f_todo_item_id : Flag()) |
					  (replyTo.pollOption.isEmpty() ? Flag() : Flag::f_poll_option) |
...
MediaCheckResult CheckMessageMedia(const MTPMessageMedia &media) {
	...
	return media.match([](const MTPDmessageMediaEmpty &) { return Result::Good; },
					   ...
					   [](const MTPDmessageMediaToDo &) { return Result::Good; },
					   ...
					   [](const MTPDmessageMediaVideoStream &) { return Result::Good; },
...
[[nodiscard]] CallId CallIdFromInput(const MTPInputGroupCall &data) {
	return data.match([&](const MTPDinputGroupCall &data) { return data.vid().v; },
					  [](const auto &) -> CallId { Unexpected("slug/msg in CallIdFromInput."); });
}
...
PreparedServiceText GenerateJoinedText(not_null<History *> history, not_null<UserData *> inviter, bool viaRequest) {
	if (inviter->id != history->session().userPeerId()) {
		...
		result.text = (history->peer->isMegagroup() ? tr::lng_action_add_you_group : tr::lng_action_add_you)(
			tr::now, lt_from, tr::link(inviter->name(), QString()), tr::marked);
		return result;
	} else if (history->peer->isMegagroup()) {
		if (viaRequest) {
			return {tr::lng_action_you_joined_by_request(tr::now, tr::marked)};
		}
		auto self = history->session().user();
		auto result = PreparedServiceText();
		result.links.push_back(self->createOpenLink());
		result.text = tr::lng_action_user_joined(tr::now, lt_from, tr::link(self->name(), QString()), tr::marked);
		return result;
	}
	return {viaRequest ? tr::lng_action_you_joined_by_request_channel(tr::now, tr::marked)
...
void CheckReactionNotificationSchedule(not_null<HistoryItem *> item, const OnStackUsers &wasUsers) {
	...
	const auto from = item->history()->session().api().reactionsNotifySettings().messagesFromCurrent();
	...
	for (const auto &[emoji, reactions] : item->recentReactions()) {
		for (const auto &reaction : reactions) {
			...
			if (from == Api::ReactionsNotifyFrom::Contacts && !user->isContact()) {
				continue;
			}
...
void CheckPollVoteNotificationSchedule(not_null<HistoryItem *> item,
	...
	const auto from = item->history()->session().api().reactionsNotifySettings().pollVotesFromCurrent();
	if (from == Api::ReactionsNotifyFrom::None) {
		return;
	}
...
[[nodiscard]] TextWithEntities UnsupportedMessageText() {
	...
	auto result =
		TextWithEntities{tr::lng_message_unsupported(tr::now, lt_link, siteLink).replace("Telegram", "AyuGram")};
...
void ShowTrialTranscribesToast(int left, TimeId until) {
	...
	const auto text = left
		? tr::lng_audio_transcribe_trials_left(tr::now, lt_count, left, lt_date, {date}, tr::marked)
	...
	for (const auto &item : list) {
		if (const auto media = item->media()) {
			const auto hasCaption =
				!item->originalText().text.isEmpty() || !media->consumedMessageText().text.isEmpty();
			if (hasCaption && (media->allowsEditCaption() || media->poll())) {
				++result;
			}
		}
	}

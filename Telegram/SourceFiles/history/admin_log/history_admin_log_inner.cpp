...
void InnerWidget::enumerateItems(Method method) {
      ...
      TopToBottom
          ? std::lower_bound(begin, end, _visibleTop,
                             [this](auto &elem, int top) {
                               return this->itemTop(elem) + elem->height() <=
                                      top;
                             })
          : std::upper_bound(
                begin, end, _visibleBottom, [this](int bottom, auto &elem) {
  ...
  while (true) {
    ...
    // Binary search should've skipped all the items that are above / below the
    // visible area.
    if (TopToBottom) {
...
template <typename Method> void InnerWidget::enumerateUserpics(Method method) {
  // Find and remember the top of an attached messages pack
  // -1 means we didn't find an attached to next message yet.
  int lowestAttachedItemTop = -1;

  auto userpicCallback = [&](not_null<Element *> view, int itemtop,
                             int itembottom) {
    ...
    // Call method on a userpic for all messages that have it and for those who
    // are not showing it because of their attachment to the next message if
    // they are bottom-most visible.
    if (view->displayFromPhoto() ||
        (view->hasFromPhoto() && itembottom >= _visibleBottom)) {
      if (lowestAttachedItemTop < 0) {
        lowestAttachedItemTop = itemtop + view->marginTop();
      }
      // Attach userpic to the bottom of the visible area with the same margin
      // as the last message.
      auto userpicMinBottomSkip =
          st::historyPaddingBottom + st::msgMargin.bottom();
      auto userpicBottom = qMin(itembottom - view->marginBottom(),
                                _visibleBottom - userpicMinBottomSkip);

      // Do not let the userpic go above the attached messages pack top line.
      userpicBottom =
          qMax(userpicBottom, lowestAttachedItemTop + st::msgPhotoSize);
...
template <typename Method> void InnerWidget::enumerateDates(Method method) {
  // Find and remember the bottom of an single-day messages pack
  // -1 means we didn't find a same-day with previous message yet.
  auto lowestInOneDayItemBottom = -1;

  auto dateCallback = [&](not_null<Element *> view, int itemtop,
                          int itembottom) {
    ...
    // Call method on a date for all messages that have it and for those who are
    // not showing it because they are in a one day together with the previous
    // message if they are top-most visible.
    if (view->displayDate() || (!item->isEmpty() && itemtop <= _visibleTop)) {
      if (lowestInOneDayItemBottom < 0) {
        lowestInOneDayItemBottom = itembottom - view->marginBottom();
      }
      // Attach date to the top of the visible area with the same margin as it
      // has in service message.
      ...
      auto dateHeight = st::msgServicePadding.bottom() +
    ...
    // Forget the found bottom of the pack, search for the next one from
    // scratch.
...
InnerWidget::InnerWidget(QWidget *parent,
    ...
    : RpWidget(parent), _controller(controller), _channel(channel),
      _history(channel->owner().history(channel)),
      _api(&_channel->session().mtp()),
      _pathGradient(HistoryView::MakePathShiftGradient(controller->chatStyle(),
                                                       [=] { update(); })),
      _scrollDateCheck([=] { scrollDateCheck(); }),
      _emptyText(st::historyAdminLogEmptyWidth -
  ...
  Window::ChatThemeValueFromPeer(controller, channel) |
      rpl::on_next(
  ...
  session().data().viewRepaintRequest() |
      rpl::on_next(
          [=](Data::RequestViewRepaint data) {
            if (myView(data.view)) {
              repaintItem(data.view, data.rect);
            }
          },
          lifetime());
  session().data().viewResizeRequest() | rpl::on_next(
                                             [=](auto view) {
                                               if (myView(view)) {
                                                 resizeItem(view);
                                               }
  ...
  session().data().itemViewRefreshRequest() |
      rpl::on_next(
          ...
          lifetime());
  session().data().viewLayoutChanged() | rpl::on_next(
                                             [=](auto view) {
                                               if (myView(view)) {
  ...
  session().data().itemVisibilityQueries() |
      rpl::filter([=](const Data::Session::ItemVisibilityQuery &query) {
        return (_history == query.item->history()) &&
               query.item->isAdminLogEntry() && isVisible();
      }) |
      rpl::on_next(
          [=](const Data::Session::ItemVisibilityQuery &query) {
            if (const auto view = viewForItem(query.item)) {
              auto top = itemTop(view);
              if (top >= 0 && top + view->height() > _visibleTop &&
                  top < _visibleBottom) {
  ...
  controller->adaptive().chatWideValue() |
      rpl::on_next([=](bool wide) { _isChatWide = wide; }, lifetime());

  updateEmptyText();

  _antiSpamValidator.resolveUser(crl::guard(this, [=] { requestAdmins(); }));
}
...
void InnerWidget::scrollDateCheck() {
  if (!_visibleTopItem) {
    ...
    scrollDateHide();
  } else if (_visibleTopItem != _scrollDateLastItem ||
             _visibleTopFromItem != _scrollDateLastItemTop) {
    // Show scroll date only if it is not the initial onScroll() event (with
    // empty _scrollDateLastItem).
...
void InnerWidget::toggleScrollDateShown() {
  ...
  _scrollDateOpacity.start([this] { repaintScrollDateCallback(); }, from, to,
                           st::historyDateFadeDuration);
}

void InnerWidget::repaintScrollDateCallback() {
  auto updateTop = _visibleTop;
  auto updateHeight = st::msgServiceMargin.top() + st::msgServicePadding.top() +
                      st::msgServiceFont->height +
...
void InnerWidget::requestAdmins() {
  ...
  _api.request(MTPchannels_GetParticipants(
                   _channel->inputChannel(), MTP_channelParticipantsAdmins(),
                   MTP_int(offset), MTP_int(kMaxChannelAdmins),
        ...
        result.match(
            [&](const MTPDchannels_channelParticipants &data) {
              ...
              for (const auto &parsed : list) {
                if (parsed.isUser()) {
                  const auto user =
                      _channel->owner().userLoaded(parsed.userId());
                  if (user) {
                    _admins.emplace_back(user);
...
void InnerWidget::showFilter(Fn<void(FilterValue &&filter)> callback) {
  ...
  _controller->uiShow()->show(Box([=](not_null<Ui::GenericBox *> box) {
    ...
    Ui::AddSubsectionTitle(box->verticalLayout(),
                           tr::lng_admin_log_filter_actions_type_subtitle());
    const auto collectFlags =
        FillFilterValueList(box->verticalLayout(), isChannel, filter);
    ...
    auto checkedPeerId = std::vector<PeerId>();
    {
      ...
      for (const auto &user : admins) {
        if (!filter.admins ||
            ranges::contains((*filter.admins), user->id, &UserData::id)) {
          checkedPeerId.push_back(user->id);
        }
      }
    }

    const auto checkbox = box->addRow(object_ptr<Ui::Checkbox>(
        box->verticalLayout(),
        tr::lng_admin_log_filter_actions_admins_section(tr::now, tr::marked),
        checkedPeerId.size() == admins.size(), st::defaultBoxCheckbox));
    ...
    Ui::AddExpandablePeerList(checkbox, controller, box->verticalLayout());

    box->addButton(tr::lng_settings_save(), [=] {
      const auto peers = controller->collectRequests();
      const auto users = ranges::views::all(peers) |
      ...
      callback(FilterValue{
          .flags = collectFlags(),
          .admins = (admins.size() == users.size()) ? std::nullopt
                                                    : std::optional(users),
      });
    });
...
void InnerWidget::updateEmptyText() {
  ...
  auto description = hasSearch   ? tr::lng_admin_log_no_results_search_text(
                                       tr::now, lt_query, _searchQuery)
                     : hasFilter ? tr::lng_admin_log_no_results_text(tr::now)
    ...
    if (const auto view = Element::Hovered()) {
      ...
      if (sentIt != end(_itemDates)) {
        dateText +=
            '\n' + tr::lng_sent_date(
                       tr::now, lt_date,
                       QLocale().toString(base::unixtime::parse(sentIt->second),
                                          QLocale::LongFormat));
      }
      return dateText;
    }
...
InnerWidget::elementInSelectionMode(const HistoryView::Element *) {
  return {};
}

bool InnerWidget::elementIntersectsRange(not_null<const Element *> view,
                                         int from, int till) {
...
void InnerWidget::elementShowPollResults(not_null<PollData *> poll,
                                         FullMsgId context) {}

void InnerWidget::elementShowAddPollOption(
    not_null<HistoryView::Element *> view, not_null<PollData *> poll,
    FullMsgId context, QRect optionRect) {}

void InnerWidget::elementSubmitAddPollOption(FullMsgId context) {}

void InnerWidget::elementOpenPhoto(not_null<PhotoData *> photo,
                                   FullMsgId context) {
  _controller->openPhoto(photo, {context});
}

void InnerWidget::elementOpenDocument(not_null<DocumentData *> document,
                                      FullMsgId context, bool showInMediaView) {
...
void InnerWidget::elementSendBotCommand(const QString &command,
                                        const FullMsgId &context) {}

void InnerWidget::elementSearchInList(const QString &query,
                                      const FullMsgId &context) {}
...
void InnerWidget::elementStartEffect(not_null<const Element *> view,
                                     Element *replacing) {}
...
void InnerWidget::preloadMore(Direction direction) {
  auto &requestId = (direction == Direction::Up) ? _preloadUpRequestId
  ...
  const auto filter = [&] {
    ...
    return empty | ((f & LocalFlag::Join) ? Flag::f_join : empty) |
  ...
  requestId =
      _api.request(MTPchannels_GetAdminLog(
                       MTP_flags(flags), _channel->inputChannel(),
                       MTP_string(_searchQuery),
                       MTP_channelAdminLogEventsFilter(MTP_flags(filter)),
                       MTP_vector<MTPInputUser>(admins), MTP_long(maxId),
                       MTP_long(minId), MTP_int(perPage)))
          .done([=, &requestId,
                 &loadedFlag](const MTPchannels_AdminLogResults &result) {
...
void InnerWidget::addEvents(Direction direction,
  ...
  // When loading items down we add them to a new vector and copy _items after
  // them.
  auto newItemsForDownDirection = std::vector<OwnedItem>();
  auto oldItemsCount = _items.size();
  auto &addToItems =
      (direction == Direction::Up) ? _items : newItemsForDownDirection;
  ...
  auto newItemsCount =
      _items.size() +
      ((direction == Direction::Up) ? 0 : newItemsForDownDirection.size());
...
void InnerWidget::paintEvent(QPaintEvent *e) {
  ...
  if (_items.empty() && _upLoaded && _downLoaded) {
    ...
    auto to = std::lower_bound(begin, end, clip.top() + clip.height(),
    ...
    if (from != end) {
      ...
      enumerateUserpics([&](not_null<Element *> view, int userpicTop) {
        ...
        if (userpicTop + st::msgPhotoSize > clip.top()) {
          ...
          from->paintUserpicLeft(p, _userpics[from], st::historyPhotoLeft,
                                 userpicTop, view->width(), st::msgPhotoSize);
        }
        return true;
      });
      ...
      enumerateDates([&](not_null<Element *> view, int itemtop, int dateTop) {
        ...
        if (dateTop < clip.top() + clip.height()) {
          auto opacity =
              (dateInPlace /* || noFloatingDate*/) ? 1. : scrollDateOpacity;
          if (opacity > 0.) {
...
void InnerWidget::paintEmpty(Painter &p, not_null<const Ui::ChatStyle *> st) {
  ...
  auto innerWidth = rectWidth - st::historyAdminLogEmptyPadding.left() -
                    st::historyAdminLogEmptyPadding.right();
  auto rectHeight = st::historyAdminLogEmptyPadding.top() +
                    _emptyText.countHeight(innerWidth) +
                    st::historyAdminLogEmptyPadding.bottom();
  auto rect = QRect((width() - rectWidth) / 2, (height() - rectHeight) / 3,
                    rectWidth, rectHeight);
  HistoryView::ServiceMessagePainter::PaintBubble(p, st, rect);

  p.setPen(st->msgServiceFg());
  _emptyText.draw(p, rect.x() + st::historyAdminLogEmptyPadding.left(),
                  rect.y() + st::historyAdminLogEmptyPadding.top(), innerWidth,
                  style::al_top);
}

TextForMimeData InnerWidget::getSelectedText() const {
  return _selectedItem ? _selectedItem->selectedText(_selectedText)
                       : TextForMimeData();
}
...
void InnerWidget::mouseDoubleClickEvent(QMouseEvent *e) {
  ...
  if (((_mouseAction == MouseAction::Selecting && _selectedItem != nullptr) ||
       (_mouseAction == MouseAction::None)) &&
...
void InnerWidget::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
  ...
  if (_selectedItem) {
    ...
    if (Element::Moused() && Element::Moused() == Element::Hovered()) {
      auto mousePos =
          mapPointToItem(mapFromGlobal(_mousePosition), Element::Moused());
      StateRequest request;
      request.flags |= Ui::Text::StateRequest::Flag::LookupSymbol;
  ...
  const auto lnkPhoto =
      link ? reinterpret_cast<PhotoData *>(
  ...
  const auto fromId =
      PeerId(link ? link->property(kPeerLinkPeerIdProperty).toULongLong() : 0);
  if (lnkPhoto || lnkDocument) {
    if (isUponSelected > 0) {
      _menu->addAction(
          tr::lng_context_copy_selected(tr::now), [=] { copySelectedText(); },
          &st::menuIconCopy);
    }
    if (lnkPhoto) {
      ...
      if (!lnkPhoto->isNull() && media && media->loaded()) {
        _menu->addAction(
            ...
            base::fn_delayed(st::defaultDropdownMenu.menu.ripple.hideDuration,
                             this, [=] { savePhotoToFile(lnkPhoto); }),
            &st::menuIconSaveImage);
        _menu->addAction(
            tr::lng_context_copy_image(tr::now),
            [=] { copyContextImage(lnkPhoto); }, &st::menuIconCopy);
      }
      if (lnkPhoto->hasAttachedStickers()) {
        ...
        _menu->addAction(tr::lng_context_attached_stickers(tr::now),
                         std::move(callback), &st::menuIconStickers);
      }
    } else {
      if (lnkDocument->loading()) {
        _menu->addAction(
            tr::lng_context_cancel_download(tr::now),
            [=] { cancelContextDownload(lnkDocument); }, &st::menuIconCancel);
      } else {
          ...
          const auto notAutoplayedGif = [&] {
            return lnkDocument->isGifv() &&
                   !Data::AutoDownload::ShouldAutoPlay(
                       ...
                       item->history()->peer, lnkDocument);
          }();
          if (notAutoplayedGif) {
            _menu->addAction(
                tr::lng_context_open_gif(tr::now),
                [=] { openContextGif(itemId); }, &st::menuIconShowInChat);
          }
        }
        ...
        _menu->addAction(
            ...
            base::fn_delayed(
                st::defaultDropdownMenu.menu.ripple.hideDuration, this,
    ...
    if (isUponSelected > 0) {
      _menu->addAction(
          ...
          [this] { copySelectedText(); }, &st::menuIconCopy);
      if (item && !Ui::SkipTranslate(getSelectedText().rich)) {
        const auto peer = item->history()->peer;
        _menu->addAction(
            tr::lng_context_translate_selected({}),
            [=] {
              _controller->show(Box(Ui::TranslateBox, peer, MsgId(),
                                    getSelectedText().rich, false));
            },
      ...
      if (item && !isUponSelected) {
        ...
        if (!item->isService() && !link &&
          ...
          _menu->addAction(
              ...
              [=] { copyContextText(itemId); }, &st::menuIconCopy);
        }
    ...
    const auto actionText =
        link ? link->copyToClipboardContextItemText() : QString();
...
void InnerWidget::savePhotoToFile(not_null<PhotoData *> photo) {
  ...
  FileDialog::GetWritePath(this, tr::lng_save_photo(tr::now), filter,
...
void InnerWidget::saveDocumentToFile(not_null<DocumentData *> document) {
  DocumentSaveClickHandler::Save(Data::FileOrigin(), document,
...
void InnerWidget::suggestRestrictParticipant(not_null<PeerData *> participant,
                                             FullMsgId realId) {
  ...
  _menu->addAction(
      ...
      [=] {
        ...
        auto editRestrictions = [=](bool hasAdminRights,
                                    ChatRestrictionsInfo currentRights,
                                    UserData *by, TimeId since) {
          auto weak = QPointer<InnerWidget>(this);
          auto weakBox = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
          auto box =
              Box<EditRestrictedBox>(_channel, user, hasAdminRights,
                                     currentRights, QString(), by, since);
          box->setSaveCallback([=](ChatRestrictionsInfo oldRights,
        ...
        if (!user) {
          const auto text =
              (_channel->isBroadcast() ? tr::lng_profile_sure_kick_channel
                                       : tr::lng_profile_sure_kick)(
                  tr::now, lt_user, participant->name());
          auto weakBox = std::make_shared<base::weak_qptr<Ui::BoxContent>>();
          const auto sure = crl::guard(this, [=] {
            restrictParticipant(
                participant, ChatRestrictionsInfo(),
          ...
          *weakBox = _controller->show(Ui::MakeConfirmBox({text, sure}));
          ...
          _api.request(MTPchannels_GetParticipant(_channel->inputChannel(),
                                                  user->input()))
              .done([=](const MTPchannels_ChannelParticipant &result) {
                user->owner().processUsers(result.data().vusers());

                const auto participant =
                    Api::ChatParticipant(result.data().vparticipant(), user);
                ...
                if (participant.type() == Type::Creator ||
                  ...
                  editRestrictions(false, participant.restrictions(),
                                   user->owner().user(participant.by()), since);
                }
              })
              .fail([=] { editRestrictions(false, {}, nullptr, 0); })
              .send();
        }
      },
...

  {
    ...
    auto handler = [=, this] {
      participant->session().changes().peerUpdates(
          _channel, Data::PeerUpdate::Flag::Members) |
          rpl::on_next(
      ...
      participant->session().api().chatParticipants().kick(
          _channel, participant, {_channel->restrictions(), 0});
    };
...
void InnerWidget::restrictParticipant(not_null<PeerData *> participant,
  ...
  const auto callback =
      SaveRestrictedCallback(_controller->uiShow(), _channel, participant,
                             crl::guard(this, done), nullptr);
  callback(oldRights, newRights);
}

void InnerWidget::restrictParticipantDone(not_null<PeerData *> participant,
                                          ChatRestrictionsInfo rights) {
  if (rights.flags) {
    _admins.erase(std::remove(_admins.begin(), _admins.end(), participant),
                  _admins.end());
    _adminsCanEdit.erase(
        std::remove(_adminsCanEdit.begin(), _adminsCanEdit.end(), participant),
        _adminsCanEdit.end());
  }
  _downLoaded = false;
  checkPreloadMore();
...
bool InnerWidget::canRestrict() const {
  return _channel->isMegagroup() && _channel->canBanMembers() &&
         !_admins.empty();
}
...
void InnerWidget::mouseActionStart(const QPoint &screenPos,
  ...
  _dragStartPosition =
      mapPointToItem(mapFromGlobal(screenPos), _mouseActionItem);
  ...
  if (_mouseAction == MouseAction::None && _mouseActionItem) {
    ...
    if (_trippleClickTimer.isActive() &&
        (screenPos - _trippleClickPoint).manhattanLength() <
      ...
      if (Element::Pressed()) {
        ...
        if (uponSelected) {
          if (!_selectedItem || _selectedItem != _mouseActionItem) {
            uponSelected = false;
          } else if (_mouseTextSymbol < _selectedText.from ||
                     _mouseTextSymbol >= _selectedText.to) {
            uponSelected = false;
          }
        }
        if (uponSelected) {
...
void InnerWidget::mouseActionFinish(const QPoint &screenPos,
                                    Qt::MouseButton button) {
  mouseActionUpdate(screenPos);
  ...
  if (activated) {
    ...
    ActivateClickHandler(
        window(), activated,
        {button, QVariant::fromValue(ClickHandlerContext{
                     .elementDelegate =
  ...
  if (_mouseAction == MouseAction::PrepareDrag && !_pressWasInactive &&
  ...
  if (QGuiApplication::clipboard()->supportsSelection() && _selectedItem &&
      _selectedText.from != _selectedText.to) {
    TextUtilities::SetClipboardText(_selectedItem->selectedText(_selectedText),
...
void InnerWidget::updateSelected() {
  ...
  auto point =
      QPoint(std::clamp(mousePosition.x(), 0, width()),
  ...
  auto selectingText = _selectedItem && (view == _mouseActionItem) &&
                       (view == Element::Hovered());
  if (view) {
    if (view != _mouseActionItem ||
        (itemPoint - _dragStartPosition).manhattanLength() >=
            QApplication::startDragDistance()) {
    ...
    if (!dragState.link && itemPoint.x() >= st::historyPhotoLeft &&
      ...
      if (!item->isService() && view->hasFromPhoto()) {
        enumerateUserpics([&](not_null<Element *> view, int userpicTop) {
          ...
          if (point.y() >= userpicTop &&
              point.y() < userpicTop + st::msgPhotoSize) {
  ...
  if (dragState.link || dragState.cursor == CursorState::Date ||
...
void InnerWidget::performDrag() {
  ...
  //{ 		uponSelected = _selected.contains(_mouseActionItem); 	} else { 		StateRequest
  //request; 		request.flags |= Ui::Text::StateRequest::Flag::LookupSymbol; 		auto
  //dragState = _mouseActionItem->textState(_dragStartPosition.x(),
  //_dragStartPosition.y(), request); 		uponSelected = (dragState.cursor ==
  //CursorState::Text); 		if (uponSelected) { 			if (_selected.isEmpty() ||
  ...
  //			} else {
  //				uint16 selFrom =
  //_selected.cbegin().value().from, selTo = _selected.cbegin().value().to; 				if
  //(dragState.symbol < selFrom || dragState.symbol >= selTo) { 					uponSelected =
  //false;
  //				}
  //			}
  //		}
  ...
  //	//if (!sel.isEmpty() && sel.at(0) != '/' && sel.at(0) != '@' &&
  //sel.at(0) != '#') {
  ...
  //		if (selectedState.count > 0 && selectedState.count ==
  //selectedState.canForwardCount) {
  ...
  //				&&
  //pressedMedia->dragItemByHandler(pressedHandler)) { 				forwardMimeType =
  //u"application/x-td-forward"_q; 				session().data().setMimeForwardIds( 					{ 1,
  //pressedLnkItem->fullId() });
  //			}
  //		}
  //	}
  //	if (!forwardMimeType.isEmpty()) {
  ...
  //		if (auto document = (pressedMedia ? pressedMedia->getDocument()
  //: nullptr)) { 			auto filepath = document->filepath(true); 			if
  //(!filepath.isEmpty()) { 				QList<QUrl> urls;
...
void InnerWidget::onTouchScrollTimer() {
  ...
  if (_touchScrollState == Ui::TouchScrollState::Acceleration &&
      _touchWaitingAcceleration && (nowTime - _touchAccelerationTime) > 40) {
...
void InnerWidget::touchUpdateSpeed() {
  ...
  if (_touchPrevPosValid) {
    ...
    if (elapsed) {
      ...
      const int newSpeedY =
          (qAbs(pixelsPerSecond.y()) > Ui::kFingerAccuracyThreshold)
              ? pixelsPerSecond.y()
              : 0;
      const int newSpeedX =
          (qAbs(pixelsPerSecond.x()) > Ui::kFingerAccuracyThreshold)
              ? pixelsPerSecond.x()
              : 0;
      if (_touchScrollState == Ui::TouchScrollState::Auto) {
        ...
        if ((oldSpeedY <= 0 && newSpeedY <= 0) ||
            ...
            (oldSpeedX >= 0 && newSpeedX >= 0)) {
          _touchSpeed.setY(std::clamp((oldSpeedY + (newSpeedY / 4)),
                                      -Ui::kMaxScrollAccelerated,
                                      +Ui::kMaxScrollAccelerated));
          _touchSpeed.setX(std::clamp((oldSpeedX + (newSpeedX / 4)),
                                      -Ui::kMaxScrollAccelerated,
                                      +Ui::kMaxScrollAccelerated));
        } else {
          _touchSpeed = QPoint();
        ...
        if (!_touchSpeed.isNull()) {
          _touchSpeed.setX(
              std::clamp((_touchSpeed.x() / 4) + (newSpeedX * 3 / 4),
                         -Ui::kMaxScrollFlick, +Ui::kMaxScrollFlick));
          _touchSpeed.setY(
              std::clamp((_touchSpeed.y() / 4) + (newSpeedY * 3 / 4),
                         -Ui::kMaxScrollFlick, +Ui::kMaxScrollFlick));
        } else {
...
void InnerWidget::touchDeaccelerate(int32 elapsed) {
  ...
  _touchSpeed.setX((x == 0)  ? x
                   : (x > 0) ? qMax(0, x - elapsed)
                             : qMin(0, x + elapsed));
  _touchSpeed.setY((y == 0)  ? y
                   : (y > 0) ? qMax(0, y - elapsed)
                             : qMin(0, y + elapsed));
}
...
void InnerWidget::touchEvent(QTouchEvent *e) {
  ...
  case QEvent::TouchUpdate: {
    ...
    } else if (!_touchScroll && (_touchPos - _touchStart).manhattanLength() >=
                                    QApplication::startDragDistance()) {
      _touchSelectTimer.cancel();
      _touchScroll = true;
      touchUpdateSpeed();
    ...
    if (_touchSelect) {
      if (notMoved) {
        ...
        auto contextMenu = QContextMenuEvent(
            QContextMenuEvent::Mouse, mapFromGlobal(_touchPos), _touchPos);

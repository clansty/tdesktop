...
[[nodiscard]] TextWithEntities SpoilerLoginCode(TextWithEntities text) {
  const auto r =
      QRegularExpression(u"(?<![\\w\\-#])(\\d[\\d\\-]{2,6}\\d)(?!\\w\\-)"_q);
...
[[nodiscard]] HistoryItemCommonFields
ForwardedFields(HistoryItemCommonFields fields, not_null<History *> history,
...
[[nodiscard]] TextWithEntities AmountAndStarCurrency(int64 amount,
...
void HistoryItem::FillForwardedInfo(CreateConfig &config,
  ...
  config.savedFromSenderId =
      data.vsaved_from_id() ? peerFromMTP(*data.vsaved_from_id()) : PeerId();
  config.savedFromSenderName = qs(data.vsaved_from_name().value_or_empty());
...
std::unique_ptr<Data::Media>
HistoryItem::CreateMedia(not_null<HistoryItem *> item,
                         const MTPMessageMedia &media) {
  using Result = std::unique_ptr<Data::Media>;
  return media.match(
      [&](const MTPDmessageMediaContact &media) -> Result {
        return std::make_unique<Data::MediaContact>(
            item, media.vuser_id().v, qs(media.vfirst_name()),
            qs(media.vlast_name()), qs(media.vphone_number()),
            Data::SharedContact::ParseVcard(qs(media.vvcard())));
      },
      [&](const MTPDmessageMediaGeo &media) -> Result {
        return media.vgeo().match(
            [&](const MTPDgeoPoint &point) -> Result {
              return std::make_unique<Data::MediaLocation>(
                  item, Data::LocationPoint(point));
            },
            [](const MTPDgeoPointEmpty &) -> Result { return nullptr; });
      },
      [&](const MTPDmessageMediaGeoLive &media) -> Result {
        return media.vgeo().match(
            [&](const MTPDgeoPoint &point) -> Result {
              return std::make_unique<Data::MediaLocation>(
                  item, Data::LocationPoint(point), media.vperiod().v);
            },
            [](const MTPDgeoPointEmpty &) -> Result { return nullptr; });
      },
      [&](const MTPDmessageMediaVenue &media) -> Result {
        return media.vgeo().match(
            [&](const MTPDgeoPoint &point) -> Result {
              return std::make_unique<Data::MediaLocation>(
                  item, Data::LocationPoint(point), qs(media.vtitle()),
                  qs(media.vaddress()));
            },
            [](const MTPDgeoPointEmpty &data) -> Result { return nullptr; });
      },
        ...
        return photo->match(
            [&](const MTPDphoto &photo) -> Result {
              return std::make_unique<Data::MediaPhoto>(
                  item, item->history()->owner().processPhoto(photo),
                  media.is_spoiler());
            },
      ...
      [&](const MTPDmessageMediaDocument &media) -> Result {
        ...
        return document->match(
            [&](const MTPDdocument &document) -> Result {
      ...
      [&](const MTPDmessageMediaWebPage &media) -> Result {
        ...
        const auto flags =
            Flag() |
            (media.is_force_large_media() ? Flag::ForceLargeMedia : Flag()) |
            (media.is_force_small_media() ? Flag::ForceSmallMedia : Flag()) |
            (media.is_manual() ? Flag::Manual : Flag()) |
            (media.is_safe() ? Flag::Safe : Flag());
        return media.vwebpage().match(
            [](const MTPDwebPageEmpty &) -> Result { return nullptr; },
            [&](const MTPDwebPagePending &webpage) -> Result {
              return std::make_unique<Data::MediaWebPage>(
                  item, item->history()->owner().processWebpage(webpage),
                  flags);
            },
      ...
      [&](const MTPDmessageMediaDice &media) -> Result {
        ...
        return std::make_unique<Data::MediaDice>(
            item, outcome, qs(media.vemoticon()), media.vvalue().v);
      },
      ...
      [](const MTPDmessageMediaEmpty &) -> Result { return nullptr; },
      [](const MTPDmessageMediaUnsupported &) -> Result { return nullptr; });
}

HistoryItem::HistoryItem(not_null<History *> history, MsgId id,
                         const MTPDmessage &data, MessageFlags localFlags)
    : HistoryItem(
          history,
          {
              .id = id,
  ...
  const auto checked =
      media ? CheckMessageMedia(*media) : MediaCheckResult::Good;
  if (checked == MediaCheckResult::Unsupported) {
    ...
    if ((GetEnhancedBool("blocked_user_spoiler_mode") &&
         blockExist(peerId.value)) ||
        (GetEnhancedBool("blocked_user_spoiler_mode") && user &&
         user->isBlocked())) {
      isBlocked = true;
    }
    ...
    if (GetEnhancedBool("blocked_user_spoiler_mode")) {
      _blockMsg = TextWithEntities{
          msg, Api::EntitiesFromMTP(
                   &history->session(), data.ventities().value_or_empty(),
                   blkMsg.length(), qs(data.vmessage()).length())};

      _originalMsg = TextWithEntities{
          qs(data.vmessage()),
          Api::EntitiesFromMTP(&history->session(),
    ...
    : HistoryItem(history,
                  {
                      .id = id,
                      .flags = FlagsFromMTP(id, data.vflags().v, localFlags),
                      .from = data.vfrom_id() ? peerFromMTP(*data.vfrom_id())
                                              : PeerId(0),
                      .date = data.vdate().v,
                  }) {
  data.vaction().match(
      [&](const MTPDmessageActionPhoneCall &data) {
        ...
        _media = std::make_unique<Data::MediaCall>(
            this, Data::ComputeCallData(&history->owner(), data));
        setTextValue({});
      },
      ...
      [&](const auto &) { createServiceFromMtp(data); });
  setReactions(data.vreactions());
  applyTTL(data);
}

HistoryItem::HistoryItem(not_null<History *> history, MsgId id,
                         const MTPDmessageEmpty &data, MessageFlags localFlags)
    : HistoryItem(
          history, {.id = id, .flags = localFlags},
          PreparedServiceText{tr::lng_message_empty(tr::now, tr::marked)}) {}

HistoryItem::HistoryItem(not_null<History *> history,
                         HistoryItemCommonFields &&fields,
                         PreparedServiceText &&message, PhotoData *photo)
    : HistoryItem(history, fields) {
  setServiceText(std::move(message));
  if (photo) {
    _media = std::make_unique<Data::MediaPhoto>(this, history->peer, photo);
  }
}
...
HistoryItem::HistoryItem(not_null<History *> history,
  ...
  const auto dropForwardInfo =
      fields.ignoreForwardFrom || original->computeDropForwardedInfo();
  ...
  if (peer->isSelf()) {
    //
    // iOS app sends you to the original post if we forward a forward from
    // channel. But server returns not the original post but the forward in
    // saved_from_...
    //
  ...
  const auto dropText = fields.ignoreForwardCaptions && _media &&
  ...
  const auto forwardText = [&] {
    ...
    if (!text.text.isEmpty()) {
      return dropForwardInfo ? DropDisallowedCustomEmoji(history->peer, text)
  ...
  if (fields.groupedId) {
    setGroupId(MessageGroupId::FromRaw(history->peer->id, fields.groupedId,
                                       _flags & MessageFlag::IsOrWasScheduled));
  }
}
...
HistoryItem::HistoryItem(not_null<History *> history, MsgId id,
    ...
    : HistoryItem(
          ...
          {
              .id = id,
              .flags = (MessageFlag::Local | MessageFlag::Sponsored |
                        (history->peer->isChannel() ? MessageFlag::Post
                                                    : MessageFlag(0))),
              .date = NewMessageDate(injectedAfter ? injectedAfter->date() : 0),
          }) {
  const auto webpage = history->peer->owner().webpage(
      history->peer->owner().nextLocalMessageId().bare, WebPageType::None,
      from.link, from.link,
      from.isRecommended ? tr::lng_recommended_message_title(tr::now)
                         : tr::lng_sponsored_message_title(tr::now),
      from.title, textWithEntities,
      (from.photoId ? history->owner().photo(from.photoId).get() : nullptr),
      nullptr, WebPageCollage(), nullptr, nullptr, nullptr, 0, QString(), false,
      false, 0);
  auto webpageMedia = std::make_unique<Data::MediaWebPage>(
      this, webpage, MediaWebPageFlag::Sponsored);
  _media = std::move(webpageMedia);
}

HistoryItem::HistoryItem(not_null<History *> history,
                         const HistoryItemCommonFields &fields)
    : id(fields.id), _history(history),
      _from((fields.flags & MessageFlag::HasFromId && fields.from)
                ? history->owner().peer(fields.from)
                : history->peer),
      _flags(FinalizeMessageFlags(history, fields.flags)), _date(fields.date),
      _starsPaid(fields.starsPaid), _shortcutId(fields.shortcutId),
      _effectId(fields.effectId) {
  Expects(!_shortcutId || isSending() ||
...
HistoryItem::HistoryItem(not_null<History *> history, MsgId id,
    ...
    : HistoryItem(
          ...
          {
              .id = id,
              .flags = (MessageFlag::Local | MessageFlag::Outgoing |
...
TimeId HistoryItem::date() const { return _date; }

int HistoryItem::starsPaid() const { return _starsPaid; }
...
void HistoryItem::dependencyStoryRemoved(not_null<Data::Story *> dependency) {
  if (const auto reply = Get<HistoryMessageReply>()) {
    const auto documentId = reply->replyToDocumentId;
...
void HistoryItem::updateServiceDependent(bool force) {
  ...
  if (!dependent->lnk) {
    ...
    if (const auto done = Get<HistoryServiceTodoCompletions>()) {
      const auto &items =
          !done->completed.empty() ? done->completed : done->incompleted;
      if (items.size() == 1) {
    ...
    dependent->lnk = JumpToMessageClickHandler(
        (dependent->peerId ? _history->owner().peer(dependent->peerId)
                           : _history->peer),
        dependent->msgId, fullId(),
  ...
  if (!dependent->msg) {
    ...
    if (dependent->msg) {
      if (dependent->msg->isEmpty()) {
        ...
        _history->owner().registerDependentMessage(this, dependent->msg);
...
void HistoryItem::resolveDependent(
  ...
  RequestDependentMessageItem(
      this, (dependent->peerId ? dependent->peerId : _history->peer->id),
      dependent->msgId);
}
...
void HistoryItem::resolveDependent(not_null<HistoryMessageReply *> reply) {
  if (!reply->acquireResolve()) {
    ...
    RequestDependentMessageItem(this, reply->externalPeerId(), messageId);
  } else if (reply->storyId()) {
    RequestDependentMessageStory(this, reply->externalPeerId(),
                                 reply->storyId());
  }
}
...
SuggestionActions
HistoryItem::computeSuggestionActions(bool accepted, bool rejected,
                                      TimeId giftOfferExpiresAt) const {
  if (giftOfferExpiresAt) {
    const auto can = isRegular() && !(accepted || rejected) && !out() &&
                     (giftOfferExpiresAt > base::unixtime::now());
    return can ? SuggestionActions::GiftOfferActions : SuggestionActions::None;
  }
  ...
  const auto canDecline =
      isRegular() && !(accepted || rejected) &&
      (channelIsAuthor ? !amMonoforumAdmin : amMonoforumAdmin);
  const auto canAccept =
      canDecline && (channelIsAuthor ? !amMonoforumAdmin
                                     : (amMonoforumAdmin && broadcast &&
                                        broadcast->canPostMessages()));
  return canAccept    ? SuggestionActions::AcceptAndDecline
         : canDecline ? SuggestionActions::Decline
                      : SuggestionActions::None;
}
...
void HistoryItem::setReplyMarkup(HistoryMessageMarkupData &&markup,
  ...
  const auto requestUpdate = [&] {
    ...
    history()->session().changes().messageUpdated(
        this, Data::MessageUpdate::Flag::ReplyMarkup);
  };
  if (markup.isNull()) {
...
bool HistoryItem::areCommentsUnread() const {
  const auto views = Get<HistoryMessageViews>();
  if (!views || !views->commentsMegagroupId ||
...
FullMsgId HistoryItem::commentsItemId() const {
  if (const auto views = Get<HistoryMessageViews>()) {
    return FullMsgId(PeerId(views->commentsMegagroupId), views->commentsRootId);
  }
  return FullMsgId();
}
...
QString GenerateServiceTime(TimeId date) {
  if (date > 0) {
    return QString(" (%1)").arg(base::unixtime::parse(date).toString(
...
HistoryItem *HistoryItem::lookupDiscussionPostOriginal() const {
  ...
  if (!forwarded || !forwarded->savedFromPeer || !forwarded->savedFromMsgId) {
    return nullptr;
  }
  return _history->owner().message(forwarded->savedFromPeer->id,
...
PeerData *HistoryItem::computeDisplayFrom() const {
  if (const auto sender = discussionPostOriginalSender()) {
    ...
    if (showForwardsFromSender(forwarded)) {
      return forwarded->forwardOfForward() ? forwarded->savedFromSender
...
HistoryItem::createView(not_null<HistoryView::ElementDelegate *> delegate,
                        HistoryView::Element *replacing) {
  if (isService()) {
    return std::make_unique<HistoryView::Service>(delegate, this, replacing);
  }
...
void HistoryItem::invalidateChatListEntry() {
  _history->session().changes().messageUpdated(
      this, Data::MessageUpdate::Flag::DialogRowRefresh);
      ...
      [&](const MTPDmessage &data) {
        ...
        return media->match(
            [&](const MTPDmessageMediaDocument &data) {
              ...
              const auto id = document->match(
                  [](const auto &data) { return DocumentId(data.vid().v); });
              const auto existingMedia = this->media();
              const auto existingDocument =
                  existingMedia ? existingMedia->document() : nullptr;
              return !existingDocument || (existingDocument->id != id) ||
...
bool HistoryItem::markContentsRead(bool fromThisClient) {
  ...
  if (isUnreadMention() || isIncomingUnreadMedia() ||
      (unsupportedTTL() && hasUnreadMediaFlag())) {
...
void HistoryItem::setIsPinned(bool pinned) {
  ...
  if (pinned) {
    ...
    storage.add(Storage::SharedMediaAddExisting(
        ...
        Storage::SharedMediaType::Pinned, id, {id, id}));
    _history->setHasPinnedMessages(true);
    if (const auto topic = this->topic()) {
      storage.add(Storage::SharedMediaAddExisting(
          _history->peer->id, topic->rootId(),
    ...
    if (const auto sublist = this->savedSublist()) {
      storage.add(Storage::SharedMediaAddExisting(
          _history->peer->id,
          MsgId(0), // topicRootId
          sublistPeerId(), Storage::SharedMediaType::Pinned, id, {id, id}));
    ...
    _history->session().storage().remove(Storage::SharedMediaRemoveOne(
        _history->peer->id, Storage::SharedMediaType::Pinned, id));
  }
}
...
PaidPostType HistoryItem::paidType() const {
  return (_flags & MessageFlag::StarsPaidSuggested) ? PaidPostType::Stars
         : (_flags & MessageFlag::TonPaidSuggested) ? PaidPostType::Ton
                                                    : PaidPostType::None;
}
...
void HistoryItem::addLogEntryOriginal(WebPageId localId, const QString &label,
                                      const TextWithEntities &content) {
  Expects(isAdminLogEntry());

  AddComponents(HistoryMessageLogEntryOriginal::Bit());
  Get<HistoryMessageLogEntryOriginal>()->page =
      _history->owner().webpage(localId, label, content);
}
...
void HistoryItem::setFactcheck(MessageFactcheck info) {
  if (!info) {
    ...
    if (factcheck->data.hash == info.hash &&
      ...
      return;
    } else if (textChanged || factcheck->data.country != info.country ||
...
PeerData *HistoryItem::specialNotificationPeer() const {
  return (mentionsMe() && !_history->peer->isUser()) ? from().get() : nullptr;
}
...
bool HistoryItem::isFromScheduled() const {
  return isHistoryEntry() && (_flags & MessageFlag::IsOrWasScheduled);
}

bool HistoryItem::isScheduled() const {
  return !isHistoryEntry() && !isAdminLogEntry() &&
         (_flags & MessageFlag::IsOrWasScheduled);
}
...
BusinessShortcutId HistoryItem::shortcutId() const { return _shortcutId; }

bool HistoryItem::isBusinessShortcut() const { return _shortcutId != 0; }

void HistoryItem::setRealShortcutId(BusinessShortcutId id) { _shortcutId = id; }
...
void HistoryItem::destroy() { _history->destroyMessage(this); }
...
void HistoryItem::clearMainView() { _mainView = nullptr; }
...
void HistoryItem::applyEdition(HistoryMessageEdition &&edition) {
  ...
  //		keyboardTop = _height - h + st::msgBotKbButton.margin -
  //marginBottom();
  //	}
  // }

  const auto editingMedia = isEditingMedia();
  const auto updatingSavedLocalEdit =
      !edition.savePreviousMedia && editingMedia;
  ...
  if (updatingSavedLocalEdit) {
    Get<HistoryMessageSavedMediaData>()->media =
        (mediaCheck != MediaCheckResult::Unsupported && edition.mtpMedia)
  ...
  auto updatedText = (mediaCheck == MediaCheckResult::Unsupported)
                     ...
                     : checkedMedia ? edition.textWithEntities
                                    : EnsureNonEmpty(edition.textWithEntities);
  auto serviceText =
      (!checkedMedia && edition.textWithEntities.empty() && edition.mtpMedia)
  ...
  if (message.vaction().type() == mtpc_messageActionHistoryClear) {
    ...
    if (const auto user = history()->peer->asUser()) {
      if (const auto conferenceId = out() ? info.conferenceId : 0) {
        Core::App().calls().unregisterConferenceInvite(conferenceId, user, id,
                                                       !out(), true);
      }
    }
    _media = nullptr;
...
void HistoryItem::applyEdition(const QVector<MTPMessageExtendedMedia> &media) {
...
void HistoryItem::applySentMessage(const MTPDmessage &data) {
  ...
  updateSentContent({qs(data.vmessage()),
                     Api::EntitiesFromMTP(&_history->session(),
                                          data.ventities().value_or_empty())},
                    data.vmedia());
...
void HistoryItem::applySentMessage(const QString &text,
                                   const MTPDupdateShortSentMessage &data,
                                   bool wasAlready) {
  updateSentContent(
      {text, Api::EntitiesFromMTP(&_history->session(),
                                  data.ventities().value_or_empty())},
      data.vmedia());
  contributeToSlowmode(data.vdate().v);
  if (!wasAlready) {
...
void HistoryItem::updateSentContent(const TextWithEntities &textWithEntities,
                                    const MTPMessageMedia *media) {
  if (isEditingMedia()) {
    return;
  }
  const auto mediaCheck =
      media ? CheckMessageMedia(*media) : MediaCheckResult::Good;
  if (mediaCheck == MediaCheckResult::Unsupported) {
    _flags &= ~MessageFlag::HasPostAuthor;
    _flags |= MessageFlag::Legacy;
...
void HistoryItem::applyEditionToHistoryCleared() {
  ...
  applyEdition(MTP_messageService(
                   MTP_flags(Flag() | (topicPost ? Flag::f_reply_to : Flag())),
                   MTP_int(id),
                   peerToMTP(PeerId(0)), // from_id
                   peerToMTP(_history->peer->id),
                   MTPPeer(), // saved_peer_id
                   NewMessageReplyHeader(action), MTP_int(date()),
                   MTP_messageActionHistoryClear(), MTPMessageReactions(),
                   MTPint() // ttl_period
                   )
                   .c_messageService());
}
...
void HistoryItem::clearMediaAsExpired() {
  ...
  if (const auto document = media->document()) {
    ...
    applyEditionToHistoryCleared();
    updateServiceText({tr::lng_ttl_photo_expired(tr::now, tr::marked)});
    _flags |= MessageFlag::ReactionsAllowed;
  }
}
...
void HistoryItem::addToUnreadThings(HistoryUnreadThings::AddType type) {
  ...
  if (mention) {
    if (history->unreadMentions().add(id, type)) {
      changes->historyUpdated(history,
                              Data::HistoryUpdate::Flag::UnreadMentions);
    }
    if (topic && topic->unreadMentions().add(id, type)) {
      changes->topicUpdated(topic, Data::TopicUpdate::Flag::UnreadMentions);
    }
  }
  if (reaction) {
    ...
    const auto toSublist = sublist && sublist->parentChat() &&
    ...
    if (toHistory || toTopic || toSublist) {
      ...
      if (hasUnreadReaction()) {
        ...
        if (toTopic) {
          changes->topicUpdated(topic,
                                Data::TopicUpdate::Flag::UnreadReactions);
        }
        if (toSublist) {
          changes->sublistUpdated(sublist,
  ...
  if (pollVote) {
    ...
    const auto toTopic = topic && topic->unreadPollVotes().add(id, type);
    if (toHistory || toTopic) {
      ...
      if (toTopic) {
        changes->topicUpdated(topic, Data::TopicUpdate::Flag::UnreadPollVotes);
      }
    }
  }
}
  ...
  if (isRegular()) {
    if (const auto types = sharedMediaTypes()) {
      _history->session().storage().add(Storage::SharedMediaAddNew(
          _history->peer->id, topicRootId(), sublistPeerId(), types, id));
  ...
  if (isRegular()) {
    if (const auto types = sharedMediaTypes()) {
      _history->session().storage().remove(
          Storage::SharedMediaRemoveOne(_history->peer->id, types, id));
    }
  }
}
...
void HistoryItem::changeReplyToTopCounter(not_null<HistoryMessageReply *> reply,
                                          int delta) {
...
bool HistoryItem::allowsSendNow() const {
  return !isService() && isScheduled() && !isSending() && !hasFailed() &&
         !isEditingMedia() && (paidType() == PaidPostType::None);
}
...
bool HistoryItem::canBeEdited() const {
  if ((!isRegular() && !isScheduled() && !isBusinessShortcut()) ||
      Has<HistoryMessageVia>() || Has<HistoryMessageForwarded>()) {
    return false;
  }
...
bool HistoryItem::canStopPoll() const { return canBeEdited() && isRegular(); }
...
bool HistoryItem::canDelete() const {
  if (isSponsored()) {
    ...
    return false;
  } else if (!isHistoryEntry() && !isScheduled() && !isBusinessShortcut()) {
    return false;
  }
...
bool HistoryItem::canDeleteForEveryone(TimeId now) const {
  ...
  const auto messageTooOld =
      messageToMyself  ? false
      : peer->isUser() ? (now - date() >= config.revokePrivateTimeLimit)
  ...
  if (peer->isChannel()) {
    ...
    if ((user->isBot() && !user->isSupport()) || user->isInaccessible()) {
      return false;
    }
  }
...
ChatRestriction HistoryItem::requiredSendRight() const {
  ...
  const auto photo = (media && !media->webpage()) ? media->photo() : nullptr;
  const auto document =
      (media && !media->webpage()) ? media->document() : nullptr;
  if (photo) {
...
Data::SendError
HistoryItem::errorTextForForward(not_null<Data::Thread *> to) const {
...
Data::SendError HistoryItem::errorTextForForwardIgnoreRights(
  ...
  const auto peer = to->peer();
  if (_media && _media->poll() && _media->poll()->publicVotes() &&
      peer->isBroadcast()) {
    return tr::lng_restricted_send_public_polls(tr::now);
  } else if (_media && _media->todolist() &&
             (peer->isBroadcast() || peer->isMonoforum())) {
    return tr::lng_restricted_send_todo_lists(tr::now);
  } else if (_media && _media->invoice() && _media->invoice()->isPaidMedia &&
             peer->isBroadcast() && peer->isFullLoaded() &&
...
void HistoryItem::translationToggle(
    not_null<HistoryMessageTranslation *> translation, bool used) {
...
Data::PaidReactionSend HistoryItem::startPaidReactionSending() {
  return _reactions ? _reactions->startPaidSending() : Data::PaidReactionSend();
}

void HistoryItem::finishPaidReactionSending(Data::PaidReactionSend send,
                                            bool success) {
...
std::vector<Data::MessageReaction> HistoryItem::reactionsWithLocal() const {
  ...
  const auto i = ranges::find(result, Data::ReactionId::Paid(),
...
PeerId HistoryItem::reactionsLocalShownPeer() const {
  return _reactions ? _reactions->localPaidShownPeer()
...
auto HistoryItem::recentReactions() const -> const
    base::flat_map<Data::ReactionId, std::vector<Data::RecentReaction>> & {
...
auto HistoryItem::topPaidReactionsWithLocal() const
  ...
  const auto i = ranges::find_if(
      result, [](const TopPaid &entry) { return entry.my != 0; });
...
bool HistoryItem::canViewReactions() const {
  return (_flags & MessageFlag::CanViewReactions) && _reactions &&
         !_reactions->list().empty();
}

std::vector<Data::ReactionId> HistoryItem::chosenReactions() const {
  return _reactions ? _reactions->chosen() : std::vector<Data::ReactionId>();
}

Data::ReactionId
HistoryItem::lookupUnreadReaction(not_null<UserData *> from) const {
  if (!_reactions) {
    return {};
  }
  const auto recent = _reactions->recent();
  for (const auto &[id, list] : _reactions->recent()) {
    const auto i = ranges::find(list, from, &Data::RecentReaction::peer);
...
QByteArray HistoryItem::lookupUnreadPollVote(not_null<PeerData *> from) const {
  const auto m = media();
...
bool HistoryItem::computeDropForwardedInfo() const {
  ...
  return (media && media->dropForwardedInfo()) ||
         (_history->peer->isSelf() && !Has<HistoryMessageForwarded>() &&
          (!media || !media->forceForwardedInfo()));
}

bool HistoryItem::inThread(MsgId rootId) const {
  return (replyToTop() == rootId) || (topicRootId() == rootId);
}

not_null<PeerData *> HistoryItem::author() const {
  return (isPostHidingAuthor() && !isSponsored()) ? _history->peer : from();
}
...
bool HistoryItem::showForwardsFromSender(
  ...
  const auto peer = history()->peer;
  return !forwarded->story && (peer->isSelf() || peer->isRepliesChat() ||
                               peer->isVerifyCodes() || forwarded->imported);
}
...
TextWithEntities HistoryItem::translatedTextWithLocalEntities() const {
  ...
  if (hideLinks()) {
    const auto isUrl = [](const EntityInText &entity) {
      ...
      return (type == EntityType::Mention) || (type == EntityType::Hashtag) ||
             (type == EntityType::Cashtag) || (type == EntityType::Url) ||
...
void HistoryItem::setForwardsCount(int count) {
  const auto views = Get<HistoryMessageViews>();
  if (!views || views->forwardsCount == count ||
...
void HistoryItem::setPostAuthor(const QString &postAuthor) {
  ...
  msgsigned->isAnonymousRank =
      !isDiscussionPost() && this->author()->isMegagroup();
  history()->owner().requestItemResize(this);
}
...
bool HistoryItem::isDeleted() const { return _deleted; }
...
bool HistoryItem::wasDeletedAnimated() const { return _deletedAnimated; }

void HistoryItem::markDeletedAnimated() { _deletedAnimated = false; }
  ...
  try {
    ...
    if (!isService()) {
      ...
      if (!_text.text.endsWith(postfix)) { // fix stacking for TTL messages
        auto prepared = PreparedServiceText{.text = _text.append(postfix),
...
void HistoryItem::refreshRepliesText(not_null<HistoryMessageViews *> views,
                                     bool forceResize) {
  if (views->commentsMegagroupId) {
    views->replies.text =
        (views->replies.count > 0)
            ? tr::lng_comments_open_count(tr::now, lt_count_short,
                                          views->replies.count)
            : tr::lng_comments_open_none(tr::now);
    views->replies.textWidth = st::semiboldFont->width(views->replies.text);
...
void HistoryItem::setReplyFields(MsgId replyTo, MsgId replyToTop,
                                 bool isForumPost) {
  if (isScheduled()) {
    return;
...
bool HistoryItem::canUpdateDate() const { return isScheduled(); }
...
bool HistoryItem::isUploading() const { return _media && _media->uploading(); }
...
bool HistoryItem::isRegular() const { return isHistoryEntry() && !isLocal(); }
...
void HistoryItem::sendFailed() {
  ...
  _history->session().changes().historyUpdated(
      _history, Data::HistoryUpdate::Flag::ClientSideMessages);
}

bool HistoryItem::needCheck() const {
  return (out() && !isEmpty()) || (!isRegular() && _history->peer->isSelf());
}

bool HistoryItem::isService() const { return Has<HistoryServiceData>(); }
...
FullStoryId HistoryItem::replyToStory() const {
  if (const auto reply = Get<HistoryMessageReply>()) {
    if (reply->storyId()) {
      const auto peerId = reply->externalPeerId() ? reply->externalPeerId()
                                                  : _history->peer->id;
...
FullReplyTo HistoryItem::replyTo() const {
  const auto monoforumPeerId =
      _history->peer->amMonoforumAdmin() ? sublistPeerId() : PeerId();
...
void HistoryItem::detectTextLinks(const TextWithEntities &textWithEntities) {
  for (const auto &entity : textWithEntities.entities) {
    auto type = entity.type();
    if (type == EntityType::Url || type == EntityType::CustomUrl ||
        type == EntityType::Phone || type == EntityType::BankCard ||
...
bool HistoryItem::showNotification() const {
  ...
  return (out() || _history->peer->isSelf()) ? isFromScheduled()
...
MessageGroupId HistoryItem::groupId() const { return _groupId; }

EffectId HistoryItem::effectId() const { return _effectId; }
...
QString HistoryItem::computeUnavailableReason() const {
  if (const auto restrictions = Get<HistoryMessageRestrictions>()) {
    ...
    return Data::UnavailableReason::Compute(&history()->session(),
...
bool HistoryItem::isMediaSensitive() const {
  ...
  return !Data::UnavailableReason::IgnoreSensitiveMark(&_history->session());
}
...
Data::SavedSublist *HistoryItem::savedSublist() const {
  if (isBusinessShortcut() || isScheduled()) {
    ...
    const auto sublist =
        _history->owner().savedMessages().sublist(_history->peer);
...
TextWithEntities
HistoryItem::notificationText(NotificationTextOptions options) const {
  auto result = [&] {
    if (_media && !isService()) {
  ...
  return Ui::Text::Mid(result, 0, kNotificationTextLimit)
      .append(Ui::kQEllipsis);
}
...
ItemPreview HistoryItem::toPreview(ToPreviewOptions options) const {
  ...
  auto result = [&]() -> ItemPreview {
    if (_media) {
      ...
      return {// wrap_rtl "adds" a newline in case text starts with quote.
  ...
  const auto fromSender = [](not_null<PeerData *> sender) {
    return sender->isSelf() ? tr::lng_from_you(tr::now) : sender->shortName();
  };
  const auto forwarded = Get<HistoryMessageForwarded>();
  const auto forwardFromSender = forwarded && showForwardsFromSender(forwarded);
  result.icon =
      (forwarded && (!forwardFromSender || forwarded->forwardOfForward()))
          ? ItemPreview::Icon::ForwardedMessage
      : replyToStory().valid() ? ItemPreview::Icon::ReplyToStory
  ...
  const auto fromForwarded = [&]() -> std::optional<QString> {
    if (forwarded) {
      const auto sender = forwarded->forwardOfForward()
                              ...
                              : forwarded->originalSender;
      return sender ? fromSender(sender)
  ...
  const auto sender = [&]() -> std::optional<QString> {
    ...
    } else if (_history->peer->isSelf() || _history->peer->isVerifyCodes()) {
      return fromForwarded();
    }
    return {};
  }();
  ...
  return Dialogs::Ui::PreviewWithSender(std::move(result), *sender,
                                        topic ? topic->titleWithIcon()
                                              : TextWithEntities());
}
...
void HistoryItem::createComponents(CreateConfig &&config) {
  uint64 mask = 0;
  if (config.reply.messageId || config.reply.externalSenderId ||
      !config.reply.externalSenderName.isEmpty() || config.reply.storyId) {
  ...
  if (!config.postAuthor.isEmpty() || config.viaBusinessBotId) {
    mask |= HistoryMessageSigned::Bit();
             ...
             && config.savedFromPeer && !config.originalPostAuthor.isEmpty()) {
    const auto savedFrom = _history->owner().peerLoaded(config.savedFromPeer);
    if (savedFrom && savedFrom->isChannel()) {
      mask |= HistoryMessageSigned::Bit();
    }
  } else if (!config.originalPostAuthor.isEmpty() &&
             (_history->peer->isSelf() || _history->peer->isRepliesChat() ||
  ...
  if (!isBusinessShortcut() && !isScheduled() &&
      (_history->peer->isSelf() || config.savedSublistPeer ||
  ...
  if (config.suggest.exists) {
    ...
    if (computeSuggestionActions(config.suggest.accepted,
                                 config.suggest.rejected,
                                 TimeId()) != SuggestionActions::None) {
      mask |= HistoryMessageReplyMarkup::Bit();
    }
  }
  ...
  if (const auto saved = Get<HistoryMessageSaved>()) {
    ...
    if (_history->peer->isSelf()) {
      saved->savedMessagesSublist = _history->owner().savedMessages().sublist(
  ...
  if (Has<HistoryMessageViews>()) {
    ...
    if (config.replies.isNull && isSending() && config.markup.isNull()) {
  ...
  if (const auto msgsigned = Get<HistoryMessageSigned>()) {
    if (config.viaBusinessBotId) {
      msgsigned->viaBusinessBot =
          _history->owner().user(config.viaBusinessBotId);
      ...
      msgsigned->author = config.postAuthor.isEmpty()
                              ...
                              : config.postAuthor;
      msgsigned->isAnonymousRank =
          !isDiscussionPost() && author()->isMegagroup();
    }
  }
  setupForwardedComponent(config);
  if (const auto markup = Get<HistoryMessageReplyMarkup>()) {
  ...
  if (const auto restrictions = Get<HistoryMessageRestrictions>()) {
    ...
    const auto i = ranges::find(restrictions->reasons, true,
...
bool HistoryItem::checkRepliesPts(const HistoryMessageRepliesData &data) const {
  const auto channel = _history->peer->asChannel();
  const auto pts =
      channel ? channel->pts() : _history->session().updates().pts();
  return (data.pts >= pts);
}
...
void HistoryItem::setupForwardedComponent(const CreateConfig &config) {
  ...
  const auto originalSender = config.originalSenderId ? config.originalSenderId
                              : !config.originalSenderName.isEmpty()
                                  ? PeerId()
                                  : from()->id;
  forwarded->originalSender =
      originalSender ? _history->owner().peer(originalSender).get() : nullptr;
  if (!forwarded->originalSender) {
    forwarded->originalHiddenSenderInfo = std::make_unique<HiddenSenderInfo>(
        config.originalSenderName, config.imported);
  }
  ...
  forwarded->savedFromPeer = _history->owner().peerLoaded(config.savedFromPeer);
  forwarded->savedFromMsgId = config.savedFromMsgId;
  forwarded->savedFromSender =
      _history->owner().peerLoaded(config.savedFromSenderId);
  forwarded->savedFromOutgoing = config.savedFromOutgoing;
  forwarded->savedFromDate = config.savedFromDate;
  if (!forwarded->savedFromSender && !config.savedFromSenderName.isEmpty()) {
    forwarded->savedFromHiddenSenderInfo =
...
TextWithEntities
HistoryItem::withLocalEntities(const TextWithEntities &textWithEntities) const {
  using namespace HistoryView;
  if (!generateLocalEntitiesByReply()) {
    if (!_media) {
    } else if (const auto document = _media->document()) {
      if (const auto duration = DurationForTimestampLinks(document)) {
        return AddTimestampLinks(textWithEntities, duration,
...
void HistoryItem::createComponentsHelper(HistoryItemCommonFields &&fields) {
  ...
  if (fields.flags & MessageFlag::HasReplyInfo) {
    ...
    config.reply.externalPeerId =
        replyTo.storyId ? replyTo.storyId.peer
        : (replyTo.messageId && replyTo.messageId.peer != history()->peer->id)
            ? replyTo.messageId.peer
            : PeerId();
    const auto to = LookupReplyTo(_history, replyTo.messageId);
    config.reply.monoforumPeerId =
        (to && to->sublistPeerId()) ? to->sublistPeerId()
    ...
    const auto topic = forum ? forum->topicFor(replyTo.topicRootId) : nullptr;
    if (!config.reply.externalPeerId && topic && to &&
        topic->rootId() != to->topicRootId()) {
      config.reply.externalPeerId = replyTo.messageId.peer;
    }
    const auto topicPost =
        config.reply.externalPeerId
            ? (replyTo.topicRootId &&
               (replyTo.topicRootId != Data::ForumTopic::kGeneralId))
            : (topic || LookupReplyIsTopicPost(to) ||
...
bool HistoryItem::changeReactions(const MTPMessageReactions *reactions) {
  ...
  if (data.is_reactions_as_tags() || (empty && _history->peer->isSelf())) {
    _flags |= MessageFlag::ReactionsAreTags;
  } else {
    _flags &= ~MessageFlag::ReactionsAreTags;
  }
...
void HistoryItem::createComponents(const MTPDmessage &data, bool blocked) {
  ...
  config.savedSublistPeer =
      data.vsaved_peer_id() ? peerFromMTP(*data.vsaved_peer_id()) : PeerId();
  ...
  config.scheduleRepeatPeriod = data.vschedule_repeat_period().value_or_empty();
...
PreparedServiceText
HistoryItem::prepareServiceTextForMessage(const MTPMessageMedia &media,
                                          bool unread) {
  return media.match(
      ...
      [&](const MTPDmessageMediaPhoto &data) -> PreparedServiceText {
        if (unread) {
          ...
          if (out()) {
            return {tr::lng_ttl_photo_sent(tr::now, tr::marked)};
          } else {
            auto result = PreparedServiceText();
            result.links.push_back(fromLink());
            result.text = tr::lng_ttl_photo_received(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
            return result;
          }
        } else {
          return {tr::lng_ttl_photo_expired(tr::now, tr::marked)};
        }
      },
      [&](const MTPDmessageMediaDocument &data) -> PreparedServiceText {
        if (unread) {
          ...
          if (data.is_video()) {
            if (out()) {
              return {tr::lng_ttl_video_sent(tr::now, tr::marked)};
            } else {
              auto result = PreparedServiceText();
              result.links.push_back(fromLink());
              result.text =
                  tr::lng_ttl_video_received(tr::now, lt_from,
                                             fromLinkText(), // Link 1.
                                             tr::marked);
              return result;
            }
          } else if (out()) {
            auto text = (data.is_voice() ? tr::lng_ttl_voice_sent
                         : data.is_round()
          ...
          auto text =
              (data.is_video()   ? tr::lng_ttl_video_expired
               : data.is_voice() ? tr::lng_ttl_voice_expired
               : data.is_round() ? tr::lng_ttl_round_expired
                                 : tr::lng_message_empty)(tr::now, tr::marked);
          return {std::move(text)};
        }
      },
      [](const auto &) { return PreparedServiceText(); });
}
...
void HistoryItem::createServiceFromMtp(const MTPDmessageService &message) {
  ...
  if (type == mtpc_messageActionPinMessage) {
        ...
        std::make_shared<LambdaClickHandler>([=](ClickContext context) {
          ...
          if (const auto item = owner->message(id)) {
            CheckoutProcess::Start(
                ...
                Payments::ProcessNonPanelPaymentFormFactory(weak.get(), item));
          }
        });
    ...
    const auto &callData =
        started ? action.c_messageActionGroupCall().vcall()
    ...
    auto hasLink = !has.has_value() ? PeerHasThisCallValue(peer, id)
                   : (*has) ? PeerHasThisCallValue(peer, id) | rpl::skip(1) |
                                  rpl::type_erased
                            : rpl::producer<bool>();
    if (!hasLink) {
      RemoveComponents(HistoryServiceOngoingCall::Bit());
          ...
          rpl::on_next(
              [=](bool has) {
                updateServiceText(prepareInvitedToCallText(
                    ParseInvitedToCallUsers(this, users), has ? id : 0));
                if (!has) {
                  RemoveComponents(HistoryServiceOngoingCall::Bit());
                }
              },
    ...
    const auto done = Get<HistoryServiceTodoCompletions>();
                      ...
                      ranges::views::transform(&MTPint::v) | ranges::to_vector;
    ...
    append->list = ranges::views::all(data.vlist().v) |
    ...
    data.vanswer().match(
        [&](const MTPDpollAnswer &answer) {
          append->answer.option = answer.voption().v;
          append->answer.text =
              Api::ParseTextWithEntities(&_history->session(), answer.vtext());
        },
        [](const auto &) {});
    ...
    UpdateComponents(HistoryServiceSuggestFinish::Bit());
    const auto finish = Get<HistoryServiceSuggestFinish>();
    finish->refundType = data.is_payer_initiated() ? SuggestRefundType::User
    ...
    if (const auto unique = gift ? gift->unique : nullptr) {
      ...
      const auto actions =
          computeSuggestionActions(accepted, rejected, expiresAt);
      if (actions != SuggestionActions::None) {
        mask |= HistoryMessageReplyMarkup::Bit();
      }
      UpdateComponents(mask);
    ...
    UpdateComponents(HistoryServiceSuggestFinish::Bit());
    const auto finish = Get<HistoryServiceSuggestFinish>();
    finish->refundType = data.is_expired() ? SuggestRefundType::Expired
    ...
    const auto period =
        _history->session().appConfig().noForwardsRequestExpirePeriod();
    const auto expiresAt = TimeId(_date + period);
    const auto isExpired = expired || (expiresAt <= base::unixtime::now());
    ...
    replyTo->match(
        [&](const MTPDmessageReplyHeader &data) {
          ...
          if (const auto dependent = GetServiceDependentData()) {
            ...
            if (id) {
              dependent->peerId = (peerId != _history->peer->id) ? peerId : 0;
              dependent->msgId = id;
              dependent->topId = data.vreply_to_top_id().value_or(id);
              dependent->topicPost =
                  data.is_forum_topic() || Has<HistoryServiceTopicInfo>();
  ...
  const auto savedSublistPeer = message.vsaved_peer_id()
                                    ...
                                    : PeerId();
  const auto requiresMonoforumPeer = _history->peer->amMonoforumAdmin();
  if (!isBusinessShortcut() && (savedSublistPeer || requiresMonoforumPeer)) {
    AddComponents(HistoryMessageSaved::Bit());
    const auto saved = Get<HistoryMessageSaved>();
    saved->sublistPeerId = savedSublistPeer ? savedSublistPeer : _from->id;
    if (_history->peer->isSelf()) {
      saved->savedMessagesSublist = _history->owner().savedMessages().sublist(
          _history->owner().peer(saved->sublistPeerId));
    }
...
void HistoryItem::setServiceMessageByAction(const MTPmessageAction &action) {
  auto prepareChatAddUserText =
      [this](const MTPDmessageActionChatAddUser &action) {
        auto result = PreparedServiceText();
        ...
        if (users.size() == 1) {
          ...
          if (u == _from) {
            result.links.push_back(fromLink());
            result.text = tr::lng_action_user_joined(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
          } else {
            result.links.push_back(fromLink());
            ...
            result.text = tr::lng_action_add_user(
                tr::now, lt_from,
                fromLinkText(),                  // Link 1.
                lt_user, tr::link(u->name(), 2), // Link 2.
                tr::marked);
          }
          ...
          result.text = tr::lng_action_add_user(
              tr::now, lt_from,
              fromLinkText(), // Link 1.
              lt_user, {.text = u"somebody"_q}, tr::marked);
        } else {
          result.links.push_back(fromLink());
          for (auto i = 0, l = int(users.size()); i != l; ++i) {
            ...
            if (i == 0) {
              ...
              result.text = tr::lng_action_add_users_and_last(
                  tr::now, lt_accumulated, result.text, lt_user, linkText,
                  tr::marked);
            } else {
          ...
          result.text =
              tr::lng_action_add_users_many(tr::now, lt_from,
                                            fromLinkText(), // Link 1.
                                            lt_users, result.text, tr::marked);
        }
        return result;
      };

  auto prepareChatJoinedByLink =
      [this](const MTPDmessageActionChatJoinedByLink &action) {
        auto result = PreparedServiceText();
        result.links.push_back(fromLink());
        result.text =
            tr::lng_action_user_joined_by_link(tr::now, lt_from,
                                               fromLinkText(), // Link 1.
                                               tr::marked);
        return result;
      };
  ...
  auto prepareChatCreate = [this](const MTPDmessageActionChatCreate &action) {
    ...
    if (_history->peer->isMonoforum()) {
      result.text = tr::lng_action_created_monoforum(tr::now, tr::marked);
    } else {
      result.links.push_back(fromLink());
      result.text = tr::lng_action_created_chat(
          tr::now, lt_from,
          fromLinkText(), // Link 1.
          lt_title, {.text = qs(action.vtitle())}, tr::marked);
    }
    return result;
  };

  auto prepareChannelCreate =
      [this](const MTPDmessageActionChannelCreate &action) {
        auto result = PreparedServiceText();
        if (_history->peer->isMonoforum()) {
          result.text = tr::lng_action_created_monoforum(tr::now, tr::marked);
        } else if (isPost()) {
          result.text = tr::lng_action_created_channel(tr::now, tr::marked);
        } else {
          result.links.push_back(fromLink());
          result.text = tr::lng_action_created_chat(
              tr::now, lt_from,
  ...
  auto prepareChatDeletePhoto =
      [&](const MTPDmessageActionChatDeletePhoto &action) {
        auto result = PreparedServiceText();
        if (isPost()) {
          result.text =
              tr::lng_action_removed_photo_channel(tr::now, tr::marked);
        } else {
          result.links.push_back(fromLink());
          result.text = tr::lng_action_removed_photo(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
        }
        return result;
  ...
  auto prepareChatDeleteUser =
      [this](const MTPDmessageActionChatDeleteUser &action) {
        auto result = PreparedServiceText();
        if (peerFromUser(action.vuser_id()) == _from->id) {
          result.links.push_back(fromLink());
          result.text = tr::lng_action_user_left(tr::now, lt_from,
                                                 fromLinkText(), // Link 1.
                                                 tr::marked);
        } else {
          ...
          result.text = tr::lng_action_kick_user(
              tr::now, lt_from,
              fromLinkText(),                     // Link 1.
              lt_user, tr::link(user->name(), 2), // Link 2.
              tr::marked);
        }
        return result;
      };

  auto prepareChatEditPhoto =
      [this](const MTPDmessageActionChatEditPhoto &action) {
        auto result = PreparedServiceText();
        if (isPost()) {
          result.text =
              tr::lng_action_changed_photo_channel(tr::now, tr::marked);
        } else {
          result.links.push_back(fromLink());
          result.text = tr::lng_action_changed_photo(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
        }
        return result;
  ...
  auto prepareChatEditTitle =
      [this](const MTPDmessageActionChatEditTitle &action) {
        auto result = PreparedServiceText();
        if (isPost()) {
          result.text = tr::lng_action_changed_title_channel(
              tr::now, lt_title, {.text = (qs(action.vtitle()))}, tr::marked);
        } else {
          result.links.push_back(fromLink());
  ...
  auto preparePaymentSentMe = [&](const MTPDmessageActionPaymentSentMe &data) {
    auto result = PreparedServiceText();
    result.text =
        (data.is_recurring_used() ? tr::lng_action_payment_bot_recurring
  ...
  auto prepareScreenshotTaken = [this](
    ...
    auto result = PreparedServiceText();
    if (out()) {
      result.text = tr::lng_action_you_took_screenshot(tr::now, tr::marked);
    } else {
      result.links.push_back(fromLink());
      result.text = tr::lng_action_took_screenshot(tr::now, lt_from,
                                                   fromLinkText(), // Link 1.
                                                   tr::marked);
    }
    return result;
  ...
  auto prepareBotAllowed = [&](const MTPDmessageActionBotAllowed &action) {
    ...
    if (action.is_attach_menu()) {
      result.text = {tr::lng_action_attach_menu_bot_allowed(tr::now)};
    } else if (action.is_from_request()) {
      result.text = {tr::lng_action_webapp_bot_allowed(tr::now)};
      ...
      result.text = tr::lng_action_bot_allowed_from_app(
          tr::now, lt_app,
          (url.isEmpty() ? TextWithEntities{u"App"_q}
      ...
      result.text = tr::lng_action_bot_allowed_from_domain(
          tr::now, lt_domain, tr::link(domain, u"http://"_q + domain),
          tr::marked);
    }
    return result;
  };

  auto prepareSecureValuesSent =
      [&](const MTPDmessageActionSecureValuesSent &action) {
        auto result = PreparedServiceText();
        ...
        result.text = tr::lng_action_secure_values_sent(
            ...
            lt_documents, {.text = documents.join(", ")}, tr::marked);
        return result;
      };

  auto prepareContactSignUp =
      [this](const MTPDmessageActionContactSignUp &data) {
        auto result = PreparedServiceText();
        result.links.push_back(fromLink());
        result.text = tr::lng_action_user_registered(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
        return result;
      };

  auto prepareProximityReached =
      [this](const MTPDmessageActionGeoProximityReached &action) {
        auto result = PreparedServiceText();
        ...
        const auto distance = [&] {
          if (distanceMeters >= 1000) {
            ...
            return tr::lng_action_proximity_distance_km(tr::now, lt_count, km);
          } else {
            return tr::lng_action_proximity_distance_m(tr::now, lt_count,
                                                       distanceMeters);
          }
        }();
        result.text = [&] {
          if (fromId == selfId) {
            ...
            return tr::lng_action_you_proximity_reached(
                tr::now, lt_distance, {.text = distance}, lt_user,
            ...
            return tr::lng_action_proximity_reached_you(
                ...
                lt_distance, {.text = distance}, tr::marked);
          } else {
            result.links.push_back(fromPeer->createOpenLink());
            result.links.push_back(toPeer->createOpenLink());
            return tr::lng_action_proximity_reached(
                tr::now, lt_from, tr::link(fromPeer->name(), 1), // Link 1.
                lt_distance, {.text = distance}, lt_user,
  ...
  auto prepareGroupCall = [this](const MTPDmessageActionGroupCall &action) {
    ...
    if (const auto duration = action.vduration()) {
      ...
      auto text = (days > 1)      ? tr::lng_days(tr::now, lt_count, days)
                  : (hours > 1)   ? tr::lng_hours(tr::now, lt_count, hours)
                  : (minutes > 1) ? tr::lng_minutes(tr::now, lt_count, minutes)
                                  : tr::lng_seconds(tr::now, lt_count, seconds);
      if (_history->peer->isBroadcast()) {
        result.text = tr::lng_action_group_call_finished(
            tr::now, lt_duration, {.text = text}, tr::marked);
      } else {
        result.links.push_back(fromLink());
        result.text = tr::lng_action_group_call_finished_group(
            tr::now, lt_from,
            fromLinkText(), // Link 1.
            lt_duration, {.text = text}, tr::marked);
      }
      return result;
    }
    if (_history->peer->isBroadcast()) {
      result.text =
          tr::lng_action_group_call_started_channel(tr::now, tr::marked);
    } else {
      result.links.push_back(fromLink());
      result.text =
          tr::lng_action_group_call_started_group(tr::now, lt_from,
                                                  fromLinkText(), // Link 1.
                                                  tr::marked);
    }
    return result;
  ...
  auto prepareInviteToGroupCall =
      [this](const MTPDmessageActionInviteToGroupCall &action) {
        ...
        const auto linkCallId =
            PeerHasThisCall(peer, callId).value_or(false) ? callId : 0;
        return prepareInvitedToCallText(
            ParseInvitedToCallUsers(this, action.vusers().v), linkCallId);
      };

  auto prepareSetMessagesTTL =
      [this](const MTPDmessageActionSetMessagesTTL &action) {
        auto result = PreparedServiceText();
        const auto period = action.vperiod().v;
        const auto duration =
            (period == 5) ? u"5 seconds"_q : Ui::FormatTTL(period);
        if (const auto from = action.vauto_setting_from(); from && period) {
          if (const auto peer = _from->owner().peer(peerFromUser(*from))) {
            result.text =
                (peer->id == peer->session().userPeerId())
                    ? tr::lng_action_ttl_global_me(
                          tr::now, lt_duration, {.text = duration}, tr::marked)
                    : tr::lng_action_ttl_global(
                          tr::now, lt_from,
                          tr::link(peer->name(), 1), // Link 1.
                          lt_duration, {.text = duration}, tr::marked);
            return result;
          }
        }
        if (isPost()) {
          if (!period) {
            result.text =
                tr::lng_action_ttl_removed_channel(tr::now, tr::marked);
          } else {
          ...
          if (!period) {
            result.text = tr::lng_action_ttl_removed_you(tr::now, tr::marked);
          } else {
          ...
          result.links.push_back(fromLink());
          if (!period) {
            result.text = tr::lng_action_ttl_removed(tr::now, lt_from,
                                                     fromLinkText(), // Link 1.
                                                     tr::marked);
          } else {
  ...
  auto prepareGroupCallScheduled =
      [&](const MTPDmessageActionGroupCallScheduled &data) {
        return prepareCallScheduledText(data.vschedule_date().v);
      };

  auto prepareSetChatTheme = [this](
                                 const MTPDmessageActionSetChatTheme &action) {
    auto result = PreparedServiceText();
    action.vtheme().match(
        [&](const MTPDchatTheme &data) {
          const auto text = qs(data.vemoticon());
          if (!text.isEmpty()) {
            if (_from->isSelf()) {
              result.text = tr::lng_action_you_theme_changed(
                  tr::now, lt_emoji, {.text = text}, tr::marked);
            } else {
              result.links.push_back(fromLink());
              result.text = tr::lng_action_theme_changed(
                  tr::now, lt_from,
                  fromLinkText(), // Link 1.
                  lt_emoji, {.text = text}, tr::marked);
            }
          } else {
            if (_from->isSelf()) {
              result.text =
                  tr::lng_action_you_theme_disabled(tr::now, tr::marked);
            } else {
              result.links.push_back(fromLink());
              result.text =
                  tr::lng_action_theme_disabled(tr::now, lt_from,
                                                fromLinkText(), // Link 1.
                                                tr::marked);
            }
          }
        ...
        [&](const MTPDchatThemeUniqueGift &data) {
          data.vgift().match(
              [&](const MTPDstarGiftUnique &data) {
                const auto giftName = tr::bold(qs(data.vtitle()) + u" #"_q +
                                               QString::number(data.vnum().v));
                if (_from->isSelf()) {
                  result.text = tr::lng_action_you_gift_theme_changed(
                      tr::now, lt_name, giftName, tr::marked);
                } else {
                  result.links.push_back(fromLink());
                  result.text = tr::lng_action_gift_theme_changed(
                      tr::now, lt_from, fromLinkText(), lt_name, giftName,
                      tr::marked);
                }
              },
              [](const MTPDstarGift &) {});
  ...
  auto prepareChatJoinedByRequest =
      [this](const MTPDmessageActionChatJoinedByRequest &action) {
        auto result = PreparedServiceText();
        result.links.push_back(fromLink());
        result.text =
            tr::lng_action_user_joined_by_request(tr::now, lt_from,
                                                  fromLinkText(), // Link 1.
                                                  tr::marked);
        return result;
      };

  auto prepareWebViewDataSent =
      [](const MTPDmessageActionWebViewDataSent &action) {
        auto result = PreparedServiceText();
        result.text = tr::lng_action_webview_data_done(
            tr::now, lt_text, {.text = qs(action.vtext())}, tr::marked);
        return result;
      };
  ...
  auto prepareGiftPremium = [&](const MTPDmessageActionGiftPremium &action) {
    ...
    if (anonymous) {
      result.text = tr::lng_action_gift_received_anonymous(tr::now, lt_cost,
                                                           cost, tr::marked);
    } else {
      result.links.push_back(peer->createOpenLink());
      result.text =
          isSelf
              ? tr::lng_action_gift_sent(tr::now, lt_cost, cost, tr::marked)
              : tr::lng_action_gift_received(
                    tr::now, lt_user, tr::link(peer->shortName(), 1), // Link 1.
                    lt_cost, cost, tr::marked);
    }
    return result;
  };
  ...
  auto prepareTopicCreate = [&](const MTPDmessageActionTopicCreate &action) {
    ...
    result.text = tr::lng_action_topic_created(
        tr::now, lt_topic,
        tr::link(Data::ForumTopicIconWithTitle(
                     id, action.vicon_emoji_id().value_or_empty(),
                     qs(action.vtitle())),
                 topicUrl),
        tr::marked);
    return result;
  ...
  auto prepareTopicEdit = [&](const MTPDmessageActionTopicEdit &action) {
    ...
    if (const auto closed = action.vclosed()) {
      result.links.push_back(fromLink());
      result.text =
          (mtpIsTrue(*closed) ? tr::lng_action_topic_closed_inside_by(
                                    tr::now, lt_from,
                                    fromLinkText(), // Link 1.
                                    tr::marked)
      ...
      if (const auto icon = action.vicon_emoji_id()) {
        if (const auto iconId = icon->v) {
          ...
          result.text = tr::lng_action_topic_icon_changed(
              tr::now, lt_from,
              fromLinkText(), // Link 1.
              lt_link, {linkText}, lt_emoji, Data::SingleCustomEmoji(iconId),
              tr::marked);
        } else {
          result.links.push_back(fromLink());
      ...
      result.text = tr::lng_action_topic_renamed(
          tr::now, lt_from,
          fromLinkText(), // Link 1.
          lt_link, {linkText}, lt_title,
          Data::ForumTopicIconWithTitle(
              topicRootId(), action.vicon_emoji_id().value_or_empty(),
              qs(*action.vtitle())),
          tr::marked);
    }
  ...
  auto prepareSuggestProfilePhoto =
      [this](const MTPDmessageActionSuggestProfilePhoto &action) {
        auto result = PreparedServiceText{};
        const auto isSelf = (_from->id == _from->session().userPeerId());
        const auto isVideo = action.vphoto().match(
        ...
        const auto name = (user && !user->firstName.isEmpty()) ? user->firstName
                                                               : peer->name();
        result.links.push_back(peer->createOpenLink());
        result.text = (isSelf ? (isVideo ? tr::lng_action_suggested_video_me
                                         : tr::lng_action_suggested_photo_me)
                              : (isVideo ? tr::lng_action_suggested_video
                                         : tr::lng_action_suggested_photo))(
            tr::now, lt_user, tr::link(name, 1), // Link 1.
            tr::marked);
        return result;
      };
  ...
  auto prepareRequestedPeer =
      [&](const MTPDmessageActionRequestedPeer &action) {
        ...
        result.text = tr::lng_action_shared_chat_with_bot(
            tr::now, lt_chat, result.text, lt_bot,
            tr::link(history()->peer->name(), 1), tr::marked);
        return result;
      };
        ...
        result.text = isSelf
                          ? ((!same && both)
                                 ? tr::lng_action_set_wallpaper_both_me(
                                       tr::now, lt_user,
                                       tr::link(tr::bold(name), 1), tr::marked)
                                 : (same ? tr::lng_action_set_same_wallpaper_me
                                         : tr::lng_action_set_wallpaper_me)(
                                       tr::now, tr::marked))
                          : (same ? tr::lng_action_set_same_wallpaper
                                  : tr::lng_action_set_wallpaper)(
                                tr::now, lt_user, tr::link(tr::bold(name), 1),
                                tr::marked);
        return result;
      };
  ...
  auto prepareGiftCode = [&](const MTPDmessageActionGiftCode &action) {
    ...
    if (const auto boosted = action.vboost_peer()) {
      result.text = {
          (action.is_unclaimed()      ? tr::lng_prize_unclaimed_about
           : action.is_via_giveaway() ? tr::lng_prize_about
                                      : tr::lng_prize_gift_about)(
              tr::now, lt_channel,
              _from->owner().peer(peerFromMTP(*action.vboost_peer()))->name()),
      };
    } else {
      const auto isSelf = (_from->id == _from->session().userPeerId());
      const auto peer = isSelf ? _history->peer : _from;
      const auto cost =
          AmountAndStarCurrency(action.vamount().value_or_empty(),
  ...
  auto prepareGiveawayLaunch =
      [&](const MTPDmessageActionGiveawayLaunch &action) {
        ...
        result.text = credits
                          ? (_history->peer->isMegagroup()
                                ...
                                tr::lng_action_giveaway_credits_started_amount(
                                    tr::now, lt_count_decimal, float64(credits),
                                    tr::bold),
                                tr::marked)
                          : (_history->peer->isMegagroup()
  ...
  auto prepareGiveawayResults =
      [&](const MTPDmessageActionGiveawayResults &action) {
        auto result = PreparedServiceText();
        ...
        result.text = {(
            !winners ? tr::lng_action_giveaway_results_none(tr::now)
            : (credits && unclaimed)
                ? tr::lng_action_giveaway_results_credits_some(tr::now)
            : (!credits && unclaimed)
                ? tr::lng_action_giveaway_results_some(tr::now)
            : (credits && !unclaimed)
                ? tr::lng_action_giveaway_results_credits(tr::now, lt_count,
                                                          winners)
  ...
  auto prepareBoostApply = [&](const MTPDmessageActionBoostApply &action) {
    ...
    result.links.push_back(fromLink());
    result.text =
        isSelf ? tr::lng_action_boost_apply_me(tr::now, tr::marked)
               : tr::lng_action_boost_apply(tr::now, lt_count, boosts, lt_from,
                                            fromLinkText(), // Link 1.
                                            tr::marked);
    const auto channel = _history->peer->asChannel();
  ...
  auto preparePaymentRefunded =
      [&](const MTPDmessageActionPaymentRefunded &action) {
        auto result = PreparedServiceText();
        const auto refund = Get<HistoryServicePaymentRefund>();
        ...
        result.text = tr::lng_action_payment_refunded(
            tr::now, lt_peer, tr::link(refund->peer->name(), 1), // Link 1.
            lt_amount, AmountAndStarCurrency(amount, currency), tr::marked);
        return result;
      };

  auto prepareGiftStars = [&](const MTPDmessageActionGiftStars &action) {
    auto result = PreparedServiceText();
    const auto isSelf = (_from->id == _from->session().userPeerId());
    const auto peer = isSelf ? _history->peer : _from;
    _history->session().giftBoxStickersPacks().load();
  ...
  auto prepareGiftTon = [&](const MTPDmessageActionGiftTon &action) {
    auto result = PreparedServiceText();
    const auto isSelf = (_from->id == _from->session().userPeerId());
    const auto peer = isSelf ? _history->peer : _from;
  ...
  auto prepareGiftPrize = [&](const MTPDmessageActionPrizeStars &action) {
    auto result = PreparedServiceText();
    _history->session().giftBoxStickersPacks().load();
    result.text = {
        (action.is_unclaimed() ? tr::lng_prize_unclaimed_about
  ...
  auto prepareStarGift = [&](const MTPDmessageActionStarGift &action) {
    auto result = PreparedServiceText();
    ...
    const auto stars = action.vgift().match(
        ...
        [](const MTPDstarGiftUnique &) { return uint64(); });
    if (!stars) {
      if (!isSelf) {
        result.links.push_back(peer->createOpenLink());
      }
      result.text =
          isSelf
              ? tr::lng_action_gift_unique_sent(tr::now, tr::marked)
    ...
    const auto giftPeer =
        action.vpeer() ? peerFromMTP(*action.vpeer()) : PeerId();
    ...
    if (toChannel) {
      const auto fromId =
          action.vfrom_id() ? peerFromMTP(*action.vfrom_id()) : PeerId();
      const auto from = fromId ? peer->owner().peer(fromId).get() : nullptr;
      const auto channel = peer->owner().channel(peerToChannel(giftPeer));
      if (!from || from->isSelf()) {
        result.links.push_back(channel->createOpenLink());
        if (upgradeGifted) {
          result.text = tr::lng_action_gift_sent_upgrade_self_channel(
              tr::now, lt_cost, cost, lt_name, tr::link(channel->name(), 1),
              tr::marked);
        } else {
          result.text = tr::lng_action_gift_sent_self_channel(
              tr::now, lt_name, tr::link(channel->name(), 1), lt_cost, cost,
              tr::marked);
        }
      } else {
        ...
        if (upgradeGifted) {
          result.text = tr::lng_action_gift_sent_upgrade_self_other(
              tr::now, lt_cost, cost, lt_name, tr::link(channel->name(), 2),
              lt_user, tr::link(from->shortName(), 1), tr::marked);
        } else {
          result.text = tr::lng_action_gift_sent_channel(
              tr::now, lt_user, tr::link(from->shortName(), 1), lt_name,
              tr::link(channel->name(), 2), lt_cost, cost, tr::marked);
        }
      }
    } else if (anonymous || _history->peer->isSelf()) {
      result.text =
          (action.is_auction_acquired() ? tr::lng_action_gift_auction_won
           : anonymous ? tr::lng_action_gift_received_anonymous
                       : tr::lng_action_gift_self_bought)(tr::now, lt_cost,
                                                          cost, tr::marked);
      ...
      if (isSelf) {
        result.links.push_back(peer->createOpenLink());
        if (!from || from->isSelf()) {
          result.text = tr::lng_action_gift_sent_upgrade_self(tr::now, lt_cost,
                                                              cost, tr::marked);
        } else {
          result.links.push_back(from->createOpenLink());
          result.text = tr::lng_action_gift_sent_upgrade_self_other(
              tr::now, lt_cost, cost, lt_name, tr::link(peer->shortName(), 1),
              lt_user, tr::link(from->shortName(), 2), tr::marked);
        }
      } else {
        result.links.push_back(peer->createOpenLink());
        if (from && from != peer && !from->isSelf()) {
          ...
          result.text = tr::lng_action_gift_sent_upgrade_other(
              tr::now, lt_from, tr::link(peer->shortName(), 1), lt_cost, cost,
              lt_user, tr::link(from->shortName(), 2), tr::marked);
        } else {
            ...
            result.text =
                resale
                    ? tr::lng_action_gift_sent_channel(
                          tr::now, lt_user, tr::link(from->shortName(), 1),
                          lt_name, tr::link(channel->name(), 2), lt_cost,
                          resaleCost, tr::marked)
                    : (action.is_upgrade()
                           ? tr::lng_action_gift_upgraded_channel
                           : tr::lng_action_gift_transferred_channel)(
                          tr::now, lt_user, tr::link(from->shortName(), 1),
                          lt_channel, tr::link(channel->name(), 2), tr::marked);
          } else {
            result.text =
                resale ? tr::lng_action_gift_sent_self_channel(
                             tr::now, lt_name, tr::link(channel->name(), 1),
                             lt_cost, resaleCost, tr::marked)
                       : (from->isServiceUser()
                          ...
                          : action.is_upgrade()
                             ...
                             tr::now, lt_channel, tr::link(channel->name(), 1),
                             tr::marked);
          }
          result.links.push_back(channel->createOpenLink());
        } else {
          if (upgradeHelped) {
            ...
            if (isSelf) {
              result.text = tr::lng_action_gift_upgraded_helped_self(
                  tr::now, lt_user, tr::link(peer->shortName(), 1), tr::marked);
            } else {
            ...
            result.text =
                fromOffer
                    ? (isSelf ? tr::lng_action_gift_sent_sold(
                                    tr::now, lt_cost, resaleCost, tr::marked)
                              : tr::lng_action_gift_received_sold(
                                    tr::now, lt_user,
                                    tr::link(peer->shortName(), 1), // Link 1.
                                    lt_cost, resaleCost, tr::marked))
                : resale
                    ? (isSelf ? tr::lng_action_gift_sent(tr::now, lt_cost,
                                                         resaleCost, tr::marked)
                              : tr::lng_action_gift_received(
                                    tr::now, lt_user,
                                    tr::link(peer->shortName(), 1), // Link 1.
                                    lt_cost, resaleCost, tr::marked))
                    : (action.is_upgrade()
                           ? (isSelf ? tr::lng_action_gift_upgraded_mine
                                     : tr::lng_action_gift_upgraded)
                           : (isSelf ? tr::lng_action_gift_transferred_mine
                                     : tr::lng_action_gift_transferred))(
                          tr::now, lt_user, tr::link(from->shortName(), 1),
                          tr::marked);
          } else if (action.is_assigned()) {
            const auto gift =
                Api::FromTL(&history()->session(), action.vgift());
            result.text = tr::lng_action_gift_displayed_self(
                tr::now, lt_name,
                TextWithEntities{
                    (gift && gift->unique) ? Data::UniqueGiftName(*gift->unique)
                                           : QString(),
                },
                tr::marked);
          } else if (action.is_craft()) {
            result.text = tr::lng_action_gift_crafted(tr::now, tr::marked);
          } else {
            result.text =
                resale ? tr::lng_action_gift_self_bought(tr::now, lt_cost,
                                                         resaleCost, tr::marked)
                       : (from->isServiceUser()
      ...
      [&](const MTPDmessageActionPaidMessagesRefunded &action) {
        ...
        if (_from->isSelf()) {
          ...
          result.text = tr::lng_action_paid_message_refund(
              tr::now, lt_count, action.vstars().v, lt_from,
              tr::link(_from->shortName(), 1), tr::marked);
        }
        return result;
      };

  auto preparePaidMessagesPrice =
      [&](const MTPDmessageActionPaidMessagesPrice &action) {
        ...
        result.text =
            _history->peer->isBroadcast()
                ? (stars > 0          ? tr::lng_action_direct_messages_paid(
                                            tr::now, lt_count, stars, tr::marked)
                   : broadcastAllowed ? tr::lng_action_direct_messages_enabled(
                                            tr::now, tr::marked)
                                      : tr::lng_action_direct_messages_disabled(
                                            tr::now, tr::marked))
            : stars ? tr::lng_action_message_price_paid(tr::now, lt_count,
                                                        stars, tr::marked)
                    : tr::lng_action_message_price_free(tr::now, tr::marked);
        return result;
      };
  ...
  auto preparePollAppendAnswer =
      [&](const MTPDmessageActionPollAppendAnswer &) {
  ...
  auto prepareSuggestedPostApproval =
      [&](const MTPDmessageActionSuggestedPostApproval &data) {
        return PreparedServiceText{
            {data.is_rejected() ? tr::lng_action_post_rejected(tr::now)
             : data.is_balance_too_low()
                 ? tr::lng_action_not_enough_funds(tr::now)
                 : tr::lng_suggest_action_agreement(tr::now)}};
      };

  auto prepareSuggestedPostSuccess =
      [&](const MTPDmessageActionSuggestedPostSuccess &data) {
        const auto price = CreditsAmountFromTL(&data.vprice());
        auto result = PreparedServiceText();
        result.links.push_back(_from->createOpenLink());
        result.text = (price.stars() ? tr::lng_action_suggest_success_stars
                                     : tr::lng_action_suggest_success_ton)(
            tr::now, lt_count_decimal, price.value(), lt_from,
            tr::link(_from->shortName(), 1), tr::marked);
        return result;
      };

  auto prepareSuggestedPostRefund =
      [&](const MTPDmessageActionSuggestedPostRefund &data) {
        return PreparedServiceText{
            {data.is_payer_initiated()
                 ? tr::lng_action_suggest_refund_user(tr::now)
                 : tr::lng_action_suggest_refund_admin(tr::now)}};
      };

  auto prepareConferenceCall =
      [&](const MTPDmessageActionConferenceCall &) -> PreparedServiceText {
    Unexpected("PhoneCall type in setServiceMessageFromMtp.");
  };

  auto prepareSuggestBirthday =
      [&](const MTPDmessageActionSuggestBirthday &action) {
        auto result = PreparedServiceText{};
        const auto isSelf = (_from->id == _from->session().userPeerId());
        const auto peer = isSelf ? history()->peer : _from;
        const auto user = peer->asUser();
        const auto name = (user && !user->firstName.isEmpty()) ? user->firstName
                                                               : peer->name();
        result.links.push_back(peer->createOpenLink());
        result.text = (isSelf ? tr::lng_action_suggested_birthday_me
  ...
  auto prepareStarGiftPurchaseOffer =
      [&](const MTPDmessageActionStarGiftPurchaseOffer &action) {
        auto result = PreparedServiceText{};
        action.vgift().match(
            [&](const MTPDstarGiftUnique &data) {
              ...
              if (_from->isSelf()) {
                result.text = tr::lng_action_gift_offer_you(
                    tr::now, lt_cost, cost, lt_name, giftName, tr::marked);
              } else {
                result.text = tr::lng_action_gift_offer_incoming(
                    tr::now, lt_amount, cost, tr::marked);
              }
            },
            [](const MTPDstarGift &) {});
        return result;
  ...
  auto prepareStarGiftPurchaseOfferDeclined =
      [&](const MTPDmessageActionStarGiftPurchaseOfferDeclined &action) {
        auto result = PreparedServiceText{};
        action.vgift().match(
            [&](const MTPDstarGiftUnique &data) {
              const auto amount = CreditsAmountFromTL(action.vprice());
              ...
              if (_from->isSelf()) {
                ...
                result.text =
                    expired
                        ? tr::lng_action_gift_offer_expired(
                              ...
                              lt_name, giftName, lt_cost, cost, tr::marked)
                ...
                result.text =
                    expired ? tr::lng_action_gift_offer_expired_your(
                                  tr::now, lt_name, giftName, lt_cost, cost,
                                  tr::marked)
                            : tr::lng_action_gift_offer_declined(
                                  tr::now, lt_user, fromLinkText(), lt_name,
                                  giftName, lt_cost, cost, tr::marked);
              }
            },
            [](const MTPDstarGift &) {});
        return result;
  ...
  auto prepareNewCreatorPending =
      [this](const MTPDmessageActionNewCreatorPending &action) {
        auto result = PreparedServiceText();
        auto user = _history->owner().user(action.vnew_creator_id().v);
        result.links.push_back(fromLink());
        result.links.push_back(user->createOpenLink());
        result.text = tr::lng_action_new_creator_pending(
            tr::now, lt_user, tr::link(user->name(), 2), lt_from,
            fromLinkText(), tr::marked);
        return result;
      };

  auto prepareChangeCreator =
      [this](const MTPDmessageActionChangeCreator &action) {
        auto result = PreparedServiceText();
        auto user = _history->owner().user(action.vnew_creator_id().v);
        result.links.push_back(fromLink());
        result.links.push_back(user->createOpenLink());
        result.text = tr::lng_action_change_creator(
            tr::now, lt_from, fromLinkText(), lt_user,
            tr::link(user->name(), 2), tr::marked);
        return result;
      };

  auto prepareNoForwardsToggle =
      [this](const MTPDmessageActionNoForwardsToggle &action) {
        auto result = PreparedServiceText();
        const auto enabled = mtpIsTrue(action.vnew_value());
        const auto prevEnabled = mtpIsTrue(action.vprev_value());
        if (enabled == prevEnabled) {
          result.text =
              tr::lng_action_no_forwards_still_disabled(tr::now, tr::marked);
        } else if (out()) {
          result.text = (enabled ? tr::lng_action_no_forwards_you_disabled
                                 : tr::lng_action_no_forwards_you_enabled)(
              tr::now, tr::marked);
        } else {
          result.links.push_back(fromLink());
          result.text = (enabled ? tr::lng_action_no_forwards_disabled
  ...
  auto prepareNoForwardsRequest =
      [this](const MTPDmessageActionNoForwardsRequest &action) {
        auto result = PreparedServiceText();
        const auto nfr = Get<HistoryServiceNoForwardsRequest>();
        if (nfr && nfr->expired && !nfr->actionTaken) {
          result.text =
              tr::lng_action_no_forwards_request_expired(tr::now, tr::marked);
        } else if (_from->isSelf()) {
          result.text =
              tr::lng_action_no_forwards_request_you(tr::now, tr::marked);
        } else {
          result.links.push_back(fromLink());
  ...
  auto prepareManagedBotCreated =
      [this](const MTPDmessageActionManagedBotCreated &action) {
        auto result = PreparedServiceText();
        ...
        result.text = tr::lng_action_managed_bot_created(
            tr::now, lt_from, fromLinkText(), lt_bot, tr::link(bot->name(), 2),
            tr::marked);
        return result;
      };

  setServiceText(action.match(
      prepareChatAddUserText, prepareChatJoinedByLink, prepareChatCreate,
      PrepareEmptyText<MTPDmessageActionChatMigrateTo>,
      PrepareEmptyText<MTPDmessageActionChannelMigrateFrom>,
      PrepareEmptyText<MTPDmessageActionHistoryClear>, prepareChannelCreate,
      prepareChatDeletePhoto, prepareChatDeleteUser, prepareChatEditPhoto,
      prepareChatEditTitle, preparePinMessage, prepareGameScore,
      preparePhoneCall, preparePaymentSent, prepareScreenshotTaken,
      prepareCustomAction, prepareBotAllowed, prepareSecureValuesSent,
      prepareContactSignUp, prepareProximityReached, preparePaymentSentMe,
      PrepareErrorText<MTPDmessageActionSecureValuesSentMe>, prepareGroupCall,
      prepareInviteToGroupCall, prepareSetMessagesTTL,
      prepareGroupCallScheduled, prepareSetChatTheme,
      prepareChatJoinedByRequest, prepareWebViewDataSent, prepareGiftPremium,
      prepareTopicCreate, prepareTopicEdit,
      PrepareErrorText<MTPDmessageActionWebViewDataSentMe>,
      prepareSuggestProfilePhoto, prepareRequestedPeer, prepareSetChatWallPaper,
      prepareGiftCode, prepareGiveawayLaunch, prepareGiveawayResults,
      prepareBoostApply, preparePaymentRefunded, prepareGiftStars,
      prepareGiftTon, prepareGiftPrize, prepareStarGift, prepareStarGiftUnique,
      preparePaidMessagesRefunded, preparePaidMessagesPrice,
      prepareConferenceCall, prepareTodoCompletions, prepareTodoAppendTasks,
      preparePollAppendAnswer, prepareSuggestedPostApproval,
      prepareSuggestedPostSuccess, prepareSuggestedPostRefund,
      prepareSuggestBirthday, prepareStarGiftPurchaseOffer,
      prepareStarGiftPurchaseOfferDeclined, prepareNewCreatorPending,
      prepareChangeCreator, prepareNoForwardsToggle, prepareNoForwardsRequest,
  ...
  action.match(
        ...
        data.vphoto().match(
            [&](const MTPDphoto &photo) {
              _media = std::make_unique<Data::MediaPhoto>(
                  this, _history->peer, _history->owner().processPhoto(photo));
            },
      ...
      [&](const MTPDmessageActionGiftPremium &data) {
        ...
        _media = std::make_unique<Data::MediaGiftBox>(
            this, _from,
            Data::GiftCode{
                .message = (data.vmessage() ? Api::ParseTextWithEntities(
                                                  session, *data.vmessage())
                                            : TextWithEntities()),
                .count = data.vdays().v,
      ...
      [&](const MTPDmessageActionGiftCode &data) {
        const auto boostedId =
            data.vboost_peer() ? peerToChannel(peerFromMTP(*data.vboost_peer()))
                               : ChannelId();
        _media = std::make_unique<Data::MediaGiftBox>(
            this, _from,
            Data::GiftCode{
                ...
                .message = (data.vmessage()
                                ? Api::ParseTextWithEntities(
                                      &history()->session(), *data.vmessage())
                                : TextWithEntities()),
                .channel =
                    (boostedId ? history()->owner().channel(boostedId).get()
                               : nullptr),
                .count = data.vdays().v,
                .type = Data::GiftType::Premium,
      ...
      [&](const MTPDmessageActionGiftStars &data) {
        _media = std::make_unique<Data::MediaGiftBox>(
            this, _from, Data::GiftType::Credits, data.vstars().v);
      },
      [&](const MTPDmessageActionGiftTon &data) {
        _media = std::make_unique<Data::MediaGiftBox>(
            this, _from, Data::GiftType::Ton, data.vcrypto_amount().v);
      },
      ...
      [&](const MTPDmessageActionStarGift &data) {
        const auto service = _from->isServiceUser();
        const auto from =
            data.vfrom_id() ? peerFromMTP(*data.vfrom_id()) : PeerId();
        const auto to = data.vpeer() ? peerFromMTP(*data.vpeer()) : PeerId();
        const auto upgradeMsgId = data.vupgrade_msg_id().value_or_empty();
        const auto giftMsgId = data.vgift_msg_id().value_or_empty();
        const auto title = data.vgift().match(
            [&](const MTPDstarGift &gift) {
              return qs(gift.vtitle().value_or_empty());
            },
            [](const MTPDstarGiftUnique &) { return QString(); });
        const auto bid = data.vgift().match(
            [&](const MTPDstarGift &gift) {
            ...
            [](const MTPDstarGiftUnique &) { return 0; });
        const auto channel =
            (service && peerIsChannel(to))
                ? history()->owner().channel(peerToChannel(to)).get()
                : nullptr;
        const auto channelSavedId =
            channel ? data.vsaved_id().value_or_empty() : uint64();
        const auto realGiftMsgId =
            (peerIsUser(to) && data.vsaved_id())
        ...
        auto fields = Fields{
            .message = (data.vmessage()
                            ? Api::ParseTextWithEntities(&history()->session(),
                                                         *data.vmessage())
                            : TextWithEntities()),
            .auctionTo =
                (service && data.is_auction_acquired() && data.vto_id())
                    ? history()->owner().peer(peerFromMTP(*data.vto_id())).get()
                    : nullptr,
            .channel = channel,
            .channelFrom =
                ((service && from) ? history()->owner().peer(from).get()
                                   : nullptr),
            ...
            .starsUpgradedBySender =
                int(data.vupgrade_stars().value_or_empty()),
            .starsBid = bid,
        ...
        _media = std::make_unique<Data::MediaGiftBox>(this, _from,
                                                      std::move(fields));
      },
      [&](const MTPDmessageActionStarGiftUnique &data) {
        ...
        auto fields = Fields{
            ...
            .channelSavedId = channelSavedId,
            .realGiftMsgId = realGiftMsgId,
            .starsForDetailsRemove =
                int(data.vdrop_original_details_stars().value_or_empty()),
            .type = Data::GiftType::StarGift,
      ...
      [&](const MTPDmessageActionSuggestBirthday &data) {
        ...
        _media = std::make_unique<Data::MediaGiftBox>(
            this, _from,
            Data::GiftCode{
                .count = Data::Birthday(fields.vday().v, fields.vmonth().v,
...
void HistoryItem::setSelfDestruct(HistorySelfDestructType type,
                                  MTPint mtpTTLvalue) {
...
PreparedServiceText HistoryItem::prepareInvitedToCallText(
    const std::vector<not_null<UserData *>> &users, CallId linkCallId) {
  auto chatText = tr::lng_action_invite_user_chat(tr::now, tr::marked);
  auto result = PreparedServiceText();
  result.links.push_back(fromLink());
  auto linkIndex = 1;
  if (linkCallId) {
  ...
  if (users.size() == 1) {
    ...
    result.text = tr::lng_action_invite_user(
        tr::now, lt_from,
        fromLinkText(),                               // Link 1.
        lt_user, tr::link(user->name(), ++linkIndex), // Link N.
        lt_chat, chatText, tr::marked);
  } else if (users.empty()) {
    result.text = tr::lng_action_invite_user(tr::now, lt_from,
                                             fromLinkText(), // Link 1.
                                             lt_user, {.text = u"somebody"_q},
                                             lt_chat, chatText, tr::marked);
  } else {
    for (auto i = 0, l = int(users.size()); i != l; ++i) {
    ...
    result.text = tr::lng_action_invite_users_many(
        tr::now, lt_from,
        fromLinkText(), // Link 1.
        lt_users, result.text, lt_chat, chatText, tr::marked);
  }
  return result;
}
  ...
  if (pinned && pinned->msg) {
    ...
    if (mediaText.isEmpty()) {
      ...
      for (; limit != 0;) {
        ...
        if (cutAt >= size)
          break;
        if (original.text.at(cutAt).isLowSurrogate() && (cutAt + 1 < size) &&
      ...
      if (!limit && cutAt + 5 < size) {
        original = Ui::Text::Mid(original, 0, cutAt).append(Ui::kQEllipsis);
      }
      original = tr::link(Ui::Text::Filtered(std::move(original),
                                             {
                                                 EntityType::Spoiler,
                          ...
                          2);
      result.text =
          tr::lng_action_pinned_message(tr::now, lt_from,
                                        fromLinkText(), // Link 1.
                                        lt_text,
      ...
      result.text = tr::lng_action_pinned_media(
          tr::now, lt_from,
          fromLinkText(),                   // Link 1.
          lt_media, tr::link(mediaText, 2), // Link 2.
          tr::marked);
    }
    ...
    result.text = tr::lng_action_pinned_media(
        tr::now, lt_from,
        fromLinkText(),                                           // Link 1.
        lt_media, tr::link(tr::lng_contacts_loading(tr::now), 2), // Link 2.
        tr::marked);
  } else {
    result.links.push_back(fromLink());
    result.text = tr::lng_action_pinned_media(
        tr::now, lt_from,
        fromLinkText(), // Link 1.
        lt_media, {.text = tr::lng_deleted_message(tr::now)}, tr::marked);
  }
  return result;
}
    ...
    if (gamescore && gamescore->msg) {
      if (const auto media = gamescore->msg->media()) {
        if (const auto game = media->game()) {
          const auto row = 0;
          const auto column = 0;
          result.links.push_back(std::make_shared<ReplyMarkupClickHandler>(
              &_history->owner(), row, column, gamescore->msg->fullId()));
  ...
  if (_from->isSelf()) {
    ...
    if (gameTitle.text.isEmpty()) {
      result.text = tr::lng_action_game_you_scored_no_game(
          tr::now, lt_count, scoreNumber, tr::marked);
    } else {
      result.text = tr::lng_action_game_you_scored(
          tr::now, lt_count, scoreNumber, lt_game, gameTitle, tr::marked);
    }
  } else {
    result.links.push_back(fromLink());
    auto gameTitle = computeGameTitle();
    if (gameTitle.text.isEmpty()) {
      result.text = tr::lng_action_game_score_no_game(tr::now, lt_count,
                                                      scoreNumber, lt_from,
                                                      fromLinkText(), // Link 1.
                                                      tr::marked);
    } else {
      result.text =
          tr::lng_action_game_score(tr::now, lt_count, scoreNumber, lt_from,
                                    fromLinkText(), // Link 1.
                                    lt_game, gameTitle, tr::marked);
    }
  }
  return result;
}
  ...
  if (invoiceTitle.text.isEmpty()) {
    if (payment->recurringUsed) {
      result.text = tr::lng_action_payment_used_recurring(
          tr::now, lt_amount, payment->amount, tr::marked);
    } else {
      result.text =
          (payment->recurringInit ? tr::lng_action_payment_init_recurring
                                  : tr::lng_action_payment_done)(
              tr::now, lt_amount, payment->amount, lt_user,
              {.text = _history->peer->name()}, tr::marked);
    }
  } else {
    result.text = (payment->recurringInit
        ...
        tr::now, lt_amount, payment->amount, lt_user,
        {.text = _history->peer->name()}, lt_invoice, invoiceTitle, tr::marked);
    if (payment->msg) {
...
PreparedServiceText HistoryItem::prepareStoryMentionText() {
  ...
  const auto phrase = (this->media() && this->media()->storyExpired(true))
                          ? (out() ? tr::lng_action_story_mention_me_unavailable
                                   : tr::lng_action_story_mention_unavailable)
                          : (out() ? tr::lng_action_story_mention_me
                                   : tr::lng_action_story_mention);
  result.text = phrase(tr::now, lt_user,
                       Ui::Text::Wrapped(tr::bold(peer->shortName()),
...
PreparedServiceText HistoryItem::prepareCallScheduledText(TimeId scheduleDate) {
  const auto call = Get<HistoryServiceOngoingCall>();
  Assert(call != nullptr);
  ...
  const auto prepareWithDate = [&](const QString &date) {
    if (_history->peer->isBroadcast()) {
      result.text = tr::lng_action_group_call_scheduled_channel(
          tr::now, lt_date, {.text = date}, tr::marked);
    } else {
      result.links.push_back(fromLink());
      result.text = tr::lng_action_group_call_scheduled_group(
          tr::now, lt_from,
          fromLinkText(), // Link 1.
          lt_date, {.text = date}, tr::marked);
    }
  };
  ...
  const auto prepareGeneric = [&] {
    prepareWithDate(tr::lng_group_call_starts_date(
        tr::now, lt_date, langDayOfMonthFull(date), lt_time, time));
  };
  ...
  if (now.date().addDays(1) < scheduled.date()) {
    ...
    prepareWithDate(tr::lng_group_call_starts_tomorrow(tr::now, lt_time, time));
  } else if (now.date() == scheduled.date()) {
    nextIn = secsToDateAddDays(1);
    prepareWithDate(tr::lng_group_call_starts_today(tr::now, lt_time, time));
  } else {
    prepareGeneric();
  }
  if (nextIn) {
    call->lifetime =
        base::timer_once((nextIn + 2) * crl::time(1000)) | rpl::on_next([=] {
...
PreparedServiceText HistoryItem::composeTodoIncompleted(
  ...
  if (out()) {
    return {
        tr::lng_action_todo_marked_not_done_self(tr::now, lt_tasks, tasks,
                                                 tr::marked),
    };
  }
  return {
      .text = tr::lng_action_todo_marked_not_done(
          tr::now, lt_from, fromLinkText(), lt_tasks, tasks, tr::marked),
      .links = {fromLink()},
  };
}
...
PreparedServiceText HistoryItem::composeTodoCompleted(
  ...
  if (out()) {
    return {
        tr::lng_action_todo_marked_done_self(tr::now, lt_tasks, tasks,
                                             tr::marked),
    };
  }
  return {
      .text = tr::lng_action_todo_marked_done(tr::now, lt_from, fromLinkText(),
                                              lt_tasks, tasks, tr::marked),
      .links = {fromLink()},
  };
}
...
PreparedServiceText HistoryItem::prepareTodoCompletionsText() {
  ...
  return done->completed.empty() ? composeTodoIncompleted(done)
...
PreparedServiceText HistoryItem::prepareTodoAppendTasksText() {
  ...
  if (out()) {
    return {
        tr::lng_action_todo_added_self(tr::now, lt_tasks, tasks, tr::marked),
    };
  }
  return {
      .text = tr::lng_action_todo_added(tr::now, lt_from, fromLinkText(),
                                        lt_tasks, tasks, tr::marked),
      .links = {fromLink()},
  };
  return result;
...
PreparedServiceText HistoryItem::preparePollAppendAnswerText() {
  const auto append = Get<HistoryServicePollAppendAnswer>();
  const auto option = append ? append->answer.text.text : QString();
  if (out()) {
    return {
        tr::lng_action_poll_added_answer_self(tr::now, lt_option, {option},
                                              tr::marked),
    };
  }
  return {
      .text = tr::lng_action_poll_added_answer(tr::now, lt_from, fromLinkText(),
                                               lt_option, {option}, tr::marked),
      .links = {fromLink()},
  };
}
...
PreparedServiceText HistoryItem::preparePollDeleteAnswerText() {
  const auto del = Get<HistoryServicePollDeleteAnswer>();
  const auto option = del ? del->answer.text.text : QString();
  if (out()) {
    return {
        tr::lng_action_poll_deleted_answer_self(tr::now, lt_option, {option},
                                                tr::marked),
    };
  }
  return {
      .text = tr::lng_action_poll_deleted_answer(
          tr::now, lt_from, fromLinkText(), lt_option, {option}, tr::marked),
      .links = {fromLink()},
  };
}
...
void HistoryItem::setupChatThemeChange() {
  if (const auto user = history()->peer->asUser()) {
    auto link = std::make_shared<LambdaClickHandler>([=](ClickContext context) {
      const auto my = context.other.value<ClickHandlerContext>();
...
void HistoryItem::setupTTLChange() {
  ...
  auto link = std::make_shared<LambdaClickHandler>([=](ClickContext context) {
    const auto my = context.other.value<ClickHandlerContext>();
    if (const auto controller = my.sessionWindow.get()) {
      const auto validator = TTLMenu::TTLValidator(controller->uiShow(), peer);
      if (validator.can()) {
        validator.showBox();
      }
    }
...
void HistoryItem::clearDependencyMessage() {
  if (const auto dependent = GetServiceDependentData()) {
    if (dependent->msg) {
      _history->owner().unregisterDependentMessage(this, dependent->msg);
      dependent->msg = nullptr;
      dependent->msgId = 0;
    }
  }

...
WithLocalFlag(HistoryItemCommonFields fields) {
  fields.flags |= MessageFlag::Local;
  return fields;
}

[[nodiscard]] Dialogs::UnreadState
AdjustedForumUnreadState(Dialogs::UnreadState state) {
...
[[nodiscard]] UserData *GuestChatBotForCurrentUser(
  ...
  if (!guestChat || !guestChat->visitor || (guestChat->visitor->id != self->id)) {
    return nullptr;
  }
  const auto bot = item->from()->asUser();
  return (bot && bot->isBot() && bot->botInfo && bot->botInfo->supportsGuestChat)
             ? bot
             : nullptr;
}
...
History::History(not_null<Data::Session *> owner, PeerId peerId)
    : Thread(owner, Type::History), peer(owner->peer(peerId)),
...
int History::height() const { return _height; }
...
void History::itemVanished(not_null<HistoryItem *> item) {
  ...
  if ((!item->out() || item->isPost()) && item->unread(this) &&
      unreadCount() > 0) {
...
void History::takeLocalDraft(not_null<History *> from) {
  ...
  const auto i =
      from->_drafts.find(Data::DraftKey::Local(topicRootId, monoforumPeerId));
  if (i == end(from->_drafts)) {
    return;
  }
  auto &draft = i->second;
  if (!draft->textWithTags.text.isEmpty() &&
      !_drafts.contains(Data::DraftKey::Local(topicRootId, monoforumPeerId))) {
...
void History::createLocalDraftFromCloud(MsgId topicRootId,
  ...
  if (Data::DraftIsNull(existing) || !existing->date ||
      draft->date >= existing->date) {
    if (!existing) {
      setLocalDraft(std::make_unique<Data::Draft>(
          draft->textWithTags, draft->reply, draft->suggest, draft->cursor,
          draft->webpage));
...
void History::setDraft(Data::DraftKey key,
                       std::unique_ptr<Data::Draft> &&draft) {
  if (!key) {
    return;
  }
  const auto cloudThread =
      key.isCloud() ? threadFor(key.topicRootId(), key.monoforumPeerId())
                    : nullptr;
  if (cloudThread) {
...
const Data::HistoryDrafts &History::draftsMap() const { return _drafts; }
...
void History::clearDraft(Data::DraftKey key) { setDraft(key, nullptr); }
...
Data::Draft *History::createCloudDraft(MsgId topicRootId,
                                       PeerId monoforumPeerId,
  ...
  if (Data::DraftIsNull(fromDraft)) {
    setCloudDraft(std::make_unique<Data::Draft>(
        ...
        SuggestOptions(), MessageCursor(), Data::WebPageDraft()));
    ...
    if (!existing) {
      ...
      setCloudDraft(std::make_unique<Data::Draft>(
          fromDraft->textWithTags, reply, fromDraft->suggest, fromDraft->cursor,
          fromDraft->webpage));
...
bool History::skipCloudDraftUpdate(MsgId topicRootId, PeerId monoforumPeerId,
                                   TimeId date) const {
...
void History::startSavingCloudDraft(MsgId topicRootId, PeerId monoforumPeerId) {
  const auto key = Data::DraftKey::Local(topicRootId, monoforumPeerId);
  ++_savingCloudDraftRequests[key];
}

void History::finishSavingCloudDraft(MsgId topicRootId, PeerId monoforumPeerId,
                                     TimeId savedAt) {
  const auto key = Data::DraftKey::Local(topicRootId, monoforumPeerId);
...
void History::applyCloudDraft(MsgId topicRootId, PeerId monoforumPeerId) {
  if (!topicRootId && session().supportMode()) {
    ...
    if (const auto thread = threadFor(topicRootId, monoforumPeerId)) {
      thread->updateChatListSortPosition();
      if (topicRootId) {
        session().changes().topicUpdated(thread->asTopic(),
                                         Data::TopicUpdate::Flag::CloudDraft);
      } else if (monoforumPeerId) {
        session().changes().sublistUpdated(
            thread->asSublist(), Data::SublistUpdate::Flag::CloudDraft);
      } else {
...
const Data::ForwardDraft &History::forwardDraft(MsgId topicRootId,
...
History::resolveForwardDraft(const Data::ForwardDraft &draft) const {
...
Data::ResolvedForwardDraft
History::resolveForwardDraft(MsgId topicRootId, PeerId monoforumPeerId) {
...
void History::setForwardDraft(MsgId topicRootId, PeerId monoforumPeerId,
  ...
  if (changed) {
    if (const auto thread = threadFor(topicRootId, monoforumPeerId)) {
      session().changes().entryUpdated(thread,
                                       Data::EntryUpdate::Flag::ForwardDraft);
    }
  }
}

not_null<HistoryItem *> History::createItem(MsgId id, const MTPMessage &message,
  ...
  const auto result = message.match(
      [&](const auto &data) { return makeMessage(id, data, localFlags); });
...
History::createItems(const QVector<MTPMessage> &data) {
  ...
  for (auto i = data.cend(), e = data.cbegin(); i != e;) {
    ...
    result.emplace_back(createItem(id, data, localFlags, detachExistingItem));
  }
  return result;
}

not_null<HistoryItem *> History::addNewMessage(MsgId id,
  ...
  const auto item =
      createItem(id, message, localFlags, detachExisting, newMessage);
...
not_null<HistoryItem *> History::insertItem(std::unique_ptr<HistoryItem> item) {
...
void History::destroyMessage(not_null<HistoryItem *> item) {
    ...
    if (item->isRegular()) {
      ...
      if (const auto types = item->sharedMediaTypes()) {
        session().storage().remove(
            Storage::SharedMediaRemoveOne(peerId, types, item->id));
      }
    }
    itemRemoved(item);
  }
  ...
  const auto documentToCancel = [&] {
    const auto media = item->isAdminLogEntry() ? nullptr : item->media();
...
void History::destroyMessagesByDates(TimeId minDate, TimeId maxDate) {
  ...
  for (const auto &message : _items) {
    if (message->isRegular() && message->date() > minDate &&
...
void History::editHistoryMessages(PeerData *peer, bool isHide) {
  for (const auto &message : _items) {
    if (message->isRegular() && !message->isService() &&
        message->from() == peer) {
      if (isHide) {
...
void History::unpinMessagesFor(MsgId topicRootId, PeerId monoforumPeerId) {
  if (topicRootId) {
    session().storage().remove(Storage::SharedMediaRemoveAll(
        peer->id, topicRootId, Storage::SharedMediaType::Pinned));
    ...
    session().storage().remove(Storage::SharedMediaRemoveAll(
        peer->id, monoforumPeerId, Storage::SharedMediaType::Pinned));
    if (const auto sublist = peer->monoforumSublistFor(monoforumPeerId)) {
      sublist->setHasPinnedMessages(false);
    }
    for (const auto &item : _items) {
      if (item->isPinned() && item->sublistPeerId() == monoforumPeerId) {
        item->setIsPinned(false);
      }
    }
  } else {
    session().storage().remove(Storage::SharedMediaRemoveAll(
        peer->id, Storage::SharedMediaType::Pinned));
...
not_null<HistoryItem *> History::addNewItem(not_null<HistoryItem *> item,
                                            bool unread) {
  if (item->isScheduled()) {
  ...
  const auto shouldMarkBottomNotLoaded =
      loadedAtBottom() && !unread && !isEmpty();
...
History::addNewLocalMessage(HistoryItemCommonFields &&fields,
  ...
  return addNewItem(makeMessage(WithLocalFlag(std::move(fields)), text, media),
                    true);
}
...
History::addNewLocalMessage(HistoryItemCommonFields &&fields,
  ...
  return addNewItem(
      makeMessage(WithLocalFlag(std::move(fields)), forwardOriginal), true);
}
...
History::addNewLocalMessage(HistoryItemCommonFields &&fields,
  ...
  return addNewItem(
      makeMessage(WithLocalFlag(std::move(fields)), document, caption), true);
}
...
History::addNewLocalMessage(HistoryItemCommonFields &&fields,
                            ...
                            const TextWithEntities &caption) {
  return addNewItem(
      makeMessage(WithLocalFlag(std::move(fields)), photo, caption), true);
}

not_null<HistoryItem *>
History::addNewLocalMessage(HistoryItemCommonFields &&fields,
                            not_null<GameData *> game) {
  return addNewItem(makeMessage(WithLocalFlag(std::move(fields)), game), true);
}

not_null<HistoryItem *>
History::addNewLocalMessage(not_null<HistoryItem *> item) {
...
not_null<HistoryItem *>
History::addSponsoredMessage(MsgId id, Data::SponsoredFrom from,
                             const TextWithEntities &textWithEntities) {
  return addNewItem(makeMessage(id, from, textWithEntities, nullptr), true);
}
...
void History::clearUnreadReactionsFor(MsgId topicRootId,
  ...
  if ((!topicRootId && !sublist) || (!topicRootId && forum) ||
      (!sublist && monoforum)) {
    if (forum) {
...
int History::unreadPollVotesCount() const { return _unreadPollVotesCount; }
...
not_null<HistoryItem *> History::addNewToBack(not_null<HistoryItem *> item,
                                              bool unread) {
  ...
  if (!unread && item->isRegular()) {
    ...
    if (const auto types = item->sharedMediaTypes()) {
      ...
      storage.add(Storage::SharedMediaAddExisting(peer->id,
                                                  MsgId(0),  // topicRootId
      ...
      if (const auto topic = item->topic()) {
        storage.add(Storage::SharedMediaAddExisting(peer->id, topic->rootId(),
                                                    PeerId(), // monoforumPeerId
                                                    types, item->id,
                                                    {item->id, item->id}));
        if (pinned) {
      ...
      if (const auto sublist = item->savedSublist()) {
        storage.add(Storage::SharedMediaAddExisting(
            peer->id,
            MsgId(), // topicRootId
            item->sublistPeerId(), types, item->id, {item->id, item->id}));
        if (pinned) {
  ...
  if (const auto user = guestMessage ? nullptr : from->asUser()) {
    const auto lastAuthors = [&]() -> std::deque<not_null<UserData *>> * {
      if (auto chat = peer->asChat()) {
        ...
        return channel->canViewMembers() ? &channel->mgInfo->lastParticipants
                                         : nullptr;
      }
      return nullptr;
    }();
    ...
    if (lastAuthors) {
      auto prev =
          ranges::find(*lastAuthors, user,
                       [](not_null<UserData *> user) { return user.get(); });
      auto index =
          (prev != lastAuthors->end()) ? (lastAuthors->end() - prev) : -1;
      if (index > 0) {
        lastAuthors->erase(prev);
      } else if (index < 0 && peer->isMegagroup()) { // nothing is outdated if
                                                     // just reordering
      ...
      if (auto megagroup = peer->asMegagroup()) {
        session().changes().peerUpdated(peer, Data::PeerUpdate::Flag::Members);
    ...
    if (!(markupFlags & ReplyMarkupFlag::Selective) || item->mentionsMe()) {
      ...
      if (markupFlags & ReplyMarkupFlag::None) {
        ...
        if (lastKeyboardFrom == from->id ||
            (!lastKeyboardInited && !peer->isChat() && !peer->isMegagroup() &&
             !item->out())) {
          clearLastKeyboard();
        }
      } else {
...
void History::applyMessageChanges(not_null<HistoryItem *> item,
                                  const MTPMessage &data) {
  if (data.type() == mtpc_messageService) {
    applyServiceChanges(item, data.c_messageService());
  }
  owner().stickers().checkSavedGif(item);
  session().changes().messageUpdated(item, Data::MessageUpdate::Flag::NewAdded);
}

void History::applyServiceChanges(not_null<HistoryItem *> item,
                                  const MTPDmessageService &data) {
  const auto replyTo = data.vreply_to();
  const auto processJoinedUser = [&](not_null<ChannelData *> megagroup,
  ...
  data.vaction().match(
        ...
        data.vphoto().match(
            [&](const MTPDphoto &data) {
              ...
              const auto chatPhoto = MTP_chatPhoto(
                  ...
                  MTP_long(photo->id), MTP_bytes(photo->inlineThumbnailBytes()),
                  data.vdc_id());
              if (const auto chat = peer->asChat()) {
      ...
      [&](const MTPDmessageActionChatMigrateTo &data) {
        if (const auto chat = peer->asChat()) {
          ...
          if (const auto channel =
                  owner().channelLoaded(data.vchannel_id().v)) {
              ...
              [&](const MTPDmessageReplyHeader &data) {
                ...
                if (id && item) {
                  session().storage().add(Storage::SharedMediaAddSlice(
                      ...
                      MsgId(0),  // topicRootId
                      PeerId(0), // monoforumPeerId
                      Storage::SharedMediaType::Pinned, {id},
                      {id, ServerMaxMsgId}));
                  setHasPinnedMessages(true);
                  if (const auto topic = item->topic()) {
                    session().storage().add(Storage::SharedMediaAddSlice(
                        peer->id, topic->rootId(),
                        PeerId(), // monoforumPeerId
                        Storage::SharedMediaType::Pinned, {id},
                        {id, ServerMaxMsgId}));
                    topic->setHasPinnedMessages(true);
                  ...
                  if (const auto sublist = item->savedSublist()) {
                    session().storage().add(Storage::SharedMediaAddSlice(
                        peer->id,
                        MsgId(), // topicRootId
                        item->sublistPeerId(), Storage::SharedMediaType::Pinned,
                        {id}, {id, ServerMaxMsgId}));
                    sublist->setHasPinnedMessages(true);
                  }
                }
              },
        ...
        if (const auto payment = item->Get<HistoryServicePayment>()) {
          ...
          if (const auto message = payment->msg) {
            if (const auto media = message->media()) {
              if (media->invoice()) {
                paid = Payments::CheckoutProcess::InvoicePaid(message);
              }
            }
          } else if (!payment->slug.isEmpty()) {
            using Payments::CheckoutProcess;
            paid = Payments::CheckoutProcess::InvoicePaid(&session(),
                                                          payment->slug);
          }
          if (paid) {
            ...
            Ui::Toast::Show({
                .text = tr::lng_payments_success(
                    ...
                    lt_title, tr::bold(paid->title), tr::marked),
      ...
      [&](const MTPDmessageActionTopicCreate &data) {
        if (const auto forum = peer->forum()) {
          forum->applyTopicAdded(item->id, qs(data.vtitle()),
                                 data.vicon_color().v,
                                 data.vicon_emoji_id().value_or(DocumentId()),
                                 item->from()->id, item->date(), item->out());
        }
      },
      ...
      [&](const MTPDmessageActionTodoCompletions &data) {
        if (const auto done = item->Get<HistoryServiceTodoCompletions>()) {
          const auto list =
              done->msg ? done->msg : owner().message(peer, done->msgId);
      ...
      [&](const MTPDmessageActionPollAppendAnswer &data) {
        if (const auto append = item->Get<HistoryServicePollAppendAnswer>()) {
          const auto list =
              append->msg ? append->msg : owner().message(peer, append->msgId);
          if (const auto media = list ? list->media() : nullptr) {
            if (const auto poll = media->poll()) {
              data.vanswer().match(
                  [&](const MTPDpollAnswer &answer) {
                    ...
                    parsed.text =
                        Api::ParseTextWithEntities(&session(), answer.vtext());
      ...
      [&](const MTPDmessageActionPollDeleteAnswer &data) {
        if (const auto del = item->Get<HistoryServicePollDeleteAnswer>()) {
          const auto list =
              del->msg ? del->msg : owner().message(peer, del->msgId);
          if (const auto media = list ? list->media() : nullptr) {
            if (const auto poll = media->poll()) {
      ...
      [&](const MTPDmessageActionNoForwardsToggle &data) {
        if (const auto user = peer->asUser()) {
          ...
          user->setNoForwardsFlags(enabled && item->out(),
                                   enabled && !item->out());
        }
      },
      [](const auto &) {});
...
void History::mainViewHeightAdjusted(not_null<Element *> view, int delta) {
  ...
  block->resizeGetHeight(_width, HistoryBlock::ResizeRequest::ResizePending);
  const auto blockInHistory = block->indexInHistory();
  for (auto i = blockInHistory + 1, count = int(blocks.size()); i != count;
       ++i) {
...
void History::mainViewRemoved(not_null<HistoryBlock *> block,
...
HistoryBlock *History::prepareBlockForAddingItem() {
  ...
  const auto addNewBlock = blocks.empty() || (blocks.back()->messages.size() >=
                                              kNewBlockEachMessage);
  if (addNewBlock) {
...
void History::viewReplaced(not_null<const Element *> was, Element *now) {
  if (scrollTopItem == was)
    scrollTopItem = now;
  if (_firstUnreadView == was)
    _firstUnreadView = now;
  if (_unreadBarView == was)
    _unreadBarView = now;
}
...
void History::addItemToBlock(not_null<HistoryItem *> item) {
  ...
  if (item->Has<HistoryServiceNoForwardsToggle>()) {
    if (const auto prev = view->previousInBlocks()) {
      if (const auto nfr =
              prev->data()->Get<HistoryServiceNoForwardsRequest>()) {
        if (!nfr->actionTaken) {
...
void History::addEdgesToSharedMedia() {
  ...
  for (auto i = 0; i != Storage::kSharedMediaTypeCount; ++i) {
    ...
    session().storage().add(
        Storage::SharedMediaAddSlice(peer->id,
                                     MsgId(0),  // topicRootId
                                     PeerId(0), // monoforumPeerId
                                     type, {}, {from, till}));
  }
  ...
  for (const auto &item : ranges::views::reverse(items)) {
    ...
    if (item->author()->id) {
      if (markupSenders) { // chats with bots
        if (!lastKeyboardInited && item->definesReplyKeyboard() &&
            !item->out()) {
      ...
      } else if (!lastKeyboardInited && item->definesReplyKeyboard() &&
  ...
  for (auto i = 0; i != Storage::kSharedMediaTypeCount; ++i) {
    if (!medias[i].empty()) {
      ...
      session().storage().add(Storage::SharedMediaAddSlice(
          ...
          MsgId(0),  // topicRootId
          PeerId(0), // monoforumPeerId
          type, std::move(medias[i]), {from, till}));
      if (type == Storage::SharedMediaType::Pinned) {
        setHasPinnedMessages(true);
      }
    }
...
std::optional<int> History::countStillUnreadLocal(MsgId readTillId) const {
  if (isEmpty() || !folderKnown()) {
    DEBUG_LOG(("Reading: countStillUnreadLocal unknown %1 and %2.")
                  .arg(Logs::b(isEmpty()), Logs::b(folderKnown())));
    return std::nullopt;
  }
  if (_inboxReadBefore) {
    ...
    if (minMsgId() <= before && maxMsgId() >= readTillId) {
      ...
      for (const auto &block : blocks) {
        for (const auto &message : block->messages) {
          const auto item = message->data();
          if (!item->isRegular() || (item->out() && !item->isFromScheduled())) {
            continue;
  ...
  DEBUG_LOG(("Reading: check at end loaded from %1 loaded %2 - %3")
                .arg(QString::number(minimalServerId.bare),
                     Logs::b(loadedAtBottom()), Logs::b(loadedAtTop())));
  if (!loadedAtBottom() || (!loadedAtTop() && !minimalServerId) ||
...
void History::applyInboxReadUpdate(FolderId folderId, MsgId upTo,
                                   int stillUnread, int32 channelPts) {
  ...
  if (const auto last = chatListMessage()) {
    if (last->out() && last->isRegular() && last->id <= upTo) {
      session().changes().messageUpdated(
          last, Data::MessageUpdate::Flag::DialogRowRepaint);
    }
  }
  updateChatListEntry();
...
int History::unreadCount() const { return _unreadCount ? *_unreadCount : 0; }

bool History::unreadCountKnown() const { return _unreadCount.has_value(); }
...
void History::setUnreadCount(int newUnreadCount) {
  ...
  const auto lastOutgoing = [&] {
    ...
    return last && last->isRegular() && loadedAtBottom() && !isEmpty() &&
           blocks.back()->messages.back()->data() == last && last->out();
  }();
  if (newUnreadCount == 1 && !lastOutgoing) {
    if (loadedAtBottom()) {
      _firstUnreadView =
          !isEmpty() ? blocks.back()->messages.back().get() : nullptr;
    }
...
void History::setUnreadMark(bool unread) {
  ...
  const auto notifier =
      unreadStateChangeNotifier(useMyUnreadInParent() && !unreadCount());
...
void History::setMuted(bool muted) {
  if (this->muted() == muted) {
    ...
    const auto state =
        useMyUnreadInParent() ? computeBadgesState() : Dialogs::BadgesState();
    const auto notify = (state.unread || state.reaction || state.poll);
    const auto notifier = unreadStateChangeNotifier(notify);
    Thread::setMuted(muted);
  }
  session().changes().peerUpdated(peer, Data::PeerUpdate::Flag::Notifications);
  owner().chatsFilters().refreshHistory(this);
  if (const auto forum = peer->forum()) {
...
bool History::folderKnown() const { return _folder.has_value(); }

Data::Folder *History::folder() const { return _folder.value_or(nullptr); }
...
void History::clearFolder() { setFolderPointer(nullptr); }
...
int History::chatListNameVersion() const { return peer->nameVersion(); }
...
TimeId History::adjustedChatListTimeId() const {
  ...
  if (const auto draft = cloudDraft(MsgId(), PeerId())) {
    if (!peer->forum() && !Data::DraftIsNull(draft) &&
        !session().supportMode()) {
...
auto History::findScrollTopItem(int top) const -> Element * {
  ...
  // go forward through history while we don't find the last item that starts
  // above
  for (auto blocksCount = int(blocks.size()); blockIndex < blocksCount;
       ++blockIndex) {
    const auto &block = blocks[blockIndex];
    for (auto itemsCount = int(block->messages.size()); itemIndex < itemsCount;
         ++itemIndex) {
...
void History::unsetFirstUnreadMessage() { _firstUnreadView = nullptr; }

HistoryView::Element *History::unreadBar() const { return _unreadBarView; }

HistoryView::Element *History::firstUnreadMessage() const {
  return _firstUnreadView;
}

not_null<HistoryItem *> History::addNewInTheMiddle(not_null<HistoryItem *> item,
                                                   int blockIndex,
                                                   int itemIndex) {
  ...
  const auto it =
      block->messages.insert(block->messages.begin() + itemIndex,
  ...
  if (itemIndex + 1 < block->messages.size()) {
    ...
    block->messages[itemIndex + 1]->previousInBlocksChanged();
             ...
             !blocks[blockIndex + 1]->messages.empty()) {
...
Dialogs::BadgesState History::chatListBadgesState() const {
  if (const auto forum = peer->forum()) {
    return adjustBadgesStateByFolder(Dialogs::BadgesForUnread(
        forum->topicsList()->unreadState(), Dialogs::CountInBadge::Chats,
        Dialogs::IncludeInBadge::UnmutedOrAll));
  } else if (const auto monoforum = peer->monoforum()) {
    return adjustBadgesStateByFolder(Dialogs::BadgesForUnread(
        withMyMuted(monoforum->chatsList()->unreadState()),
        Dialogs::CountInBadge::Chats, Dialogs::IncludeInBadge::All));
  }
...
Dialogs::BadgesState History::computeBadgesState() const {
  return adjustBadgesStateByFolder(Dialogs::BadgesForUnread(
      computeUnreadState(), Dialogs::CountInBadge::Messages,
      Dialogs::IncludeInBadge::All));
}

Dialogs::BadgesState
History::adjustBadgesStateByFolder(Dialogs::BadgesState state) const {
  if (folder()) {
    state.mentionMuted = state.reactionMuted = state.pollMuted =
        state.unreadMuted = true;
  }
  return state;
}
...
Dialogs::UnreadState History::computeUnreadState() const {
  ...
  const auto hideReactions =
      (peer->isChannel() && !peer->isMegagroup() &&
       !settings.showChannelReactions()) ||
...
void History::resolveChatListMessageGroup() {
  ...
  if (!(_flags & Flag::ResolveChatListMessage) || !item ||
      !hasOrphanMediaGroupPart()) {
    return;
  }
  // If we set a single album part, request the full album.
  const auto withImages =
      !item->toPreview({.hideSender = true, .hideCaption = true})
           .images.empty();
  if (withImages) {
...
void History::paintUserpic(Painter &p, Ui::PeerUserpicView &view,
                           const Dialogs::Ui::PaintContext &context) const {
  peer->paintUserpic(p, view, context.st->padding.left(),
                     context.st->padding.top(), context.st->photoSize);
}
...
bool History::loadedAtBottom() const { return _loadedAtBottom; }

bool History::loadedAtTop() const { return _loadedAtTop; }
...
void History::clearSharedMedia() {
  session().storage().remove(Storage::SharedMediaRemoveAll(peer->id));
}

void History::setLastServerMessage(HistoryItem *item) {
  _lastServerMessage = item;
  if (_lastMessage && *_lastMessage && !(*_lastMessage)->isRegular() &&
...
void History::setFakeChatListMessageFrom(const MTPmessages_Messages &data) {
  ...
  const auto other = data.match(
      ...
      [&](const auto &data) {
        for (const auto &message : data.vmessages().v) {
          const auto id =
              message.match([](const auto &data) { return data.vid().v; });
          if (id != last->id) {
            return &message;
          }
        }
  ...
  const auto item =
      owner().addNewMessage(*other, MessageFlags(), NewMessageType::Existing);
...
void History::applyChatListGroup(PeerId dataPeerId,
                                 const MTPmessages_Messages &data) {
  if (!isEmpty() || !_chatListMessage || !*_chatListMessage ||
      (*_chatListMessage)->history() != this || !_lastMessage ||
      !*_lastMessage || dataPeerId != peer->id) {
    return;
  }
  ...
  data.match([&](const MTPDmessages_messagesNotModified &) {},
             [&](const auto &data) { processMessages(data.vmessages()); });
}
...
bool History::lastMessageKnown() const { return _lastMessage.has_value(); }
...
void History::updateChatListExistence() { Entry::updateChatListExistence(); }
...
bool History::shouldBeInChatList() const {
  if (peer->migrateTo() || !folderKnown()) {
    ...
    return chat->amIn() || !lastMessageKnown() || (lastMessage() != nullptr);
  } else if (const auto user = peer->asUser()) {
    if (user->isBot() && isTopPromoted()) {
      return true;
    }
  }
  return !lastMessageKnown() || (lastMessage() != nullptr);
}
...
bool History::isServerSideUnread(not_null<const HistoryItem *> item) const {
  ...
  return item->out() ? (!_outboxReadBefore || (item->id >= *_outboxReadBefore))
                     : (!_inboxReadBefore || (item->id >= *_inboxReadBefore));
}

void History::applyDialog(Data::Folder *requestFolder, const MTPDdialog &data) {
  const auto folderId = data.vfolder_id();
  const auto folder = !folderId     ? requestFolder
                      : folderId->v ? owner().folder(folderId->v).get()
                                    : nullptr;
  applyDialogFields(folder, data.vunread_count().v, data.vread_inbox_max_id().v,
  ...
  session().changes().historyUpdated(
      this, Data::HistoryUpdate::Flag::UnreadPollVotes);
  if (const auto channel = peer->asChannel()) {
    if (const auto pts = data.vpts()) {
      channel->ptsReceived(pts->v);
    }
    if (!channel->amCreator()) {
      const auto topMessageId = FullMsgId(channel->id, data.vtop_message().v);
  ...
  owner().notifySettings().apply(MTP_notifyPeer(data.vpeer()),
                                 data.vnotify_settings());

  const auto draft = data.vdraft();
  if (draft && draft->type() == mtpc_draftMessage) {
    Data::ApplyPeerCloudDraft(&session(), peer->id,
                              MsgId(),  // topicRootId
                              PeerId(), // monoforumPeerId
...
void History::dialogEntryApplied() {
  ...
  if (chatListTimeId() != 0 && loadedAtBottom()) {
    if (const auto channel = peer->asChannel()) {
      const auto inviter = channel->inviter;
      if (inviter && chatListTimeId() <= channel->inviteDate &&
          channel->amIn() && owner().userLoaded(inviter)) {
        insertJoinedMessage();
      }
    }
  }
...
void History::cacheTopPromotion(bool promoted, const QString &type,
...
QStringView History::topPromotionType() const {
  return topPromotionAboutShown() ? base::StringViewMid(_topPromotedType, 5)
...
QString History::topPromotionMessage() const { return _topPromotedMessage; }
...
bool History::skipUnreadUpdate() const { return clearUnreadOnClientSide(); }

void History::applyDialogFields(Data::Folder *folder, int unreadCount,
                                MsgId maxInboxRead, MsgId maxOutboxRead) {
  if (folder) {
    setFolder(folder);
  } else {
    clearFolder();
  }
  if (!skipUnreadUpdate() && maxInboxRead + 1 >= _inboxReadBefore.value_or(1)) {
...
void History::tryMarkForumIntervalRead(MsgId wasInboxReadBefore,
                                       MsgId nowInboxReadBefore) {
  if (!isForum() || !peer->useSubsectionTabs() ||
...
void History::tryMarkMonoforumIntervalRead(MsgId wasInboxReadBefore,
                                           MsgId nowInboxReadBefore) {
...
MsgId History::msgIdForRead() const {
  const auto last = lastMessage();
  const auto result = (last && last->isRegular()) ? last->id : MsgId(0);
  return loadedAtBottom() ? std::max(result, maxMsgId()) : result;
}
...
void History::resizeToWidth(int newWidth) {
  ...
  const auto request = (_flags & Flag::PendingAllItemsResize)
                           ? Request::ReinitAll
                       : (_width != newWidth) ? Request::ResizeAll
...
Data::Thread *History::threadFor(MsgId topicRootId, PeerId monoforumPeerId) {
  return topicRootId       ? peer->forumTopicFor(topicRootId)
         : monoforumPeerId ? peer->monoforumSublistFor(monoforumPeerId)
                           : static_cast<Data::Thread *>(this);
}

const Data::Thread *History::threadFor(MsgId topicRootId,
                                       PeerId monoforumPeerId) const {
  return const_cast<History *>(this)->threadFor(topicRootId, monoforumPeerId);
}

void History::forumChanged(Data::Forum *old) {
  if (inChatList()) {
    notifyUnreadStateChange(
        old ? AdjustedForumUnreadState(old->topicsList()->unreadState())
            : computeUnreadState());
  }
  ...
  if (const auto forum = peer->forum()) {
    ...
    forum->topicsList()->unreadStateChanges() |
        rpl::filter([=] { return (_flags & Flag::IsForum) && inChatList(); }) |
        rpl::map(AdjustedForumUnreadState) |
        rpl::on_next(
            [=](const Dialogs::UnreadState &old) {
              notifyUnreadStateChange(old);
            },
            forum->lifetime());

    forum->chatsListChanges() |
        rpl::on_next([=] { updateChatListEntry(); }, forum->lifetime());
  } else {
    _flags &= ~Flag::IsForum;
  }
...
bool History::isForum() const { return (_flags & Flag::IsForum); }

void History::monoforumChanged(Data::SavedMessages *old) {
  if (inChatList()) {
    notifyUnreadStateChange(old ? AdjustedForumUnreadState(withMyMuted(
                                      old->chatsList()->unreadState()))
                                : computeUnreadState());
  }
  ...
  if (const auto monoforum = peer->monoforum()) {
    ...
    monoforum->chatsList()->unreadStateChanges() | rpl::filter([=] {
    ...
    monoforum->chatsListChanges() |
        rpl::on_next([=] { updateChatListEntry(); }, monoforum->lifetime());
  } else {
    _flags &= ~Flag::IsMonoforumAdmin;
  }
  if (cloudDraft(MsgId(), PeerId())) {
...
Data::HistoryMessages &History::messages() {
  if (!_messages) {
    ...
    list.reserve(std::min(int(_items.size()),
...
Data::HistoryMessages *History::maybeMessages() { return _messages.get(); }

HistoryItem *History::insertJoinedMessage() {
  const auto channel = peer->asChannel();
  if (!channel || channel->isMonoforum() || _joinedMessage ||
      !channel->amIn() ||
      (channel->isMegagroup() && channel->mgInfo->joinedMessageFound)) {
  ...
  if (peer->isMegagroup() && peer->migrateFrom() && !blocks.empty() &&
      blocks.front()->messages.front()->data()->id == 1) {
    channel->mgInfo->joinedMessageFound = true;
    return nullptr;
  }

  _joinedMessage = GenerateJoinedMessage(this, channel->inviteDate, inviter,
...
void History::checkNewPeerMessages() {
  ...
  const auto when = [](TimeId date) {
    ...
    if (passed < 3600) {
      ...
      return tr::lng_new_contact_updated_hours(tr::now, lt_count,
                                               (passed / 3600));
    } else if (passed < 60 * 24 * 3600) {
      return tr::lng_new_contact_updated_days(tr::now, lt_count,
                                              (passed / (24 * 3600)));
    }
    return tr::lng_new_contact_updated_months(tr::now, lt_count,
                                              (passed / (30 * 24 * 3600)));
  };
  ...
  if (!firstDate) {
    firstDate =
        base::unixtime::serialize(QDateTime(QDate(2013, 8, 1), QTime(0, 0)));
  }
...
void History::checkLocalMessages() {
  ...
  const auto firstDate =
      loadedAtTop() ? 0 : blocks.front()->messages.front()->data()->date();
  ...
  if (peer->isChannel() && !_joinedMessage && peer->asChannel()->inviter &&
...
HistoryItem *History::joinedMessageInstance() const { return _joinedMessage; }
...
bool History::isEmpty() const { return blocks.empty(); }
...
bool History::isDisplayedEmpty() const {
  ...
  if (first == last) {
    return first->data()->isGroupEssential() || isChangePhoto(first->data());
  }
...
void History::clear(ClearType type, bool markEmpty) {
  ...
  if (type == ClearType::Unload) {
    ...
    const auto tillId =
        (_lastMessage && (*_lastMessage) && (*_lastMessage)->isRegular())
            ? (*_lastMessage)->id
...
bool History::isTopPromoted() const { return (_flags & Flag::IsTopPromoted); }
...
void History::translateOfferFrom(LanguageId id) {
  if (!id) {
    if (translatedTo()) {
      ...
      session().changes().historyUpdated(this, UpdateFlag::TranslateFrom);
    }
  } else if (!_translation) {
    _translation = std::make_unique<HistoryTranslation>(this, id);
    using Flag = PeerData::TranslationFlag;
    if (peer->autoTranslation() && (peer->translationFlag() == Flag::Enabled)) {
...
HistoryTranslation *History::translation() const { return _translation.get(); }
...
HistoryBlock::HistoryBlock(not_null<History *> history) : _history(history) {}
...
int HistoryBlock::resizeGetHeight(int newWidth, ResizeRequest request) {
  ...
  if (request == ResizeRequest::ReinitAll) {
    ...
    for (const auto &message : messages) {
      message->setY(y);
      y += message->pendingResize() ? message->resizeGetHeight(newWidth)
                                    : message->height();
    }
  }
  _height = y;
...
void HistoryBlock::remove(not_null<Element *> view) {
  ...
  if (messages.empty()) {
    ...
    _history->blocks[blockIndex + 1]
        ->messages.front()
        ->previousInBlocksChanged();
  } else if (!_history->blocks.empty() &&
             !_history->blocks.back()->messages.empty()) {
...
void HistoryBlock::refreshView(not_null<Element *> view) {
  ...
  const auto item = view->data();
  auto refreshed =
      item->createView(_history->delegateMixin()->delegate(), view);

...
TabbedPanel::TabbedPanel(QWidget *parent, TabbedPanelDescriptor &&descriptor)
    : RpWidget(parent), _regularWindow(descriptor.regularWindow),
      _ownedSelector(std::move(descriptor.ownedSelector)),
      _selector(descriptor.nonOwnedSelector ? descriptor.nonOwnedSelector
                                            : _ownedSelector.data()),
  ...
  _selector->showRequests() |
      rpl::on_next([=] { showFromSelector(); }, lifetime());

  resize(QRect(0, 0, st::emojiPanWidth, st::emojiPanMaxHeight)
  ...
  _selector->slideFinished() | rpl::on_next(
                                   [=] {
                                     InvokeQueued(this, [=] {
                                       if (_hideAfterSlide) {
  ...
  macWindowDeactivateEvents() |
      rpl::filter([=] { return !isHidden() && !preventAutoHide(); }) |
      rpl::on_next([=] { hideAnimated(); }, lifetime());
...
void TabbedPanel::setDesiredHeightValues(float64 ratio, int minHeight,
                                         int maxHeight) {
  _heightRatio = ratio;
...
void TabbedPanel::updateContentHeight() {
  ...
  auto wantedContentHeight =
      qRound(_heightRatio * availableHeight) - addedHeight;
  auto contentHeight =
      marginsHeight +
      std::clamp(wantedContentHeight, _minContentHeight, _maxContentHeight);
...
void TabbedPanel::moveHorizontally() {
  ...
  const auto right =
      std::max(parentWidget()->width() - std::max(_right, width), 0);
  moveToRight(right, y());
  updateContentHeight();
}
...
void TabbedPanel::startOpacityAnimation(bool hiding) {
  ...
  hideChildren();
  _a_opacity.start([=] { opacityAnimationCallback(); }, _hiding ? 1. : 0.,
...
void TabbedPanel::startShowAnimation() {
  if (!_a_show.animating()) {
    ...
    const auto origin = _showAnimationOrigin.value_or(
        _dropDown ? Ui::PanelAnimation::Origin::TopRight
...
bool TabbedPanel::overlaps(const QRect &globalRect) const {
  ...
  return inner.marginsRemoved(QMargins(radius, 0, radius, 0))
             .contains(testRect) ||
         inner.marginsRemoved(QMargins(0, radius, 0, radius))
             .contains(testRect);
}

...
public:
  PreviewDelegate(not_null<QWidget *> parent, not_null<Ui::ChatStyle *> st,
                  rpl::producer<bool> chatWideValue, Fn<void()> update);
...
PreviewDelegate::PreviewDelegate(not_null<QWidget *> parent,
...
PreviewWrap::PreviewWrap(not_null<Ui::RpWidget *> parent,
  ...
  const auto isRound = _item && _item->media() && _item->media()->document() &&
                       _item->media()->document()->isVideoMessage();
  const auto &settings = AyuSettings::getInstance();

  std::move(theme) | rpl::on_next(
  ...
  session->data().itemViewRefreshRequest() |
      rpl::on_next(
          ...
          lifetime());
  session->data().itemDataChanges() | rpl::on_next(
                                          [=](not_null<HistoryItem *> item) {
                                            if (item == _item) {
                                          ...
                                          lifetime());
  session->data().itemRemoved() | rpl::on_next(
                                      [=](not_null<const HistoryItem *> item) {
                                        if (item == _item) {
                                          _closeRequests.fire({});
                                        }
...

  {
    ...
    rpl::combine(sizeValue(), _elementInner.value()) |
        rpl::on_next(
            [=](QSize size, QRect inner) {
              close->moveToLeft(inner.x() +
                                    (inner.width() - close->width()) / 2,
                                (size.height() - close->height() -
  ...
  {
    auto text = item->out()
                    ? (isRound ? tr::lng_ttl_round_tooltip_out
                               : tr::lng_ttl_voice_tooltip_out)(
                          lt_user,
                          rpl::single(item->history()->peer->shortName()) |
                              rpl::map(tr::rich),
                          tr::rich)
                    : (isRound ? settings.saveDeletedMessages()
    ...
    const auto tooltip = Ui::CreateChild<Ui::ImportantTooltip>(
        ...
        object_ptr<Ui::PaddingWrap<Ui::FlatLabel>>(
            this,
            Ui::MakeNiceTooltipLabel(parent, std::move(text),
    ...
    _elementInner.value() |
        rpl::filter([](const QRect &inner) { return !inner.isEmpty(); }) |
        rpl::on_next(
            [=](const QRect &inner) {
              tooltip->pointAt(inner, RectPart::Top, [=](QSize size) {
                return QPoint{
                    inner.x() + (inner.width() - size.width()) / 2,
                    (inner.y() - st::normalFont->height - size.height() -
...
bool PreviewWrap::goodItem() const {
  ...
  return document && (document->isVoiceMessage() || document->isVideoMessage());
}
...
void PreviewWrap::createView() {
  ...
  rpl::combine(sizeValue(), _globalViewport.value()) |
      rpl::on_next(
          [=](QSize outer, QRect globalViewport) {
            ...
            _elementGeometry =
                QRect((_viewport.width() - _element->width()) / 2,
                      (_viewport.height() - _element->height()) / 2,
                      _element->width(), _element->height())
...
void PreviewWrap::paintEvent(QPaintEvent *e) {
  ...
  if (!_lastFrameCache.isNull()) {
    ...
    auto context = _theme->preparePaintContext(
        _style.get(), Rect(_element->currentSize()),
        Rect(_element->currentSize()), Rect(_element->currentSize()),
...
rpl::producer<QRect>
GlobalViewportForWindow(not_null<Window::SessionController *> controller) {
  const auto delegate = controller->window().floatPlayerDelegate();
  return rpl::single(rpl::empty) |
         rpl::then(delegate->floatPlayerAreaUpdates()) | rpl::map([=] {
...
void ShowTTLMediaLayerWidget(not_null<Window::SessionController *> controller,
                             not_null<HistoryItem *> item) {
  const auto parent = controller->content();
  const auto show = controller->uiShow();
  auto preview = base::make_unique_q<PreviewWrap>(
      parent, item, GlobalViewportForWindow(controller),
      controller->adaptive().chatWideValue(),
      Window::ChatThemeValueFromPeer(controller, item->history()->peer));
  preview->closeRequests() |
      rpl::on_next([=] { show->hideLayer(); }, preview->lifetime());
  auto layer =
      std::make_unique<Editor::LayerWidget>(parent, std::move(preview));

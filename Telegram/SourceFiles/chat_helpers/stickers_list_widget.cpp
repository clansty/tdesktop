...
constexpr auto kRecentDisplayLimit = 60;
...
struct StickersListWidget::Set {
  Set(uint64 id, Data::StickersSet *set, Data::StickersSetFlags flags,
      const QString &title, const QString &shortName, int count,
      bool externalLayout, std::vector<Sticker> &&stickers = {});
  Set(Set &&other);
...
auto StickersListWidget::PrepareStickers(const QVector<DocumentData *> &pack,
                                         bool skipPremium)
    -> std::vector<Sticker> {
  return ranges::views::all(pack) |
         ranges::views::filter([&](DocumentData *document) {
           return !skipPremium || !document->isPremiumSticker();
         }) |
         ranges::views::transform(
             [](DocumentData *document) { return Sticker{document}; }) |
         ranges::to_vector;
}

StickersListWidget::Set::Set(uint64 id, StickersSet *set,
                             Data::StickersSetFlags flags, const QString &title,
                             const QString &shortName, int count,
                             bool externalLayout,
                             std::vector<Sticker> &&stickers)
    : id(id), set(set), flags(flags), title(title), shortName(shortName),
      stickers(std::move(stickers)), count(count),
      externalLayout(externalLayout) {}

StickersListWidget::Set::Set(Set &&other) = default;
StickersListWidget::Set &
StickersListWidget::Set::operator=(Set &&other) = default;
...
StickersListWidget::StickersListWidget(QWidget *parent,
                                       StickersListDescriptor &&descriptor)
    : Inner(parent, descriptor.st ? *descriptor.st : st::defaultEmojiPan,
            descriptor.show, descriptor.paused),
      _mode(descriptor.mode),
      _requireConfirmation(descriptor.requireConfirmation),
      _show(std::move(descriptor.show)), _features(descriptor.features),
      _overBg(st::roundRadiusLarge, st().overBg), _api(&session().mtp()),
      _localSetsManager(std::make_unique<LocalStickersManager>(&session())),
      _customRecentIds(std::move(descriptor.customRecentList)),
      _section(Section::Stickers), _isMasks(_mode == Mode::Masks),
      _isEffects(_mode == Mode::MessageEffects),
      _excludeSetId(descriptor.excludeSetId),
      _updateItemsTimer([=] { updateItems(); }),
      _updateSetsTimer([=] { updateSets(); }),
      _trendingAddBgOver(ImageRoundRadius::Large,
                         st::stickersTrendingAdd.textBgOver),
      _trendingAddBg(ImageRoundRadius::Large, st::stickersTrendingAdd.textBg),
      _inactiveButtonBg(ImageRoundRadius::Large,
                        st::stickersTrendingInstalled.textBg),
      _groupCategoryAddBgOver(ImageRoundRadius::Large,
                              st::stickerGroupCategoryAdd.textBgOver),
      _groupCategoryAddBg(ImageRoundRadius::Large,
                          st::stickerGroupCategoryAdd.textBg),
      _pathGradient(std::make_unique<Ui::PathShiftGradient>(
          st().pathBg, st().pathFg, [=] { update(); })),
      _megagroupSetAbout(st::columnMinimalWidthThird - st::emojiScroll.width -
                         st().headerLeft),
      ...
      _premiumMark(std::make_unique<StickerPremiumMark>(
          &session(), st::stickersPremiumLock)),
  ...
  _settings->addClickHandler([=] {
    if (const auto window = _show->resolveWindow()) {
      ...
      window->show(Box<StickersBox>(_show, Section::Installed, _isMasks));
  ...
  session().changes().peerUpdates(Data::PeerUpdate::Flag::StickersSet) |
      rpl::filter([=](const Data::PeerUpdate &update) {
        return (update.peer.get() == _megagroupSet);
      }) |
      rpl::on_next([=] { refreshStickers(); }, lifetime());

  if (!_isEffects) {
    session().data().stickers().recentUpdated(
        _isMasks ? Data::StickersType::Masks : Data::StickersType::Stickers) |
        rpl::on_next([=] { refreshRecent(); }, lifetime());
  }

  positionValue() | rpl::skip(1) | rpl::map_to(TabbedSelector::Action::Update) |
      rpl::start_to_stream(_choosingUpdated, lifetime());

  if (_isEffects) {
    refreshStickers();
  } else {
    rpl::merge(Data::AmPremiumValue(&session()) | rpl::to_empty,
...
object_ptr<TabbedSelector::InnerFooter> StickersListWidget::createFooter() {
  ...
  _footer->setChosen() |
      rpl::on_next([=](uint64 setId) { showStickerSet(setId); },
                   _footer->lifetime());
  ...
  _footer->openSettingsRequests() |
      rpl::on_next(
          [=] {
            ...
            _show->showBox(
                Box<StickersBox>(_show,
                                 (onlyFeatured ? StickersBox::Section::Featured
                                  : _isMasks ? StickersBox::Section::Masks
...
void StickersListWidget::visibleTopBottomUpdated(int visibleTop,
                                                 int visibleBottom) {
  ...
  if (_footer) {
    _footer->validateSelectedIcon(currentSet(visibleTop),
                                  ValidateIconAnimations::Full);
  }
}

void StickersListWidget::checkVisibleFeatured(int visibleTop,
                                              int visibleBottom) {
  ...
  const auto destroyAbove = floorclamp(visibleTop - visibleHeight, rowHeight, 0,
                                       _officialSets.size());
  const auto destroyBelow = ceilclamp(visibleBottom + visibleHeight, rowHeight,
                                      0, _officialSets.size());
...
void StickersListWidget::preloadMoreOfficial() {
  ...
  _officialRequestId =
      _api.request(MTPmessages_GetOldFeaturedStickers(
                       MTP_int(_officialOffset), MTP_int(kOfficialLoadLimit),
                       MTP_long(0) // hash
                       ))
          .done([=](const MTPmessages_FeaturedStickers &result) {
            _officialRequestId = 0;
            result.match(
                [&](const MTPDmessages_featuredStickersNotModified &d) {
                ...
                [&](const MTPDmessages_featuredStickers &data) {
                  ...
                  for (int i = 0, l = list.size(); i != l; ++i) {
                    const auto set =
                        session().data().stickers().feedSet(list[i]);
                    if (set->stickers.empty() && set->covers.empty()) {
                      continue;
                    }
                    const auto externalLayout = true;
                    appendSet(_officialSets, set->id, externalLayout,
...
void StickersListWidget::readVisibleFeatured(int visibleTop,
                                             int visibleBottom) {
  const auto rowHeight = featuredRowHeight();
...
int StickersListWidget::featuredRowHeight() const {
  return st::stickersTrendingHeader + _singleSize.height() +
...
bool StickersListWidget::enumerateSections(Callback callback) const {
  ...
  for (auto i = 0; i != sets.size(); ++i) {
    ...
    const auto firstAfterShortcuts =
        !i && searchShortcutsShown() && !searchShortcutSelected();
    const auto titleSkip = set.externalLayout ? st::stickersTrendingHeader
                           : setHasTitle(set) ? st().header
                           : firstAfterShortcuts
    ...
    if (set.externalLayout) {
      ...
      info.rowsCount = 0;
      info.rowsBottom = info.rowsTop + _megagroupSetButtonRect.y() +
                        _megagroupSetButtonRect.height() +
                        st::stickerGroupCategoryAddMargin.bottom();
    } else {
      info.rowsCount =
          (info.count / _columnCount) + ((info.count % _columnCount) ? 1 : 0);
...
StickersListWidget::SectionInfo
StickersListWidget::sectionInfo(int section) const {
  Expects(section >= 0 && section < shownSets().size());

  auto result = SectionInfo();
  enumerateSections(
      [searchForSection = section, &result](const SectionInfo &info) {
...
StickersListWidget::SectionInfo
StickersListWidget::sectionInfoByOffset(int yOffset) const {
  auto result = SectionInfo();
...
int StickersListWidget::countDesiredHeight(int newWidth) {
  const auto minSize =
      _isEffects ? st::stickerEffectWidthMin : st::stickerPanWidthMin;
  ...
  _rowsLeft =
      fullWidth - columnCount * singleWidth - rowsRight - st().margin.left();
  ...
  const auto minimalLastHeight =
      (_section == Section::Stickers) ? minimalHeight : 0;
...
void StickersListWidget::sendSearchRequest() {
  ...
  const auto stickersCached =
      _searchStickersCache.find(_searchQuery) != _searchStickersCache.cend();
  const auto setsCached =
      _searchSetsCache.find(_searchQuery) != _searchSetsCache.cend();
...
void StickersListWidget::sendSearchSetsRequest(const QString &query) {
  ...
  _searchSetsRequestId =
      _api.request(MTPmessages_SearchStickerSets(
              MTP_flags(0), MTP_string(query), MTP_long(hash)))
...
void StickersListWidget::requestSearchStickers(
  ...
  _searchStickersRequestId =
      _api.request(MTPmessages_SearchStickers(
              MTP_flags(0),
              MTP_string(query),
              MTPstring(),
...
void StickersListWidget::searchForSets(const QString &query,
  ...
  if (_isEffects) {
    ...
    _filteredStickers =
        session().data().stickers().getListByEmoji(std::move(emoji), 0, true);
  }
...
void StickersListWidget::refreshSearchRows() {
  ...
  auto sets = (it != end(_searchSetsCache)) ? &it->second : nullptr;
...
void StickersListWidget::refreshSearchRows(
  ...
  const auto hasCloudFoundStickers =
      true && (foundStickersIt != _searchStickersCache.end()) &&
  ...
  if (!cloudSets && _searchNextQuery.isEmpty()) {
    showStickerSet(!_mySets.empty() ? _mySets[0].id
...
void StickersListWidget::refreshSearchShortcuts(
  ...
  if (_searchSelectedSetId &&
      !ranges::contains(_searchShortcutSets, _searchSelectedSetId, &Set::id)) {
    _searchSelectedSetId = 0;
  }
...
bool StickersListWidget::addSearchShortcut(not_null<StickersSet *> set) {
  ...
  auto elements = PrepareStickers(
      set->stickers.empty() ? set->covers : set->stickers, skipPremium);
  if (elements.empty()) {
    return false;
  }
  _searchShortcutSets.emplace_back(
      set->id, set, set->flags, set->title, set->shortName, set->count, false,
      std::move(elements));
  _searchShortcutSets.back().thumbnailDocument = set->lookupThumbnailDocument();
  return true;
}
...
void StickersListWidget::fillSelectedSearchShortcut() {
  ...
  _searchSets.emplace_back(
      set->id, set, set->flags | SetFlag::Special,
      tr::lng_stickers_count(tr::now, lt_count, set->count), set->shortName,
      set->count, false, std::move(elements));
}
...
void StickersListWidget::startSearchSwapAnimation(
  ...
  const auto top =
      searchShortcutsTop() + (packToPack ? searchShortcutsHeight() : 0);
  ...
  _searchSwapAnimation.start(
      ...
      0., 1., st().searchSwapDuration, anim::sineInOut);
}
...
int StickersListWidget::searchShortcutsHeight() const {
  ...
  auto result =
      st().searchPacksTop + st().searchPackHeight + st().searchPacksBottom;
  result +=
      searchShortcutSelected() ? st().searchBackHeight : st().searchResultsHeight;
  return result;
}

QRect StickersListWidget::searchBackRect() const {
  return QRect(0, searchShortcutsTop(), width(),
...
QRect StickersListWidget::searchShortcutRect(int index) const {
  ...
  const auto left = st().headerLeft - st().margin.left() -
                    _searchShortcutsScroll +
                    index * (st().searchPackWidth + st().searchPackSkip);
  const auto top = searchShortcutsTop() +
                   (searchShortcutSelected() ? st().searchBackHeight : 0) +
                   st().searchPacksTop;
  return QRect(left, top, st().searchPackWidth, st().searchPackHeight);
}
...
void StickersListWidget::refreshSearchShortcutsScroll(int newWidth) {
  ...
  const auto full = st().headerLeft - st().margin.left() +
...
void StickersListWidget::scrollSearchShortcutsTo(int value) {
  const auto scroll = std::clamp(value, 0, _searchShortcutsScrollMax);
...
void StickersListWidget::toggleSearchShortcut(int index) {
  ...
  const auto packToPack =
      _searchSelectedSetId && target && _searchSelectedSetId != target;
...
void StickersListWidget::fillFoundStickersRow(
  ...
  _searchSets.emplace_back(SearchEmojiSectionSetId(), nullptr,
                           Data::StickersSetFlag::Special, QString(), QString(),
                           elements.size(),
...
void StickersListWidget::fillFilteredStickersRow() {
  ...
  auto elements =
      ranges::views::all(_filteredStickers) |
      ranges::views::transform(
          [](not_null<DocumentData *> document) { return Sticker{document}; }) |
      ranges::to_vector;

  _searchSets.emplace_back(
      SearchEmojiSectionSetId(), nullptr, Data::StickersSetFlag::Special,
...
void StickersListWidget::takeHeavyData(std::vector<Set> &to,
                                       std::vector<Set> &from) {
  auto indices = base::flat_map<uint64, int>();
...
void StickersListWidget::takeHeavyData(Set &to, Set &from) {
  ...
  const auto same = ranges::equal(toList, fromList, ranges::equal_to(),
...
void StickersListWidget::searchStickersResultsDone(
  ...
  result.match(
      [&](const MTPDmessages_foundStickersNotModified &data) {
        ...
        checkPaginateSearchStickers(getVisibleTop(), getVisibleBottom());
      },
      [&](const MTPDmessages_foundStickers &data) {
        auto it = _searchStickersCache.find(query);
        if (it == _searchStickersCache.cend()) {
          it = _searchStickersCache
                   .emplace(query, std::vector<DocumentId>())
                   .first;
        }
...
void StickersListWidget::loadMoreSearchStickers() {
  if (_searchStickersRequestId || _searchQuery.isEmpty() || _isEffects ||
      (_searchQuery != _searchNextQuery)) {
    return;
  }
...
void StickersListWidget::checkPaginateSearchStickers(
    int visibleTop,
    int visibleBottom) {
  if (_section != Section::Search || _searchQuery.isEmpty() ||
      (_searchQuery != _searchNextQuery) || _searchStickersRequestId) {
    return;
  }
  const auto visibleHeight = visibleBottom - visibleTop;
  if (visibleHeight <= 0) {
...
void StickersListWidget::searchResultsDone(
  ...
  result.match(
      [&](const MTPDmessages_foundStickerSetsNotModified &data) {
        LOG(("API Error: "
      ...
      [&](const MTPDmessages_foundStickerSets &data) {
        ...
        if (it == _searchSetsCache.cend()) {
          it = _searchSetsCache.emplace(query, std::vector<uint64>())
                   .first;
        }
...
int StickersListWidget::stickersLeft() const { return _rowsLeft; }
...
QRect StickersListWidget::stickerRect(int section, int sel) {
  ...
  const auto rowsToSkip =
      (countTillItem / _columnCount) + ((countTillItem % _columnCount) ? 1 : 0);
  const auto x = stickersLeft() + ((sel % _columnCount) * _singleSize.width());
...
void StickersListWidget::paintSearchShortcuts(Painter &p, QRect clip) {
  if (!searchShortcutsShown() || clip.bottom() < searchShortcutsTop() ||
  ...
  if (back.height() > 0) {
    ...
    const auto &icon =
        selected ? st().search.back.iconOver : st().search.back.icon;
    icon.paint(p, st().searchBackIconLeft,
               back.y() + st().searchBackIconTop, width());
    const auto text = tr::lng_search_back_to_results(tr::now);
    const auto &font = st::emojiPanHeaderFont;
    const auto available =
        width() - st().searchBackTextLeft - st().margin.right();
    auto shown = text;
    ...
    p.drawTextLeft(st().searchBackTextLeft,
                   back.y() + st().searchBackTextTop, width(), shown,
                   textWidth);
  }
  ...
  p.save();
  p.setClipRect(
      QRect(0, searchShortcutsTop() + back.height(), width(),
            st().searchPacksTop + st().searchPackHeight +
  ...
  for (auto i = 0, count = int(_searchShortcutSets.size()); i != count; ++i) {
    ...
    if (set.ripple) {
      set.ripple->paint(p, myrtlrect(rect).x(), rect.y(), width());
      if (set.ripple->empty()) {
        set.ripple.reset();
      }
    }
    ...
    const auto available = rect.width() - 2 * st().searchPackTextPadding;
    ...
    p.setPen(st().textFg);
    p.drawTextLeft(titleLeft, rect.y() + st().searchPackTextTop, width(),
                   title, titleWidth);
  }
  p.restore();

  if (!searchShortcutSelected()) {
    const auto top = searchShortcutsTop() + st().searchPacksTop +
                     st().searchPackHeight + st().searchPacksBottom;
    p.setFont(st::emojiPanHeaderFont);
    p.setPen(st().headerFg);
    p.drawTextLeft(st().headerLeft - st().margin.left(),
                   top + st().searchResultsTextTop, width(),
...
void StickersListWidget::paintSearchShortcutIcon(
  ...
  const auto point =
      rect.topLeft() +
      QPoint((rect.width() - size.width()) / 2,
  ...
  if (const auto image = media->getStickerSmall()) {
    ...
    PaintStickerThumbnailPath(p, media, QRect(point, size),
                              _pathGradient.get());
  }
}

void StickersListWidget::paintStickers(Painter &p, QRect clip) {
  auto fromColumn = floorclamp(clip.x() - stickersLeft(), _singleSize.width(),
                               0, _columnCount);
  auto toColumn = ceilclamp(clip.x() + clip.width() - stickersLeft(),
                            _singleSize.width(), 0, _columnCount);
  if (rtl()) {
  ...
  enumerateSections([&](const SectionInfo &info) {
    ...
    if (set.externalLayout) {
      ...
      const auto count =
          (set.flags & SetFlag::NotLoaded) ? set.count : loadedCount;

      auto widthForTitle =
          stickersRight() - (st().headerLeft - st().margin.left());
      {
        const auto installedSet = !featuredHasAddButton(info.section);
        const auto add = featuredAddRect(info, installedSet);
        const auto selected =
            selectedButton ? (selectedButton->section == info.section) : false;
        (installedSet ? _inactiveButtonBg
         : selected   ? _trendingAddBgOver
                      : _trendingAddBg)
            .paint(p, myrtlrect(add));
        if (set.ripple) {
        ...
        const auto textWidth = installedSet ? _installedWidth : _addWidth;
        const auto &st = installedSet ? st::stickersTrendingInstalled
                                      : st::stickersTrendingAdd;
        p.setFont(st.style.font);
        p.setPen(selected ? st.textFgOver : st.textFg);
        p.drawTextLeft(add.x() - (st.width / 2), add.y() + st.textTop, width(),
                       text, textWidth);
      ...
      if (set.flags & SetFlag::Unread) {
        widthForTitle -=
            st::stickersFeaturedUnreadSize + st::stickersFeaturedUnreadSkip;
      }
      ...
      if (titleWidth > widthForTitle) {
        titleText =
            st::stickersTrendingHeaderFont->elided(titleText, widthForTitle);
      ...
      p.drawTextLeft(st().headerLeft - st().margin.left(),
                     info.top + st::stickersTrendingHeaderTop, width(),
      ...
      if (set.flags & SetFlag::Unread) {
...

        {
          ...
          p.drawEllipse(
              style::rtlrect(st().headerLeft - st().margin.left() + titleWidth +
      ...
      const auto statusText =
          (count > 0) ? tr::lng_stickers_count(tr::now, lt_count, count)
                      : tr::lng_contacts_loading(tr::now);
      p.setFont(st::stickersTrendingSubheaderFont);
      p.setPen(st().trendingSubheaderFg);
      p.drawTextLeft(st().headerLeft - st().margin.left(),
                     info.top + st::stickersTrendingSubheaderTop, width(),
                     statusText);
      ...
      for (auto j = fromColumn; j < toColumn; ++j) {
        ...
        paintSticker(p, set, info.rowsTop, info.section, index, now, paused,
    ...
    if (setHasTitle(set) && clip.top() < info.rowsTop) {
      ...
      if (hasRemoveButton(info.section)) {
        ...
        if (set.ripple) {
          set.ripple->paint(p, remove.x() + removeSt.rippleAreaPosition.x(),
        ...
        icon.paint(p, remove.x() + (remove.width() - icon.width()) / 2,
                   remove.y() + (remove.height() - icon.height()) / 2, width());

        widthForTitle -= remove.width();
      }
      const auto amCreator = (set.flags & Data::StickersSetFlag::AmCreator);
      if (amCreator) {
        widthForTitle -= badgeWidth + st::stickersFeaturedUnreadSkip +
      ...
      p.setPen(st().headerFg);
      p.drawTextLeft(st().headerLeft - st().margin.left(),
                     info.top + st().headerTop, width(), titleText, titleWidth);
      if (amCreator) {
        const auto badgeLeft = st().headerLeft - st().margin.left() +
                               titleWidth + st::stickersFeaturedUnreadSkip;
        {
          ...
          p.drawRoundedRect(
              style::rtlrect(
                  badgeLeft, info.top + st::stickersHeaderBadgeFontTop,
                  badgeWidth + badgeFont->height, badgeFont->height, width()),
              badgeFont->height / 2., badgeFont->height / 2.);
        }
        p.setPen(st().headerFg);
        p.setBrush(Qt::NoBrush);
        p.setFont(badgeFont);
        p.drawText(QRect(badgeLeft + badgeFont->height / 2,
                         info.top + st::stickersHeaderBadgeFontTop, badgeWidth,
                         badgeFont->height),
                   badgeText, style::al_center);
      }
    }
    if (clip.top() + clip.height() <= info.rowsTop) {
      return true;
               ...
               set.stickers.empty()) {
      const auto buttonSelected =
          (std::get_if<OverGroupAdd>(&_selected) != nullptr);
      paintMegagroupEmptySet(p, info.rowsTop, buttonSelected);
      return true;
    }
    const auto fromRow = floorclamp(clip.y() - info.rowsTop,
                                    _singleSize.height(), 0, info.rowsCount);
    const auto toRow = ceilclamp(clip.y() + clip.height() - info.rowsTop,
                                 _singleSize.height(), 0, info.rowsCount);
      ...
      for (auto j = fromColumn; j < toColumn; ++j) {
        ...
        const auto selected = selectedSticker
                                  ? (selectedSticker->section == info.section &&
                                     selectedSticker->index == index)
                                  : false;
        const auto deleteSelected = selected && selectedSticker->overDelete;
        paintSticker(p, set, info.rowsTop, info.section, index, now, paused,
                     selected, deleteSelected);
      }
    }
...
void StickersListWidget::checkVisibleLottie() {
  ...
  enumerateSections([&](const SectionInfo &info) {
    if (destroyBelow <= info.rowsTop || destroyAbove >= info.rowsBottom) {
...
void StickersListWidget::pauseInvisibleLottieIn(const SectionInfo &info) {
  ...
  if (visibleTop >= info.rowsTop + _singleSize.height() &&
    ...
    const auto pauseRows =
        std::min(pauseHeight / _singleSize.height(), info.rowsCount);
...
void StickersListWidget::paintMegagroupEmptySet(Painter &p, int y,
                                                bool buttonSelected) {
  p.setPen(st().headerFg);
  ...
  (buttonSelected ? _groupCategoryAddBgOver : _groupCategoryAddBg)
      .paint(p, myrtlrect(button));
  ...
  p.setPen(buttonSelected ? st::stickerGroupCategoryAdd.textFgOver
                          : st::stickerGroupCategoryAdd.textFg);
  p.drawTextLeft(button.x() - (st::stickerGroupCategoryAdd.width / 2),
                 button.y() + st::stickerGroupCategoryAdd.textTop, width(),
                 _megagroupSetButtonText, _megagroupSetButtonTextWidth);
}
...
void StickersListWidget::ensureLottiePlayer(Set &set) {
  ...
  set.lottiePlayer = std::make_unique<Lottie::MultiPlayer>(
      Lottie::Quality::Default, getLottieRenderer());
  const auto raw = set.lottiePlayer.get();

  raw->updates() | rpl::on_next(
                       [=] {
                         auto &sets = shownSets();
                         enumerateSections([&](const SectionInfo &info) {
...
void StickersListWidget::setupLottie(Set &set, int section, int index) {
  ...
  sticker.lottie = LottieAnimationFromDocument(
      set.lottiePlayer.get(), sticker.documentMedia.get(),
...
void StickersListWidget::setupWebm(Set &set, int section, int index) {
  ...
  sticker.webm = Media::Clip::MakeReader(
      ...
      sticker.documentMedia->bytes(), std::move(callback));
}

void StickersListWidget::clipCallback(Media::Clip::Notification notification,
                                      uint64 setId,
                                      not_null<DocumentData *> document,
                                      int indexHint) {
  Expects(indexHint >= 0);
  ...
  enumerateSections([&](const SectionInfo &info) {
    ...
    case Notification::Reinit: {
      ...
      if (webm->state() == State::Error) {
        ...
        const auto size = ComputeStickerSize(j->document, boundingBoxSize());
...
bool StickersListWidget::itemVisible(const SectionInfo &info, int index) const {
  const auto visibleTop = getVisibleTop();
  const auto visibleBottom = getVisibleBottom();
...
void StickersListWidget::updateSet(const SectionInfo &info) {
  ...
  const auto now = crl::now();
  const auto delay = std::max(_lastScrolledAt + kMinAfterScrollDelay - now,
...
void StickersListWidget::repaintItems(const SectionInfo &info, crl::time now) {
  update(0, info.rowsTop, width(), info.rowsBottom - info.rowsTop);
  auto &set = shownSets()[info.section];
...
QSize StickersListWidget::boundingBoxSize() const {
  return QSize(_singleSize.width() - st::roundRadiusSmall * 2,
               _singleSize.height() - st::roundRadiusSmall * 2);
}

void StickersListWidget::paintSticker(Painter &p, Set &set, int y, int section,
                                      int index, crl::time now, bool paused,
                                      bool selected, bool deleteSelected) {
  auto &sticker = set.stickers[index];
  sticker.ensureMediaCreated();
  const auto document = sticker.document;
  ...
  if (isLottie && !sticker.lottie && media->loaded()) {
  ...
  const auto pos = QPoint(stickersLeft() + col * _singleSize.width(),
  ...
  const auto ppos = pos + QPoint((_singleSize.width() - size.width()) / 2,
                                 (_singleSize.height() - size.height()) / 2);

  auto lottieFrame = QImage();

  QPainterPath path;
  path.addRoundedRect(QRectF(ppos, size), st::stickerRoundingSize,
                      st::stickerRoundingSize);
  ...
  if (sticker.lottie && sticker.lottie->ready()) {
    ...
    p.drawImage(QRect(ppos, lottieFrame.size() / style::DevicePixelRatio()),
                lottieFrame);
    ...
    const auto frame = sticker.webm->current({.frame = size, .keepAlpha = true},
                                             paused ? 0 : now);
    if (sticker.savedFrame.isNull()) {
    ...
    const auto useSavedFrame =
        !sticker.savedFrame.isNull() && (sticker.savedFrameFor == _singleSize);
    if (useSavedFrame) {
      ...
      p.setOpacity(1.);
      PaintStickerThumbnailPath(p, media.get(), QRect(ppos, size),
                                _pathGradient.get());
    }
  }
  ...
  if (selected && stickerHasDeleteButton(set, index)) {
    const auto xPos =
        pos +
        QPoint(_singleSize.width() - st::stickerPanDeleteIconBg.width(), 0);
    p.setOpacity(deleteSelected ? st::stickerPanDeleteOpacityBgOver
                                : st::stickerPanDeleteOpacityBg);
    st::stickerPanDeleteIconBg.paint(p, xPos, width());
    p.setOpacity(deleteSelected ? st::stickerPanDeleteOpacityFgOver
  ...
  const auto corner = (set.id == Data::Stickers::RecentSetId) ? &_cornerEmoji
  ...
  if (corner && !corner->empty() && _paintAsPremium) {
    ...
    if (const auto emoji = (*corner)[index]) {
      ...
      const auto position =
          pos + QPoint(_singleSize.width(), _singleSize.height()) -
  ...
  if (!cornerPainted && premium) {
    _premiumMark->paint(p, lottieFrame, sticker.premiumLock, pos, _singleSize,
                        width());
  }
}
...
QRect StickersListWidget::featuredAddRect(const SectionInfo &info,
...
void StickersListWidget::setPressed(OverState newPressed) {
  ...
  if (auto button = std::get_if<OverButton>(&_pressed)) {
    ...
    if (shortcut->index >= 0 && shortcut->index < _searchShortcutSets.size()) {
      auto &set = _searchShortcutSets[shortcut->index];
      if (!set.ripple) {
    ...
    if (!_megagroupSetButtonRipple) {
      auto mask = Ui::RippleAnimation::RoundRectMask(
          _megagroupSetButtonRect.size(), st::roundRadiusLarge);
      _megagroupSetButtonRipple = std::make_unique<Ui::RippleAnimation>(
          st::stickerGroupCategoryAdd.ripple, std::move(mask),
          [this] { rtlupdate(megagroupSetButtonRectFinal()); });
    }
    _megagroupSetButtonRipple->add(
...
std::unique_ptr<Ui::RippleAnimation>
StickersListWidget::createButtonRipple(int section) {
  Expects(section >= 0 && section < shownSets().size());

  if (shownSets()[section].externalLayout) {
    const auto maskSize = QSize(_addWidth - st::stickersTrendingAdd.width,
                                st::stickersTrendingAdd.height);
    auto mask =
        Ui::RippleAnimation::RoundRectMask(maskSize, st::roundRadiusLarge);
    return std::make_unique<Ui::RippleAnimation>(
        st::stickersTrendingAdd.ripple, std::move(mask),
...
void StickersListWidget::showStickerSetBox(not_null<DocumentData *> document,
  ...
  if (document->sticker() && document->sticker()->set) {
    showBoxPreventHide(Box<StickerSetBox>(_show, document->sticker()->set,
    ...
    rpl::merge(base::timer_once(kTimeout), document->owner().stickers().updated(
    ...
    document->session().api().requestSpecialStickersForce(
        ...
        setId == Data::Stickers::RecentSetId, false);
  }
}

base::unique_qptr<Ui::PopupMenu>
StickersListWidget::fillContextMenu(const SendMenu::Details &details) {
  ...
  SendMenu::FillSendMenu(menu,
                         nullptr, // showForEffect
                         details, SendMenu::DefaultCallback(_show, send),
                         icons);
  ...
  menu->addAction((isFaved ? tr::lng_faved_stickers_remove
  ...
  if (_features.openStickerSets) {
    menu->addAction(
        ...
        [=, id = set.id] { showStickerSetBox(document, id); },
        &icons->menuStickerSet);
  }

  if (const auto id = set.id; id == Data::Stickers::RecentSetId) {
    menu->addAction(
        tr::lng_recent_stickers_remove(tr::now),
        [=] {
          Api::ToggleRecentSticker(document, Data::FileOriginStickerSet(id, 0),
                                   false);
        },
        &icons->menuRecentRemove);
  }

  SendMenu::AttachSendMenuEffect(menu, _show, details,
...
Ui::MessageSendingAnimationFrom StickersListWidget::messageSentAnimationInfo(
    int section, int index, not_null<DocumentData *> document) {
  const auto rect = stickerRect(section, index);
  const auto size = ComputeStickerSize(document, boundingBoxSize());
  const auto innerPos = QPoint((rect.width() - size.width()) / 2,
                               (rect.height() - size.height()) / 2);
  ...
  return {
      ...
      .globalStartGeometry =
          mapToGlobal(QRect(rect.topLeft() + innerPos, size)),
  };
}
...
void StickersListWidget::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (!v::is_null(pressed) && pressed == _selected) {
      ...
      const auto document = set.stickers[sticker->index].document;
      if (_features.openStickerSets && (e->modifiers() & Qt::ControlModifier)) {
        showStickerSetBox(document, set.id);
      } else {
        const auto &settings = AyuSettings::getInstance();
        auto from = messageSentAnimationInfo(sticker->section, sticker->index,
                                             document);
        ...
        if (settings.stickerConfirmation() &&
            (_mode == Mode::Full || _mode == Mode::ChatIntro) &&
            _requireConfirmation) {
          Ui::show(Ui::MakeConfirmBox({.text = tr::ayu_ConfirmationSticker(),
...
void StickersListWidget::removeRecentSticker(int section, int index) {
  if ((_section != Section::Stickers) || (section >= int(_mySets.size())) ||
...
void StickersListWidget::removeFavedSticker(int section, int index) {
  ...
  Api::ToggleFavedSticker(
      _show, document,
      Data::FileOriginStickerSet(Data::Stickers::FavedSetId, 0), false);
}
...
void StickersListWidget::resizeEvent(QResizeEvent *e) {
  _settings->moveToLeft((width() - _settings->width()) / 2, height() / 3);
  if (!_megagroupSetAbout.isEmpty()) {
    refreshMegagroupSetGeometry();
  }
}

void StickersListWidget::leaveEventHook(QEvent *e) { clearSelection(); }
...
void StickersListWidget::setSection(Section section) {
  ...
  clearHeavyData();
  _section = section;
  _recentShownCount = (section == Section::Search) ? _filteredStickers.size()
                      : _mySets.empty() ? 0
...
void StickersListWidget::refreshFeaturedSets() {
  ...
  _officialSets.reserve(session().data().stickers().featuredSetsOrder().size() +
                        wereOfficial.size() - wasFeaturedSetsCount);
...
uint64 StickersListWidget::currentSet(int yOffset) const {
  ...
  const auto &sets = shownSets();
  return sets.empty() ? Data::Stickers::RecentSetId
                      : sets[sectionInfoByOffset(yOffset).section].id;
}

bool StickersListWidget::appendSet(std::vector<Set> &to, uint64 setId,
                                   bool externalLayout, AppendSkip skip) {
  ...
  const auto set = it->second.get();
  if ((skip == AppendSkip::Archived) && (set->flags & SetFlag::Archived)) {
    return false;
  }
  if ((skip == AppendSkip::Installed) && (set->flags & SetFlag::Installed) &&
  ...
  const auto skipPremium = !session().premiumPossible();
  auto elements = PrepareStickers(
      ((set->stickers.empty() && externalLayout) ? set->covers : set->stickers),
      skipPremium);
  if (elements.empty()) {
    return false;
  }
  to.emplace_back(set->id, set, set->flags, set->title, set->shortName,
                  set->count, externalLayout, std::move(elements));
...
auto StickersListWidget::collectRecentStickers() -> std::vector<Sticker> {
  ...
  const auto &sets = session().data().stickers().sets();
  const auto &recent = _isMasks ? RecentStickerPack()
                                : session().data().stickers().getRecentPack();
  const auto customIt =
      _isMasks ? sets.cend() : sets.find(Data::Stickers::CustomSetId);
  const auto cloudIt =
      sets.find(_isMasks ? Data::Stickers::CloudRecentAttachedSetId
                         : Data::Stickers::CloudRecentSetId);
  const auto customCount =
      (customIt != sets.cend()) ? customIt->second->stickers.size() : 0;
  const auto cloudCount =
      (cloudIt != sets.cend()) ? cloudIt->second->stickers.size() : 0;
...
void StickersListWidget::refreshRecentStickers(bool performResize) {
  ...
  const auto recentIt = ranges::find_if(
      _mySets, [](auto &set) { return set.id == Data::Stickers::RecentSetId; });
  if (!recentPack.empty()) {
    const auto shortName = QString();
    const auto externalLayout = false;
    auto set = Set(Data::Stickers::RecentSetId, nullptr,
                   (SetFlag::Official | SetFlag::Special),
                   (_isEffects ? tr::lng_effect_stickers_title(tr::now)
                               : tr::lng_recent_stickers(tr::now)),
                   shortName, recentPack.size(), externalLayout,
                   std::move(recentPack));
    if (recentIt == _mySets.end()) {
      const auto where =
          (_mySets.empty() || _mySets.begin()->id != Data::Stickers::FavedSetId)
              ? _mySets.begin()
              : (_mySets.begin() + 1);
...
void StickersListWidget::refreshFavedStickers() {
  ...
  _mySets.insert(_mySets.begin(),
                 Set{Data::Stickers::FavedSetId, nullptr,
                     (SetFlag::Official | SetFlag::Special),
                     Lang::Hard::FavedSetTitle(), shortName, set->count,
                     externalLayout, std::move(elements)});
  _favedStickersMap = base::flat_set<not_null<DocumentData *>>{
      set->stickers.begin(), set->stickers.end()};
}
...
void StickersListWidget::refreshMegagroupStickers(GroupStickersPlace place) {
  ...
  if (!_megagroupSet->mgInfo->stickerSet) {
    if (canEdit) {
      auto hidden =
          session().settings().isGroupStickersSectionHidden(_megagroupSet->id);
      if (isShownHere(hidden)) {
        const auto shortName = QString();
        const auto externalLayout = false;
        const auto count = 0;
        _mySets.emplace_back(Data::Stickers::MegagroupSetId, nullptr,
                             SetFlag::Special, tr::lng_group_stickers(tr::now),
                             shortName, count, externalLayout);
      }
    }
    return;
  }
  ...
  if (it != sets.cend()) {
    const auto set = it->second.get();
    auto isInstalled =
        (set->flags & SetFlag::Installed) && !(set->flags & SetFlag::Archived);
    if (isInstalled && !canEdit) {
      ...
      auto elements = PrepareStickers(set->stickers, skipPremium);
      if (!elements.empty()) {
        _mySets.emplace_back(Data::Stickers::MegagroupSetId, set,
                             SetFlag::Special, tr::lng_group_stickers(tr::now),
                             shortName, set->count, externalLayout,
                             std::move(elements));
      }
    }
    return;
  ...
  _api.request(MTPmessages_GetStickerSet(Data::InputStickerSet(set),
                                         MTP_int(0) // hash
                                         ))
...
std::vector<StickerIcon> StickersListWidget::fillIcons() {
  ...
  for (auto l = _mySets.size(); i != l; ++i) {
    ...
    const auto size =
        set->hasThumbnail() ? QSize(set->thumbnailLocation().width(),
                                    set->thumbnailLocation().height())
        : s->hasThumbnail() ? QSize(s->thumbnailLocation().width(),
...
void StickersListWidget::updateSelected() {
  ...
  if (!rect().contains(p) || p.y() < getVisibleTop() ||
      p.y() >= getVisibleBottom() || !isVisible()) {
    clearSelection();
    return;
  }
	if (searchShortcutsShown()
      ...
      if (set.id == Data::Stickers::MegagroupSetId && set.stickers.empty()) {
        ...
        if (index >= 0 && index < set.stickers.size()) {
          ...
          if (stickerHasDeleteButton(set, index)) {
            const auto inx = sx - (columnIndex * _singleSize.width());
            const auto iny = yOffset - (rowIndex * _singleSize.height());
...
bool StickersListWidget::stickerHasDeleteButton(const Set &set,
                                                int index) const {
  if (set.id == Data::Stickers::RecentSetId) {
...
void StickersListWidget::setSelected(OverState newSelected) {
  if (_selected != newSelected) {
    setCursor(!v::is_null(newSelected) ? style::cur_pointer
                                       : style::cur_default);
    ...
    auto updateSelected = [&]() {
      if (auto sticker = std::get_if<OverSticker>(&_selected)) {
        ...
        if (button->section >= 0 && button->section < sets.size() &&
...
void StickersListWidget::refreshIcons(ValidateIconAnimations animations) {
  if (_footer) {
    _footer->refreshIcons(
        fillIcons(), currentSet(getVisibleTop()),
        [=] { return getLottieRenderer(); }, animations);
  }
}
...
void StickersListWidget::refreshMegagroupSetGeometry() {
  ...
  _megagroupSetButtonTextWidth = st.style.font->width(_megagroupSetButtonText);
...
void StickersListWidget::showMegagroupSet(ChannelData *megagroup) {
  ...
  if (_megagroupSet != megagroup) {
    ...
    if (_megagroupSetAbout.isEmpty()) {
      _megagroupSetAbout.setText(st::stickerGroupCategoryAbout,
...
void StickersListWidget::setupSearch() {
  ...
  const auto type = (_mode == Mode::UserpicBuilder)
                    ...
                    : (_mode == Mode::ChatIntro) ? TabbedSearchType::Greeting
                                                 : TabbedSearchType::Stickers;
  _search = MakeSearch(
      this, st(),
      [=](std::vector<QString> &&query) { applySearchQuery(std::move(query)); },
      session, type);
}

void StickersListWidget::applySearchQuery(std::vector<QString> &&query) {
  auto set = base::flat_set<EmojiPtr>();
  auto text = ranges::accumulate(query, QString(), [](QString a, QString b) {
...
void StickersListWidget::removeMegagroupSet(bool locally) {
  ...
  showBoxPreventHide(Ui::MakeConfirmBox({
      ...
      .confirmed =
          crl::guard(this,
                     [this, group = _megagroupSet](Fn<void()> &&close) {
...
void StickersListWidget::removeSet(uint64 setId) {
  ...
  if (setId == Data::Stickers::MegagroupSetId) {
    ...
    Assert(i != end(sets));
    const auto removeLocally =
        i->stickers.empty() || !_megagroupSet->canEditStickers();
...
const Data::StickersSetsOrder &StickersListWidget::defaultSetsOrder() const {
  return _isMasks ? session().data().stickers().maskSetsOrder()
                  : session().data().stickers().setsOrder();
}

Data::StickersSetsOrder &StickersListWidget::defaultSetsOrderRef() {
  return _isMasks ? session().data().stickers().maskSetsOrderRef()
                  : session().data().stickers().setsOrderRef();
}

bool StickersListWidget::mySetsEmpty() const { return _mySets.empty(); }

void StickersListWidget::filterEffectsByEmoji(
    const std::vector<EmojiPtr> &emoji) {
  _filteredStickers.clear();
  _filterStickersCornerEmoji.clear();
  if (_mySets.empty() || _mySets.front().id != Data::Stickers::RecentSetId ||
...
MakeConfirmRemoveSetBox(not_null<Main::Session *> session,
                        const style::FlatLabel &st, uint64 setId) {
  const auto &sets = session->data().stickers().sets();
  ...
  const auto set = it->second.get();
  const auto text =
      tr::lng_stickers_remove_pack(tr::now, lt_sticker_pack, set->title);
          ...
          [=](Fn<void()> &&close) {
            ...
            if (it != sets.cend()) {
              ...
              if (set->id && set->accessHash) {
                session->api()
                    .request(
                        MTPmessages_UninstallStickerSet(MTP_inputStickerSetID(
                            MTP_long(set->id), MTP_long(set->accessHash))))
                    .send();

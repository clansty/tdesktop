...
void StickersListWidget::sendSearchRequest() {
  if (_searchSetsRequestId || _searchStickersRequestId ||
      _searchNextQuery.isEmpty() || _isEffects) {
    return;
  }
  ...
  _searchStickersRequestId =
      _api.request(
              MTPmessages_SearchStickers(MTP_flags(0), MTP_string(_searchQuery),
                                         MTPstring(),             // emoticon
          ...
          .fail([=] {
            ...
            _searchStickersCache.emplace(_searchQuery,
                                         std::vector<DocumentId>());

            _searchSetsRequestId =
                _api
                    .request(MTPmessages_SearchStickerSets(
                        MTP_flags(0), MTP_string(_searchQuery), MTP_long(hash)))
...
void StickersListWidget::fillLocalSearchRows(const QString &query) {
  ...
  auto searchWordInTitle = [](const QStringList &titleWords,
  ...
  auto allSearchWordsInTitle = [&](const QStringList &titleWords) {
...
void StickersListWidget::addSearchRow(not_null<StickersSet *> set) {
  ...
  _searchSets.emplace_back(set->id, set, set->flags, set->title, set->shortName,
                           set->count, !SetInMyList(set->flags),
                           std::move(elements));
}
      ...
      [&](const MTPDmessages_foundStickers &data) {
        ...
        if (it == _searchStickersCache.cend()) {
          it = _searchStickersCache
                   .emplace(_searchQuery, std::vector<DocumentId>())
                   .first;
        }
  ...
  result.match(
      ...
      [&](const MTPDmessages_foundStickerSets &data) {
        ...
        if (it == _searchSetsCache.cend()) {
          it = _searchSetsCache.emplace(_searchQuery, std::vector<uint64>())
                   .first;
        }
...
void StickersListWidget::showStickerSetBox(not_null<DocumentData *> document,
  ...
  if (document->sticker() && document->sticker()->set) {
    checkHideWithBox(Box<StickerSetBox>(_show, document->sticker()->set,

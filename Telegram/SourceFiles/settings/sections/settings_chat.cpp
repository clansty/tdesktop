...
private:
  class Button {
  public:
    Button(not_null<QWidget *> parent, std::vector<QColor> &&colors,
           bool selected);
  ...
  void show(not_null<const Scheme *> scheme, std::vector<QColor> &&colors,
            int selected);
...
void PaintCustomButton(QPainter &p, const std::vector<QColor> &colors) {
  ...
  const auto drawAround = [&](QPointF center, int index) {
    const auto where =
        QPointF{size * (1. + center.x()) / 2, size * (1. + center.y()) / 2};
    p.setBrush(colors[index]);
    p.drawEllipse(where.x() - smallSize, where.y() - smallSize, 2 * smallSize,
                  2 * smallSize);
  };
...
ColorsPalette::Button::Button(not_null<QWidget *> parent,
                              std::vector<QColor> &&colors, bool selected)
    : _widget(parent.get()), _colors(std::move(colors)), _selected(selected) {
  _widget.show();
  _widget.resize(st::settingsAccentColorSize, st::settingsAccentColorSize);
  _widget.paintRequest() | rpl::on_next([=] { paint(); }, _widget.lifetime());
}
...
void ColorsPalette::Button::update(std::vector<QColor> &&colors,
                                   bool selected) {
  if (_colors != colors) {
  ...
  if (_selected != selected) {
    _selected = selected;
    _selectedAnimation.start([=] { _widget.update(); }, _selected ? 0. : 1.,
                             _selected ? 1. : 0.,
...
bool ColorsPalette::Button::selected() const { return _selected; }
...
void ColorsPalette::Button::paint() {
  ...
  if (_colors.size() == 1) {
    PaintRoundColorButton(p, st::settingsAccentColorSize, _colors.front(),
...
void ColorsPalette::show(Type type) {
  ...
  const auto messageShotSelected =
      AyuFeatures::MessageShot::isChoosingTheme()
  ...
  const auto selected =
      std::clamp(int(i - begin(list)), 0, int(list.size()) - 1);
...
void ColorsPalette::show(not_null<const Scheme *> scheme,
                         std::vector<QColor> &&colors, int selected) {
  ...
  const auto pushButton = [&](std::vector<QColor> &&colors) {
    ...
    if (_buttons.size() > index) {
      ...
      _buttons.push_back(
          std::make_unique<Button>(inner, std::move(colors), chosen));
  ...
  for (const auto &color : colors) {
    ...
    if (clicks) {
      std::move(clicks) |
          rpl::map([=] { return _buttons[index - 1]->color(); }) |
          rpl::on_next([=](QColor color) { _selected.fire_copy(color); },
                       inner->lifetime());
    }
  }
  ...
  if (clicks) {
    std::move(clicks) |
        rpl::on_next([=] { selectCustom(scheme); }, inner->lifetime());
  }
}
...
void ColorsPalette::selectCustom(not_null<const Scheme *> scheme) {
  ...
  const auto colorizer =
      Window::Theme::ColorizerFrom(*scheme, scheme->accentColor);
  Ui::show(Box([=](not_null<Ui::GenericBox *> box) {
    const auto editor = box->addRow(object_ptr<ColorEditor>(
        box, ColorEditor::Mode::HSL, (*selected)->color()));
    ...
    editor->setLightnessLimits(colorizer.lightnessMin, colorizer.lightnessMax);
...
BackgroundRow::BackgroundRow(QWidget *parent,
    ...
    : RpWidget(parent), _controller(controller),
      _chooseFromGallery(this, tr::lng_settings_bg_from_gallery(tr::now),
                         st::settingsLink),
      _chooseFromFile(this, tr::lng_settings_bg_from_file(tr::now),
                      st::settingsLink),
  ...
  _chooseFromFile->addClickHandler([=] { ChooseFromFile(controller, this); });
  ...
  Window::Theme::Background()->updates() |
      ...
      rpl::on_next([=] { updateImage(); }, lifetime());
}
...
void BackgroundRow::paintEvent(QPaintEvent *e) {
  ...
  if (radial) {
    ...
    if (!backThumb) {
      ...
      const auto &pix = backThumb->pix(st::settingsBackgroundThumb,
                                       {.options = Images::Option::Blur});
      const auto factor = style::DevicePixelRatio();
      p.drawPixmap(0, 0, st::settingsBackgroundThumb,
                   st::settingsBackgroundThumb, pix, 0,
    ...
    const auto inner = QRect(
        QPoint(outer.x() + (outer.width() - st::radialSize.width()) / 2,
    ...
    const auto arc = inner.marginsRemoved(QMargins(
        st::radialLine, st::radialLine, st::radialLine, st::radialLine));
...
QRect BackgroundRow::radialRect() const {
  return QRect(0, 0, st::settingsBackgroundThumb, st::settingsBackgroundThumb);
}
...
crl::time BackgroundRow::radialTimeShift() const { return st::radialDuration; }

void BackgroundRow::radialAnimationCallback(crl::time now) {
  const auto updated = _radial.update(radialProgress(), !radialLoading(),
                                      now + radialTimeShift());
...
void BackgroundRow::updateImage() {
  ...
  const auto preparePattern = [&] {
    ...
    return Ui::GenerateBackgroundImage(
        {fullsize, fullsize}, paper.backgroundColors(),
        paper.gradientRotation(), paper.patternOpacity(), paintPattern);
  };
  const auto prepareNormal = [&] {
    auto result =
        QImage(QSize{fullsize, fullsize}, QImage::Format_ARGB32_Premultiplied);
    ...
    p.drawImage(QRect(0, 0, size, size), prepared,
...
void ChooseFromFile(not_null<Window::SessionController *> controller,
                    not_null<QWidget *> parent) {
  auto filters =
      QStringList(u"Theme files (*.tdesktop-theme *.tdesktop-palette *"_q +
                  Ui::ImageExtensions().join(u" *"_q) + u")"_q);
  ...
  const auto callback =
      crl::guard(controller, [=](const FileDialog::OpenResult &result) {
        ...
        local.setLocalImageAsThumbnail(
            std::make_shared<Image>(std::move(image)));
        controller->show(Box<BackgroundPreviewBox>(controller, local));
      });
  FileDialog::GetOpenPath(parent.get(), tr::lng_choose_image(tr::now),
                          filters.join(u";;"_q), crl::guard(parent, callback));
}
...
void SetupSupportSwitchSettings(
  ...
  const auto add = [&](SwitchType value, const QString &label) {
    container->add(object_ptr<Ui::Radioenum<SwitchType>>(
                       container, group, value, label, st::settingsSendType),
...
void SetupSupportChatsLimitSlice(
  ...
  const auto options = std::vector<Option>{
      ...
      {365, "1 year"}, {0, "All of them"},
  };
  const auto current = controller->session().settings().supportChatsTimeSlice();
  const auto days = current / kDayDuration;
  const auto best =
      ranges::min_element(options, std::less<>(), [&](const Option &option) {
        return std::abs(option.days - days);
      });

  const auto group = std::make_shared<Ui::RadiobuttonGroup>(best->days);
  for (const auto &option : options) {
    container->add(object_ptr<Ui::Radiobutton>(container, group, option.days,
                                               option.label,
                                               st::settingsSendType),
                   st::settingsSendTypePadding);
  }
  group->setChangedCallback([=](int days) {
    controller->session().settings().setSupportChatsTimeSlice(days *
                                                              kDayDuration);
    controller->session().saveSettingsDelayed();
  });
}
...
void BuildThemeOptionsSection(SectionBuilder &builder) {
  ...
  builder.add(
      [controller, highlights](const WidgetContext &ctx) {
...
void BuildThemeSettingsSection(SectionBuilder &builder) {
  ...
  builder.add(
      [controller](const WidgetContext &ctx) {
...
void BuildSensitiveContentSection(SectionBuilder &builder) {
  ...
  builder.add(
      [controller, highlights](const WidgetContext &ctx) {
        auto updateOnTick =
            rpl::single() | rpl::then(base::timer_each(60 * crl::time(1000)));
        Ui::AddDivider(ctx.container.get());
        SetupSensitiveContent(controller, ctx.container.get(),
                              std::move(updateOnTick), highlights);
        return SectionBuilder::WidgetToAdd{};
      },
      [] {
        return SearchEntry{
...
void BuildSupportSection(SectionBuilder &builder) {
  ...
  builder.add(
      [controller](const WidgetContext &ctx) {
        ...
        const auto addRadio = [&](SwitchType value, const QString &label) {
          inner->add(object_ptr<Ui::Radioenum<SwitchType>>(
                         inner, group, value, label, st::settingsSendType),
                     st::settingsSendTypePadding);
        };
  ...
  builder.add(
      [controller](const WidgetContext &ctx) {
        ...
        const auto current =
            controller->session().settings().supportChatsTimeSlice();
        const auto days = current / kDayDuration;
        ...
        const auto group = std::make_shared<Ui::RadiobuttonGroup>(best->days);
        for (const auto &option : options) {
          inner->add(object_ptr<Ui::Radiobutton>(inner, group, option.days,
                                                 option.label,
                                                 st::settingsSendType),
                     st::settingsSendTypePadding);
        }
...
const auto kMeta = BuildHelper(
    ...
    [](SectionBuilder &builder) { BuildChatSectionContent(builder); });
...
void Chat::fillTopBarMenu(const Ui::Menu::MenuCallback &addAction) {
  ...
  createTheme->setProperty("highlight-control-id", u"chat/themes-create"_q);
}
...
void PaintRoundColorButton(QPainter &p, int size, QBrush brush,
                           float64 selected) {
...
void SetupStickersEmoji(not_null<Window::SessionController *> controller,
                        not_null<Ui::VerticalLayout *> container,
  ...
  container->add(object_ptr<Ui::OverrideMargins>(
      container, std::move(wrap),
      QMargins(0, 0, 0, st::settingsCheckbox.margin.bottom())));

  const auto checkbox = [&](const QString &label, bool checked) {
    return object_ptr<Ui::Checkbox>(container, label, checked,
                                    st::settingsCheckbox);
  };
  const auto addWithReturn = [&](const QString &label, bool checked,
                                 auto &&handle) {
    const auto result =
        inner->add(checkbox(label, checked), st::settingsCheckboxPadding);
    result->checkedChanges() |
        rpl::on_next(std::move(handle), inner->lifetime());
    return result;
  };
  const auto addSliding = [&](const QString &label, bool checked, auto &&handle,
                              rpl::producer<bool> shown) {
    const auto wrap = inner->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
        inner, checkbox(label, checked), st::settingsCheckboxPadding));
    wrap->setDuration(0)
  ...
  const auto largeEmoji =
      addWithReturn(tr::lng_settings_large_emoji(tr::now),
                    Core::App().settings().largeEmoji(), [=](bool checked) {
                      Core::App().settings().setLargeEmoji(checked);
                      Core::App().saveSettingsDelayed();
                    });
  if (highlights) {
    highlights->push_back(
        {u"chat/large-emoji"_q, {largeEmoji, {.radius = st::boxRadius}}});
  }

  const auto replaceEmoji =
      addWithReturn(tr::lng_settings_replace_emojis(tr::now),
                    Core::App().settings().replaceEmoji(), [=](bool checked) {
  ...
  const auto suggestEmoji = inner->lifetime().make_state<rpl::variable<bool>>(
      Core::App().settings().suggestEmoji());
  const auto suggestEmojiCheckbox =
      addWithReturn(tr::lng_settings_suggest_emoji(tr::now),
                    Core::App().settings().suggestEmoji(), [=](bool checked) {
  ...
  const auto suggestAnimated = addSliding(
      ...
      rpl::combine(Data::AmPremiumValue(session), suggestEmoji->value(),
                   _1 && _2));
  if (highlights) {
    highlights->push_back({u"chat/suggest-animated-emoji"_q,
                           {suggestAnimated, {.radius = st::boxRadius}}});
  }

  const auto suggestByEmoji = addWithReturn(
      tr::lng_settings_suggest_by_emoji(tr::now),
      Core::App().settings().suggestStickersByEmoji(), [=](bool checked) {
  ...
  const auto loopStickers = addWithReturn(
      ...
      Core::App().settings().loopAnimatedStickers(), [=](bool checked) {
  ...
  const auto stickersButton =
      AddButtonWithIcon(container, tr::lng_stickers_you_have(),
                        st::settingsButton, {&st::menuIconStickers});
  stickersButton->addClickHandler([=] {
    controller->show(Box<StickersBox>(controller->uiShow(),
  ...
  const auto emojiSetsButton =
      AddButtonWithIcon(container, tr::lng_emoji_manage_sets(),
                        st::settingsButton, {&st::menuIconEmoji});
...
void SetupMessages(not_null<Window::SessionController *> controller,
                   not_null<Ui::VerticalLayout *> container,
                   HighlightRegistry *highlights) {
  ...
  auto wrap = object_ptr<Ui::VerticalLayout>(container);
  const auto inner = wrap.data();
  container->add(object_ptr<Ui::OverrideMargins>(container, std::move(wrap),
  ...
  const auto addSend = [&](SendByType value, const QString &text) {
    return inner->add(object_ptr<Ui::Radioenum<SendByType>>(
                          inner, groupSend, value, text, st::settingsSendType),
                      st::settingsSendTypePadding);
  };
  const auto sendEnter =
  ...
  addSend(SendByType::CtrlEnter,
          (Platform::IsMac() ? tr::lng_settings_send_cmdenter(tr::now)
  ...
  const auto addQuick = [&](Quick value, const QString &text) {
    return inner->add(object_ptr<Ui::Radioenum<Quick>>(
                          inner, groupQuick, value, text, st::settingsSendType),
                      st::settingsSendTypePadding);
  };
  addQuick(Quick::Reply, tr::lng_settings_chat_quick_action_reply(tr::now));
  const auto react =
      addQuick(Quick::React, tr::lng_settings_chat_quick_action_react(tr::now));
  if (highlights) {
  ...
  const auto buttonRight =
      Ui::CreateSimpleCircleButton(inner, st::stickersRemove.ripple);
  ...
  auto idValue =
      rpl::single(reactions.favoriteId()) |
      rpl::then(reactions.favoriteUpdates() | rpl::map([=] {
                  return controller->session().data().reactions().favoriteId();
                })) |
      rpl::filter([](const Data::ReactionId &id) { return !id.empty(); });
  auto selected = rpl::duplicate(idValue);
  std::move(selected) |
      rpl::on_next(
          [=, idValue = std::move(idValue)](const Data::ReactionId &id) {
            ...
            if (i != end(list)) {
              AddReactionAnimatedIcon(
                  inner,
                  buttonRight->geometryValue() | rpl::map([=](const QRect &r) {
                    return QPoint(r.left() + (r.width() - iconSize) / 2,
  ...
  react->geometryValue() |
      rpl::on_next(
          [=](const QRect &r) {
            ...
            buttonRight->moveToRight(st::settingsButtonRightSkip,
                                     r.y() +
                                         (r.height() - rightSize.height()) / 2);
          },
          buttonRight->lifetime());
  ...
  const auto cornerReply = inner->add(
      object_ptr<Ui::Checkbox>(
          inner, tr::lng_settings_chat_corner_reply(tr::now),
          Core::App().settings().cornerReply(), st::settingsCheckbox),
  ...
  const auto cornerReaction = inner->add(
      object_ptr<Ui::Checkbox>(
          inner, tr::lng_settings_chat_corner_reaction(tr::now),
          Core::App().settings().cornerReaction(), st::settingsCheckbox),
      st::settingsCheckboxPadding);
...
void SetupArchive(not_null<Window::SessionController *> controller,
                  not_null<Ui::VerticalLayout *> container,
                  Fn<void(Type)> showOther) {
  Ui::AddSkip(container);

  AddButtonWithIcon(container, tr::lng_settings_shortcuts(), st::settingsButton,
                    {&st::menuIconShortcut})
      ->addClickHandler([=] { showOther(ShortcutsId()); });

  PreloadArchiveSettings(&controller->session());
  AddButtonWithIcon(container, tr::lng_context_archive_settings(),
                    st::settingsButton, {&st::menuIconArchive})
      ->addClickHandler([=] {
        controller->show(Box(Settings::ArchiveSettingsBox, controller));
      });
}

void SetupExport(not_null<Window::SessionController *> controller,
                 not_null<Ui::VerticalLayout *> container,
                 Fn<void(Type)> showOther) {
  AddButtonWithIcon(container, tr::lng_settings_export_data(),
                    st::settingsButton, {&st::menuIconExport})
      ->addClickHandler([=] {
        const auto session = &controller->session();
  ...
  AddButtonWithIcon(container, tr::lng_settings_experimental(),
                    st::settingsButton, {&st::menuIconExperimental})
      ->addClickHandler([=] { showOther(Experimental::Id()); });
}

void SetupLocalStorage(not_null<Window::SessionController *> controller,
                       not_null<Ui::VerticalLayout *> container) {
  AddButtonWithIcon(container, tr::lng_settings_manage_local_storage(),
                    st::settingsButton, {&st::menuIconStorage})
      ->addClickHandler([=] { LocalStorageBox::Show(controller); });
}

void SetupDataStorage(not_null<Window::SessionController *> controller,
                      not_null<Ui::VerticalLayout *> container) {
  using namespace rpl::mappers;
  ...
  SetupConnectionType(&controller->window(), &controller->session().account(),
                      container);
  ...
  const auto path = container->add(object_ptr<Ui::SlideWrap<Button>>(
      container,
      CreateButtonWithIcon(container, tr::lng_download_path(),
  ...
  CreateRightLabel(path->entity(), std::move(pathtext), st::settingsButton,
  ...
  AddButtonWithIcon(container, tr::lng_downloads_section(), st::settingsButton,
                    {&st::menuIconDownload})
      ->setClickedCallback([=] {
  ...
  const auto ask =
      container
          ->add(object_ptr<Ui::SettingsButton>(
              container, tr::lng_download_path_ask(), st::settingsButtonNoIcon))
          ->toggleOn(rpl::single(Core::App().settings().askDownloadPath()));

  ask->toggledValue() | rpl::filter([](bool checked) {
...
void SetupAutoDownload(not_null<Window::SessionController *> controller,
                       not_null<Ui::VerticalLayout *> container) {
  Ui::AddDivider(container);
  Ui::AddSkip(container);
  ...
  const auto add = [&](rpl::producer<QString> label, Source source,
    ...
    AddButtonWithIcon(container, std::move(label), st::settingsButton,
                      std::move(descriptor))
        ->addClickHandler([=] {
          controller->show(
  ...
  add(tr::lng_media_auto_in_private(), Source::User, {&st::menuIconProfile});
  add(tr::lng_media_auto_in_groups(), Source::Group, {&st::menuIconGroups});
...
void SetupChatBackground(not_null<Window::SessionController *> controller,
                         not_null<Ui::VerticalLayout *> container,
                         HighlightRegistry *highlights) {
  Ui::AddDivider(container);
  Ui::AddSkip(container);
  ...
  const auto row =
      container->add(object_ptr<BackgroundRow>(container, controller),
  ...
  const auto inner = wrap.data();
  container->add(object_ptr<Ui::OverrideMargins>(
      container, std::move(wrap), QMargins(0, skipTop, 0, skipBottom)));

  Ui::AddSkip(container, st::settingsTileSkip);

  const auto background = Window::Theme::Background();
  const auto tile = inner->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
      inner,
      object_ptr<Ui::Checkbox>(inner, tr::lng_settings_bg_tile(tr::now),
                               background->tile(), st::settingsCheckbox),
      st::settingsSendTypePadding));
  const auto adaptive = inner->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
      inner,
      object_ptr<Ui::Checkbox>(inner, tr::lng_settings_adaptive_wide(tr::now),
                               Core::App().settings().adaptiveForWide(),
                               st::settingsCheckbox),
      st::settingsSendTypePadding));

  tile->entity()->checkedChanges() |
      rpl::on_next([=](bool checked) { background->setTile(checked); },
                   tile->lifetime());

  const auto shown = [=] {
    return !background->paper().isPattern() && !background->colorForFill();
  };
  tile->toggle(shown(), anim::type::instant);

  using Update = const Window::Theme::BackgroundUpdate;
  background->updates() | rpl::filter([](const Update &update) {
...
void SetupChatListQuickAction(not_null<Window::SessionController *> controller,
                              not_null<Ui::VerticalLayout *> container,
                              HighlightRegistry *highlights) {
  Ui::AddDivider(container);
  Ui::AddSkip(container);
  ...
  const auto actionToLabel = [](Type value) {
    switch (value) {
    case Type::Mute:
      return LabelType::Mute;
    case Type::Pin:
      return LabelType::Pin;
    case Type::Read:
      return LabelType::Read;
    case Type::Archive:
      return LabelType::Archive;
    case Type::Delete:
      return LabelType::Delete;
    default:
      return LabelType::Disabled;
    }
  };
  static constexpr auto kDisabledIconRatio = 1.25;

  const auto addPreview = [=](not_null<Ui::VerticalLayout *> container) {
    const auto widget = container->add(object_ptr<Ui::RpWidget>(container));
    ...
    widget->paintRequest() |
        rpl::on_next(
            [=] {
              ...
              const auto rect =
                  QRect(widget->width() - actionWidth -
                            st::boxRowPadding.right() - rightOffset,
                        0, actionWidth, height);

              auto path = QPainterPath();
              path.addRoundedRect(QRect(-actionWidth, 0,
              ...
              p.fillRect(QRect(0, 0, rect::right(rect), st::lineWidth),
                         st::windowBgOver);
              p.fillRect(QRect(0, rect::bottom(rect) - st::lineWidth,
                               rect::right(rect), st::lineWidth),
                         st::windowBgOver);
              p.fillRect(rect, Dialogs::ResolveQuickActionBg(label));
              if (state->icon) {
                Dialogs::DrawQuickAction(p, rect, state->icon.get(), label,
              ...
              p.drawRoundedRect(height, height / 2 - h * 1.5,
                                st::dialogsQuickActionRippleSize * 0.6, h,
                                h / 2, h / 2);
              p.drawRoundedRect(height, height / 2 + h,
                                st::dialogsQuickActionRippleSize * 1.0, h,
                                h / 2, h / 2);
              ...
              p.drawText(
                  QRect(widget->width() - st::dialogsQuickActionRippleSize -
                            st::boxRowPadding.right(),
                        0, st::dialogsQuickActionRippleSize, height),
                  isDisabled
...

  {
    ...
    button->sizeValue() |
        rpl::on_next(
            [=, left = st.iconLeft](QSize size) {
              icon->moveToLeft(left, (size.height() - icon->height()) / 2,
                               size.width());
            },
            icon->lifetime());
    icon->paintRequest() |
  ...
  button->setClickedCallback([=] {
    controller->uiShow()->showBox(Box([=](not_null<Ui::GenericBox *> box) {
      ...
      const auto addRadio = [&](Type value, tr::phrase<> phrase) {
        box->verticalLayout()->add(object_ptr<Ui::Radioenum<Type>>(
                                       box->verticalLayout(), group, value,
                                       phrase(tr::now), st::settingsSendType),
                                   st::settingsSendTypePadding);
      };
      ...
      addRadio(Type::Delete, tr::lng_settings_quick_dialog_action_delete);
      addRadio(Type::Disabled, tr::lng_settings_quick_dialog_action_disabled);
  ...
  Ui::AddSkip(container);
  Ui::AddDividerText(container, tr::lng_settings_quick_dialog_action_about());
  Ui::AddSkip(container);
}

void SetupDefaultThemes(not_null<Window::Controller *> window,
                        not_null<Ui::VerticalLayout *> container,
                        HighlightRegistry *highlights) {
  using Type = Window::Theme::EmbeddedType;
  using Scheme = Window::Theme::EmbeddedScheme;
  ...
  const auto block =
      container->add(object_ptr<Ui::FixedHeightWidget>(container));
  const auto palette =
      Ui::CreateChild<ColorsPalette>(container.get(), container.get());
  const auto systemAccentWrap = container->add(
      object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
          container,
          object_ptr<Ui::Checkbox>(
              container, tr::lng_settings_theme_system_accent_color(tr::now),
  ...
  const auto updateMessageShotPalette = [=](const QString &path) {
    if (path.isEmpty()) { // for Default theme (otherwise doesn't dispaly name
                          // properly)
    ...
    if (const auto color =
      ...
      if (scheme != end(kSchemesList)) {
        ...
        if (Window::Theme::LoadFromFile(path, &instance, nullptr, nullptr,
                                        colorizer)) {
  ...
  const auto apply = [=](const Scheme &scheme) {
    ...
    const auto currentlyIsCustom =
        (chosen() == Type(-1)) && !Background()->themeObject().cloud.id;
    const auto keep = [=] {
    ...
    if (IsNightMode() == isNight(scheme)) {
      ...
      Window::Theme::ToggleNightModeWithConfirmation(window,
                                                     [=, path = scheme.path] {
                                                       ToggleNightMode(path);
                                                       keep();
                                                     });
    }
  };
  const auto schemeClicked = [=](const Scheme &scheme,
  ...
  auto buttons =
      ranges::views::all(kSchemesList) |
      ranges::views::transform([&](const Scheme &scheme) {
        auto check = std::make_unique<Check>(ColorsFromScheme(scheme), false);
        const auto weak = check.get();
        const auto result = Ui::CreateChild<Ui::Radioenum<Type>>(
            block, group, scheme.type, QString(), st::settingsTheme,
            std::move(check));
        rpl::duplicate(scheme.name) |
            rpl::on_next(
                [=](const QString &themeName) { result->setText(themeName); },
                result->lifetime());
  ...
  const auto refreshSystemAccentVisibility = [=](Type type) {
    systemAccentWrap->toggle(IsSystemAccentColorSupported() &&
                                 (type != Type(-1)),
                             anim::type::instant);
  };
  ...
  if (highlights) {
    ...
    if (IsSystemAccentColorSupported()) {
      highlights->push_back(
          {u"chat/themes-system-accent"_q,
           {systemAccentWrap->entity(), {.radius = st::boxRadius}}});
    }
  }

  Background()->updates() | rpl::filter([](const BackgroundUpdate &update) {
    return (update.type == BackgroundUpdate::Type::ApplyingTheme);
  }) | rpl::map([=] { return chosen(); }) |
      rpl::on_next(
          [=](Type type) {
            refreshColorizer(type);
  ...
  block->widthValue() |
      rpl::on_next(
          [buttons = std::move(buttons)](int width) {
  ...
  if (AyuFeatures::MessageShot::isChoosingTheme()) {
    palette->selected() |
        rpl::on_next(
            [=](QColor color) {
              ...
              refreshColorizer(
                  AyuFeatures::MessageShot::getSelectedFromDefault());

              const auto type = chosen();
              const auto scheme =
                  ranges::find(kSchemesList, type, &Scheme::type);
              if (scheme == end(kSchemesList)) {
                return;
              }
    ...
    AyuFeatures::MessageShot::resetDefaultSelectedEvents() |
        rpl::on_next(
            [=] {
              refreshColorizer(AyuFeatures::MessageShot::
                                   getSelectedFromDefault()); // hide colorizer
...
void SetupThemeOptions(not_null<Window::SessionController *> controller,
                       not_null<Ui::VerticalLayout *> container,
                       HighlightRegistry *highlights) {
  using namespace Window::Theme;
...
void SetupCloudThemes(not_null<Window::SessionController *> controller,
                      not_null<Ui::VerticalLayout *> container,
                      HighlightRegistry *highlights) {
  using namespace Window::Theme;
  using namespace rpl::mappers;
  ...
  const auto showAll = Ui::CreateChild<Ui::LinkButton>(
      inner, tr::lng_settings_bg_show_all(tr::now));

  rpl::combine(title->topValue(), inner->widthValue(), showAll->widthValue()) |
      rpl::on_next(
          [=](int top, int outerWidth, int width) {
            showAll->moveToRight(st::defaultSubsectionTitlePadding.left(), top,
                                 outerWidth);
          },
          showAll->lifetime());
  ...
  const auto list = inner->lifetime().make_state<CloudList>(inner, controller);
  inner->add(list->takeWidget(),
             style::margins(st::settingsButtonNoIcon.padding.left(), 0,
                            st::settingsButtonNoIcon.padding.right(), 0));

  list->allShown() |
      rpl::on_next([=](bool shown) { showAll->setVisible(!shown); },
                   showAll->lifetime());

  showAll->addClickHandler([=] { list->showAll(); });

  const auto editWrap = inner
                            ->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
  ...
  AddButtonWithIcon(edit, tr::lng_settings_bg_theme_edit(), st::settingsButton,
                    {&st::menuIconPalette})
      ->addClickHandler([=] {
        StartEditor(&controller->window(), Background()->themeObject().cloud);
      });

  editWrap->toggleOn(
      rpl::single(BackgroundUpdate(BackgroundUpdate::Type::ApplyingTheme,
                                   Background()->tile())) |
...
void SetupThemeSettings(not_null<Window::SessionController *> controller,
                        not_null<Ui::VerticalLayout *> container,
                        HighlightRegistry *highlights) {
  Ui::AddDivider(container);
  Ui::AddSkip(container, st::settingsPrivacySkip);
  ...
  AddPeerColorButton(container, controller->uiShow(),
                     controller->session().user(), st::settingsColorButton);

  const auto settings = &Core::App().settings();
  if (settings->systemDarkMode().has_value()) {
    auto label = settings->systemDarkModeEnabledValue() |
                 rpl::map([=](bool enabled) {
                   return enabled ? tr::lng_settings_auto_night_mode_on()
                                  : tr::lng_settings_auto_night_mode_off();
                 }) |
                 rpl::flatten_latest();
    const auto button = AddButtonWithLabel(
        container, tr::lng_settings_auto_night_mode(), std::move(label),
        st::settingsButton, {&st::menuIconNightMode});
    button->setClickedCallback([=] {
  ...
  const auto family = container->lifetime().make_state<rpl::variable<QString>>(
      settings->customFontFamily());
  auto label = family->value() | rpl::map([](QString family) {
                 return family.isEmpty() ? tr::lng_font_default(tr::now)
                        : (family == style::SystemFontTag())
                            ? tr::lng_font_system(tr::now)
                            : family;
               });
  const auto fontButton = AddButtonWithLabel(
      container, tr::lng_settings_font_family(), std::move(label),
      st::settingsButton, {&st::menuIconFont});
  fontButton->setClickedCallback([=] {
    ...
    const auto generateBg = [=] {
      ...
      auto result = QImage(QSize(size, size) * ratio,
                           QImage::Format_ARGB32_Premultiplied);
      auto p = QPainter(&result);
      Window::SectionWidget::PaintBackground(
          p, theme.get(), QSize(size, size * 3), QRect(0, 0, size, size));
      p.end();
...
void SetupSupport(not_null<Window::SessionController *> controller,
                  not_null<Ui::VerticalLayout *> container) {
  Ui::AddSkip(container);
  ...
  inner->add(object_ptr<Ui::Checkbox>(
                 inner, "Send all messages without sound",

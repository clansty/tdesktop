...
public:
  Cover(QWidget *parent, not_null<Window::SessionController *> controller,
...
Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller,
    ...
    : FixedHeightWidget(parent, st::settingsPhotoTop +
      ...
      _controller(controller), _user(user),
      _badge(
          this, st::settingsCoverBadge, &user->session(),
          Info::Profile::BadgeContentForPeer(user), &_emojiStatusPanel,
          [=] {
      ...
      _userpic(this, controller, _user, Ui::UserpicButton::Role::OpenPhoto,
  ...
  _badge.setPremiumClickCallback([=] {
    _emojiStatusPanel.show(_controller, _badge.widget(), _badge.sizeTag());
  });
  _badge.updated() |
  ...
  Info::Profile::UsernamesValue(_user) |
      rpl::on_next(
...
void Cover::setupChildGeometry() {
  ...
  widthValue() | rpl::on_next(
                     [=](int newWidth) {
                       _userpic->moveToLeft(st::settingsPhotoLeft,
                                            st::settingsPhotoTop, newWidth);
...
void Cover::initViewers() {
  Info::Profile::NameValue(_user) | rpl::on_next(
                                        [=](const QString &name) {
                                          _name->setText(name);
                                          refreshNameGeometry(width());
                                        },
  ...
  rpl::single(tr::marked(IDString(_user))) |
      rpl::on_next(
  ...
  Info::Profile::UsernameValue(_user) |
      rpl::on_next(
          [=](const TextWithEntities &value) {
            _username->setMarkedText(tr::link(
                value.text.isEmpty() ? tr::lng_settings_username_add(tr::now)
                                     : value.text));
...
void Cover::refreshNameGeometry(int newWidth) {
  ...
  const auto qrButtonWidth =
      (_qrButton && !_qrButton->isHidden())
          ? (_qrButton->width() + st::infoProfileCover.rightSkip)
          : 0;
  auto nameWidth =
      newWidth - nameLeft - st::infoProfileCover.rightSkip - qrButtonWidth;
...
void Cover::refreshPhoneGeometry(int newWidth) {
  ...
  const auto phoneWidth = newWidth - phoneLeft - st::infoProfileCover.rightSkip;
...
void BuildSectionButtons(SectionBuilder &builder) {
    ...
    auto shownProducer =
        hasFilters
            ...
            : (rpl::single(rpl::empty) |
               rpl::then(session->appConfig().refreshed()) | rpl::map([=] {
  ...
  builder.addButton({
      ...
      .label = rpl::single(Lang::GetInstance().id()) |
               rpl::then(Lang::GetInstance().idChanges()) |
...
void BuildPremiumSection(SectionBuilder &builder) {
  ...
  builder.addPremiumButton({
      ...
      .label =
          session->credits().balanceValue() | rpl::map([](CreditsAmount c) {
            return c ? Lang::FormatCreditsAmountToShort(c).string : QString();
          }),
      .credits = true,
      .onClick =
          [=] {
  ...
  builder.addButton({
      ...
      .icon = {&st::menuIconTon},
      .label =
          session->credits().tonBalanceValue() | rpl::map([](CreditsAmount c) {
...
Main::Main(QWidget *parent, not_null<Window::SessionController *> controller)
    : Section(parent, controller) {
  setupContent();
}

rpl::producer<QString> Main::title() { return tr::lng_menu_settings(); }
...
void Main::fillTopBarMenu(const Ui::Menu::MenuCallback &addAction) {
  ...
  if (!controller()->session().supportMode()) {
    addAction(
        ...
        [=] { showOther(InformationId()); }, &st::menuIconEdit);
  }
...
void Main::keyPressEvent(QKeyEvent *e) {
  crl::on_main(this,
               [=, text = e->text()] { CodesFeedString(controller(), text); });
...
void Main::setupContent() {
  ...
  const auto cover =
      content->add(object_ptr<Cover>(content, window, session->user()));
  _userpic = cover->userpic();

  const SectionBuildMethod buildMethod =
      [](not_null<Ui::VerticalLayout *> container,
         not_null<Window::SessionController *> controller,
         Fn<void(Type)> showOther, rpl::producer<> showFinished) {
        auto &lifetime = container->lifetime();
        const auto highlights = lifetime.make_state<HighlightRegistry>();
        const auto isPaused =
            Window::PausedIn(controller, Window::GifPauseReason::Layer);
                  ...
                  for (const auto &[id, entry] : *highlights) {
                    if (entry.widget) {
                      controller->checkHighlightControl(
                          id, entry.widget, base::duplicate(entry.args));
                    }
                  }
                },
                lifetime);
...
void Main::showFinished() {
  controller()->checkHighlightControl(
      u"profile-photo"_q, _userpic.data(),
      {
...
void SetupLanguageButton(not_null<Window::Controller *> window,
                         not_null<Ui::VerticalLayout *> container) {
  const auto button = AddButtonWithLabel(
      container, tr::lng_settings_language(),
      rpl::single(Lang::GetInstance().id()) |
          rpl::then(Lang::GetInstance().idChanges()) |
          rpl::map([] { return Lang::GetInstance().nativeName(); }),
      st::settingsButton, {&st::menuIconTranslate});
...
void SetupValidatePhoneNumberSuggestion(
    not_null<Window::SessionController *> controller,
    not_null<Ui::VerticalLayout *> container, Fn<void(Type)> showOther) {
  ...
  Ui::AddSubsectionTitle(
      ...
      tr::lng_settings_suggestion_phone_number_title(
          lt_phone,
          rpl::single(Ui::FormatPhone(controller->session().user()->phone()))),
      QMargins(st::boxRowPadding.left() -
      ...
      object_ptr<Ui::FlatLabel>(
          ...
          tr::lng_settings_suggestion_phone_number_about(
              ...
              tr::lng_collectible_learn_more(
                  tr::url(tr::lng_settings_suggestion_phone_number_about_link(
                      tr::now))),
              tr::marked),
          st::boxLabel),
      st::boxRowPadding);
  label->setClickHandlerFilter(
      [=, weak = base::make_weak(controller)](const auto &...) {
  ...
  const auto wrap = content->add(
      object_ptr<Ui::FixedHeightWidget>(content, st::inviteLinkButton.height),
  ...
  no->setClickedCallback([=] {
    ...
    const auto customEmojiFactory = [=](QStringView data,
      ...
      *sharedLabel = box->verticalLayout()->add(
          object_ptr<Ui::FlatLabel>(
              ...
              tr::lng_settings_suggestion_phone_number_change(
                  lt_emoji, rpl::single(Ui::Text::SingleCustomEmoji(u"@"_q)),
                  tr::marked),
              st::boxLabel, st::defaultPopupMenu,
  ...
  wrap->widthValue() | rpl::on_next(
                           [=](int width) {
                             const auto buttonWidth =
                                 (width - st::inviteLinkButtonsSkip) / 2;
...
void SetupValidatePasswordSuggestion(
  ...
  const auto mainWrap =
      container->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(
          container, object_ptr<Ui::VerticalLayout>(container)));
  const auto content = mainWrap->entity();
  Ui::AddSubsectionTitle(content, tr::lng_settings_suggestion_password_title(),
                         QMargins(st::boxRowPadding.left() -
                                      st::defaultSubsectionTitlePadding.left(),
                                  0, 0, 0));
  content->add(
      object_ptr<Ui::FlatLabel>(
          content, tr::lng_settings_suggestion_password_about(), st::boxLabel),
      st::boxRowPadding);
...
bool HasInterfaceScale() { return true; }

void SetupInterfaceScale(not_null<Window::Controller *> window,
                         not_null<Ui::VerticalLayout *> container, bool icon) {
  if (!HasInterfaceScale()) {
    return;
  }
  ...
  const auto button =
      AddButtonWithIcon(container, tr::lng_settings_default_scale(),
  ...
  auto sliderWithLabel = MakeSliderWithLabel(
      container, st::settingsScale, st::settingsScaleLabel,
      st::normalFont->spacew * 2,
      st::settingsScaleLabel.style.font->width("300%"), true);
  container->add(std::move(sliderWithLabel.widget),
    ...
    *inSetScale = true;
    ...
    if (cEvalScale(scale) != cEvalScale(cConfigScale())) {
      ...
      const auto cancelled = crl::guard(button, [=](Fn<void()> close) {
        base::call_delayed(st::defaultSettingsSlider.duration, button, [=] {
  ...
  const auto toggleForScale = [=](int scale) {
    scale = cEvalScale(scale);
    const auto show =
        *shown ? ScalePreviewShow::Update : ScalePreviewShow::Show;
    *shown = true;
  ...
  slider->setPseudoDiscrete(
      valuesCount, [=](int index) { return values[index]; }, cConfigScale(),
      [=](int scale) {
        updateLabel(scale);
        toggleForScale(scale);
      },
      [=](int scale) {
        toggleHidePreview();
  ...
  button->toggledValue() | rpl::map([](bool checked) {
    return checked ? style::kScaleAuto : cEvalScale(cConfigScale());
  }) |
      rpl::on_next([=](int scale) { setScale(scale, setScale); },
                   button->lifetime());

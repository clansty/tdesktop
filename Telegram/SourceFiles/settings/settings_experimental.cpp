...
[[nodiscard]] DecodeOptionsResult DecodeOptionsFromText(const QString &text) {
  ...
  auto encoded =
      QStringView(text).mid(kOptionsClipboardPrefix.size()).toLatin1();
...
[[nodiscard]] ResolvedReferrer
ResolveReferrer(const QString &controlId, not_null<Main::Session *> session) {
...
[[nodiscard]] QString
OptionReferrer(const base::options::option<bool> &option) {
...
void AddOption(not_null<Window::Controller *> window,
               not_null<Window::SessionController *> controller,
               not_null<Ui::VerticalLayout *> container,
               base::options::option<bool> &option, rpl::producer<> resetClicks,
  ...
  std::move(resetClicks) | rpl::map_to(option.defaultValue()) |
      rpl::start_to_stream(*toggles, lifetime);
  std::move(reloadOptionsRequests) |
      rpl::on_next([=, &option] { toggles->fire_copy(option.value()); },
                   lifetime);

  const auto referrer = OptionReferrer(option);
  Button *button = nullptr;
  if (!referrer.isEmpty()) {
    button = container->add(object_ptr<Button>(container, rpl::single(name),
                                               st::settingsButtonNoIcon));
    button->addClickHandler([=] {
      const auto resolved = ResolveReferrer(referrer, &controller->session());
    ...
    button = container
                 ->add(object_ptr<Button>(container, rpl::single(name),
                                          (option.relevant()
                                               ? st::settingsButtonNoIcon
  ...
  const auto restarter =
      (referrer.isEmpty() && option.relevant() && option.restartRequired())
...
void SetupExperimental(not_null<Window::Controller *> window,
                       not_null<Window::SessionController *> controller,
                       not_null<Ui::VerticalLayout *> container,
                       rpl::producer<> reloadOptionsRequests) {
  Ui::AddSkip(container, st::settingsCheckboxesSkip);
  ...
  container->add(
      object_ptr<Ui::FlatLabel>(
          container, tr::lng_settings_experimental_about(), st::boxLabel),
  ...
  if (base::options::changed()) {
    ...
    reset = inner->add(
        object_ptr<Button>(inner, tr::lng_settings_experimental_restore(),
                           st::settingsButtonNoIcon));
  ...
  const auto addToggle = [&](const char name[]) {
    AddOption(window, controller, container, base::options::lookup<bool>(name),
              (reset ? (reset->clicks() | rpl::to_empty) : rpl::producer<>()),
...
void Experimental::fillTopBarMenu(const Ui::Menu::MenuCallback &addAction) {
  ...
  addAction(
      ...
      [=] {
        const auto decoded =
            DecodeOptionsFromText(QGuiApplication::clipboard()->text());
        if (!decoded.ok) {
...
void Experimental::setupContent() {
  ...
  SetupExperimental(&controller()->window(), controller(), content,
